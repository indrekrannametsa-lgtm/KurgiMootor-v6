    ...
    if _incomplete_yesterday:
        ...
        if _retry_due and _last_retry_raw:
            try:
                ...
                _retry_due = True
        if _retry_due:
            _retry_before = db.get_app_setting("weather_last_refresh_at", "")
            _weather_service.safe_refresh_all(TODAY)
            _retry_after = db.get_app_setting("weather_last_refresh_at", "")
            db.set_app_setting("weather_incomplete_retry_at", datetime.now(ZoneInfo("Europe/Tallinn")).isoformat())
            if _retry_after and _retry_after != _retry_before:
                _mark_forecast_dirty("hilinenud mõõdetud ilm kontrollitud")
...
if page == "Mootori tähelepanekud":
    ...
    _manual_layered_needed = bool(
        _manual_latest_key
        and _manual_light_done_key == _manual_latest_key
        and str(_last_research_harvest_day or "") != _manual_latest_key
    )
    ...
    elif str(_last_research_harvest_day or "") == _manual_latest_key:
        st.button(
            "🔬 Selle korjepäeva lai uurimisvoor tehtud",
            disabled=True,
            key="layered_manual_done_harvest_disabled",
        )
        st.caption(f"Viimane uuritud täielik korjepäev: {_manual_latest_complete_harvest.strftime('%d.%m.%Y')}.")
    ...
    elif _manual_layered_needed:
        _layered_run_now = st.button(
            "🔬 Jätka selle korjepäeva laia uurimisvooru",
            key="layered_manual_run",
            help="Kasutatav ainult siis, kui Jäljeotsija on uue täieliku korjepäeva juba läbi töötanud, kuid lai voor jäi pooleli või vahele."
        )
        if _layered_run_now:
...
# Prognoosi lehe avamine ja ilma muutus võivad käivitada ainult operatiivse
# prognoosiringi. Jäljeotsija saab automaatselt õiguse ainult uue täieliku
# korjepäeva järel. Käsitsi lai uuring ei sunni Jäljeotsijat uuesti läbi jooksma.
...
if page == "Prognoos" or _forecast_refresh_due or _run_light_research_requested or _run_layered_manual_requested:
    ...
    if _run_light_research_requested:
        ...
        _light_cycle_reason = "Prognoosi vaade · operatiivne"
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            if _layered_run_now:
                ...
                _best_challenger_confirmed = False
                ...
                if _current_results and _abc_confirmation_days:
                    ...
                    if _selected_specs:
                        ...
                        _best_challenger_confirm_stats = _confirm_stats
                        if _confirm_stats and _confirm_stats["Stabiilne"]:
                            _best_challenger_confirmed = True
                ...
                if _current_results:
                    _best_challenger = _current_results[0]
                    _save_json_setting("layered_best_challenger_json", {
                        ...
                        "confirmed": bool(_best_challenger_confirmed),
...
if page == "Mootori tähelepanekud":
    ...
    st.caption(
        ...
        "võib saada sama päeva ühisest uurimispotist ühe piiratud jätkuvooru. "
        "Ilmauuendus üksi uurimist ei käivita; viimane edukas tulemus säilib vahepeal."
    )
    ...
    if not _obs_ready:
        ...
        if not _obs:
            ...
            _shadow = _load_json_setting("shadow_challenger_forecast_json", {})
            if isinstance(_shadow, dict) and _shadow.get("target_date") and _shadow.get("challenger_total") is not None:
                try:
                    _shadow_day = date.fromisoformat(str(_shadow.get("target_date")))
                    _official_total = float(_shadow.get("official_total"))
                    _challenger_total = float(_shadow.get("challenger_total"))
                    _shadow_diff = _challenger_total - _official_total
                    st.markdown("### 🌓 Parima alternatiivi variprognoos")
                    st.info(
                        f"**{_weekday_letter(_shadow_day)} {_short_date(_shadow_day)}** · "
                        f"ametlik {str(_shadow.get('official_model') or 'champion')} **{_official_total:.1f} kasti** · "
                        f"parim kinnitamata challenger **{_challenger_total:.1f} kasti** · vahe **{_shadow_diff:+.1f}**."
                    )
                    _shadow_parts = _shadow.get("challenger_parts") or []
                    if _shadow_parts:
                        st.caption("Challenger: " + " | ".join(map(str, _shadow_parts)))
                    st.caption(
                        "Variprognoos on ainult võrdlus. See ei muuda ametlikku prognoosi ega championit; "
                        "tegeliku korje saabudes saab võrrelda, kumb oli lähemal."
                    )
                except Exception:
                    pass
