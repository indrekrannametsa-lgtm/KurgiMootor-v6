...
if page == "Täna":
    ...
    if st.button("Testi Häädemeeste"):
        with st.spinner("Loen Häädemeeste WMO 86031 värsket ametlikku vaatlust..."):
            try:
                hdm_test = WeatherService().test_haademeeste_current()
                temp = hdm_test.get("airtemperature_c")
                wind = hdm_test.get("windspeed_ms")
                rain = hdm_test.get("precipitations_mm")
                rh = hdm_test.get("relativehumidity_pct")
                temp_text = "—" if temp is None else f"{temp:.1f} °C"
                wind_text = "—" if wind is None else f"{wind:.1f} m/s"
                rain_text = "—" if rain is None else f"{rain:.1f} mm"
                rh_text = "—" if rh is None else f"{rh:.0f}%"
                st.success(
                    f"Häädemeeste {hdm_test.get('wmocode') or '86031'} · "
                    f"õhutemperatuur {temp_text} · tuul {wind_text} · "
                    f"sade {rain_text} · RH {rh_text}"
                )
                st.caption(
                    f"Jaam: {hdm_test.get('name') or 'Häädemeeste'} · "
                    f"vaatluse aeg {hdm_test.get('observed_at') or '—'} · "
                    "test ei kirjuta Supabase'i."
                )
            except Exception as exc:
                st.error(f"Häädemeeste test ebaõnnestus: {exc}")

    if st.button("Testi Häädemeeste 09.08 ajalugu"):
        with st.spinner("Loen Häädemeeste 09.08.2026 ajaloolist tunni-temperatuuri..."):
            try:
                hdm_hist = WeatherService().test_haademeeste_history(date(2026, 8, 9))
                rows_n = int(hdm_hist.get("rows") or 0)
                tmin = hdm_hist.get("tmin_c")
                tmax = hdm_hist.get("tmax_c")
                names = ", ".join(hdm_hist.get("station_names") or []) or "—"
                if rows_n and tmin is not None and tmax is not None:
                    st.success(
                        f"Häädemeeste 09.08.2026 · {rows_n} tunni-rida · "
                        f"Tmin {float(tmin):.1f} °C · Tmax {float(tmax):.1f} °C"
                    )
                else:
                    st.warning(
                        f"Häädemeeste 09.08.2026 · {rows_n} tunni-rida · "
                        "Tmin/Tmax ei arvutunud."
                    )
                st.caption(f"API jaamanimi: {names} · test ei kirjuta Supabase'i.")
            except Exception as exc:
                st.error(f"Häädemeeste ajaloo test ebaõnnestus: {exc}")

    if st.button("Testi Häädemeeste 15.06 kell 11:00"):
        with st.spinner("Küsin Ilmateenistuse tunniandmete lehelt 15.06.2026 kell 11:00..."):
            try:
                hdm_html = WeatherService().test_haademeeste_hourly_page_value(
                    date(2026, 6, 15), "11:00"
                )
                temp = hdm_html.get("temperature_c")
                st.success(
                    f"Häädemeeste 15.06.2026 kell 11:00 · "
                    f"õhutemperatuur {float(temp):.1f} °C"
                )
                st.caption(
                    f"Leitud tabelirida: {hdm_html.get('row')} · "
                    "test ei kirjuta Supabase'i."
                )
            except Exception as exc:
                st.error(f"Häädemeeste HTML tunni-test ebaõnnestus: {exc}")

    if st.button("Testi Häädemeeste 09.08 ööpäeva"):
        with st.spinner("Loen 09.08.2026 Häädemeeste 24 tunnitemperatuuri..."):
            try:
                profile = WeatherService().test_haademeeste_day_profile(date(2026, 8, 9))
                n = int(profile.get("hours_found") or 0)
                day_n = int(profile.get("day_hours_found") or 0)
                night_n = int(profile.get("night_hours_found") or 0)
                tmin = profile.get("tmin_c")
                tmax = profile.get("tmax_c")
                day_avg = profile.get("day_avg_c")
                night_avg = profile.get("night_avg_c")
                ...
                if n:
                    st.success(
                        f"Häädemeeste 09.08.2026 · {n}/24 tundi · "
                        f"öö keskmine {_ft(night_avg)} ({night_n}/12) · "
                        f"päeva keskmine {_ft(day_avg)} ({day_n}/12) · "
                        f"Tmin {_ft(tmin)} · Tmax {_ft(tmax)}"
                    )
                    with st.expander("Näita 24 tunni väärtusi"):
                        rows = [
                            {
                                "Kell": f"{int(item['hour']):02d}:00",
                                "Temperatuur °C": item.get("temperature_c"),
                            }
                            for item in profile.get("hourly", [])
                        ]
                        st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
                else:
                    st.warning("Häädemeeste 09.08.2026 kohta ei leitud ühtegi tunnitemperatuuri.")
                st.caption("Öö = 20:00–08:00 · päev = 08:00–20:00 · test ei kirjuta Supabase'i.")
            except Exception as exc:
                st.error(f"Häädemeeste ööpäevatest ebaõnnestus: {exc}")

    if st.button("Uuri tunniandmete päringut"):
        with st.spinner("Uurin Ilmateenistuse tunniandmete lehe vormi ja päringuparameetreid..."):
            try:
                probe = WeatherService().probe_hourly_page_backend()
                st.success(
                    f"Tunniandmete leht vastas HTTP {probe.get('status')} · "
                    f"HTML {probe.get('html_length')} märki · "
                    f"vorme kokku {probe.get('forms_total')}"
                )
                st.caption(
                    f"Häädemeeste nimi HTML-is: {'jah' if probe.get('html_has_haademeeste') else 'ei'}"
                )
                forms = probe.get("forms") or []
                if forms:
                    st.write("**Kuupäeva/kella jaoks huvitavad vormid:**")
                    ...
                    st.warning("HTML-ist ei leitud selget kuupäeva/kella vormi.")

                hits = probe.get("js_hits") or []
                if hits:
                    st.write("**Võimalikud JavaScripti päringuread:**")
                    st.code("\n".join(hits))
            except Exception as exc:
                st.error(f"Tunniandmete backend'i uuring ebaõnnestus: {exc}")

    st.markdown("##### Häädemeeste ajalootäide")
    _hdm_rows = db.get_weather_rows(SEASON_START, TODAY - timedelta(days=1))
    _hdm_by_day = {str(r.get("weather_date")): r for r in _hdm_rows if r.get("data_kind") == "measured"}
    _hdm_missing = []
    _d = SEASON_START
    while _d <= TODAY - timedelta(days=1):
        _r = _hdm_by_day.get(_d.isoformat()) or {}
        if (
            _r.get("temp_night_avg_c") is None
            or _r.get("temp_day_avg_c") is None
            or "Häädemeeste" not in str(_r.get("source_station") or "")
        ):
            _hdm_missing.append(_d)
        _d += timedelta(days=1)

    if _hdm_missing:
        _batch = _hdm_missing[:7]
        st.caption(
            f"Häädemeeste ajaloost täitmata {len(_hdm_missing)} päeva. "
            f"Järgmine partii {_batch[0].strftime('%d.%m')}–{_batch[-1].strftime('%d.%m')}."
        )
        if st.button("Täida järgmised 7 Häädemeeste päeva"):
            with st.spinner("Loen Häädemeeste tunnivaatlusi ja koondan päeva..."):
                try:
                    _result = WeatherService().backfill_haademeeste(_batch[0], _batch[-1])
                    _mark_model_dirty(
                        f"Häädemeeste ajalootäide {_batch[0]}–{_batch[-1]}"
                    )
                    st.success(
                        f"Valmis: {_result.get('saved', 0)} päeva. "
                        "Vajuta nuppu uuesti järgmise partii jaoks."
                    )
                    st.rerun()
                except Exception as exc:
                    st.error(f"Häädemeeste ajalootäide ebaõnnestus: {exc}")
    else:
        st.success("Häädemeeste mõõdetud T/tuule ajalugu on 15.06–eilseni täielik.")
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            weather_candidate_groups = {
                ...
                # Tmin/Tmax jäävad lisatunnusteks; põhikõver kasutab öö/päeva keskmisi.
            ...
            if not AUTONOMOUS_DISCOVERY_ENABLED:
                ...
                nonlinear_sources = [
                    "T kesk", "ÖöT kesk", "PäevT kesk", "Tmin kesk", "Tmax kesk", "Tmin min", "Tmax max",
                ...
                interaction_sources = [
                    "ÖöT kesk", "PäevT kesk", "Tmin kesk", "Tmax kesk", "Radiatsioon/p", "Sademed Σ",
