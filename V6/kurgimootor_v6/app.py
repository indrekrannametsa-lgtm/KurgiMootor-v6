...
with tabs[3]:
    ...
    if training_rows:
        ...
        # -------------------------------------------------------------------------
        # Vaba ideegeneraator — mootor loob ise uusi tunnusehüpoteese
        # -------------------------------------------------------------------------
        ...
        # 1) Mittelineaarsus: ruut ja kõrvalekalle tavatasemest.
        nonlinear_sources = [
        ...
        for _c in nonlinear_sources:
            ...
            _med = np.nanmedian(_x)
            _register_discovery_feature(f"|{_c} − mediaan|", np.abs(_x - _med), "Mittelineaarsed")
        ...
        def _autonomous_stats(candidate_pred):
        ...
        # ESIMENE RING: kõik genereeritud ideed ühekaupa tänase championi peale.
        first_round_rows = []
        for _idea_name, _idea_col in autonomous_features:
            _cols = list(champion_cols) + [_idea_col]
            _pred = _walk_forward_with_extra(_cols)
            _stats = _autonomous_stats(_pred)
            autonomous_candidate_count += 1
            if _stats:
                _meta = next((m for m in autonomous_feature_meta if m["Veerg"] == _idea_col), {})
                _stats["Idee"] = _idea_name
                _stats["Kategooria"] = _meta.get("Kategooria", "—")
                _stats["Ring"] = 1
                _stats["_Veerg"] = _idea_col
                first_round_rows.append(_stats)
                autonomous_discovery_rows.append(_stats.copy())

        first_round_df = pd.DataFrame(first_round_rows)
        if not first_round_df.empty:
            first_round_df = first_round_df.sort_values(
                ["Paranemine", "Võidab ridu %"],
                ascending=[False, False],
            ).reset_index(drop=True)

        # TEINE RING: kombineeri omavahel esimese ringi paljulubavamad ideed.
        second_round_seed = []
        if not first_round_df.empty:
            promising = first_round_df[first_round_df["Paranemine"] > -0.05].head(8)
        ...
        for _i, _left in enumerate(second_round_seed):
            for _right in second_round_seed[_i + 1:]:
                ...
                _cols = list(champion_cols) + [_left_col, _right_col]
                _pred = _walk_forward_with_extra(_cols)
                _stats = _autonomous_stats(_pred)
                autonomous_candidate_count += 1
                autonomous_category_counts["Teise ringi kombinatsioonid"] += 1
                if _stats:
                    _stats["Idee"] = _combo_name
                    _stats["Kategooria"] = "Teise ringi kombinatsioonid"
                    _stats["Ring"] = 2
                    _stats["_Veerg"] = ""
                    autonomous_discovery_rows.append(_stats)
        ...
        try:
            ...
            if today_vals:
                ...
                if today_adjust_pct is None:
                    today_adjust_text = "muutus —"
                else:
                    today_adjust_text = f"{'↑' if today_adjust_pct > 0 else '↓' if today_adjust_pct < 0 else '→'} {today_adjust_pct:+.0f}%"
            ...
            motor_accuracy_3p, motor_accuracy_days = _motor_accuracy_3p()
            ...
            with home_future_forecast_slot.container():
                for target_day, rows_day in forecast_days:
                    ...
                    adj_text = "muutus —" if adj_pct is None else f"{'↑' if adj_pct > 0 else '↓' if adj_pct < 0 else '→'} {adj_pct:+.0f}%"
                    trend = lead >= 6
...
with tabs[4]:
    ...
    if not _obs_ready:
        ...
        if "autonomous_trace_df" not in locals() or autonomous_trace_df.empty:
            ...
            st.caption(
                "1. ring testib uusi ideid ühekaupa. 2. ring kombineerib omavahel kuni 8 esimese ringi "
                "paljulubavamat ideed. Ühtegi enda leitud ideed EI võeta automaatselt kasutusse."
            )

            _auto_top = autonomous_trace_df.head(10)
            for _, _r in _auto_top.iterrows():
                _status = "✅ stabiilne leid" if bool(_r["Stabiilne"]) else "🧪 uurimisleid"
                _direction = "parem" if float(_r["Paranemine"]) > 0 else "halvem"
                st.write(
                    f"• **{_r['Idee']}** — {_status} · ring {int(_r.get('Ring', 1))}; "
                    f"MAE {_r['Idee MAE']:.2f} vs champion {_r['Championi MAE']:.2f} "
                    f"({_direction} {_r['Paranemine']:+.2f} kasti), "
                    f"võitis {_r['Võidab ridu %']:.0f}% testiridadest."
                )

            _better = autonomous_trace_df[autonomous_trace_df["Paranemine"] > 0]
            if _better.empty:
                st.info("Praegu ei leidnud ideegeneraator ühtegi tunnust, mis vähendaks championi walk-forward MAE-d.")
            else:
                _best = _better.iloc[0]
                st.success(
                    f"Parim mootori enda leid praegu: **{_best['Idee']}**. "
                    f"MAE {_best['Idee MAE']:.2f} vs champion {_best['Championi MAE']:.2f}; "
                    f"paranemine {_best['Paranemine']:.2f} kasti. "
                    "See on ainult kandidaat ja ei muuda operatiivset prognoosi."
                )
