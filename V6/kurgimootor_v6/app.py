...
APP_PATCH_VERSION = "v7.0-production-6"  # CYCLE + WEATHER · harvest totals summary
...
if page == "Täna":
    ...
    _prev_month_last = _month_start - timedelta(days=1)
    _current_month_total = 0.0
    _previous_month_total = 0.0
    _season_total = 0.0
    for _row in _harvest_summary_rows:
        ...
        if _day.year == TODAY.year and _day.month == TODAY.month:
            _current_month_total += _total
        if _day.year == _prev_month_last.year and _day.month == _prev_month_last.month:
            _previous_month_total += _total
    ...
    st.caption(
        f"{_month_names_et[_prev_month_last.month - 1]} {_prev_month_last.year}: "
        f"{_fmt(_previous_month_total)} kasti"
    )
    ...
    if existing_row:
        st.warning(
            ...
            "Allolevad väärtused on praegu andmebaasis."
        )
