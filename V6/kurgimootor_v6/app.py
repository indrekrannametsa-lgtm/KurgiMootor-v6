...
_ORDER_TEST_VERSION = "prev-order3-shadow-v1"
    ...
    with _forecast_page_placeholder.container():
        ...
        for sample in usable_samples:
            ...
            training_rows.append({
                ...
                # LAB-155 / production-shadow: kas eelmine sama põllu korje oli päeva viimane (order 3).
                # Ei kasuta eelmist saaki ega XL-i ankru/sisendina.
                "Eelmine korje viimane": 1.0 if int(sample.get("previous_order", 1)) == 3 else 0.0,
        ...
        if training_rows:
            ...
            # -------------------------------------------------------------
            # LAB-156 / PRODUCTION-SHADOW: WD incumbent vs WD + previous_order3
            #
            # Täpselt sama production X_base, sama model_engine ja sama strict
            # walk-forward nagu aktiivsel WIND×DRY A+B+C mootoril.
            # Ainuke lisatunnus: kas selle põllu EELMINE korje oli päeva kolmas.
            # Eelmist saaki ega XL-i ei kasutata sisendina.
            # -------------------------------------------------------------
            _order_acceptance_stats = None
            if USE_WINDDRY_ENGINE and "Eelmine korje viimane" in model_df.columns:
                _order_candidate_pred = _walk_forward_with_extra(
                    list(WD_LOCKED_COLS) + ["Eelmine korje viimane"],
                    cache_key="wd_plus_prev_order3_shadow_v1",
                )
                ...
                if _order_acceptance_stats:
                    _save_json_setting("wd_prev_order_acceptance_json", {
                        **_order_acceptance_stats,
                        ...
                        "candidate": "WIND×DRY + previous_order3",
                        "model_version": WD_MODEL_VERSION,
                        "tested_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "plant_index_in_test": False,
            ...
            if USE_WINDDRY_ENGINE:
                ...
                st.markdown("##### Eelmise korjejärjekorra shadow-test")
                if (
                    isinstance(_oa, dict)
                    ...
                    _o3.metric("+ eelmine order 3 MAE", f"{float(_oa.get('Katse MAE')):.2f}")
                    ...
                    st.caption("Eelmise korjejärjekorra shadow-test: piisavaid ühiseid walk-forward ridu veel ei ole.")
            elif champion_stats:
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    if (
        ...
        _ro3.metric("+ eelmine order 3 MAE", f"{float(_ro_oa.get('Katse MAE')):.2f}")
        ...
        st.warning("Eelmise korjejärjekorra shadow-test käis, kuid ühiseid ridu ei olnud piisavalt.")
    else:
        st.info("Eelmise korjejärjekorra shadow-test pole veel arvutatud; äpp käivitab selle ühe korra automaatselt.")
