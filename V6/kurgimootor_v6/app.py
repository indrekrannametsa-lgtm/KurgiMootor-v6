            ...
            if isinstance(_shadow, dict) and _shadow.get("target_date") and _shadow.get("challenger_total") is not None:
                try:
                    ...
                    st.info(
                        ...
                        f"parim alternatiiv **{_challenger_total:.1f} kasti** · vahe **{_shadow_diff:+.1f}**."
                    )
                    _shadow_parts = _shadow.get("challenger_parts") or []
                    if _shadow_parts:
                        st.caption("Alternatiiv: " + " | ".join(map(str, _shadow_parts)))
                    st.caption(
                        ("Kinnituse staatus: ✅ kinnitatud alternatiiv. " if bool(_shadow.get("confirmed")) else "Kinnituse staatus: veel kinnitamata. ")
