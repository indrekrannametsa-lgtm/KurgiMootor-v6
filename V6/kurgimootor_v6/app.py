import time as _km_boot_clock
_KM_STARTUP_T0 = _km_boot_clock.perf_counter()
_KM_STARTUP_MARKS = [("script start", 0.0)]

def _km_startup_mark(label: str) -> None:
    try:
        _KM_STARTUP_MARKS.append((str(label), max(0.0, _km_boot_clock.perf_counter() - _KM_STARTUP_T0)))
    except Exception:
        pass
...
_km_startup_mark("stdlib + PIL imported")

import numpy as np
_km_startup_mark("numpy imported")
import pandas as pd
_km_startup_mark("pandas imported")
import streamlit as st
import streamlit.components.v1 as components
_km_startup_mark("streamlit imported")

import db
_km_startup_mark("db imported")


# =============================================================================
# UI / DB SPEED PATCH (no model changes)
# =============================================================================
# Streamlit reruns the whole script on every click.  The DB module historically
# created a new Supabase client and fetched each app_setting with a separate
# network request.  Keep the public db API unchanged, but add a small cached
# facade so normal page navigation stays responsive.  Writes always invalidate
# the relevant caches immediately.
def _install_fast_db_facade() -> None:
    if getattr(db, "_kurgimootor_fast_facade_v1", False):
        return

    # Reuse one Supabase client across reruns.
    if hasattr(db, "_client"):
        _orig_client = db._client
        db._client = st.cache_resource(show_spinner=False)(_orig_client)

    # Collapse dozens of app_settings reads into one small cached snapshot.
    if all(hasattr(db, name) for name in ("_client", "_execute", "get_app_setting", "set_app_setting")):
        _orig_get_setting = db.get_app_setting
        _orig_set_setting = db.set_app_setting

        @st.cache_data(ttl=8, show_spinner=False)
        def _settings_snapshot():
            try:
                response = db._execute(db._client().table("app_settings").select("key,value"))
                return {str(r.get("key")): str(r.get("value")) for r in list(response.data or []) if r.get("key") is not None}
            except Exception:
                return None

        def _fast_get_setting(key: str, default: str = "") -> str:
            snap = _settings_snapshot()
            if isinstance(snap, dict):
                return snap.get(str(key), default)
            return _orig_get_setting(key, default)

        def _fast_set_setting(key: str, value: str) -> None:
            _orig_set_setting(key, value)
            _settings_snapshot.clear()

        db.get_app_setting = _fast_get_setting
        db.set_app_setting = _fast_set_setting

    # The same forecast/history data is read several times during one rerun.
    # Cache only reads; every write clears the cache, so operational snapshots
    # and newly entered harvests become visible immediately.
    if hasattr(db, "get_yield_forecasts"):
        _orig_get_yield_forecasts = db.get_yield_forecasts

        @st.cache_data(ttl=8, show_spinner=False)
        def _cached_yield_forecasts(limit: int = 1000):
            return _orig_get_yield_forecasts(limit=int(limit))

        def _fast_get_yield_forecasts(limit: int = 1000):
            return _cached_yield_forecasts(int(limit))

        db.get_yield_forecasts = _fast_get_yield_forecasts

        if hasattr(db, "save_yield_forecasts"):
            _orig_save_yield_forecasts = db.save_yield_forecasts

            def _fast_save_yield_forecasts(rows):
                result = _orig_save_yield_forecasts(rows)
                _cached_yield_forecasts.clear()
                return result

            db.save_yield_forecasts = _fast_save_yield_forecasts

    if hasattr(db, "get_harvest_history"):
        _orig_get_harvest_history = db.get_harvest_history

        @st.cache_data(ttl=5, show_spinner=False)
        def _cached_harvest_history(limit: int = 500):
            return _orig_get_harvest_history(limit=int(limit))

        def _fast_get_harvest_history(limit: int = 500):
            return _cached_harvest_history(int(limit))

        db.get_harvest_history = _fast_get_harvest_history

        if hasattr(db, "save_harvest"):
            _orig_save_harvest = db.save_harvest

            def _fast_save_harvest(*args, **kwargs):
                result = _orig_save_harvest(*args, **kwargs)
                _cached_harvest_history.clear()
                return result

            db.save_harvest = _fast_save_harvest

        if hasattr(db, "delete_harvest"):
            _orig_delete_harvest = db.delete_harvest

            def _fast_delete_harvest(*args, **kwargs):
                result = _orig_delete_harvest(*args, **kwargs)
                _cached_harvest_history.clear()
                return result

            db.delete_harvest = _fast_delete_harvest

    db._kurgimootor_fast_facade_v1 = True


_install_fast_db_facade()
_km_startup_mark("DB cache facade installed")

def _km_commit_startup_diag(note: str = "") -> None:
    """Preserve this script run timing in Streamlit session state.

    The first run is especially useful after Streamlit Cloud has slept: it lets us
    separate time spent inside app.py from platform wake-up time before app.py starts.
    """
    try:
        _km_startup_mark(note or "diagnostic snapshot")
        _rows = []
        _prev = 0.0
        for _label, _cum in _KM_STARTUP_MARKS:
            _cum = float(_cum)
            _rows.append({
                "Etapp": str(_label),
                "Lõik s": round(max(0.0, _cum - _prev), 3),
                "Kokku s": round(_cum, 3),
            })
            _prev = _cum
        _snap = {
            "captured_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(timespec="seconds"),
            "note": str(note or ""),
            "total_s": round(float(_prev), 3),
            "rows": _rows,
            "patch": "v7.0-production-10",
        }
        if "_km_startup_first_run" not in st.session_state:
            st.session_state["_km_startup_first_run"] = _snap
        st.session_state["_km_startup_last_run"] = _snap
    except Exception:
        pass

from core import WeatherService
_km_startup_mark("core imported")
# V7.0 production does not import model_engine. The old research/champion block remains
# below as dead archive code behind _run_operational_cycle = False.
...
APP_PATCH_VERSION = "v7.0-production-10"  # startup timing diagnostics only; model unchanged
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor V7.0", page_icon=APP_ICON, layout="wide")
_km_startup_mark("page config ready")

# Background operational/research calculation may still run when inputs changed,
# but its full Prognoos UI must never be painted into another menu page.
st.markdown(
    """<style>
    .st-key-kurgi_hidden_forecast_cycle { display: none !important; }
    </style>""",
...
# =============================================================================
# WIND×DRY PRODUCTION A+B+C MOOTOR
# =============================================================================
# A+B+C production-mootor ei kasuta vana app-128 championit otsustajana. A+B+C mootor on:
#   app-128 puhas weather-first baas + lukustatud LAB-154 tunnused
#   WIND×DRY HIGH L3–7 LEVEL+DELTA.
# Taimeindeksi loogika jääb samaks, kuid WIND×DRY saab oma puhta state/raw ajaloo ja alustab 1.00-st.
USE_WINDDRY_ENGINE = False  # legacy research path retained only for rollback/diagnostics
WD_MODEL_VERSION = "v6.5-winddry-leveldelta-production-4"
CYCLE_MODEL_VERSION = "v7.0-cycle-weather-production-1"
WD_WEATHER_START = date(TODAY.year, 7, 1)  # LAB-154 lukustatud ilma algus
WD_LOCKED_COLS = [
    "WD HIGH L3–7 päevad",
    "WD HIGH L3–7 jada",
...
def _wd_daily_value(row):
    if not row:
        return None
    wind = _wd_float(row.get("wind_avg_ms"))
    rh = _wd_float(row.get("humidity_avg_pct"))
    if wind is None or rh is None:
        return None
    return float(wind * (100.0 - rh))


def _wd_max_run(flags):
    best = cur = 0
    for flag in flags:
        if flag:
            cur += 1
...
def _wd_threshold(measured_map, before_day):
    vals = []
    for key, row in measured_map.items():
        dd = _wd_parse_day(key)
        if dd is None or dd >= before_day:
            continue
        # LAB-154 parity: lävi õpib ainult 01.07+ päriselt kontrollitud mõõdetud päevadest.
        if dd < WD_WEATHER_START:
            continue
        if str(row.get("data_kind") or "").strip().lower() != "measured" or not bool(row.get("checked")):
            continue
        v = _wd_daily_value(row)
        if v is not None:
            vals.append(v)
...
def _wd_l3_7_window(target_day, values_map, measured_threshold_map):
    days = [target_day - timedelta(days=lag) for lag in range(7, 2, -1)]
    vals = []
    kinds = []
    for dd in days:
        row = values_map.get(dd.isoformat()) if isinstance(values_map, dict) else None
        v = _wd_daily_value(row)
        if v is None:
            return None
        vals.append(v)
        kinds.append(str((row or {}).get("data_kind") or "?")[:1].upper())
    hi = _wd_threshold(measured_threshold_map, min(days))
    if hi is None:
        return None
    flags = [v >= hi for v in vals]
    return {
        "high_days": float(sum(flags)),
        "high_run": float(_wd_max_run(flags)),
        "threshold": float(hi),
...
def _wd_locked_features(target_day, previous_target_day, values_map, measured_threshold_map, allow_delta=True):
    cur = _wd_l3_7_window(target_day, values_map, measured_threshold_map)
    if cur is None:
        return {c: None for c in WD_LOCKED_COLS}
    prev = _wd_l3_7_window(previous_target_day, values_map, measured_threshold_map) if previous_target_day else None
    if allow_delta and prev is not None:
        d_days = float(cur["high_days"] - prev["high_days"])
        d_run = float(cur["high_run"] - prev["high_run"])
    else:
        d_days = 0.0
        d_run = 0.0
    return {
        "WD HIGH L3–7 päevad": float(cur["high_days"]),
        "WD HIGH L3–7 jada": float(cur["high_run"]),
        "Δ WD HIGH L3–7 päevad": d_days,
        "Δ WD HIGH L3–7 jada": d_run,
        "_WD threshold": float(cur["threshold"]),
        "_WD avg": float(cur["avg"]),
        "_WD kinds": cur["kinds"],
        "_WD window": cur["window"],
    }
...
def _ensure_active_winddry_engine() -> None:
    """One-time bootstrap for the official CYCLE + WEATHER production engine.

    The historical function name is kept only to avoid touching unrelated UI plumbing.
    """
    _engine_key = "active_forecast_engine"
    _current = db.get_app_setting(_engine_key, "")
    if _current != CYCLE_MODEL_VERSION:
        db.set_app_setting(_engine_key, CYCLE_MODEL_VERSION)
        _mark_forecast_dirty("KurgiMootor V7.0 · CYCLE + WEATHER kasutuselevõtt")


try:
    _ensure_active_winddry_engine()
...
def _basis_number(basis, key, default=None):
    """Loe snapshoti basis-tekstist masinloetav arvuline lisaväli."""
    try:
        m = re.search(rf"(?:^|;\s*){re.escape(str(key))}=([-+0-9.eE]+)", str(basis or ""))
        return float(m.group(1)) if m else default
    except (TypeError, ValueError):
        return default


def _row_plant_index(row):
    value = _basis_number((row or {}).get("basis"), "taimeindeks", 1.0)
    try:
...
def _combined_plant_index(rows):
    """Päeva ekvivalentne taimeindeks: korrigeeritud kogusaak / puhas mootoripotentsiaal."""
    adjusted = 0.0
    raw = 0.0
    seen = False
    for row in rows or []:
        try:
            total = float(row.get("total_forecast"))
        except (TypeError, ValueError):
            continue
        raw_total = _basis_number(row.get("basis"), "raw_total", None)
        if raw_total is None:
            idx = _row_plant_index(row)
            raw_total = total / idx if idx > 1e-9 else total
        try:
        ...
        raw += raw_total
        seen = True
    if not seen or raw <= 0:
        return 1.0
    return max(0.0, min(1.0, adjusted / raw))
...
def _history_integrity_strict_from():
    """Esimene päev, millest alates nõuame iga mineviku päeva kohta teadlikku plaani.

    Vana hooaja ajalugu võetakse kasutuselevõtu hetkel baasina ega hakata tagantjärele
    3-põllu eeldusega ümber hindama. Esmakordsel käivitamisel alustame eilsest, et
    just viimane teadmata päev saaks kohe kontrollitud.
    """
    key = "harvest_integrity_strict_from"
    raw = db.get_app_setting(key, "")
    try:
        return date.fromisoformat(str(raw)[:10])
    except Exception:
        value = TODAY - timedelta(days=1)
        db.set_app_setting(key, value.isoformat())
        return value


def _history_integrity_retro_days():
    """Vanad päevad, mida on pärast strict-gate kasutuselevõttu teadlikult muudetud."""
    raw = db.get_app_setting("harvest_integrity_retro_days", "[]")
    try:
...
def _set_history_integrity_retro_days(days):
    vals = sorted({d.isoformat() for d in days if isinstance(d, date)})
    db.set_app_setting("harvest_integrity_retro_days", json.dumps(vals))


def _ensure_retro_history_baseline(day):
    """Lukusta vana päeva praegune põllu-/järjekorraseis ENNE selle muutmist.

    Nii ei muutu vana, enne strict-gate'i kogutud 1- või 2-põlluline päev iseenesest
    veaks. Kui kasutaja seda päeva hiljem muudab, on muutmise-eelne struktuur baasiks
    ning kogemata kustutatud/lisatud põld peatab prognoosi kuni päeva parandamiseni.
    """
    try:
        dd = day if isinstance(day, date) else date.fromisoformat(str(day)[:10])
    except Exception:
        return
    if dd >= TODAY or dd >= _history_integrity_strict_from():
        return
    retro = _history_integrity_retro_days()
    if dd in retro:
        return
    rows = db.get_harvest_for_day(dd) or []
    ordered = sorted(rows, key=lambda r: (int(r.get("harvest_order") or 99), int(r.get("field_no") or 99)))
    baseline = [int(r.get("field_no")) for r in ordered if r.get("field_no") is not None]
    db.save_harvest_plan(dd, baseline)
    retro.add(dd)
    _set_history_integrity_retro_days(retro)


def _harvest_history_integrity_from(rows, plans, today, strict_from=None, retro_days=None):
    """Kontrolli ainult usaldusväärselt jälgitavat korjeajalugu.

    - Vana legacy-ajalugu enne strict_from kuupäeva EI hinnata 3-põllu eeldusega.
    - strict_from ... eile: igal päeval peab olema explicit plaan, ka [] = korjet ei olnud.
    - Vana päeva hilisemal muutmisel lisatakse see retro_days hulka ja selle päeva
      tegelikud korjeread peavad edaspidi explicit plaaniga täpselt klappima.
    - Tänast päeva ei blokeerita, sest korje võib olla alles töös.
    """
    today = today if isinstance(today, date) else date.fromisoformat(str(today)[:10])
    if strict_from is None:
        strict_from = today
    elif not isinstance(strict_from, date):
        strict_from = date.fromisoformat(str(strict_from)[:10])
    retro_days = set(retro_days or [])

    by_day = {}
    for row in rows or []:
        try:
            dd = row.get("harvest_date")
            dd = dd if isinstance(dd, date) else date.fromisoformat(str(dd)[:10])
            fno = int(row.get("field_no"))
            order = int(row.get("harvest_order") or 1)
        except (TypeError, ValueError):
            continue
        if dd >= today or not 1 <= fno <= 14:
            continue
        by_day.setdefault(dd, []).append((order, fno, row))

    norm_plans = {}
    for key, value in (plans or {}).items():
        try:
            dd = key if isinstance(key, date) else date.fromisoformat(str(key)[:10])
        except (TypeError, ValueError):
            continue
        if dd >= today:
            continue
        try:
            vals = [int(f) for f in list(value or [])]
        except TypeError:
            vals = [int(value)] if value is not None else []
        norm_plans[dd] = vals

    issues = []
    yesterday = today - timedelta(days=1)
    days_to_check = set()
    if strict_from <= yesterday:
        dd = strict_from
        while dd <= yesterday:
            days_to_check.add(dd)
            dd += timedelta(days=1)
    days_to_check.update(dd for dd in retro_days if dd < today)

    for dd in sorted(days_to_check):
        ordered = sorted(by_day.get(dd, []), key=lambda x: (x[0], x[1]))
        actual = [f for _, f, _ in ordered]
        if dd not in norm_plans:
            issues.append({
                "day": dd,
                "reason": "korjeandmed sisestamata",
                "expected": None,
                "actual": actual,
            })
            continue
        plan = list(norm_plans.get(dd) or [])
        if actual != plan:
            issues.append({
                "day": dd,
                "reason": "korjeandmed sisestamata" if plan else "korjeplaan ja saak ei klapi",
                "expected": plan,
                "actual": actual,
            })
    ...
    return {
        ...
        "tracking_start": strict_from,
    }


def _harvest_history_integrity():
    try:
        rows = db.get_harvest_history(limit=5000)
        plans = db.get_harvest_plans() or {}
        strict_from = _history_integrity_strict_from()
        retro_days = _history_integrity_retro_days()
        return _harvest_history_integrity_from(rows, plans, TODAY, strict_from, retro_days)
    except Exception as exc:
        # DB kontrolli enda viga ei tohi anda ekslikult rohelist prognoosi.
        return {
            "ok": False,
            "issues": [{"day": None, "reason": f"korjeajaloo kontroll ebaõnnestus: {exc}", "expected": None, "actual": []}],
            "first_issue": {"day": None, "reason": "korjeajaloo kontroll ebaõnnestus"},
            "tracking_start": None,
        }


def _history_gate_warning_text(gate):
    issue = (gate or {}).get("first_issue") or {}
    dd = issue.get("day")
    return f"⚠️ Korjeandmed sisestamata: {_dateday_label(dd)}" if dd else "⚠️ Korjeandmed sisestamata"


def _zero_forecast_rows(rows):
    """Kuvakiht: säilita snapshoti põllud/intervallid, kuid ära kuva ebausaldusväärset prognoosi."""
    out = []
    for row in rows or []:
        item = dict(row)
        for key in ("abc_forecast", "xl_forecast", "total_forecast", "cb_forecast"):
            item[key] = 0.0
        out.append(item)
    return out


def _history_rows_with_calendar_intervals(rows):
    """Return row copies with display interval = days since the same field's previous real harvest.

    This is a UI/history helper only. It does not write to DB and does not alter the
    CYCLE model's order-adjusted growth calculation.
    """
    copies = [dict(r) for r in (rows or [])]
    sortable = []
    for idx, r in enumerate(copies):
        try:
            dd = date.fromisoformat(str(r.get("harvest_date"))[:10])
            field_no = int(r.get("field_no"))
            order = int(r.get("harvest_order") or 99)
        except Exception:
            r["interval_days"] = None
            continue
        sortable.append((dd, order, field_no, idx))

    previous_by_field = {}
    for dd, order, field_no, idx in sorted(sortable):
        prev = previous_by_field.get(field_no)
        copies[idx]["interval_days"] = (dd - prev).days if prev is not None else None
        previous_by_field[field_no] = dd
    return copies
...
def _format_interval_value(value):
    try:
        if value is None or pd.isna(value):
            return "—"
        return f"{int(round(float(value)))} p"
    except Exception:
        return "—"
...
def _render_day_block(day_label, rows, show_quality=False, planned_fields=None):
    ...
    if planned_fields is None:
        field_count_text = f"{s['count']} põld" if s["count"] == 1 else f"{s['count']} põldu"
    else:
        expected_count = len(planned_fields)
        field_count_text = f"{s['count']}/{expected_count} põldu" if expected_count else "korjevaba päev"
    st.caption(
        f"A {_fmt(s['a'])} · B {_fmt(s['b'])} · C {_fmt(s['c'])} · "
        f"XL {_fmt(s['xl'])} · C/B {cb_text} · {field_count_text}"
    )

    df, missing_rows = _field_table(rows, planned_fields=planned_fields)

    formatters = {
        "Intervall": _format_interval_value,
...
_km_startup_mark("startup definitions ready")
...
_km_startup_mark("automatic weather check done")

# V7.0 production gate: prognoos on usaldusväärne ainult siis, kui KOGU mineviku
# korjeajalugu on lõpetatud. Esimene vigane kuupäev jääb ekraanile kuni parandamiseni.
_history_gate = _harvest_history_integrity()
_history_gate_ok = bool(_history_gate.get("ok"))
_history_gate_issue = _history_gate.get("first_issue") or {}
_history_gate_bad_day = _history_gate_issue.get("day")
_history_gate_warning = _history_gate_warning_text(_history_gate)
_km_startup_mark("harvest history gate done")

_hdr_logo, _hdr_title = st.columns([1, 7])
with _hdr_logo:
    st.image(str(APP_ICON_PATH), width=72)
with _hdr_title:
    st.markdown("## KurgiMootor V7.0")
    st.caption("Saagi ennustamise tööriist. Aktiivne mootor: CYCLE + WEATHER.")

page = st.radio(
    "Vaade",
    ["Täna", "Korjed", "Ilm", "Prognoos", "Mootor"],
    horizontal=True,
    label_visibility="collapsed",
    key="main_page",
)
_km_startup_mark("header + navigation painted")
...
if page == "Täna":
    ...
    _km_startup_mark("Today plan + harvest history loaded")
    ...
    if saved_today_plan != selected_today_fields:
        ...
        _mark_forecast_dirty(
            "tänane korjeplaan muutus: " + ",".join(str(f) for f in selected_today_fields)
        )
    ...
    if not _history_gate_ok:
        st.warning(_history_gate_warning)
    ...
    try:
        _home_saved_all = db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []
        _home_saved = [
            r for r in _home_saved_all
            if str(r.get("model_version") or "") == CYCLE_MODEL_VERSION
        ]
    except db.DatabaseError:
        _home_saved = []
    _km_startup_mark("Today forecast snapshots loaded")
    ...
    def _home_motor_accuracy_3p():
        """3P accuracy for audited A+B+C separately from total (A+B+C+XL)."""
        ...
        for hr in harvest_rows_home:
            try:
                ...
                abc = float(hr.get("a") or 0.0) + float(hr.get("b") or 0.0) + float(hr.get("c") or 0.0)
                total = float(hr.get("total"))
            except (TypeError, ValueError):
                continue
            if d > TODAY:
                continue
            actual_by_day.setdefault(d, {})[fno] = {"abc": abc, "total": total}
        ...
        for d, rows_for_day in actual_by_day.items():
            ...
            if saved_plan is not None:
                ...
                is_complete = len(rows_for_day) == 3 and len(set(rows_for_day.keys())) == 3
            if is_complete:
                complete_days.append(d)

        evaluated = []
        for d in sorted(complete_days, reverse=True):
            expected = set(actual_by_day[d].keys())
            actual_abc = sum(actual_by_day[d][f]["abc"] for f in expected)
            actual_total = sum(actual_by_day[d][f]["total"] for f in expected)
            if actual_abc <= 0 or actual_total <= 0:
                continue
            by_date = _home_snapshot_map(d)
            candidates = []
            for fdate, rows_for_date in by_date.items():
                if not fdate or fdate > d.isoformat() or set(rows_for_date.keys()) != expected:
                    continue
                try:
                    forecast_abc = sum(float(rows_for_date[f]["abc_forecast"]) for f in expected)
                    forecast_total = sum(float(rows_for_date[f]["total_forecast"]) for f in expected)
                except (TypeError, ValueError, KeyError):
                    continue
                candidates.append((fdate, forecast_abc, forecast_total))
            if not candidates:
                continue
            candidates.sort(key=lambda x: x[0])
            fdate, forecast_abc, forecast_total = candidates[-1]
            evaluated.append({
                "day": d,
                "abc_abs_pct_error": abs(forecast_abc / actual_abc - 1.0) * 100.0,
                "total_abs_pct_error": abs(forecast_total / actual_total - 1.0) * 100.0,
            })
            if len(evaluated) == 3:
                break
        ...
        abc_mean_err = float(np.mean([r["abc_abs_pct_error"] for r in evaluated]))
        total_mean_err = float(np.mean([r["total_abs_pct_error"] for r in evaluated]))
        abc_accuracy = max(0.0, min(100.0, 100.0 - abc_mean_err))
        total_accuracy = max(0.0, min(100.0, 100.0 - total_mean_err))
        return abc_accuracy, total_accuracy, evaluated
    ...
    # Päevakaart peab alati järgima PRAEGUST tööplaani. Lead=0 snapshot võib olla
    # ajaloo/no-hindsight jaoks lukus ning sisaldada enne plaanimuutust rohkem põlde;
    # kasutajale kuvatav päevaprognoos summeerib ainult hetkel valitud põllud.
    _today_plan_set = {int(f) for f in selected_today_fields}
    today_saved_rows = [
        r for r in today_saved_rows
        if int(r.get("field_no")) in _today_plan_set
    ] if _today_plan_set else []
    if not _history_gate_ok:
        today_saved_rows = _zero_forecast_rows(today_saved_rows)
    today_saved = _snapshot_totals(today_saved_rows)
    if not _history_gate_ok and today_saved is None:
        today_saved = {"total": 0.0, "abc": 0.0, "xl": 0.0, "cb": 0.0}
    with home_today_forecast_slot.container():
        if today_saved:
            adj_pct = None if not _history_gate_ok else _home_forecast_adjustment(TODAY, today_saved_rows)
            ...
            st.markdown(
                ...
                <div style="border:2px solid rgba(76,160,92,.38);border-radius:12px;padding:12px 14px;">
                  ...
                  <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;margin-top:3px;">
    ...
    motor_abc_accuracy_3p, motor_total_accuracy_3p, motor_accuracy_days = _home_motor_accuracy_3p()
    _km_startup_mark("Today accuracy block calculated")
    with home_engine_info_slot.container():
        if motor_abc_accuracy_3p is None:
            abc_accuracy_value = "—"
            total_accuracy_value = "—"
            accuracy_sub = f"{len(motor_accuracy_days)}/3 päeva"
        else:
            abc_accuracy_value = f"{motor_abc_accuracy_3p:.0f}%"
            total_accuracy_value = f"{motor_total_accuracy_3p:.0f}%"
        ...
        st.markdown(
            ...
            <div style="border:1px solid rgba(128,128,128,.24);border-radius:11px;
              ...
              <div style="font-size:.80rem;opacity:.62;margin-top:3px;">
                {accuracy_sub} · kokku koos XL-ga {total_accuracy_value}
              </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.caption(
            "A+B+C: CYCLE + WEATHER · XL: eelmise tsükli päevatootlikkus · "
            "C/B: viimase tegeliku korje suhe"
        )

    # Järgmised päevad – vana kaardidisain, andmed ainult snapshotidest.
    # Käsitsi salvestatud tulevikuplaan (ka []) on kuvamisel autoriteetne.
    try:
        _home_future_plans = db.get_harvest_plans() or {}
    except Exception:
        _home_future_plans = {}
    ...
    with home_future_forecast_slot.container():
        ...
        for lead in range(1, 10):
            ...
            _home_manual_plan = _home_future_plans.get(target_day.isoformat())
            _home_explicit_zero = False
            if _home_manual_plan is not None:
                _home_plan_list = [int(f) for f in list(_home_manual_plan)]
                if not _home_plan_list:
                    rows_day = []
                    _home_explicit_zero = True
                else:
                    _home_plan_set = set(_home_plan_list)
                    rows_day = [r for r in rows_day if int(r.get("field_no")) in _home_plan_set]
                    if {int(r.get("field_no")) for r in rows_day} != _home_plan_set:
                        rows_day = []
            if not _history_gate_ok:
                rows_day = _zero_forecast_rows(rows_day)

            if _home_explicit_zero:
                totals = {"total": 0.0, "abc": 0.0, "xl": 0.0, "cb": None}
            else:
            ...
            adj_pct = None if (not _history_gate_ok or _home_explicit_zero) else _home_forecast_adjustment(target_day, rows_day)
            adj_text = "—" if adj_pct is None else (
                "0%" if abs(adj_pct) < 0.5 else f"{'↑' if adj_pct > 0 else '↓'} {adj_pct:+.0f}%"
            )
            _plan_badge = " · plaan" if _home_manual_plan is not None else ""
            far_forecast = lead >= 6
            ...
            st.markdown(
                ...
                <div style="background:{bg};border:1px solid {border};border-radius:10px;
                  ...
                  <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;">
                    ...
                    <div style="text-align:right;">
                      <strong style="font-size:1.15rem;">{_fmt(totals['total'])} kasti · {adj_text}{_plan_badge}{badge}</strong>
                    </div>
                  </div>
    ...
    _km_startup_mark("Today page rendered")

elif page == "Korjed":
    st.subheader("Korjed")
    st.caption("Andmebaasis hoitakse iga põllu korje eraldi. Äpis vaatame saaki eelkõige päevade kaupa.")

    # Kumulatiivne saagivaade. Arvutatakse alati otse päris korjeridadest,
    # mitte eraldi käsitsi hoitavast koondist. Hooaeg algab SEASON_START-ist.
    _harvest_summary_rows = db.get_harvest_history(limit=5000) or []

    def _summary_row_day(_row):
        _value = _row.get("harvest_date")
        if isinstance(_value, datetime):
            return _value.date()
        if isinstance(_value, date):
            return _value
        try:
            return date.fromisoformat(str(_value)[:10])
        except Exception:
            return None

    def _summary_row_total(_row) -> float:
        _stored = _row.get("total")
        if _stored is not None:
            return max(0.0, _n(_stored))
        return max(0.0, sum(_n(_row.get(_k)) for _k in ("a", "b", "c", "xl")))
    ...
    _month_totals = {}
    _season_total = 0.0
    for _row in _harvest_summary_rows:
        _day = _summary_row_day(_row)
        if _day is None or _day > TODAY:
            continue
        _total = _summary_row_total(_row)
        if SEASON_START <= _day <= TODAY:
            _season_total += _total
            _month_key = (_day.year, _day.month)
            _month_totals[_month_key] = _month_totals.get(_month_key, 0.0) + _total

    _current_month_total = _month_totals.get((TODAY.year, TODAY.month), 0.0)

    _sum_c1, _sum_c2 = st.columns(2)
    _sum_c1.metric(
        f"{_month_names_et[TODAY.month - 1]} {TODAY.year}",
        f"{_fmt(_current_month_total)} kasti",
    )
    _sum_c2.metric(
        f"Hooaeg {TODAY.year}",
        f"{_fmt(_season_total)} kasti",
    )

    # Näita kõiki varasemaid aktiivse hooaja kuid, ka 0-saagiga kuud.
    _season_months = []
    _cursor = date(SEASON_START.year, SEASON_START.month, 1)
    while _cursor <= _month_start:
        _season_months.append(_cursor)
        if _cursor.month == 12:
            _cursor = date(_cursor.year + 1, 1, 1)
        else:
            _cursor = date(_cursor.year, _cursor.month + 1, 1)

    _previous_month_labels = []
    for _mday in reversed(_season_months[:-1]):
        _mtotal = _month_totals.get((_mday.year, _mday.month), 0.0)
        _previous_month_labels.append(
            f"{_month_names_et[_mday.month - 1]} {_mday.year}: {_fmt(_mtotal)} kasti"
        )
    if _previous_month_labels:
        st.caption(" · ".join(_previous_month_labels))
    ...
    planned_for_form = (
        ...
        if saved_plan_for_form is not None
    ...
    if not _history_gate_ok:
        st.warning(_history_gate_warning)
        if isinstance(_history_gate_bad_day, date):
            _focus_iso = _history_gate_bad_day.isoformat()
            if st.session_state.get("history_fix_focus_day") != _focus_iso:
                st.session_state["history_fix_focus_day"] = _focus_iso
                st.session_state["manual_harvest_date"] = _history_gate_bad_day
    else:
        st.session_state.pop("history_fix_focus_day", None)

    c1, c2, c3 = st.columns(3)
    _entry_default_day = _history_gate_bad_day if (not _history_gate_ok and isinstance(_history_gate_bad_day, date)) else TODAY
    entry_date = c1.date_input(
        "Kuupäev", value=_entry_default_day, max_value=TODAY, key="manual_harvest_date"
    )
    ...
    # Mineviku päeva saab samas kalendrivaates lõpuni parandada: muuta selle päeva
    # korjepõlde või märkida [] ehk korjet ei olnud. Esmane kuvamine EI salvesta midagi;
    # andmebaasi kirjutatakse alles siis, kui kasutaja valikut päriselt muudab.
    if entry_date < TODAY:
        try:
            _past_saved_plan = db.get_harvest_plan(entry_date)
        except Exception:
            _past_saved_plan = None
        if _past_saved_plan is not None:
            _past_plan_initial = [int(f) for f in _past_saved_plan]
        elif ordered_selected_rows:
            _past_plan_initial = [int(r.get("field_no")) for r in ordered_selected_rows if r.get("field_no") is not None]
        else:
            _past_plan_initial = _planned_fields_for_day(entry_date, selected_day_rows, history_for_form)

        _past_plan_key = f"history_plan_fields_{entry_date.isoformat()}"
        _past_base_key = f"history_plan_baseline_{entry_date.isoformat()}"
        if _past_plan_key not in st.session_state:
            st.session_state[_past_plan_key] = list(_past_plan_initial)
            st.session_state[_past_base_key] = list(_past_plan_initial)
        _past_selected_plan = st.multiselect(
            "Selle päeva korjepõllud",
            options=list(range(1, 15)),
            key=_past_plan_key,
            max_selections=4,
            help="Eemalda kõik põllud, kui sellel päeval korjet ei olnud.",
        )
        _past_baseline = [int(f) for f in st.session_state.get(_past_base_key, _past_plan_initial)]
        _past_selected_plan = [int(f) for f in _past_selected_plan]
        if _past_selected_plan != _past_baseline:
            _ensure_retro_history_baseline(entry_date)
            db.save_harvest_plan(entry_date, _past_selected_plan)
            st.session_state[_past_base_key] = list(_past_selected_plan)
            _mark_forecast_dirty(f"mineviku korjeplaan muudetud {entry_date}")
            st.rerun()
        elif _past_saved_plan is None and entry_date >= _history_integrity_strict_from():
            if st.button("Kinnita selle päeva põllud", key=f"confirm_history_plan_{entry_date.isoformat()}"):
                db.save_harvest_plan(entry_date, _past_selected_plan)
                _mark_forecast_dirty(f"mineviku korjeplaan kinnitatud {entry_date}")
                st.rerun()
    ...
    if existing_row:
        st.caption(
            f"Olemasolev korje · {entry_date.strftime('%d.%m.%Y')} · põld {entry_field}"
        )
        change_confirmed = st.checkbox(
            "Kinnitan olemasoleva korje muutmise või kustutamise",
            value=False,
            key=f"manual_change_confirm_{form_version}_{entry_date.isoformat()}_{entry_field}",
        )
        if st.button(
            "Kustuta korjerida",
            key=f"manual_delete_harvest_{form_version}_{entry_date.isoformat()}_{entry_field}",
            disabled=not change_confirmed,
        ):
            if entry_date < TODAY:
                _ensure_retro_history_baseline(entry_date)
            db.delete_harvest(entry_date, entry_field)

            # Kontrolli kohe otse päevapäringuga, et DB kustutus päriselt toimus.
            _after_delete_rows = db.get_harvest_for_day(entry_date) or []
            _still_exists = any(
                int(r.get("field_no") or r.get("field_id") or 0) == int(entry_field)
                for r in _after_delete_rows
            )
            if _still_exists:
                st.error(
                    f"Korjerida ei kustunud andmebaasist: {entry_date} · põld {entry_field}."
                )
            else:
    ...
    with st.form("manual_harvest_form"):
        ...
        submitted = st.form_submit_button(
            "Salvesta muudatus" if existing_row else "Salvesta korje"
        )
        ...
        if submitted:
            ...
            if entry_date > TODAY:
                ...
                st.warning("Olemasoleva korje muutmiseks märgi kinnitus.")
            elif total_preview <= 0:
                st.warning("0-korjet ei salvestata. Kustutamiseks kasuta eraldi nuppu „Kustuta korjerida“.")
            else:
                if entry_date < TODAY:
                    _ensure_retro_history_baseline(entry_date)
                db.save_harvest(
                ...
                # Mootoriotsing on lõpetatud. Täielik korjepäev märgib ainult
                # operatiivse CYCLE prognoosi dirty (see tehti juba ülal); eraldi
                # Jäljeotsijat/challenger-search'i enam ei käivitata.
    ...
    rows = _history_rows_with_calendar_intervals(db.get_harvest_history())
    if rows:
        by_day = {}
        for row in rows:
        ...
        for day_str, day_rows in by_day.items():
            ...
            if day_date == TODAY:
                ...
                _strict_from_for_display = _history_integrity_strict_from()
                _retro_for_display = _history_integrity_retro_days()
                if day_date >= _strict_from_for_display or day_date in _retro_for_display:
                    try:
                        _display_plan = db.get_harvest_plan(day_date)
                    except Exception:
                        _display_plan = None
                    if _display_plan is not None:
                        planned_fields = [int(f) for f in _display_plan]
...
_season_oos_run_now = False
# Kui kasutaja valib kihilise kandidaadi otse tabelireast, pannakse valik
# session_state'i ning järgmine rerun teeb kogu-hooaja OOS-i ainult sellele kandidaadile.
_season_oos_selected_parts = st.session_state.get("_season_oos_selected_parts_run")
...
if page == "Mootori tähelepanekud":
    ...
    # Kogu hooaja OOS audit on eraldi diagnostika. Aktiivse WIND×DRY benchmarki saab
    # arvutada oma nupust; challengerit EI valita enam automaatselt hulgi. Kasutaja saab
    # allpool kihiliste mudelite tabelist ühe rea ise valida ning testida ainult seda.
    _season_oos_clicked = st.button(
        "📊 Arvuta aktiivse WIND×DRY kogu hooaja OOS",
        key="layered_season_oos_run",
        help=(
            "Arvutab ainult aktiivse WIND×DRY fikseeritud kogu-hooaja strict walk-forward benchmarki. "
            "Challenger valitakse eraldi otse kihiliste mudelite tabelist."
        ),
    )
    if _season_oos_clicked:
        # Püsiv päringulipp: kui Streamlit teeb enne mudeliplokini jõudmist vahe-rerun'i,
        # ei kao kasutaja vajutus ära. Lipp eemaldatakse alles siis, kui arvutus päriselt algab.
        st.session_state["_season_oos_active_request"] = True
    _season_oos_run_now = bool(st.session_state.get("_season_oos_active_request"))
    st.caption(
        "Aktiivne WIND×DRY annab fikseeritud võrdluspunkti. Kui tahad challengerit kontrollida, "
        "vali allpool kihiliste mudelite tabelist üks rida ja vajuta `Testi valitud kandidaati`. "
        "Nii ei kuluta me CPU-d kõigi ideede peale."
    )

    # Aktiivse OOS tulemuse koht on fikseeritud KOHE nupu all.
    # Arvutus ise toimub allpool seal, kus production walk-forward objektid on juba olemas,
    # kuid st.empty() lubab sama jooksu lõpus tulemuse siia tagasi kirjutada. Nii ei sõltu
    # nähtav tulemus enam eraldi st.rerun()-ist ega jää "järjekorda" kinni.
    _season_oos_panel = st.empty()

    def _render_active_oos_panel(slot, active_data=None, error=None, running=False):
        with slot.container():
            if running:
                st.info("⏳ Arvutan aktiivse WIND×DRY kogu-hooaja strict OOS-i…")
            if error:
                st.error(str(error))
            if isinstance(active_data, dict) and active_data:
                st.success("Kogu hooaja OOS arvutus on valmis.")
                st.markdown("#### 🟢 Aktiivne WIND×DRY · kogu hooaja võrdluspunkt")
                _at1, _at2, _at3, _at4 = st.columns(4)
                _at1.metric("Põllu MAE", f"{float(active_data.get('row_mae')):.2f}" if active_data.get('row_mae') is not None else "—")
                _at2.metric("3 põllu päeva MAE", f"{float(active_data.get('daily_mae')):.2f}" if active_data.get('daily_mae') is not None else "—")
                _at3.metric("Viimase 5 päeva MAE", f"{float(active_data.get('daily_recent_mae')):.2f}" if active_data.get('daily_recent_mae') is not None else "—")
                _at4.metric("Täispäevi", str(int(active_data.get('daily_days') or 0)))

                _active_table = pd.DataFrame([{
                    "Mudel": "Aktiivne WIND×DRY",
                    "Põllu OOS MAE": active_data.get("row_mae"),
                    "3 põllu päeva MAE": active_data.get("daily_mae"),
                    "Viimase 5 päeva MAE": active_data.get("daily_recent_mae"),
                    "OOS ridu": int(active_data.get("rows") or 0),
                    "Täispäevi": int(active_data.get("daily_days") or 0),
                }])
                st.dataframe(
                    _active_table.style.format({
                        "Põllu OOS MAE": "{:.2f}",
                        "3 põllu päeva MAE": "{:.2f}",
                        "Viimase 5 päeva MAE": "{:.2f}",
                    }),
                    use_container_width=True, hide_index=True,
                )
                _at_recent_start = active_data.get("daily_recent_start")
                _at_recent_end = active_data.get("daily_recent_end")
                _at_recent_days = int(active_data.get("daily_recent_days") or 0)
                _at_we = active_data.get("daily_worst_error")
                _at_wd = active_data.get("daily_worst_day")
                _at_bits = []
                if active_data.get("daily_recent_mae") is not None:
                    _range_txt = ""
                    if _at_recent_start and _at_recent_end:
                        _range_txt = f" ({_at_recent_start}…{_at_recent_end})"
                    _at_bits.append(f"värske kontroll: viimased {_at_recent_days} täispäeva{_range_txt}")
                if _at_we is not None and _at_wd:
                    _at_bits.append(f"suurim päevaviga {float(_at_we):.1f} kasti ({_at_wd})")
                if _at_bits:
                    st.caption(" · ".join(_at_bits))

    _season_oos_saved_top = st.session_state.get("_layered_season_oos_last_result")
    if not isinstance(_season_oos_saved_top, dict) or not _season_oos_saved_top:
        _season_oos_saved_top = _load_json_setting("layered_season_oos_json", {})
    _active_top = (_season_oos_saved_top or {}).get("active_baseline") if isinstance(_season_oos_saved_top, dict) else None
    _active_oos_error = st.session_state.get("_season_oos_active_error")
    _render_active_oos_panel(
        _season_oos_panel,
        active_data=_active_top,
        error=_active_oos_error,
        running=bool(_season_oos_run_now),
    )
    ...
    _season_oos_saved = st.session_state.get("_layered_season_oos_last_result")
    if not isinstance(_season_oos_saved, dict) or not _season_oos_saved:
        _season_oos_saved = _load_json_setting("layered_season_oos_json", {})
    if isinstance(_season_oos_saved, dict) and (
        (_season_oos_saved.get("rows") or []) or _season_oos_saved.get("active_baseline")
    ):
        _active_baseline = _season_oos_saved.get("active_baseline") or {}
        st.markdown("### 📊 Kogu senine hooaeg · strict OOS")
        if _active_baseline:
            st.markdown("#### 🟢 Aktiivne WIND×DRY · fikseeritud võrdluspunkt")
            _ab1, _ab2, _ab3, _ab4 = st.columns(4)
            _ab1.metric("Põllu OOS MAE", f"{float(_active_baseline.get('row_mae')):.2f}" if _active_baseline.get('row_mae') is not None else "—")
            _ab2.metric("Päevakoondi MAE", f"{float(_active_baseline.get('daily_mae')):.2f}" if _active_baseline.get('daily_mae') is not None else "—")
            _ab3.metric("OOS ridu", str(int(_active_baseline.get('rows') or 0)))
            _ab4.metric("Täispäevi", str(int(_active_baseline.get('daily_days') or 0)))
            _recent_mae = _active_baseline.get("daily_recent_mae")
            _recent_days = int(_active_baseline.get("daily_recent_days") or 0)
            _recent_start = _active_baseline.get("daily_recent_start")
            _recent_end = _active_baseline.get("daily_recent_end")
            _we = _active_baseline.get("daily_worst_error")
            _wd = _active_baseline.get("daily_worst_day")
            st.caption(
                "See on aktiivse production-mootori enda kogu-hooaja benchmark, mitte ühe challengeri ühine alamvalim. "
                + (f"Värske kontroll: viimase {_recent_days} täispäeva MAE {float(_recent_mae):.2f}"
                   + (f" ({_recent_start}…{_recent_end})" if _recent_start and _recent_end else "") + ". "
                   if _recent_mae is not None else "")
                + (f"Suurim päevakoondi absoluutviga {float(_we):.1f} kasti ({_wd})." if _we is not None and _wd else "")
            )
            _pool_total = int(_season_oos_saved.get("candidate_pool_total") or 0)
            _pool_audited = int(_season_oos_saved.get("candidate_pool_audited") or 0)
            st.caption(
                f"Käsitsi kogu-hooaja OOS-is läbi testitud challengereid: {_pool_audited}/{_pool_total}. "
                "Vali järgmine kandidaat kohe siitsamast; korraga arvutatakse ainult üks."
            )

            # Näita viimatise käsitsi testitud challengeri tulemust KOHE benchmarki juures.
            # Nii ei pea kasutaja pärast 20 s arvutust kusagilt lehe alt tabelit otsima.
            _last_tested_name = str(_season_oos_saved.get("last_tested_candidate") or "")
            _saved_candidate_rows = list(_season_oos_saved.get("rows") or [])
            _last_tested_row = None
            if _last_tested_name:
                for _lr in _saved_candidate_rows:
                    if str(_lr.get("Koosseis") or "") == _last_tested_name:
                        _last_tested_row = _lr
                        break
            if _last_tested_row is None and _saved_candidate_rows:
                _last_tested_row = _saved_candidate_rows[-1]
                _last_tested_name = str(_last_tested_row.get("Koosseis") or "")

            if isinstance(_last_tested_row, dict) and _last_tested_row:
                st.success("Valitud challengeri kogu-hooaja OOS-test on valmis.")
                st.markdown("#### 🔵 Viimati testitud challenger")
                if _last_tested_name:
                    st.caption(_last_tested_name)
                _lrow1, _lrow2, _lrow3, _lrow4 = st.columns(4)
                _lrow1.metric(
                    "Põllu OOS MAE",
                    f"{float(_last_tested_row.get('oos_candidate_mae')):.2f}"
                    if _last_tested_row.get("oos_candidate_mae") is not None else "—",
                    delta=(
                        f"{float(_last_tested_row.get('oos_improvement')):+.2f} vs WD"
                        if _last_tested_row.get("oos_improvement") is not None else None
                    ),
                )
                _lrow2.metric(
                    "3 põllu päeva MAE",
                    f"{float(_last_tested_row.get('daily_candidate_mae')):.2f}"
                    if _last_tested_row.get("daily_candidate_mae") is not None else "—",
                    delta=(
                        f"{float(_last_tested_row.get('daily_improvement')):+.2f} vs WD"
                        if _last_tested_row.get("daily_improvement") is not None else None
                    ),
                )
                _lrow3.metric(
                    "Põlluridu võidab",
                    f"{float(_last_tested_row.get('oos_win_share')):.0f}%"
                    if _last_tested_row.get("oos_win_share") is not None else "—",
                )
                _lrow4.metric(
                    "Päevi võidab",
                    f"{float(_last_tested_row.get('daily_win_share')):.0f}%"
                    if _last_tested_row.get("daily_win_share") is not None else "—",
                )

            # Kandidaatide valik peab olema OOS võrdluspunkti JUURES, mitte lehe lõpus
            # diagnostika-expanderis. Laeme viimase kihilise uuringu tabeli otse DB-st,
            # et valik oleks nähtav ka siis, kui selles Streamliti jooksus laia uuringut ei käivitata.
            _top_layered_raw = db.get_app_setting("layered_research_trace_json", "")
            try:
                _top_layered_rows = json.loads(_top_layered_raw) if _top_layered_raw else []
            except Exception:
                _top_layered_rows = []
            _top_layered_df = pd.DataFrame(_top_layered_rows if isinstance(_top_layered_rows, list) else [])

            if not _top_layered_df.empty:
                # Säilita sama järjestus, mida detailtabel kasutab. Kui vanemas salvestuses
                # mõni sortimisveerg puudub, jääb lihtsalt DB järjekord alles.
                _sort_cols = [c for c in ["Stabiilne", "Paranemine", "Võidab ridu %"] if c in _top_layered_df.columns]
                if _sort_cols:
                    _asc_map = {"Stabiilne": False, "Paranemine": False, "Võidab ridu %": False}
                    _top_layered_df = _top_layered_df.sort_values(
                        _sort_cols, ascending=[_asc_map[c] for c in _sort_cols], na_position="last"
                    ).reset_index(drop=True)
                else:
                    _top_layered_df = _top_layered_df.reset_index(drop=True)

                st.markdown("#### 🎯 Vali challenger kogu-hooaja OOS-testiks")
                st.caption("Allpool on praeguse andmeseisu kihilised kandidaadid. Test ei muuda ametlikku mootorit.")

                _top_show = _top_layered_df.copy()
                _top_show.insert(0, "Rida", range(1, len(_top_show) + 1))
                _top_cols = [
                    c for c in [
                        "Rida", "Kihte", "Koosseis", "Paranemine", "Võidab ridu %",
                        "Kinnitus Δ", "Kinnitus võidab %", "Kinnitus stabiilne"
                    ] if c in _top_show.columns
                ]
                _top_fmt = {}
                if "Paranemine" in _top_cols:
                    _top_fmt["Paranemine"] = "{:+.2f}"
                if "Võidab ridu %" in _top_cols:
                    _top_fmt["Võidab ridu %"] = "{:.0f}%"
                if "Kinnitus Δ" in _top_cols:
                    _top_fmt["Kinnitus Δ"] = "{:+.2f}"
                if "Kinnitus võidab %" in _top_cols:
                    _top_fmt["Kinnitus võidab %"] = "{:.0f}%"
                st.dataframe(
                    _top_show[_top_cols].style.format(_top_fmt),
                    use_container_width=True, hide_index=True,
                )

                _top_choices = [0] + list(range(1, len(_top_layered_df) + 1))

                def _top_row_label(_row_no):
                    if int(_row_no) == 0:
                        return "— vali kandidaat —"
                    _rr = _top_layered_df.iloc[int(_row_no) - 1]
                    _parts = str(_rr.get("Koosseis") or "")
                    _short = _parts if len(_parts) <= 70 else (_parts[:67] + "…")
                    _imp = _rr.get("Paranemine")
                    _conf = _rr.get("Kinnitus Δ")
                    _imp_txt = f"{float(_imp):+.2f}" if pd.notna(_imp) else "—"
                    _conf_txt = f"{float(_conf):+.2f}" if pd.notna(_conf) else "—"
                    return f"Rida {int(_row_no)} · avastus {_imp_txt} · kinnitus {_conf_txt} · {_short}"

                _top_pick = st.selectbox(
                    "Vali rida",
                    options=_top_choices,
                    format_func=_top_row_label,
                    key="season_oos_candidate_top_pick",
                    help="Vali üks kandidaat. Kogu-hooaja strict OOS arvutatakse ainult sellele reale.",
                )
                if int(_top_pick) > 0:
                    _top_sel = _top_layered_df.iloc[int(_top_pick) - 1]
                    _top_parts = str(_top_sel.get("Koosseis") or "")
                    st.caption(f"Valitud: rida {int(_top_pick)} · {_top_parts}")
                    if st.button(
                        "📊 Testi valitud kandidaati kogu hooajal",
                        key="season_oos_candidate_top_run",
                        help="Võrdleb seda ühte kandidaati aktiivse WIND×DRY-ga kogu senise strict OOS ajaloo peal.",
                    ):
                        st.session_state["_season_oos_selected_parts_run"] = _top_parts
                        st.rerun()
            else:
                st.warning(
                    "Kihilise uuringu kandidaaditabelit ei leitud. Käivita üks lai uuring; "
                    "pärast seda ilmuvad kandidaadid siia automaatselt."
                )

        _season_rows = pd.DataFrame(_season_oos_saved.get("rows") or [])
        if not _season_rows.empty:
            _season_hday = str(_season_oos_saved.get("harvest_day") or "")
            _season_when = str(_season_oos_saved.get("updated_at") or "")
            st.caption(
                "See on see kontroll, mida otsisime: iga ajalooline korje ennustatakse ainult talle eelnenud "
                "andmetega. `OOS` on põllu MAE; `Päev` on kolme põllu päevakoondi MAE. Väiksem MAE on parem."
                + (f" Andmeseis kuni {_season_hday}." if _season_hday else "")
            )
            _season_rows = _season_rows.sort_values(
                ["daily_improvement", "oos_improvement"], ascending=[False, False], na_position="last"
            ).reset_index(drop=True)
            _sb = _season_rows.iloc[0]
            _sb_parts = str(_sb.get("Koosseis") or "")
            _sb_daily_imp = _sb.get("daily_improvement")
            _sb_oos_imp = _sb.get("oos_improvement")
            st.info(
                f"**Parim senise hooaja päevakoondis:** {_sb_parts} · "
                f"päev MAE {_sb.get('daily_base_mae'):.2f} → {_sb.get('daily_candidate_mae'):.2f} "
                f"(Δ {_sb_daily_imp:+.2f}); põlluread Δ {_sb_oos_imp:+.2f}."
            )
            _season_show = _season_rows.rename(columns={
                "oos_base_mae": "OOS baas MAE",
                "oos_candidate_mae": "OOS katse MAE",
                "oos_improvement": "OOS Δ",
                "oos_win_share": "OOS võidab %",
                "oos_rows": "OOS ridu",
                "daily_base_mae": "Päev baas MAE",
                "daily_candidate_mae": "Päev katse MAE",
                "daily_improvement": "Päev Δ",
                "daily_win_share": "Päev võidab %",
                "daily_days": "Päevi",
                "recent_base_mae": "Viimased 5 baas MAE",
                "recent_candidate_mae": "Viimased 5 katse MAE",
                "recent_improvement": "Viimased 5 Δ",
                "recent_win_share": "Viimased 5 võidab %",
                "recent_days": "Värskeid päevi",
            })
            _season_cols = [
                "Kihte", "Koosseis", "OOS baas MAE", "OOS katse MAE", "OOS Δ", "OOS võidab %", "OOS ridu",
                "Päev baas MAE", "Päev katse MAE", "Päev Δ", "Päev võidab %", "Päevi",
                "Viimased 5 baas MAE", "Viimased 5 katse MAE", "Viimased 5 Δ", "Viimased 5 võidab %", "Värskeid päevi",
                "Kinnitus stabiilne",
            ]
            _season_cols = [c for c in _season_cols if c in _season_show.columns]
            st.dataframe(
                _season_show[_season_cols].style.format({
                    "OOS baas MAE": "{:.2f}", "OOS katse MAE": "{:.2f}", "OOS Δ": "{:+.2f}",
                    "OOS võidab %": "{:.0f}%", "Päev baas MAE": "{:.2f}", "Päev katse MAE": "{:.2f}",
                    "Päev Δ": "{:+.2f}", "Päev võidab %": "{:.0f}%",
                    "Viimased 5 baas MAE": "{:.2f}", "Viimased 5 katse MAE": "{:.2f}",
                    "Viimased 5 Δ": "{:+.2f}", "Viimased 5 võidab %": "{:.0f}%",
                }),
                use_container_width=True, hide_index=True,
            )
            st.caption(
                "See tabel on diagnostika. Kogu-hooaja võit üksi ei vaheta championit; eraldi hilisem kinnitus jääb kaitseks alles."
            )
...
# Mootoriotsing/Jäljeotsija on productionis lõpetatud. Vana research-dirty lipp
# puhastatakse ära, kuid ükski korje ega ilm ei käivita enam challenger-search'i.
_auto_research_dirty = False
_research_dirty_reason = ""
_new_complete_harvest_trigger = False
try:
    if _model_is_dirty():
...
# Bootstrap guard: uue CYCLE production-versiooni esimene ametlik snapshot peab
# tekkima kohe ka siis, kui varasem forecast_dirty lipp on mõne eelmise ringi
# käigus ekslikult maha võetud. Prognoos-leht ise jääb tavaliselt read-only,
# kuid snapshot'i puudumine on päris operatiivne põhjus üheks arvutusringiks.
_cycle_snapshot_missing = False
try:
    if db.yield_forecasts_available():
        _boot_saved_forecasts = db.get_yield_forecasts(limit=5000)
        _cycle_snapshot_missing = not any(
            str(r.get("model_version") or "") == CYCLE_MODEL_VERSION
            for r in (_boot_saved_forecasts or [])
        )
except Exception:
    # DB lugemisvea korral ei tekita siin lõputut bootstrap-loopi; tavapärane
    # forecast_dirty mehhanism jääb endiselt tööle.
    _cycle_snapshot_missing = False

_forecast_refresh_due = bool(_forecast_is_dirty() or _cycle_snapshot_missing)
if _cycle_snapshot_missing:
    _forecast_dirty_reason = "CYCLE + WEATHER esimene ametlik snapshot"
else:
    _forecast_dirty_reason = db.get_app_setting("forecast_dirty_reason", "") if _forecast_refresh_due else ""
_km_startup_mark("forecast refresh decision done")
...
# Auditeeritud WD production-vastuvõtutest peab iga uue WD_MODEL_VERSION-i kohta
# vähemalt ühe korra päris production walk-forward koodiga läbi jooksma.
# See ei käivita Jäljeotsijat ega muuda championit; ainult arvutab/salvestab acceptance statistika.
_wd_acceptance_saved_boot = _load_json_setting("wd_production_acceptance_json", {})
_wd_acceptance_due = bool(
    page == "Mootori tähelepanekud"
    and USE_WINDDRY_ENGINE
    and (
        not isinstance(_wd_acceptance_saved_boot, dict)
        or str(_wd_acceptance_saved_boot.get("model_version") or "") != WD_MODEL_VERSION
    )
)

# Ühekordne production-shadow test: aktiivne WD vs sama WD + previous_order3.
# See EI muuda championit ega ametlikku prognoosi.
_ORDER_TEST_VERSION = "prev-order123-categorical-shadow-v1"
_order_acceptance_saved_boot = _load_json_setting("wd_prev_order_acceptance_json", {})
_order_acceptance_due = bool(
    page == "Mootori tähelepanekud"
    and USE_WINDDRY_ENGINE
    and (
        not isinstance(_order_acceptance_saved_boot, dict)
        or str(_order_acceptance_saved_boot.get("model_version") or "") != WD_MODEL_VERSION
        or str(_order_acceptance_saved_boot.get("test_version") or "") != _ORDER_TEST_VERSION
    )
)

# Järgmine shadow-test mõõdab hüpoteesi õiges lõppsihtmärgis:
# eelmise korje järjekord XL-mudelis ning selle mõju KOGU = ABC + XL prognoosile.
# Ametlikku prognoosi/championit see ei muuda.
_ORDER_TOTAL_TEST_VERSION = "prev-order123-xl-total-shadow-v1"
_order_total_saved_boot = _load_json_setting("wd_prev_order_xl_total_acceptance_json", {})
_order_total_due = bool(
    page == "Mootori tähelepanekud"
    and USE_WINDDRY_ENGINE
    and (
        not isinstance(_order_total_saved_boot, dict)
        or str(_order_total_saved_boot.get("model_version") or "") != WD_MODEL_VERSION
        or str(_order_total_saved_boot.get("test_version") or "") != _ORDER_TOTAL_TEST_VERSION
    )
)

# Ühekordne shadow-test: otsene B/C/XL compositional-arhitektuur.
# Kaks sama mudeliruumi log-suhet:
#   log(C/B) ja log(XL/B)
# annavad korraga ühe B-C-XL jaotuse. Eelmise korje järjekorda selles katses
# EI kasutata — kõigepealt testime ainult arhitektuuri ennast.
# Ametlikku prognoosi/championit see ei muuda.
_SIZE_DIST_TEST_VERSION = "direct-bcxl-alr-no-order-v1"
_size_dist_saved_boot = _load_json_setting("joint_size_distribution_shadow_json", {})
_size_dist_due = bool(
    page == "Mootori tähelepanekud"
    and USE_WINDDRY_ENGINE
    and (
        not isinstance(_size_dist_saved_boot, dict)
        or str(_size_dist_saved_boot.get("model_version") or "") != WD_MODEL_VERSION
        or str(_size_dist_saved_boot.get("test_version") or "") != _SIZE_DIST_TEST_VERSION
    )
)

# LAB-161 / production-shadow: kas eelmise korje järjekord seletab veel
# PRAEGUSE C/B + XL süsteemi allesjäänud viga? Uut mudelit ei ehitata.
# Primary residual = tegelik (C+XL) - prognoositud (C+XL).
# Lisaks kontrollime C/B ja XL residuaali eraldi ning teeme strict expanding
# residual-korrektsiooni ainult varasemate OOS vigade pealt.
_RESIDUAL_ORDER_TEST_VERSION = "prev-order-current-size-residual-v1"
_residual_order_saved_boot = _load_json_setting("wd_prev_order_residual_json", {})
_residual_order_due = bool(
    page == "Mootori tähelepanekud"
    and USE_WINDDRY_ENGINE
    and (
        not isinstance(_residual_order_saved_boot, dict)
        or str(_residual_order_saved_boot.get("model_version") or "") != WD_MODEL_VERSION
        or str(_residual_order_saved_boot.get("test_version") or "") != _RESIDUAL_ORDER_TEST_VERSION
    )
)
...
# =============================================================================
# FAST CYCLE + WEATHER FORECAST PATH
# =============================================================================
# Vana Jäljeotsija/champion walk-forward plokk on failis arhiivina alles, kuid seda
# ei käivitata enam operatiivse prognoosi jaoks. See funktsioon sisaldab sama
# LAB44–47 lukustatud CYCLE matemaatikat ja teeb ainult selle töö, mida production
# prognoos tegelikult vajab.
def _run_cycle_weather_snapshot_fast():
    # CYCLE + WEATHER PRODUCTION ENGINE (LAB44–47 frozen architecture)
    # -----------------------------------------------------------------
    # Official A+B+C forecast is no longer produced by the legacy champion,
    # plant-index or WIND×DRY correction stack.  The engine predicts the
    # cycle-to-cycle change of DAILY A+B+C productivity for each field.
    #
    # Frozen architecture:
    #   rate_t = ABC_t / order-adjusted growth_days_t
    #   y_t    = log(rate_t / rate_(t-1))
    #   target features = cycle-vs-previous-cycle weather deltas,
    #                     d_growth, previous log-rate and season age
    #   ridge lambda = 8, min train rows = 24
    #
    # Historical fit uses only checked measured weather.  An operational
    # future cycle uses checked measured days where available and forecast
    # weather for the still-future days.  The target harvest day itself is
    # excluded from weather exactly as in the audited LAB architecture.
    _CE_HOURS_PER_FIELD = 3.0
    _CE_ABC_EPS = 0.20
    _CE_RIDGE_LAMBDA = 8.0
    _CE_MIN_TRAIN_ROWS = 24
    _CE_FEATURES = [
        "d_growth", "d_rad", "d_tmean", "d_tmin", "d_winddry",
        "d_precip", "d_et0", "prev_log_rate", "season_day",
    ]
    _CE_WEATHER_KEYS = ("rad", "tmean", "tmin", "winddry", "precip", "et0")
    ...
    def _ce_event_from_row(row):
        dd = _ce_day(row.get("harvest_date"))
        if dd is None or not _ce_reliable(row):
            return None
        try:
            field_no = int(row.get("field_no"))
        except Exception:
            return None
        if not 1 <= field_no <= 14:
            return None
        vals = [_ce_float(row.get(k)) for k in ("a", "b", "c")]
        ...
        try:
            ...
            order = 1
        abc = float(sum(vals))
        xl = _ce_float(row.get("xl"))
        b = vals[1]
        c = vals[2]
        return {
            "day": dd, "field": field_no, "order": order,
            "abc": max(0.0, abc), "xl": max(0.0, float(xl or 0.0)),
            "cb": (float(c) / float(b)) if b and b > 0 else None,
        }

    harvest_rows = db.get_harvest_history(limit=2000)
    _ce_events_all = []
    for _ce_hr in harvest_rows:
        _ce_ev = _ce_event_from_row(_ce_hr)
        if _ce_ev is not None:
            _ce_events_all.append(_ce_ev)
    _ce_events_all.sort(key=lambda e: (e["day"], e["order"], e["field"]))
    _ce_season_origin = min((e["day"] for e in _ce_events_all), default=SEASON_START)

    def _ce_growth(prev_ev, cur_day, cur_order):
        g = float((cur_day - prev_ev["day"]).days)
        g += (int(cur_order) - int(prev_ev["order"])) * (_CE_HOURS_PER_FIELD / 24.0)
        return max(0.5, float(g))

    _ce_weather_start = min((e["day"] for e in _ce_events_all), default=SEASON_START) - timedelta(days=1)
    _ce_weather_end = TODAY + timedelta(days=9)
    _ce_weather_rows = db.get_weather_rows(_ce_weather_start, _ce_weather_end)

    def _ce_weather_value(row):
        tmin = _ce_float(row.get("temp_min_c"))
        tmax = _ce_float(row.get("temp_max_c"))
        wind = _ce_float(row.get("wind_avg_ms"))
        rad = _ce_float(row.get("radiation_mj_m2"))
        rh = _ce_float(row.get("humidity_avg_pct"))
        precip = _ce_float(row.get("precipitation_mm"))
        et0 = _ce_float(row.get("et0_mm"))
    ...
    _ce_measured_weather = {}
    _ce_operational_weather = {}
    _ce_operational_kind = {}
    for _ce_wr in _ce_weather_rows:
        _ce_dd = _ce_day(_ce_wr.get("weather_date"))
        _ce_wv = _ce_weather_value(_ce_wr)
        if _ce_dd is None or _ce_wv is None:
            continue
        _ce_kind = str(_ce_wr.get("data_kind") or "").strip().lower()
        _ce_checked_measured = _ce_kind == "measured" and bool(_ce_wr.get("checked"))
        if _ce_checked_measured:
            # Historical model training remains exactly LAB47: only checked measured weather.
            _ce_measured_weather[_ce_dd] = _ce_wv
            _ce_operational_weather[_ce_dd] = _ce_wv
            _ce_operational_kind[_ce_dd] = "measured"
        elif _ce_kind == "measured" and _ce_dd not in _ce_operational_weather:
            # Operational forecast must not disappear merely because yesterday's
            # official Pärnu components are not checked yet.  A complete temporary
            # measured row is allowed for NOW/FUTURE prediction only; it never enters
            # the historical training map above.
            _ce_operational_weather[_ce_dd] = _ce_wv
            _ce_operational_kind[_ce_dd] = "measured-ajutine"
        elif _ce_kind == "forecast" and _ce_dd not in _ce_operational_weather:
            _ce_operational_weather[_ce_dd] = _ce_wv
            _ce_operational_kind[_ce_dd] = "forecast"

    # weather_daily has one row per calendar day.  When a new measured row
    # replaces that day's forecast, the previous forecast still exists in
    # app_settings as weather_forecast_snapshot_YYYY-MM-DD.  This matters
    # especially on the morning after a day whose official measured data is
    # not complete yet.  Fill only operational gaps from those locked snapshots;
    # historical training stays checked-measured-only and therefore unchanged.
    if hasattr(db, "get_weather_forecast_snapshot"):
        _ce_snap_start = max(_ce_weather_start, TODAY - timedelta(days=14))
        _ce_snap_end = TODAY + timedelta(days=9)
        _ce_snap_day = _ce_snap_start
        while _ce_snap_day <= _ce_snap_end:
            if _ce_snap_day not in _ce_operational_weather:
                try:
                    _ce_snap = db.get_weather_forecast_snapshot(_ce_snap_day)
                except Exception:
                    _ce_snap = None
                if isinstance(_ce_snap, dict):
                    _ce_snap_wv = _ce_weather_value(_ce_snap)
                    if _ce_snap_wv is not None:
                        _ce_operational_weather[_ce_snap_day] = _ce_snap_wv
                        _ce_operational_kind[_ce_snap_day] = "forecast-snapshot"
            _ce_snap_day += timedelta(days=1)

    def _ce_cycle_weather(start_day, target_day, weather_map, kind_map=None):
        n = (target_day - start_day).days
        if n <= 0:
            return None
        days = [start_day + timedelta(days=k) for k in range(n)]
        if any(dd not in weather_map for dd in days):
            return None
        out = {}
        for k in _CE_WEATHER_KEYS:
            vals = np.asarray([weather_map[dd][k] for dd in days], dtype=float)
            out[k] = float(np.mean(vals))
        # Extra raw summaries are only for transparent UI diagnostics.
        out["_days"] = days
        out["_estimated_days"] = {
            dd for dd in days
            if kind_map is not None and str(kind_map.get(dd) or "") != "measured"
        }
        out["_tmax_mean"] = float(np.mean([weather_map[dd]["tmax"] for dd in days]))
        out["_tmax_max"] = float(np.max([weather_map[dd]["tmax"] for dd in days]))
        out["_tmin_min"] = float(np.min([weather_map[dd]["tmin"] for dd in days]))
        out["_rh_mean"] = float(np.mean([weather_map[dd]["rh"] for dd in days]))
        out["_rad_sum"] = float(np.sum([weather_map[dd]["rad"] for dd in days]))
        out["_precip_sum"] = float(np.sum([weather_map[dd]["precip"] for dd in days]))
        out["_et0_sum"] = float(np.sum([weather_map[dd]["et0"] for dd in days]))
        out["_warm16"] = int(np.sum([weather_map[dd]["tmin"] >= 16.0 for dd in days]))
        return out

    def _ce_training_frame():
        rows = []
        for field_no in range(1, 15):
            hist = sorted(
                [e for e in _ce_events_all if e["field"] == field_no],
                key=lambda e: (e["day"], e["order"]),
            )
            intervals = []
            for i in range(1, len(hist)):
                ...
                if c["day"] <= p["day"]:
                    continue
                growth = _ce_growth(p, c["day"], c["order"])
                wx = _ce_cycle_weather(p["day"], c["day"], _ce_measured_weather)
                if wx is None:
                    continue
                rate = max(_CE_ABC_EPS, float(c["abc"])) / growth
                intervals.append({
                    "field": field_no,
                    "start_date": p["day"],
                    "target_date": c["day"],
                    ...
                    "growth": float(growth),
                    "rate": float(rate),
                    **{k: float(wx[k]) for k in _CE_WEATHER_KEYS},
                })
            for i in range(1, len(intervals)):
                ...
                if prev["target_date"] != cur["start_date"]:
                    continue
                prev_rate = max(_CE_ABC_EPS, float(prev["rate"]))
                cur_rate = max(_CE_ABC_EPS, float(cur["rate"]))
                row = {
                    "field": int(field_no),
                    ...
                    "season_day": float((cur["target_date"] - _ce_season_origin).days),
                }
                for k in _CE_WEATHER_KEYS:
        ...
        if not df.empty:
            df = df.sort_values(["target_date", "field"]).reset_index(drop=True)
        return df

    _ce_training = _ce_training_frame()

    def _ce_fit_for(target_day):
        if _ce_training.empty:
            return None
        train = _ce_training[_ce_training["target_date"] < target_day].copy()
        if len(train) < _CE_MIN_TRAIN_ROWS:
            return None
        X = train[_CE_FEATURES].to_numpy(dtype=float)
        ...
        penalty = np.eye(A.shape[1], dtype=float) * _CE_RIDGE_LAMBDA
        penalty[0, 0] = 0.0
        beta = np.linalg.solve(A.T @ A + penalty, A.T @ y)
        return {"mu": mu, "sd": sd, "beta": beta, "n": int(len(train))}

    def _ce_actual_state(field_no, before_day=None, include_day=True):
        hist = [e for e in _ce_events_all if e["field"] == int(field_no)]
        if before_day is not None:
            if include_day:
                hist = [e for e in hist if e["day"] <= before_day]
            else:
                hist = [e for e in hist if e["day"] < before_day]
        hist = sorted(hist, key=lambda e: (e["day"], e["order"]))
        if len(hist) < 2:
            return None
        p, c = hist[-2], hist[-1]
        growth = _ce_growth(p, c["day"], c["order"])
        # Operational state must survive the morning after a harvest. Historical
        # TRAINING remains checked-measured-only, but the latest actual field state
        # may need yesterday's complete temporary measured row or the locked forecast
        # snapshot until official checked weather arrives. Otherwise a freshly harvested
        # field disappears from its next rotation even though the harvest itself is known.
        wx = _ce_cycle_weather(
            p["day"], c["day"],
            _ce_operational_weather, _ce_operational_kind,
        )
        if wx is None or growth <= 0:
            return None
        return {
            "start_day": c["day"],
            "start_order": int(c["order"]),
            "prev_rate": max(_CE_ABC_EPS, float(c["abc"])) / float(growth),
            "prev_growth": float(growth),
            "prev_weather": {k: float(wx[k]) for k in _CE_WEATHER_KEYS},
            "xl_rate": max(0.0, float(c.get("xl") or 0.0)) / float(growth),
            "cb": c.get("cb"),
            "source": "tegelik",
        }

    def _ce_explain(persist, prediction, fit, feature_values):
        # Exact multiplicative decomposition in a fixed order.  Effects are
        # reported in boxes so the displayed pieces reconstruct prediction.
        x = np.asarray([feature_values[k] for k in _CE_FEATURES], dtype=float)
        z = (x - fit["mu"]) / fit["sd"]
        contrib = {k: float(fit["beta"][i + 1] * z[i]) for i, k in enumerate(_CE_FEATURES)}
        baseline = float(persist * math.exp(float(fit["beta"][0])))
        groups = [
            ("Intervall", ["d_growth"]),
            ("Radiatsioon", ["d_rad"]),
            ("Temperatuur", ["d_tmean", "d_tmin"]),
            ("Tuul", ["d_winddry"]),
            ("Sademed", ["d_precip"]),
            ("ET0", ["d_et0"]),
            ("Eelmine tootlikkus", ["prev_log_rate"]),
            ("Hooaeg", ["season_day"]),
        ]
        effects = {}
        cur = baseline
        for label, cols in groups:
            nxt = cur * math.exp(sum(contrib.get(c, 0.0) for c in cols))
            effects[label] = float(nxt - cur)
            cur = nxt
        # Numerical roundoff only; keep exact final reconstruction.
        if abs(cur - prediction) > 1e-8:
            effects["Muu"] = float(prediction - cur)
        return {
    ...
    def _ce_predict(field_no, state, target_day, target_order):
        if not state:
            return None
        fit = _ce_fit_for(target_day)
        if fit is None:
            return None
        cur_wx = _ce_cycle_weather(
            state["start_day"], target_day,
            _ce_operational_weather, _ce_operational_kind,
        )
        if cur_wx is None:
            return None
        growth = float((target_day - state["start_day"]).days)
        growth += (int(target_order) - int(state["start_order"])) * (_CE_HOURS_PER_FIELD / 24.0)
        growth = max(0.5, growth)
        prev_rate = max(_CE_ABC_EPS, float(state["prev_rate"]))
        feature_values = {
            "d_growth": float(growth - float(state["prev_growth"])),
            "prev_log_rate": float(math.log(prev_rate)),
            "season_day": float((target_day - _ce_season_origin).days),
        }
        for k in _CE_WEATHER_KEYS:
            feature_values[f"d_{k}"] = float(cur_wx[k] - float(state["prev_weather"][k]))
        x = np.asarray([feature_values[k] for k in _CE_FEATURES], dtype=float)
        z = (x - fit["mu"]) / fit["sd"]
        yhat = float(fit["beta"][0] + np.dot(z, fit["beta"][1:]))
        # Frozen LAB safety rail: not a tuned production cap.
        yhat = float(np.clip(yhat, -1.5, 1.5))
        persist = float(prev_rate * growth)
        abc_pred = float(max(0.0, persist * math.exp(yhat)))

        # XL and C/B are deliberately transparent persistence auxiliaries.
        # They do not feed back into the audited A+B+C engine.
        xl_pred = float(max(0.0, float(state.get("xl_rate") or 0.0) * growth))
        cb_pred = state.get("cb")
        try:
            cb_pred = float(cb_pred) if cb_pred is not None else None
        except (TypeError, ValueError):
            cb_pred = None
        total_pred = float(abc_pred + xl_pred)

        explain = _ce_explain(persist, abc_pred, fit, feature_values)
        wx_summary = {
            "Tmin kesk": float(cur_wx["tmin"]),
            "Tmin min": float(cur_wx.get("_tmin_min", cur_wx["tmin"])),
            "Tmax kesk": float(cur_wx.get("_tmax_mean", cur_wx["tmean"])),
            "Tmax max": float(cur_wx.get("_tmax_max", cur_wx["tmean"])),
            "Soojad ööd 16+": float(cur_wx.get("_warm16", 0)),
            "Intervall p": float((target_day - state["start_day"]).days),
            "Radiatsioon Σ": float(cur_wx.get("_rad_sum", cur_wx["rad"])),
            "Radiatsioon/p": float(cur_wx["rad"]),
            "Sademed Σ": float(cur_wx.get("_precip_sum", cur_wx["precip"])),
            "Niiskus kesk": float(cur_wx.get("_rh_mean", 0.0)),
            "ET0 Σ": float(cur_wx.get("_et0_sum", cur_wx["et0"])),
            "Päevapikkus": None,
            "Päevapikkus Δ7p": None,
        }
        return {
            "abc": abc_pred,
            "xl": xl_pred,
            "cb": cb_pred,
            "total": total_pred,
            "interval": int((target_day - state["start_day"]).days),
            "growth": float(growth),
            "estimated_days": set(cur_wx.get("_estimated_days") or set()),
            "abc_explain": explain,
            "wx": wx_summary,
            "cycle_weather": {k: float(cur_wx[k]) for k in _CE_WEATHER_KEYS},
            "rate": float(abc_pred / growth),
            "xl_rate": float(xl_pred / growth),
            "train_n": int(fit["n"]),
            "source": state.get("source") or "tegelik",
        }

    def _ce_missing_reason(state, target_day):
        if not state:
            return "eelkorje state puudub"
        fit = _ce_fit_for(target_day)
        if fit is None:
            n = 0 if _ce_training.empty else int((_ce_training["target_date"] < target_day).sum())
            return f"õppimisridu {n}/{_CE_MIN_TRAIN_ROWS}"
        n = (target_day - state["start_day"]).days
        if n <= 0:
            return "vigane korjeintervall"
        days = [state["start_day"] + timedelta(days=k) for k in range(n)]
        missing = [dd for dd in days if dd not in _ce_operational_weather]
        if missing:
            return "ilm puudub: " + ", ".join(dd.strftime("%d.%m") for dd in missing)
        return "prognoos ei arvutunud"

    # Today's plan is predicted strictly from state that existed before TODAY.
    ...
    try:
        _saved_today_plan_fast = db.get_harvest_plan(TODAY)
    except Exception:
        _saved_today_plan_fast = None
    selected_home = st.session_state.get("home_today_fields")
    if _saved_today_plan_fast is not None:
        today_plan = [int(f) for f in list(_saved_today_plan_fast)[:4]]
    elif selected_home is not None:
        today_plan = [int(f) for f in list(selected_home)[:4]]
    else:
        today_plan = [int(f) for f in today_plan_default]
    today_actual = {
        int(r.get("field_no")): r for r in today_rows_live
        if r.get("field_no") is not None
    }
    ...
    internal_today = []
    for _ce_pos, f in enumerate(today_plan, start=1):
        prev_state = _ce_actual_state(int(f), TODAY, include_day=False)
        pred = _ce_predict(int(f), prev_state, TODAY, _ce_pos)
        if pred is None:
            today_forecast_rows.append({
                "Põld": int(f), "Taimeindeks": 1.0, "A+B+C": None,
                "C/B": None, "XL": None, "Kokku": None, "Intervall": None,
                "Alus": "CYCLE · " + _ce_missing_reason(prev_state, TODAY), "Hinnanguline ilm": "—",
            })
            continue
        today_predictions_by_field[int(f)] = pred
        _est_txt = ", ".join(sorted(d.strftime("%d.%m") for d in pred["estimated_days"])) or "—"
        today_forecast_rows.append({
            "Põld": int(f), "Taimeindeks": 1.0,
            "A+B+C": pred["abc"], "C/B": pred["cb"], "XL": pred["xl"], "Kokku": pred["total"],
            "Intervall": pred["interval"],
            "Alus": f"CYCLE+WEATHER · tegelik eelkorje · train N={pred['train_n']}",
            "Hinnanguline ilm": _est_txt,
            "_Raw ABC": pred["abc"], "_Raw XL": pred["xl"], "_Raw Kokku": pred["total"],
            "_ABC_selgitus": pred["abc_explain"], "_WX": pred["wx"],
        })

    # Future runtime starts from the latest ACTUAL state available now.
    # If today's planned field is not yet harvested, today's locked forecast
    # becomes only that field's temporary state for recursive +1…+9 planning.
    _ce_runtime_state = {f: _ce_actual_state(f, TODAY, include_day=True) for f in range(1, 15)}
    for _ce_pos, f in enumerate(today_plan, start=1):
        if int(f) in today_actual:
            _ce_runtime_state[int(f)] = _ce_actual_state(int(f), TODAY, include_day=True)
            continue
        pred = today_predictions_by_field.get(int(f))
        old_state = _ce_actual_state(int(f), TODAY, include_day=False)
        if pred is None or old_state is None:
            continue
        _ce_runtime_state[int(f)] = {
            "start_day": TODAY,
            "start_order": int(_ce_pos),
            "prev_rate": float(pred["rate"]),
            "prev_growth": float(pred["growth"]),
            "prev_weather": dict(pred["cycle_weather"]),
            "xl_rate": float(pred["xl_rate"]),
            "cb": pred.get("cb"),
            "source": "prognoos",
        }

    # Future work plans are authoritative for the FORECAST SIMULATION only.
    # Missing saved plan => default 3-field proposal from the current simulated cursor.
    # Saved [] => no harvest; simulated rotation does not advance.
    # Saved 1..4 fields => predict exactly those fields in that order and continue the
    # simulated cursor after the last planned field. ACTUAL crop state/rotation still
    # changes only when real harvest rows are entered.
    try:
        _ce_saved_future_plans = db.get_harvest_plans() or {}
    except Exception:
        _ce_saved_future_plans = {}

    forecast_days = []
    if today_plan:
        current_first = _next_field(int(today_plan[-1]))
    else:
        _past = [e for e in _ce_events_all if e["day"] <= TODAY]
        current_first = _next_field(int(_past[-1]["field"])) if _past else 1
    any_weather_imputation = set()

    for offset in range(1, 10):
        target_day = TODAY + timedelta(days=offset)

        _saved_future_plan = _ce_saved_future_plans.get(target_day.isoformat())
        if _saved_future_plan is not None:
            try:
                _saved_future_plan_list = [int(f) for f in list(_saved_future_plan)[:4]]
            except TypeError:
                _saved_future_plan_list = [int(_saved_future_plan)]
            day_fields = _saved_future_plan_list
        else:
            day_fields = [
                int(current_first),
                int(_next_field(current_first)),
                int(_next_field(_next_field(current_first))),
            ]

        if not day_fields:
            forecast_days.append((target_day, []))
            continue

        day_rows = []
        for target_order, f in enumerate(day_fields, start=1):
            state = _ce_runtime_state.get(int(f))
            pred = _ce_predict(int(f), state, target_day, target_order)
            if pred is None:
                day_rows.append({
                    "Põld": int(f), "Taimeindeks": 1.0, "A+B+C": None,
                    "C/B": None, "XL": None, "Kokku": None,
                    "Intervall": (target_day - state["start_day"]).days if state else None,
                    "Alus": "CYCLE · " + _ce_missing_reason(state, target_day), "Hinnanguline ilm": "puudub",
                })
                continue
            any_weather_imputation.update(pred["estimated_days"])
            source_label = "tegelik eelkorje" if state.get("source") == "tegelik" else "prognoositud eelkorje"
            _est_txt = ", ".join(sorted(d.strftime("%d.%m") for d in pred["estimated_days"])) or "—"
            day_rows.append({
                "Põld": int(f), "Taimeindeks": 1.0,
                "A+B+C": pred["abc"], "C/B": pred["cb"], "XL": pred["xl"], "Kokku": pred["total"],
                "Intervall": pred["interval"],
                "Alus": f"CYCLE+WEATHER · {source_label} · train N={pred['train_n']}",
                "Hinnanguline ilm": _est_txt,
                "_Raw ABC": pred["abc"], "_Raw XL": pred["xl"], "_Raw Kokku": pred["total"],
                "_ABC_selgitus": pred["abc_explain"], "_WX": pred["wx"],
            })
        ...
        if day_fields:
            current_first = _next_field(int(day_fields[-1]))
    ...
    # Uuel production-mootoril on oma model_version, et vana app-128 ajalugu ei seguneks aktiivse prognoosiga.
    ...
    if not today_rows_live:
        for row in today_forecast_rows:
            ...
            forecast_payloads.append({
                ...
                "basis": (
                    f"{row.get('Alus') or 'CYCLE+WEATHER'}; "
                    f"abc_engine=cycle_productivity; xl_engine=previous_daily_productivity; "
                    f"cb_engine=previous_ratio; raw_total={float(row.get('_Raw Kokku') or row.get('Kokku')):.6f}"
                ),
    ...
    # Production completeness guard: a partially computed 9-day forecast may be
    # persisted for diagnostics, but it must NOT clear forecast_dirty. Otherwise one
    # missing field can silently disappear until some unrelated input changes.
    missing_forecasts = []
    if not today_rows_live:
        for row in today_forecast_rows:
            if row.get("A+B+C") is None or row.get("XL") is None or row.get("Kokku") is None:
                missing_forecasts.append(f"{TODAY.strftime('%d.%m')} põld {row.get('Põld')}: {row.get('Alus') or 'prognoos puudub'}")
    for target_day, rows_day in forecast_days:
        for row in rows_day:
            if row.get("A+B+C") is None or row.get("XL") is None or row.get("Kokku") is None:
                missing_forecasts.append(f"{target_day.strftime('%d.%m')} põld {row.get('Põld')}: {row.get('Alus') or 'prognoos puudub'}")

    try:
        _cycle_fit_next = _ce_fit_for(TODAY + timedelta(days=1))
        db.set_app_setting(
            "cycle_last_training_rows",
            str(int(_cycle_fit_next.get("n", 0))) if _cycle_fit_next else "0",
        )
    except Exception:
        pass

    # Number of forecast rows that the current work plan actually asks for.
    # This may legitimately be zero at season end when all future days are explicit no-harvest days.
    _expected_forecast_rows = 0
    if not today_rows_live:
        _expected_forecast_rows += len(today_forecast_rows)
    _expected_forecast_rows += sum(len(rows_day) for _, rows_day in forecast_days)
    ...
    try:
        if db.yield_forecasts_available():
            ...
            forecast_store_ok = bool(forecast_payloads) or _expected_forecast_rows == 0
        else:
    ...
    if missing_forecasts:
        db.set_app_setting("forecast_partial_error", " | ".join(missing_forecasts[:12]))
    else:
        db.set_app_setting("forecast_partial_error", "")
    complete_ok = bool(forecast_store_ok and not missing_forecasts)
    return complete_ok, len(forecast_payloads)


_km_startup_mark("CYCLE forecast function ready")
_fast_cycle_saved = False
_fast_cycle_payload_count = 0
if _forecast_refresh_due and not _history_gate_ok:
    # Ära arvuta ega kirjuta ebausaldusväärset snapshoti üle. Dirty jääb üles ja
    # esimene parandatud rerun arvutab kohe uue päris prognoosi.
    db.set_app_setting("forecast_dirty", "1")
    db.set_app_setting("forecast_last_error", _history_gate_warning)
    db.set_app_setting("forecast_cycle_last_reason", "FAST CYCLE · blokeeritud korjeajaloo tõttu")

elif _forecast_refresh_due:
    _fast_cycle_t0 = time.perf_counter()
    try:
        _fast_cycle_saved, _fast_cycle_payload_count = _run_cycle_weather_snapshot_fast()
        _fast_cycle_error = ""
    except Exception as _fast_exc:
        _fast_cycle_saved = False
        _fast_cycle_payload_count = 0
        _fast_cycle_error = str(_fast_exc)

    _fast_cycle_duration_s = max(0.0, time.perf_counter() - _fast_cycle_t0)
    _fast_cycle_finished_at = datetime.now(ZoneInfo("Europe/Tallinn"))
    if _fast_cycle_saved:
        db.set_app_setting("forecast_dirty", "0")
        db.set_app_setting("forecast_last_error", "")
    else:
        db.set_app_setting("forecast_dirty", "1")
        _partial_detail = str(db.get_app_setting("forecast_partial_error", "") or "")
        db.set_app_setting(
            "forecast_last_error",
            _fast_cycle_error or _partial_detail or f"CYCLE snapshot salvestamata: payload_rows={_fast_cycle_payload_count}",
        )
    db.set_app_setting("forecast_last_checked_at", _fast_cycle_finished_at.isoformat())
    db.set_app_setting("forecast_cycle_last_duration_s", f"{_fast_cycle_duration_s:.2f}")
    db.set_app_setting(
        "forecast_cycle_last_reason",
        "FAST CYCLE · " + str(_forecast_dirty_reason or "sisendimuutus"),
    )

    _km_startup_mark("CYCLE snapshot refresh done")

    # Täna-leht oli selle skriptiringi alguses veel vana snapshoti peal.
    # Pärast edukat salvestust tee üks odav rerun, et uus plaan/prognoos kohe nähtav oleks.
    if page == "Täna" and _fast_cycle_saved:
        # Preserve cold/first-run timings before Streamlit aborts this run for rerun.
        _km_commit_startup_diag("before automatic forecast rerun")
        st.rerun()
else:
    _km_startup_mark("CYCLE snapshot refresh skipped")

# Mootoriotsing on lõpetatud: vana mitmetuhanderealine walk-forward/research plokk
# ei käivitu enam ei uue päeva, ilma ega korje järel. Operatiivse prognoosi teeb
# ülal olev FAST CYCLE tee.
_run_operational_cycle = False
...
if _run_operational_cycle:
    ...
    elif _season_oos_run_now:
        _light_cycle_reason = "aktiivse WIND×DRY kogu-hooaja strict OOS"
    elif _season_oos_selected_parts:
        _light_cycle_reason = "valitud challengeri kogu-hooaja strict OOS"
    elif _wd_acceptance_due:
        _light_cycle_reason = "WIND×DRY production-vastuvõtutest"
    elif _order_acceptance_due:
        _light_cycle_reason = "WIND×DRY + eelmise korje järjekorra ABC shadow-test"
    elif _order_total_due:
        _light_cycle_reason = "eelmise korjejärjekorra XL + kogusaagi shadow-test"
    elif _size_dist_due:
        _light_cycle_reason = "ühise B/C/XL suurusjaotuse shadow-test"
    elif _residual_order_due:
        _light_cycle_reason = "korjejärjekorra residual-test praeguse C/B + XL peal"
    else:
        _light_cycle_reason = "operatiivne prognoos"

    _run_light_research = False

    # Operatiivne arvutusring jookseb alati peidetult. Prognoos-leht on ainult
    # puhas snapshoti lugemisvaade; vana Jäljeotsija/championi UI ei vilgu enam kasutajale.
    _forecast_page_placeholder = st.container(key="kurgi_hidden_forecast_cycle")
    _forecast_cycle_ui = _forecast_page_placeholder

    with _forecast_cycle_ui:
        ...
        for sample in usable_samples:
            ...
            training_rows.append({
                ...
                # LAB-157 / production-shadow: eelmise korje järjekord kategooriana.
                # Order 1 on referents; order 2 ja order 3 saavad eraldi dummy-tunnuse.
                # Ei kasuta eelmist saaki ega XL-i ankru/sisendina.
                "Eelmine korje order 2": 1.0 if int(sample.get("previous_order", 1)) == 2 else 0.0,
                "Eelmine korje order 3": 1.0 if int(sample.get("previous_order", 1)) == 3 else 0.0,
        ...
        if training_rows:
            ...
            # -------------------------------------------------------------
            # PRODUCTION: lukustatud LAB-154 tunnused ajaloolisele õppimisandmestikule.
            # Iga rea L3–7 aken kasutab ainult mõõdetud/kontrollitud ilma. HIGH lävi on
            # expanding-past 75. protsentiil enne akna algust. DELTA võrdleb sama põllu
            # eelmise korje L3–7 aknaga; esimese mudelirea delta on 0 nagu LAB-154-s.
            # -------------------------------------------------------------
            if USE_WINDDRY_ENGINE:
                _wd_hist_map = {
                    str(k): v for k, v in weather_by_day.items()
                    if (
                        v
                        and bool(v.get("checked"))
                        and str(v.get("data_kind") or "").strip().lower() == "measured"
                        and (_wd_parse_day(k) is not None and _wd_parse_day(k) >= WD_WEATHER_START)
                    )
                }
                _wd_prev_window_by_field = {}
                _wd_seen_model_row = set()
                _wd_cols_values = {c: [] for c in WD_LOCKED_COLS}
                for _, _mr in model_df.iterrows():
                    _md = pd.to_datetime(_mr["Kuupäev"]).date()
                    _mf = int(_mr["Põld"])
                    _curw = _wd_l3_7_window(_md, _wd_hist_map, _wd_hist_map)
                    _prevw = _wd_prev_window_by_field.get(_mf)
                    if _curw is None:
                        _vals = {c: np.nan for c in WD_LOCKED_COLS}
                    else:
                        _allow_delta = _mf in _wd_seen_model_row and _prevw is not None
                        _vals = {
                            "WD HIGH L3–7 päevad": float(_curw["high_days"]),
                            "WD HIGH L3–7 jada": float(_curw["high_run"]),
                            "Δ WD HIGH L3–7 päevad": float(_curw["high_days"] - _prevw["high_days"]) if _allow_delta else 0.0,
                            "Δ WD HIGH L3–7 jada": float(_curw["high_run"] - _prevw["high_run"]) if _allow_delta else 0.0,
                        }
                        _wd_prev_window_by_field[_mf] = _curw
                        _wd_seen_model_row.add(_mf)
                    for _c in WD_LOCKED_COLS:
                        _wd_cols_values[_c].append(_vals[_c])
                for _c in WD_LOCKED_COLS:
                    model_df[_c] = np.asarray(_wd_cols_values[_c], dtype=float)
            ...
            # -------------------------------------------------------------
            _light_cache_version = "v17-light-wf-cache-wd-orderdiag1"  # WD lävendi/parity muutus: vana WF cache ei tohi üle tulla
            ...
            if valid_abc.any():
                ...
                r1.metric("Weather-first baas MAE" if USE_WINDDRY_ENGINE else "A+B+C MAE", f"{abc_mae:.1f} kasti")
            ...
            # Shadow-research peab võrdlema uusi ideid AKTIIVSE production-mootoriga,
            # mitte vana puhta weather-first baasiga. Kui WIND×DRY on aktiivne,
            # rekonstrueerime tema walk-forward ajaloo ja kasutame seda incumbent'ina.
            if USE_WINDDRY_ENGINE:
                _research_incumbent_pred = _walk_forward_with_extra(
                    list(WD_LOCKED_COLS), cache_key="wd_locked_incumbent"
                )
            else:
                _research_incumbent_pred = predictions.copy()
            ...
            def _active_engine_full_oos_stats(base_pred, target, recent_n=5):
                """Aktiivse production-mootori fikseeritud kogu-hooaja strict OOS benchmark.

                Kasutab kõiki ridu, millel aktiivne mootor ja tegelik siht on olemas.
                Päev MAE-sse lähevad ainult täpselt 3 põlluga päevad. Kalendri- või
                "hooaja pooli" ei tekitata; ajakohasuse kontrolliks näidatakse eraldi
                viimase recent_n täispäeva MAE-d.
                """
                base_pred = np.asarray(base_pred, dtype=float)
                target = np.asarray(target, dtype=float)
                mask = np.isfinite(base_pred) & np.isfinite(target)
                idx = np.where(mask)[0]
                if len(idx) == 0:
                    return None

                row_abs = np.abs(base_pred[idx] - target[idx])
                row_mae = float(np.mean(row_abs))
                unique_days = sorted(set(dates[idx]))

                daily_rows = []
                for _day in unique_days:
                    _didx = np.array([i for i in idx if dates[i] == _day], dtype=int)
                    if len(_didx) != 3:
                        continue
                    _pred_total = float(np.sum(base_pred[_didx]))
                    _actual_total = float(np.sum(target[_didx]))
                    daily_rows.append((_day, _pred_total, _actual_total, abs(_pred_total - _actual_total)))

                daily_mae = recent_mae = worst_error = None
                worst_day = recent_start = recent_end = None
                recent_days = 0
                if daily_rows:
                    _de = np.asarray([r[3] for r in daily_rows], dtype=float)
                    daily_mae = float(np.mean(_de))
                    _recent_n = max(1, min(int(recent_n), len(daily_rows)))
                    _recent = daily_rows[-_recent_n:]
                    recent_days = len(_recent)
                    recent_mae = float(np.mean([r[3] for r in _recent]))
                    recent_start = _recent[0][0].isoformat() if hasattr(_recent[0][0], "isoformat") else str(_recent[0][0])
                    recent_end = _recent[-1][0].isoformat() if hasattr(_recent[-1][0], "isoformat") else str(_recent[-1][0])
                    _wi = int(np.argmax(_de))
                    worst_error = float(_de[_wi])
                    worst_day = daily_rows[_wi][0].isoformat() if hasattr(daily_rows[_wi][0], "isoformat") else str(daily_rows[_wi][0])

                return {
                    "row_mae": row_mae,
                    "rows": int(len(idx)),
                    "row_days": int(len(unique_days)),
                    "daily_mae": daily_mae,
                    "daily_days": int(len(daily_rows)),
                    "daily_recent_mae": recent_mae,
                    "daily_recent_days": int(recent_days),
                    "daily_recent_start": recent_start,
                    "daily_recent_end": recent_end,
                    "daily_worst_error": worst_error,
                    "daily_worst_day": worst_day,
                }

            def _full_season_oos_stats(base_pred, candidate_pred, target, recent_n=5):
                """Kogu strict walk-forward ajalugu + päris 3-põllu päevakoond.

                Võrdlus toimub ainult ühistel OOS-ridadel. Päevakoondi MAE-sse lähevad
                ainult täpselt 3 ühise põllureaga päevad. Lisaks kogu ajaloo tulemusele
                näidatakse viimase recent_n täispäeva võrdlust; kunstlikku hooaja
                poolitamist ei kasutata.
                """
                base_pred = np.asarray(base_pred, dtype=float)
                candidate_pred = np.asarray(candidate_pred, dtype=float)
                target = np.asarray(target, dtype=float)
                mask = np.isfinite(base_pred) & np.isfinite(candidate_pred) & np.isfinite(target)
                idx = np.where(mask)[0]
                if len(idx) == 0:
                    return None
                ...
                base_abs = np.abs(base_pred[idx] - target[idx])
                cand_abs = np.abs(candidate_pred[idx] - target[idx])
                row_base_mae = float(np.mean(base_abs))
                row_cand_mae = float(np.mean(cand_abs))
                row_win = float(np.mean(cand_abs < base_abs)) * 100.0

                daily_rows = []
                for _day in sorted(set(dates[idx])):
                    _didx = np.array([i for i in idx if dates[i] == _day], dtype=int)
                    if len(_didx) != 3:
                        continue
                    _b = float(np.sum(base_pred[_didx]))
                    _c = float(np.sum(candidate_pred[_didx]))
                    _a = float(np.sum(target[_didx]))
                    daily_rows.append((_day, abs(_b - _a), abs(_c - _a)))

                day_base_mae = day_cand_mae = day_win = None
                recent_base_mae = recent_cand_mae = recent_imp = recent_win = None
                recent_days = 0
                recent_start = recent_end = None
                if daily_rows:
                    _db = np.asarray([r[1] for r in daily_rows], dtype=float)
                    _dc = np.asarray([r[2] for r in daily_rows], dtype=float)
                    day_base_mae = float(np.mean(_db))
                    day_cand_mae = float(np.mean(_dc))
                    day_win = float(np.mean(_dc < _db)) * 100.0
                    _recent_n = max(1, min(int(recent_n), len(daily_rows)))
                    _recent = daily_rows[-_recent_n:]
                    _rdb = np.asarray([r[1] for r in _recent], dtype=float)
                    _rdc = np.asarray([r[2] for r in _recent], dtype=float)
                    recent_days = len(_recent)
                    recent_base_mae = float(np.mean(_rdb))
                    recent_cand_mae = float(np.mean(_rdc))
                    recent_imp = recent_base_mae - recent_cand_mae
                    recent_win = float(np.mean(_rdc < _rdb)) * 100.0
                    recent_start = _recent[0][0].isoformat() if hasattr(_recent[0][0], "isoformat") else str(_recent[0][0])
                    recent_end = _recent[-1][0].isoformat() if hasattr(_recent[-1][0], "isoformat") else str(_recent[-1][0])

                return {
                    "oos_base_mae": row_base_mae,
                    "oos_candidate_mae": row_cand_mae,
                    "oos_improvement": row_base_mae - row_cand_mae,
                    "oos_win_share": row_win,
                    "oos_rows": int(len(idx)),
                    "oos_days": int(len(set(dates[idx]))),
                    "daily_base_mae": day_base_mae,
                    "daily_candidate_mae": day_cand_mae,
                    "daily_improvement": (None if day_base_mae is None else day_base_mae - day_cand_mae),
                    "daily_win_share": day_win,
                    "daily_days": int(len(daily_rows)),
                    "recent_base_mae": recent_base_mae,
                    "recent_candidate_mae": recent_cand_mae,
                    "recent_improvement": recent_imp,
                    "recent_win_share": recent_win,
            ...
            # -------------------------------------------------------------
            # WIND×DRY PRODUCTION VASTUVÕTUTEST
            # Täpselt sama production model_engine / X_base / walk-forward:
            # weather-first baas vs weather-first + 4 lukustatud WD tunnust.
            # Taimeindeks EI osale selles testis. See mõõdab ainult A+B+C mootori lisakasu.
            # PASS lävend on sama konservatiivne kui Jäljeotsija tavapoliitika:
            # >=12 ühist rida, >=0.10 kasti MAE paranemine, >=50% ridu võidetud,
            # ning kummagi ajapoole halvim paranemine mitte alla -0.05 kasti.
            _wd_acceptance_stats = None
            if USE_WINDDRY_ENGINE:
                _wd_accept_days = set(sorted(set(
                    dates[np.where(
                        np.isfinite(predictions)
                        & np.isfinite(_research_incumbent_pred)
                        & np.isfinite(y)
                    )[0]]
                )))
                _wd_acceptance_stats = _period_stability_stats(
                    predictions, _research_incumbent_pred, y, _wd_accept_days,
                    min_rows=12, improvement_threshold=0.10, min_half_threshold=-0.05,
                )
                if _wd_acceptance_stats:
                    _save_json_setting("wd_production_acceptance_json", {
                        **_wd_acceptance_stats,
                        "status": "tested",
                        "engine": "WIND×DRY HIGH L3–7 · LEVEL+DELTA",
                        "model_version": WD_MODEL_VERSION,
                        "weather_start": WD_WEATHER_START.isoformat(),
                        "tested_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "plant_index_in_test": False,
                    })
                else:
                    # Ära tee sama rasket acceptance-ring'i igal rerun'il, kui ühiseid
                    # walk-forward ridu veel üldse ei tekkinud.
                    _save_json_setting("wd_production_acceptance_json", {
            ...
            # -------------------------------------------------------------
            # LAB-157 / PRODUCTION-SHADOW: WD incumbent vs WD + previous order 1/2/3
            #
            # Täpselt sama production X_base, sama model_engine ja sama strict
            # walk-forward nagu aktiivsel WIND×DRY A+B+C mootoril.
            # Eelmise korje järjekord on kategooriline: order 1 = referents,
            # order 2 ja order 3 eraldi dummy-tunnustena.
            # Eelmist saaki ega XL-i ei kasutata sisendina.
            # -------------------------------------------------------------
            _order_acceptance_stats = None
            if USE_WINDDRY_ENGINE and all(c in model_df.columns for c in ["Eelmine korje order 2", "Eelmine korje order 3"]):
                _order_candidate_pred = _walk_forward_with_extra(
                    list(WD_LOCKED_COLS) + ["Eelmine korje order 2", "Eelmine korje order 3"],
                    cache_key="wd_plus_prev_order123_categorical_shadow_v1",
                )
                _order_accept_days = set(sorted(set(
                    dates[np.where(
                        np.isfinite(_research_incumbent_pred)
                        & np.isfinite(_order_candidate_pred)
                        & np.isfinite(y)
                    )[0]]
                )))
                _order_acceptance_stats = _period_stability_stats(
                    _research_incumbent_pred,
                    _order_candidate_pred,
                    y,
                    _order_accept_days,
                    min_rows=12,
                ...
                if _order_acceptance_stats:
                    _save_json_setting("wd_prev_order_acceptance_json", {
                        **_order_acceptance_stats,
                        "status": "tested",
                        "test_version": _ORDER_TEST_VERSION,
                        "engine_base": "WIND×DRY HIGH L3–7 · LEVEL+DELTA",
                        "candidate": "WIND×DRY + previous_order 1/2/3",
                        "model_version": WD_MODEL_VERSION,
                        "tested_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "plant_index_in_test": False,
                        "changes_official_forecast": False,
                    })
                else:
                    _save_json_setting("wd_prev_order_acceptance_json", {
                        "status": "insufficient_rows",
            ...
            _abc_discovery_days, _abc_confirmation_days = _chronological_discovery_confirmation_days(_research_incumbent_pred, y)

            # -------------------------------------------------------------
            # KOGU HOOAJA OOS AUDIT — kasutaja käsitsi käivitatav, read-only diagnostika.
            # Arvutab juba salvestatud kihilised kandidaadid sama production strict
            # walk-forward mootoriga. Tulemused lähevad eraldi settingusse ning ei muuda
            # challenger-valikut, championit ega ametlikku prognoosi.
            # -------------------------------------------------------------
            if _season_oos_run_now or _season_oos_selected_parts:
                # Võta püsiv päringulipp maha alles siin: nüüd oleme päriselt mudeli/OOS-plokini jõudnud.
                # Nii ei kao nupuvajutus Streamliti võimaliku vahe-rerun'i tõttu, kuid arvutusvea korral
                # ei jää rakendus lõputult sama rasket testi kordama.
                if _season_oos_run_now:
                    st.session_state.pop("_season_oos_active_request", None)
                    st.session_state.pop("_season_oos_active_error", None)
                if _season_oos_selected_parts:
                    st.session_state.pop("_season_oos_selected_parts_run", None)
                    st.session_state.pop("_season_oos_candidate_error", None)

                _oos_hash_cols = [c for c in model_df.columns if c not in {"Andmekvaliteet"}]
                _oos_hash_frame = model_df[_oos_hash_cols].copy()
                for _c in _oos_hash_frame.columns:
                    if _c == "Kuupäev":
                        _oos_hash_frame[_c] = _oos_hash_frame[_c].map(
                            lambda v: v.isoformat() if hasattr(v, "isoformat") else str(v)
                        )
                _oos_data_hash = hashlib.sha256(
                    pd.util.hash_pandas_object(
                        _oos_hash_frame.astype(str), index=True
                    ).values.tobytes()
                ).hexdigest()[:20]

                _oos_state_raw = db.get_app_setting("layered_research_state_json", "")
                try:
                    _oos_state = json.loads(_oos_state_raw) if _oos_state_raw else {}
                except Exception:
                    _oos_state = {}
                _oos_all_candidates = [
                    r for r in list((_oos_state or {}).get("results", []))
                    if r.get("data_hash") == _oos_data_hash and (r.get("cols") or [])
                ]

                # Kasutaja valitud kandidaat läheb auditisse sõltumata confirmation-staatusest:
                # see on diagnostika, mitte championiks ülendamine. Ühe vajutusega arvutatakse
                # ainult üks kandidaat. Kui valikut pole, arvutatakse ainult aktiivne benchmark.
                _oos_candidates = []
                if _season_oos_selected_parts:
                    for _r in _oos_all_candidates:
                        if " | ".join(_r.get("parts") or []) == str(_season_oos_selected_parts):
                            _oos_candidates = [_r]
                            break

                _active_oos_stats = _active_engine_full_oos_stats(_research_incumbent_pred, y)
                if _active_oos_stats is None:
                    st.session_state["_season_oos_active_error"] = (
                        "Kogu-hooaja OOS arvutus jõudis mudelini, kuid aktiivsel WIND×DRY-l ei tekkinud "
                        "ühtegi strict OOS rida. Kontrolli, et ajaloolisi õppimisridu oleks piisavalt."
                    )
                else:
                    st.session_state.pop("_season_oos_active_error", None)
                if _season_oos_selected_parts and not _oos_candidates:
                    st.session_state["_season_oos_candidate_error"] = (
                        "Valitud kandidaati ei leitud selle andmeseisu research-state'ist. "
                        "Vali kandidaat uuesti kihiliste mudelite tabelist."
                    )

                # Hoia sama andmeseisu varem käsitsi auditeeritud kandidaadid alles, et kasutaja
                # saaks neid ükshaaval juurde testida ja tulemusi omavahel võrrelda.
                _prev_oos_saved = _load_json_setting("layered_season_oos_json", {})
                if isinstance(_prev_oos_saved, dict) and _prev_oos_saved.get("data_hash") == _oos_data_hash:
                    _oos_rows = list(_prev_oos_saved.get("rows") or [])
                else:
                    _oos_rows = []

                _oos_progress = st.progress(0.0, text="Valitud kandidaadi kogu hooaja OOS…") if _oos_candidates else None
                for _oi, _orow in enumerate(_oos_candidates):
                    _ocols = [str(c) for c in (_orow.get("cols") or []) if str(c) in model_df.columns]
                    if not _ocols:
                        continue
                    _okey = str(_orow.get("combo_key") or " | ".join(_orow.get("parts") or []))
                    _ocache = "season_oos_" + hashlib.sha256(
                        (_oos_data_hash + "|" + _okey).encode("utf-8")
                    ).hexdigest()[:20]
                    _opred = _walk_forward_with_extra(_ocols, cache_key=_ocache)
                    _ostat = _full_season_oos_stats(_research_incumbent_pred, _opred, y)
                    if not _ostat:
                        continue
                    _new_row = {
                        "Koosseis": " | ".join(_orow.get("parts") or []),
                        "Kihte": len(_orow.get("layers") or []),
                        "Kinnitus stabiilne": bool(_orow.get("confirmation_stable")),
                        **_ostat,
                    }
                    # Sama kandidaadi kordustest asendab tema vana rea, mitte ei tee duplikaati.
                    _oos_rows = [r for r in _oos_rows if str(r.get("Koosseis") or "") != _new_row["Koosseis"]]
                    _oos_rows.append(_new_row)
                    if _oos_progress is not None:
                        _oos_progress.progress(1.0, text="Valitud kandidaadi kogu hooaja OOS valmis")
                if _oos_progress is not None:
                    _oos_progress.empty()

                _oos_rows.sort(
                    key=lambda r: (
                        float(r.get("daily_improvement") if r.get("daily_improvement") is not None else -999.0),
                        float(r.get("oos_improvement") if r.get("oos_improvement") is not None else -999.0),
                    ),
                    reverse=True,
                )
                _oos_payload = {
                    "data_hash": _oos_data_hash,
                    "updated_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                    "harvest_day": last_complete_harvest.isoformat() if last_complete_harvest else None,
                    "rows": _oos_rows,
                    "active_engine": "WIND×DRY HIGH L3–7 · LEVEL+DELTA",
                    "active_baseline": _active_oos_stats,
                    "candidate_pool_total": int(len(_oos_all_candidates)),
                    "candidate_pool_audited": int(len(_oos_rows)),
                    "candidate_filter": "kasutaja valib ühe kandidaadi tabeli reanumbri järgi",
                    "last_tested_candidate": (
                        str(_season_oos_selected_parts)
                        if _season_oos_selected_parts
                        else str((_prev_oos_saved or {}).get("last_tested_candidate") or "")
                    ),
                    "changes_official_forecast": False,
                }
                # Hoia vastus kohe sessioonis, et UI saaks selle pärast rerun'i viivituseta
                # otse nupu all kuvada. Püsiseadistus jääb alles järgmiste sessioonide jaoks.
                st.session_state["_layered_season_oos_last_result"] = _oos_payload
                _save_json_setting("layered_season_oos_json", _oos_payload)

                # Kirjuta aktiivse benchmarki tulemus SAMAS Streamliti jooksus otse
                # ülal loodud placeholder'isse. Eraldi rerun ei ole enam vajalik.
                _render_active_oos_panel(
                    _season_oos_panel,
                    active_data=_active_oos_stats,
                    error=st.session_state.get("_season_oos_active_error"),
                    running=False,
                )

            def _stability_stats(candidate_pred):
                # Üldine diagnostika kogu walk-forward ajalool; championit selle järgi enam ei valita.
                all_days = set(sorted(set(dates[np.where(np.isfinite(_research_incumbent_pred) & np.isfinite(y))[0]])))
                return _period_stability_stats(
                    _research_incumbent_pred, candidate_pred, y, all_days,
                    min_rows=12, improvement_threshold=0.10, min_half_threshold=-0.05,
                )
            ...
            if _run_light_research:
                for name, cols in _all_candidate_groups.items():
                    _research_cols = list(cols)
                    if USE_WINDDRY_ENGINE:
                        _research_cols = list(dict.fromkeys(list(WD_LOCKED_COLS) + _research_cols))
                    cp = _walk_forward_with_extra(_research_cols, cache_key=("wd+" + name if USE_WINDDRY_ENGINE else name))
                ...
                try:
                    for _f in sorted(set(int(v) for v in fields)):
                        _idx = np.where(
                            (fields == _f) & np.isfinite(_research_incumbent_pred) & np.isfinite(y)
                        )[0]
                        if len(_idx) < 2:
                            continue
                        _idx = _idx[np.argsort(dates[_idx])]
                        _last2 = _idx[-2:]
                        _res = y[_last2] - _research_incumbent_pred[_last2]
                        if np.all(_res <= -0.75):
            ...
            # Aktiivne production A+B+C mootor on lukustatud WIND×DRY peale.
            # Jäljeotsija võib allpool kandidaate hinnata, kuid ei tohi neid
            # operatiivseks championiks ülendada.
            _shadow_selected_confirmed_row = None
            if USE_WINDDRY_ENGINE:
                champion_name = "WIND×DRY HIGH L3–7 · LEVEL+DELTA"
                champion_cols = list(WD_LOCKED_COLS)
                champion_mae = None
                champion_stats = None
            layered_trace_rows = []
            ...
            if _layered_run_now:
                ...
                def _eval_open_candidate(specs):
                    added_cols = _merge_cols_many(specs)
                    if not added_cols:
                        return None
                    cols = list(added_cols)
                    if USE_WINDDRY_ENGINE:
                        cols = list(dict.fromkeys(list(WD_LOCKED_COLS) + cols))
                    cp = _walk_forward_with_extra(cols)
                    stats = _period_stability_stats(
                        _research_incumbent_pred, cp, y, _abc_discovery_days,
                        min_rows=6,
                    ...
                    confirm_stats = _period_stability_stats(
                        _research_incumbent_pred, cp, y, _abc_confirmation_days,
                        min_rows=6,
                        improvement_threshold=0.05,
                        min_half_threshold=-0.05,
                    ...
                    _full_oos = _full_season_oos_stats(_research_incumbent_pred, cp, y)
                ...
                while (
                    ...
                    _row = {
                        ...
                        "oos_base_mae": (None if not _cand.get("full_oos") else _cand["full_oos"].get("oos_base_mae")),
                        "oos_candidate_mae": (None if not _cand.get("full_oos") else _cand["full_oos"].get("oos_candidate_mae")),
                        "oos_improvement": (None if not _cand.get("full_oos") else _cand["full_oos"].get("oos_improvement")),
                        "oos_win_share": (None if not _cand.get("full_oos") else _cand["full_oos"].get("oos_win_share")),
                        "oos_rows": (0 if not _cand.get("full_oos") else int(_cand["full_oos"].get("oos_rows") or 0)),
                        "daily_base_mae": (None if not _cand.get("full_oos") else _cand["full_oos"].get("daily_base_mae")),
                        "daily_candidate_mae": (None if not _cand.get("full_oos") else _cand["full_oos"].get("daily_candidate_mae")),
                        "daily_improvement": (None if not _cand.get("full_oos") else _cand["full_oos"].get("daily_improvement")),
                        "daily_win_share": (None if not _cand.get("full_oos") else _cand["full_oos"].get("daily_win_share")),
                        "daily_days": (0 if not _cand.get("full_oos") else int(_cand["full_oos"].get("daily_days") or 0)),
                        "recent_base_mae": (None if not _cand.get("full_oos") else _cand["full_oos"].get("recent_base_mae")),
                        "recent_candidate_mae": (None if not _cand.get("full_oos") else _cand["full_oos"].get("recent_candidate_mae")),
                        "recent_improvement": (None if not _cand.get("full_oos") else _cand["full_oos"].get("recent_improvement")),
                        "recent_win_share": (None if not _cand.get("full_oos") else _cand["full_oos"].get("recent_win_share")),
                        "recent_days": (0 if not _cand.get("full_oos") else int(_cand["full_oos"].get("recent_days") or 0)),
                    }
                ...
                for _r in _top_rows:
                    layered_trace_rows.append({
                        ...
                        "OOS MAE": _r.get("oos_candidate_mae"),
                        "OOS Δ": _r.get("oos_improvement"),
                        "OOS võidab %": _r.get("oos_win_share"),
                        "OOS ridu": _r.get("oos_rows"),
                        "Päev MAE": _r.get("daily_candidate_mae"),
                        "Päev Δ": _r.get("daily_improvement"),
                        "Päev võidab %": _r.get("daily_win_share"),
                        "Päevi": _r.get("daily_days"),
                        "Viimased 5 MAE": _r.get("recent_candidate_mae"),
                        "Viimased 5 Δ": _r.get("recent_improvement"),
                        "Viimased 5 võidab %": _r.get("recent_win_share"),
                    })
                ...
                if _selected_confirmed_row is not None:
                    ...
                    if _selected_cols:
                        # SHADOW RESEARCH: kinnitatud kandidaat talletatakse uurimistulemusena,
                        # kuid aktiivset WIND×DRY production-mootorit EI vahetata automaatselt.
                        _shadow_selected_confirmed_row = dict(_selected_confirmed_row)
                        _save_json_setting("abc_shadow_research_state_json", {
                            "saved_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                            "name": "Avatud kihiline challenger",
                            "parts": list(_selected_confirmed_row.get("parts") or []),
                            "cols": _selected_cols,
                            "base_mae": _selected_confirmed_row.get("confirmation_base_mae"),
                            "candidate_mae": _selected_confirmed_row.get("confirmation_candidate_mae"),
                            "improvement": _selected_confirmed_row.get("confirmation_improvement"),
                            "win_share": _selected_confirmed_row.get("confirmation_win_share"),
                            "worst_half": _selected_confirmed_row.get("confirmation_worst_half"),
                            "test_rows": _selected_confirmed_row.get("confirmation_test_rows"),
                            "stable": bool(_selected_confirmed_row.get("confirmation_stable", True)),
                            "status": "shadow_only",
                            "active_engine": "WIND×DRY HIGH L3–7 · LEVEL+DELTA",
                        })
            ...
            ch1.metric("Aktiivne A+B+C", champion_name)
            if USE_WINDDRY_ENGINE:
                ch2.metric("Režiim", "LUKUSTATUD")
                _shadow_state = _load_json_setting("abc_shadow_research_state_json", {})
                _shadow_impr = (_shadow_state or {}).get("improvement")
                ch3.metric(
                    "Shadow challenger",
                    "—" if _shadow_impr is None else f"{float(_shadow_impr):+.2f} MAE",
                )
                st.info(
                    "9 päeva A+B+C prognoos kasutab lukustatud **WIND×DRY HIGH L3–7 · LEVEL+DELTA** mootorit. "
                    "Jäljeotsija jätkab taustal uute kandidaatide walk-forward uurimist, kuid ei tohi aktiivset "
                    "mootorit automaatselt vahetada. Võimalik parem kandidaat jääb shadow-research olekusse kuni "
                    "teadliku kinnitamiseni."
                )
                _wa = _wd_acceptance_stats or _load_json_setting("wd_production_acceptance_json", {})
                if isinstance(_wa, dict) and _wa.get("Testiridu"):
                    _wa_pass = bool(_wa.get("Stabiilne"))
                    wa1, wa2, wa3, wa4 = st.columns(4)
                    wa1.metric("WD vastuvõtt", "PASS" if _wa_pass else "EI LÄBINUD")
                    wa2.metric("Weather-first MAE", f"{float(_wa.get('Baas MAE')):.2f}")
                    wa3.metric("WIND×DRY MAE", f"{float(_wa.get('Katse MAE')):.2f}")
                    wa4.metric("Paranemine", f"{float(_wa.get('Paranemine')):+.2f}")
                    st.caption(
                        f"Production walk-forward · {_wa.get('Testiridu')} rida / {_wa.get('Testipäevi')} päeva · "
                        f"WD võidab {float(_wa.get('Võidab ridu %')):.0f}% ridu · "
                        f"halvim ajapool {float(_wa.get('Halvim pool')):+.2f} kasti · taimeindeks testis ei osale."
                    )
                    if _wa_pass:
                        st.success("WIND×DRY production-vastuvõtutest läbitud.")
                    else:
                        st.warning(
                            "WIND×DRY production-vastuvõtutest ei läbinud veel lukustatud stabiilsuslävendit. "
                            "Mootor jääb selles RC-s teadliku otsusena aktiivseks, kuid tulemust tuleb enne lõplikku lukustamist hinnata."
                        )
                else:
                    st.caption("WIND×DRY production-vastuvõtutest: piisavaid ühiseid walk-forward ridu veel ei ole.")

                _oa = _order_acceptance_stats or _load_json_setting("wd_prev_order_acceptance_json", {})
                st.markdown("##### Kõigi eelmiste korjejärjekordade shadow-test")
                if (
                    isinstance(_oa, dict)
                    and str(_oa.get("model_version") or "") == WD_MODEL_VERSION
                    and str(_oa.get("test_version") or "") == _ORDER_TEST_VERSION
                    and _oa.get("Testiridu")
                ):
                    _oa_pass = bool(_oa.get("Stabiilne"))
                    _o1, _o2, _o3, _o4 = st.columns(4)
                    _o1.metric("Tulemus", "KASU" if _oa_pass else "EI PIISA")
                    _o2.metric("WIND×DRY MAE", f"{float(_oa.get('Baas MAE')):.2f}")
                    _o3.metric("+ eelmine order 1/2/3 MAE", f"{float(_oa.get('Katse MAE')):.2f}")
                    _o4.metric("Paranemine", f"{float(_oa.get('Paranemine')):+.2f}")
                    st.caption(
                        f"Production A+B+C walk-forward · {_oa.get('Testiridu')} rida / {_oa.get('Testipäevi')} päeva · "
                        f"kandidaat võidab {float(_oa.get('Võidab ridu %')):.0f}% ridu · "
                        f"halvim ajapool {float(_oa.get('Halvim pool')):+.2f} kasti · "
                        f"PASS-lävi: vähemalt +0.05 MAE, ≥50% ridu, halvim pool ≥−0.05. "
                        f"Ametlikku prognoosi see test ei muuda."
                    )
                else:
                    st.caption("Kõigi eelmiste korjejärjekordade shadow-test: piisavaid ühiseid walk-forward ridu veel ei ole.")
            elif champion_stats:
                ...
                st.success(
                    ...
                    f"Kinnituses võitis ta {champion_stats['Võidab ridu %']:.0f}% testiridadest ning "
            ...
            # -------------------------------------------------------------
            # LAB-158 / PRODUCTION-SHADOW: previous order mõju XL-le ja KOGULE
            #
            # Võrdlused:
            #   BASE      = aktiivne WD ABC + aktiivne XL champion
            #   XL+ORDER  = aktiivne WD ABC + sama XL champion + previous order 1/2/3
            #   BOTH      = WD ABC + previous order 1/2/3 + XL+ORDER
            #
            # Nii testime lõpuks kasutaja tegelikku hüpoteesi. A+B+C ja XL jäävad
            # eraldi mudeliteks; eelmist saaki/XL-i ankru või sisendina juurde ei lisata.
            # -------------------------------------------------------------
            _order_total_stats = None
            if USE_WINDDRY_ENGINE and all(
                c in model_df.columns for c in ["Eelmine korje order 2", "Eelmine korje order 3"]
            ):
                _order_cols = ["Eelmine korje order 2", "Eelmine korje order 3"]

                # Aktiivse XL championi aus walk-forward jälg.
                if xl_champion_cols:
                    _xl_incumbent_pred = _xl_walk_with_extra(
                        list(xl_champion_cols),
                        cache_key="xl_active_incumbent_for_order_total_v1",
                    )
                else:
                    _xl_incumbent_pred = xl_predictions.copy()

                _xl_order_cols = list(xl_champion_cols) + [
                    c for c in _order_cols if c not in set(xl_champion_cols)
                ]
                _xl_order_pred = _xl_walk_with_extra(
                    _xl_order_cols,
                    cache_key="xl_active_plus_prev_order123_for_total_v1",
                )

                # ABC kandidaat rekonstrueeritakse production-WD pealt, mitte weather-first baasilt.
                _abc_order_pred_totaltest = _walk_forward_with_extra(
                    list(WD_LOCKED_COLS) + _order_cols,
                    cache_key="wd_plus_prev_order123_for_total_v1",
                )

                _base_total_pred = _research_incumbent_pred + _xl_incumbent_pred
                _xl_order_total_pred = _research_incumbent_pred + _xl_order_pred
                _both_order_total_pred = _abc_order_pred_totaltest + _xl_order_pred

                _all_total_days = set(sorted(set(
                    dates[np.where(
                        np.isfinite(_base_total_pred)
                        & np.isfinite(_xl_order_total_pred)
                        & np.isfinite(_both_order_total_pred)
                        & np.isfinite(y_total)
                    )[0]]
                )))
                _all_xl_days = set(sorted(set(
                    dates[np.where(
                        np.isfinite(_xl_incumbent_pred)
                        & np.isfinite(_xl_order_pred)
                        & np.isfinite(y_xl)
                    )[0]]
                )))

                _xl_order_stats = _period_stability_stats(
                    _xl_incumbent_pred, _xl_order_pred, y_xl, _all_xl_days,
                    min_rows=12, improvement_threshold=0.03, min_half_threshold=-0.05,
                )
                _total_xl_only_stats = _period_stability_stats(
                    _base_total_pred, _xl_order_total_pred, y_total, _all_total_days,
                    min_rows=12, improvement_threshold=0.05, min_half_threshold=-0.05,
                )
                _total_both_stats = _period_stability_stats(
                    _base_total_pred, _both_order_total_pred, y_total, _all_total_days,
                    min_rows=12, improvement_threshold=0.05, min_half_threshold=-0.05,
                )

                if _xl_order_stats and _total_xl_only_stats and _total_both_stats:
                    _order_total_stats = {
                        "status": "tested",
                        "test_version": _ORDER_TOTAL_TEST_VERSION,
                        "model_version": WD_MODEL_VERSION,
                        "xl_incumbent": xl_champion_name,
                        "XL": _xl_order_stats,
                        "TOTAL_XL_ONLY": _total_xl_only_stats,
                        "TOTAL_BOTH": _total_both_stats,
                        "tested_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "plant_index_in_test": False,
                        "changes_official_forecast": False,
                    }
                    _save_json_setting("wd_prev_order_xl_total_acceptance_json", _order_total_stats)
                else:
                    _order_total_stats = {
                        "status": "insufficient_rows",
                        "test_version": _ORDER_TOTAL_TEST_VERSION,
            ...
            _ot = _order_total_stats or _load_json_setting("wd_prev_order_xl_total_acceptance_json", {})
            st.markdown("##### Korjejärjekord → XL + kogusaak shadow-test")
            if (
                isinstance(_ot, dict)
                and str(_ot.get("model_version") or "") == WD_MODEL_VERSION
                and str(_ot.get("test_version") or "") == _ORDER_TOTAL_TEST_VERSION
                and _ot.get("status") == "tested"
            ):
                _ox = _ot.get("XL") or {}
                _otl = _ot.get("TOTAL_XL_ONLY") or {}
                _otb = _ot.get("TOTAL_BOTH") or {}
                _best_total = _otl if float(_otl.get("Katse MAE", 1e9)) <= float(_otb.get("Katse MAE", 1e9)) else _otb
                _best_label = "order ainult XL-s" if _best_total is _otl else "order ABC-s + XL-s"
                _p1, _p2, _p3, _p4 = st.columns(4)
                _p1.metric("XL baas MAE", f"{float(_ox.get('Baas MAE')):.2f}")
                _p2.metric("XL + order MAE", f"{float(_ox.get('Katse MAE')):.2f}")
                _p3.metric("Kogu baas MAE", f"{float(_best_total.get('Baas MAE')):.2f}")
                _p4.metric("Parim kogu + order MAE", f"{float(_best_total.get('Katse MAE')):.2f}")
                st.caption(
                    f"XL paranemine {float(_ox.get('Paranemine')):+.2f} · "
                    f"XL kandidaat võidab {float(_ox.get('Võidab ridu %')):.0f}% ridu · "
                    f"KOGU, order ainult XL-s: {float(_otl.get('Baas MAE')):.2f} → {float(_otl.get('Katse MAE')):.2f} "
                    f"({float(_otl.get('Paranemine')):+.2f}); "
                    f"KOGU, order ABC+XL: {float(_otb.get('Baas MAE')):.2f} → {float(_otb.get('Katse MAE')):.2f} "
                    f"({float(_otb.get('Paranemine')):+.2f}). Parim variant: {_best_label}. "
                    f"Taimeindeks testis ei osale; ametlikku prognoosi test ei muuda."
                )
            elif isinstance(_ot, dict) and _ot.get("status") == "insufficient_rows":
                st.caption("XL + kogusaagi korjejärjekorra shadow-test: piisavaid ühiseid walk-forward ridu veel ei ole.")
            ...
            # -------------------------------------------------------------------------
            # LAB-160 / PRODUCTION-SHADOW: otsene B/C/XL compositional-mudel
            #
            # Eesmärk: vältida C+XL -> XL/C hierarhiat, kus kahe järjestikuse mudeli
            # vead võivad kuhjuda. Ennustame otse kaks additive-log-ratio telge:
            #   R_C  = log((C+eps)/(B+eps))
            #   R_XL = log((XL+eps)/(B+eps))
            # ning rekonstrueerime neist korraga B, C ja XL.
            #
            # Absoluutne B+C maht tuleb samast aktiivsest WD A+B+C prognoosist ja
            # samast A-osakaalu walk-forward hinnangust nagu incumbendil. Seega test
            # mõõdab ainult suurusjaotuse arhitektuuri, mitte kogusaagi mootorit.
            # Eelmise korje järjekorda selles testis EI kasutata.
            # -------------------------------------------------------------------------
            _size_dist_stats = None
            if USE_WINDDRY_ENGINE:
                _sd_eps = 0.05
                _sd_a = pd.to_numeric(model_df["A"], errors="coerce").to_numpy(dtype=float)
                _sd_b = pd.to_numeric(model_df["B"], errors="coerce").to_numpy(dtype=float)
                _sd_c = pd.to_numeric(model_df["C"], errors="coerce").to_numpy(dtype=float)
                _sd_xl = pd.to_numeric(model_df["XL"], errors="coerce").to_numpy(dtype=float)
                _sd_abc_actual = _sd_a + _sd_b + _sd_c

                _sd_cb_log = np.where(
                    np.isfinite(_sd_b) & np.isfinite(_sd_c) & (_sd_b >= 0) & (_sd_c >= 0),
                    np.log((_sd_c + _sd_eps) / (_sd_b + _sd_eps)),
                    np.nan,
                )
                _sd_xlb_log = np.where(
                    np.isfinite(_sd_b) & np.isfinite(_sd_xl) & (_sd_b >= 0) & (_sd_xl >= 0),
                    np.log((_sd_xl + _sd_eps) / (_sd_b + _sd_eps)),
                    np.nan,
                )
                _sd_a_share = np.where(
                    np.isfinite(_sd_a) & np.isfinite(_sd_abc_actual) & (_sd_abc_actual > 0),
                    np.clip((_sd_a + 0.01) / (_sd_abc_actual + 0.02), 0.001, 0.35),
                    np.nan,
                )
                _sd_a_logit = np.where(
                    np.isfinite(_sd_a_share),
                    np.log(_sd_a_share / (1.0 - _sd_a_share)),
                    np.nan,
                )

                def _sd_walk_transformed(target, extra_cols=None, alpha=10.0):
                    ...
                    extra_arrays = [raw_extra[:, j] for j in range(raw_extra.shape[1])]
                    preds = np.full(len(model_df), np.nan, dtype=float)
                    for _test_day in sorted(set(dates)):
                        _test_idx = np.where(dates == _test_day)[0]
                        _train_idx = np.where((dates < _test_day) & np.isfinite(target))[0]
                        if len(_train_idx) < min_train_rows:
                            continue
                        preds[_test_idx] = _ridge_walk_predict(
                            target, extra_arrays, _train_idx, _test_idx,
                            alpha=alpha, floor_zero=False,
                        )
                    return preds

                # Sama A-osakaalu hinnang incumbendile ja kandidaadile.
                _sd_a_logit_pred = _sd_walk_transformed(_sd_a_logit, [])
                _sd_a_share_pred = 1.0 / (1.0 + np.exp(-np.clip(_sd_a_logit_pred, -8.0, 2.0)))
                _sd_a_share_pred = np.clip(_sd_a_share_pred, 0.0, 0.35)

                # Otsene ALR-kandidaat. Primary kasutab täpselt neid lisatunnuseid,
                # mis aktiivsed C/B ja XL championid juba kasutavad; orderit ei lisata.
                _sd_cb_direct_base_log = _sd_walk_transformed(_sd_cb_log, [])
                _sd_xlb_direct_base_log = _sd_walk_transformed(_sd_xlb_log, [])
                _sd_cb_direct_prod_log = _sd_walk_transformed(_sd_cb_log, list(cb_champion_cols))
                _sd_xlb_direct_prod_log = _sd_walk_transformed(_sd_xlb_log, list(xl_champion_cols))

                def _sd_ratio_from_log(values, lo, hi):
                    return np.exp(np.clip(values, np.log(lo), np.log(hi)))

                _sd_cb_direct_base = _sd_ratio_from_log(_sd_cb_direct_base_log, 0.05, 12.0)
                _sd_xlb_direct_base = _sd_ratio_from_log(_sd_xlb_direct_base_log, 0.005, 12.0)
                _sd_cb_direct_prod = _sd_ratio_from_log(_sd_cb_direct_prod_log, 0.05, 12.0)
                _sd_xlb_direct_prod = _sd_ratio_from_log(_sd_xlb_direct_prod_log, 0.005, 12.0)

                # Production incumbentid: WD ABC + aktiivne C/B champion + aktiivne XL champion.
                _sd_abc_pred = np.asarray(_research_incumbent_pred, dtype=float)
                if cb_champion_cols:
                    _sd_cb_inc = _cb_walk_with_extra(list(cb_champion_cols))
                else:
                    _sd_cb_inc = np.asarray(cb_predictions, dtype=float).copy()
                if xl_champion_cols:
                    _sd_xl_inc = _xl_walk_with_extra(list(xl_champion_cols))
                else:
                    _sd_xl_inc = np.asarray(xl_predictions, dtype=float).copy()

                def _sd_reconstruct_current(abc_pred, cb_pred, xl_pred, a_share_pred):
                    bc_pool = np.maximum(0.0, abc_pred * (1.0 - a_share_pred))
                    cb_safe = np.clip(cb_pred, 0.05, 12.0)
                    b_pred = bc_pool / (1.0 + cb_safe)
                    c_pred = bc_pool - b_pred
                    return b_pred, c_pred, np.maximum(0.0, xl_pred), cb_safe

                def _sd_reconstruct_direct(abc_pred, cb_ratio, xlb_ratio, a_share_pred):
                    # B+C summa on lukustatud aktiivse ABC prognoosi sisse.
                    # Mõlemad suurusteljed jagavad sama B-referentsi:
                    # C = (C/B)*B, XL = (XL/B)*B.
                    bc_pool = np.maximum(0.0, abc_pred * (1.0 - a_share_pred))
                    cb_ratio = np.clip(cb_ratio, 0.05, 12.0)
                    xlb_ratio = np.clip(xlb_ratio, 0.005, 12.0)
                    b_pred = bc_pool / (1.0 + cb_ratio)
                    c_pred = cb_ratio * b_pred
                    xl_pred = xlb_ratio * b_pred
                    return b_pred, c_pred, xl_pred, cb_ratio

                _sd_cur = _sd_reconstruct_current(_sd_abc_pred, _sd_cb_inc, _sd_xl_inc, _sd_a_share_pred)
                _sd_direct_base = _sd_reconstruct_direct(
                    _sd_abc_pred, _sd_cb_direct_base, _sd_xlb_direct_base, _sd_a_share_pred
                )
                _sd_direct_prod = _sd_reconstruct_direct(
                    _sd_abc_pred, _sd_cb_direct_prod, _sd_xlb_direct_prod, _sd_a_share_pred
                )

                def _sd_metrics(pred_tuple):
                    pb, pc, px, pcb = [np.asarray(x, dtype=float) for x in pred_tuple]
                    common = (
                        np.isfinite(pb) & np.isfinite(pc) & np.isfinite(px) & np.isfinite(pcb)
                        & np.isfinite(_sd_b) & np.isfinite(_sd_c) & np.isfinite(_sd_xl)
                        & np.isfinite(y_cb) & np.isfinite(y_total)
                    )
                    if int(common.sum()) < 12:
                        return None
                    actual_sum = _sd_b + _sd_c + _sd_xl
                    pred_sum = pb + pc + px
                    share_ok = common & (actual_sum > 0) & (pred_sum > 0)
                    if int(share_ok.sum()) < 12:
                        return None
                    a_shares = np.column_stack([
                        _sd_b[share_ok] / actual_sum[share_ok],
                        _sd_c[share_ok] / actual_sum[share_ok],
                        _sd_xl[share_ok] / actual_sum[share_ok],
                    ])
                    p_shares = np.column_stack([
                        pb[share_ok] / pred_sum[share_ok],
                        pc[share_ok] / pred_sum[share_ok],
                        px[share_ok] / pred_sum[share_ok],
                    ])
                    row_dist_loss = np.mean(np.abs(p_shares - a_shares), axis=1) * 100.0
                    total_pred = _sd_abc_pred[share_ok] + px[share_ok]
                    return {
                        "rows": int(share_ok.sum()),
                        "days": int(len(set(dates[np.where(share_ok)[0]]))),
                        "dist_mae_pp": float(np.mean(row_dist_loss)),
                        "cb_mae": float(np.mean(np.abs(pcb[share_ok] - y_cb[share_ok]))),
                        "b_mae": float(np.mean(np.abs(pb[share_ok] - _sd_b[share_ok]))),
                        "c_mae": float(np.mean(np.abs(pc[share_ok] - _sd_c[share_ok]))),
                        "xl_mae": float(np.mean(np.abs(px[share_ok] - _sd_xl[share_ok]))),
                        "large_mae": float(np.mean(np.abs((pc[share_ok] + px[share_ok]) - (_sd_c[share_ok] + _sd_xl[share_ok])))),
                        "total_mae": float(np.mean(np.abs(total_pred - y_total[share_ok]))),
                        "mask": share_ok,
                    }

                _sd_m_cur = _sd_metrics(_sd_cur)
                _sd_m_direct_base = _sd_metrics(_sd_direct_base)
                _sd_m_direct_prod = _sd_metrics(_sd_direct_prod)

                _sd_variants = {
                    "Praegune C/B + eraldi XL": (_sd_cur, _sd_m_cur),
                    "Otsene ALR baas": (_sd_direct_base, _sd_m_direct_base),
                    "Otsene ALR + praegused tunnused": (_sd_direct_prod, _sd_m_direct_prod),
                }
                _sd_valid_variants = {k:v for k,v in _sd_variants.items() if v[1] is not None}

                if _sd_m_cur and len(_sd_valid_variants) >= 2:
                    _sd_summary = {}
                    for _name, (_pt, _mm) in _sd_valid_variants.items():
                        common = _sd_m_cur["mask"] & _mm["mask"]
                        idx = np.where(common)[0]
                        if len(idx) < 12:
                            continue

                        def _comp_loss(_pred_tuple):
                            pb, pc, px, _ = [np.asarray(x, dtype=float) for x in _pred_tuple]
                            asum = _sd_b[idx] + _sd_c[idx] + _sd_xl[idx]
                            psum = pb[idx] + pc[idx] + px[idx]
                            ash = np.column_stack([_sd_b[idx]/asum, _sd_c[idx]/asum, _sd_xl[idx]/asum])
                            psh = np.column_stack([pb[idx]/psum, pc[idx]/psum, px[idx]/psum])
                            return np.mean(np.abs(psh-ash), axis=1)*100.0

                        base_loss = _comp_loss(_sd_cur)
                        cand_loss = _comp_loss(_pt)
                        day_values = dates[idx]
                        unique_days = sorted(set(day_values))
                        cut = max(1, len(unique_days)//2)
                        halves = [set(unique_days[:cut]), set(unique_days[cut:])]
                        half_improvements = []
                        for hs in halves:
                            hm = np.array([d in hs for d in day_values], dtype=bool)
                            if hm.any():
                                half_improvements.append(float(np.mean(base_loss[hm] - cand_loss[hm])))

                        _sd_summary[_name] = {
                            "rows": int(len(idx)),
                            "days": int(len(unique_days)),
                            "dist_mae_pp": float(np.mean(cand_loss)),
                            "dist_improvement_pp": float(np.mean(base_loss - cand_loss)),
                            "win_rows_pct": float(100.0*np.mean(cand_loss < base_loss)),
                            "worst_half_pp": float(min(half_improvements)) if half_improvements else None,
                            "cb_mae": float(np.mean(np.abs(np.asarray(_pt[3])[idx] - y_cb[idx]))),
                            "b_mae": float(np.mean(np.abs(np.asarray(_pt[0])[idx] - _sd_b[idx]))),
                            "c_mae": float(np.mean(np.abs(np.asarray(_pt[1])[idx] - _sd_c[idx]))),
                            "xl_mae": float(np.mean(np.abs(np.asarray(_pt[2])[idx] - _sd_xl[idx]))),
                            "large_mae": float(np.mean(np.abs((np.asarray(_pt[1])[idx] + np.asarray(_pt[2])[idx]) - (_sd_c[idx] + _sd_xl[idx])))),
                            "total_mae": float(np.mean(np.abs((_sd_abc_pred[idx] + np.asarray(_pt[2])[idx]) - y_total[idx]))),
                        }

                    _size_dist_stats = {
                        "status": "tested",
                        "test_version": _SIZE_DIST_TEST_VERSION,
                        "model_version": WD_MODEL_VERSION,
                        "incumbent_cb": cb_champion_name,
                        "incumbent_xl": xl_champion_name,
                        "primary_variant": "Otsene ALR + praegused tunnused",
                        "primary_metric": "B/C/XL composition MAE percentage-points",
                        "variants": _sd_summary,
                        "tested_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "previous_order_in_test": False,
                        "plant_index_in_test": False,
                        "changes_official_forecast": False,
                    }
                    _save_json_setting("joint_size_distribution_shadow_json", _size_dist_stats)
                else:
                    _size_dist_stats = {
                        "status": "insufficient_rows",
                        "test_version": _SIZE_DIST_TEST_VERSION,
            ...
            _sd_show = _size_dist_stats or _load_json_setting("joint_size_distribution_shadow_json", {})
            st.markdown("##### Otsene B/C/XL jaotus · shadow-test")
            if (
                isinstance(_sd_show, dict)
                and str(_sd_show.get("model_version") or "") == WD_MODEL_VERSION
                and str(_sd_show.get("test_version") or "") == _SIZE_DIST_TEST_VERSION
                and _sd_show.get("status") == "tested"
            ):
                _sv = _sd_show.get("variants") or {}
                _cur = _sv.get("Praegune C/B + eraldi XL") or {}
                _primary = _sv.get(str(_sd_show.get("primary_variant") or "")) or {}
                _plain = _sv.get("Otsene ALR baas") or {}
                _js1, _js2, _js3, _js4 = st.columns(4)
                _js1.metric("Praegune jaotuse MAE", "—" if not _cur else f"{float(_cur.get('dist_mae_pp')):.2f} pp")
                _js2.metric("Otsene ALR MAE", "—" if not _primary else f"{float(_primary.get('dist_mae_pp')):.2f} pp")
                _js3.metric("Paranemine", "—" if not _primary else f"{float(_primary.get('dist_improvement_pp')):+.2f} pp")
                _js4.metric("Võidab ridu", "—" if not _primary else f"{float(_primary.get('win_rows_pct')):.0f}%")
                if _primary:
                    st.caption(
                        f"Põhitest (ilma korjejärjekorrata): C/B MAE {float(_cur.get('cb_mae')):.2f} → {float(_primary.get('cb_mae')):.2f} · "
                        f"B {float(_cur.get('b_mae')):.2f} → {float(_primary.get('b_mae')):.2f} · "
                        f"C {float(_cur.get('c_mae')):.2f} → {float(_primary.get('c_mae')):.2f} · "
                        f"XL {float(_cur.get('xl_mae')):.2f} → {float(_primary.get('xl_mae')):.2f} · "
                        f"C+XL {float(_cur.get('large_mae')):.2f} → {float(_primary.get('large_mae')):.2f} · "
                        f"KOGU {float(_cur.get('total_mae')):.2f} → {float(_primary.get('total_mae')):.2f}. "
                        f"Halvim ajapool {float(_primary.get('worst_half_pp')):+.2f} pp. "
                        f"Eelmise korje order testis ei osale; ametlikku prognoosi test ei muuda."
                    )
                if _plain and _primary:
                    st.caption(
                        f"Diagnostika: otsene ALR ilma champion-lisatunnusteta {float(_plain.get('dist_mae_pp')):.2f} pp; "
                        f"samade praeguste lisatunnustega {float(_primary.get('dist_mae_pp')):.2f} pp."
                    )
            elif isinstance(_sd_show, dict) and _sd_show.get("status") == "insufficient_rows":
                st.caption("Otsese B/C/XL jaotuse shadow-test: piisavaid ühiseid walk-forward ridu veel ei ole.")

            # -------------------------------------------------------------------------
            # LAB-161 / PRODUCTION-SHADOW: previous-order residual test
            #
            # EI lisa orderit aktiivsesse C/B ega XL mudelisse. Võtab praeguse süsteemi
            # ausad walk-forward ennustused ja küsib, kas previous order seletab veel
            # allesjäänud viga. Positiivne residual = mudel ennustas liiga vähe.
            # -------------------------------------------------------------------------
            _residual_order_stats = None
            if USE_WINDDRY_ENGINE:
                _rr_pb, _rr_pc, _rr_px, _rr_pcb = [np.asarray(x, dtype=float) for x in _sd_cur]
                _rr_o2 = pd.to_numeric(model_df["Eelmine korje order 2"], errors="coerce").to_numpy(dtype=float)
                _rr_o3 = pd.to_numeric(model_df["Eelmine korje order 3"], errors="coerce").to_numpy(dtype=float)
                _rr_order = np.where(_rr_o3 > 0.5, 3, np.where(_rr_o2 > 0.5, 2, 1)).astype(int)

                _rr_large_actual = _sd_c + _sd_xl
                _rr_large_pred = _rr_pc + _rr_px
                _rr_res_large = _rr_large_actual - _rr_large_pred
                _rr_res_c = _sd_c - _rr_pc
                _rr_res_xl = _sd_xl - _rr_px
                _rr_res_cb = y_cb - _rr_pcb

                _rr_common = (
                    np.isfinite(_rr_large_actual) & np.isfinite(_rr_large_pred)
                    & np.isfinite(_rr_res_c) & np.isfinite(_rr_res_xl) & np.isfinite(_rr_res_cb)
                )

                def _rr_group(order_value):
                    m = _rr_common & (_rr_order == int(order_value))
                    if int(m.sum()) == 0:
                        return None
                    return {
                        "n": int(m.sum()),
                        "cxl_mean": float(np.mean(_rr_res_large[m])),
                        "cxl_median": float(np.median(_rr_res_large[m])),
                        "cxl_positive_pct": float(100.0 * np.mean(_rr_res_large[m] > 0)),
                        "c_mean": float(np.mean(_rr_res_c[m])),
                        "xl_mean": float(np.mean(_rr_res_xl[m])),
                        "cb_mean": float(np.mean(_rr_res_cb[m])),
                    }

                _rr_groups = {str(o): _rr_group(o) for o in (1, 2, 3)}

                def _rr_expanding(mode):
                    # Strict OOS: sihtpäeva korrektsioon võib kasutada ainult varasemate
                    # testipäevade incumbent-residuaale. MAE jaoks kasutame robustset
                    # varasema sama-order residuali mediaani; vähemalt 4 varasemat rida.
                    cand = _rr_large_pred.copy()
                    used = np.zeros(len(model_df), dtype=bool)
                    corrections = np.zeros(len(model_df), dtype=float)
                    for td in sorted(set(dates)):
                        test_idx = np.where((dates == td) & _rr_common)[0]
                        prior_mask = (dates < td) & _rr_common
                        for ii in test_idx:
                            oo = int(_rr_order[ii])
                            if mode == "order3" and oo != 3:
                                continue
                            same = prior_mask & (_rr_order == oo)
                            if int(same.sum()) < 4:
                                continue
                            corr = float(np.median(_rr_res_large[same]))
                            cand[ii] = max(0.0, cand[ii] + corr)
                            corrections[ii] = corr
                            used[ii] = True

                    eval_mask = _rr_common & np.isfinite(cand)
                    if int(eval_mask.sum()) < 12:
                        return None
                    base_ae = np.abs(_rr_large_pred[eval_mask] - _rr_large_actual[eval_mask])
                    cand_ae = np.abs(cand[eval_mask] - _rr_large_actual[eval_mask])
                    used_eval = used[eval_mask]
                    idx_eval = np.where(eval_mask)[0]
                    used_idx = idx_eval[used_eval]

                    corrected_win_pct = None
                    corrected_improvement = None
                    worst_half = None
                    if len(used_idx):
                        bu = np.abs(_rr_large_pred[used_idx] - _rr_large_actual[used_idx])
                        cu = np.abs(cand[used_idx] - _rr_large_actual[used_idx])
                        corrected_win_pct = float(100.0 * np.mean(cu < bu))
                        corrected_improvement = float(np.mean(bu - cu))
                        used_days = sorted(set(dates[used_idx]))
                        cut = max(1, len(used_days) // 2)
                        half_imps = []
                        for hs in (set(used_days[:cut]), set(used_days[cut:])):
                            if not hs:
                                continue
                            hm = np.array([d in hs for d in dates[used_idx]], dtype=bool)
                            if hm.any():
                                half_imps.append(float(np.mean(bu[hm] - cu[hm])))
                        if half_imps:
                            worst_half = float(min(half_imps))

                    return {
                        "rows": int(eval_mask.sum()),
                        "days": int(len(set(dates[np.where(eval_mask)[0]]))),
                        "corrected_rows": int(used.sum()),
                        "base_mae": float(np.mean(base_ae)),
                        "candidate_mae": float(np.mean(cand_ae)),
                        "improvement": float(np.mean(base_ae - cand_ae)),
                        "corrected_win_pct": corrected_win_pct,
                        "corrected_improvement": corrected_improvement,
                        "worst_half": worst_half,
                        "median_abs_correction": float(np.median(np.abs(corrections[used]))) if used.any() else None,
                    }

                _rr_order3 = _rr_expanding("order3")
                _rr_all = _rr_expanding("all")

                if int(_rr_common.sum()) >= 12:
                    _residual_order_stats = {
                        "status": "tested",
                        "test_version": _RESIDUAL_ORDER_TEST_VERSION,
                        "model_version": WD_MODEL_VERSION,
                        "rows": int(_rr_common.sum()),
                        "days": int(len(set(dates[np.where(_rr_common)[0]]))),
                        "groups": _rr_groups,
                        "order3_only_correction": _rr_order3,
                        "all_orders_correction": _rr_all,
                        "data_hash": str(locals().get("_light_data_hash") or ""),
                        "tested_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "changes_official_forecast": False,
                        "plant_index_in_test": False,
                    }
                ...
                _save_json_setting("wd_prev_order_residual_json", _residual_order_stats)

            _rr_show = _residual_order_stats or _load_json_setting("wd_prev_order_residual_json", {})
            st.markdown("##### Korjejäägi residual-test")
            if (
                isinstance(_rr_show, dict)
                and str(_rr_show.get("model_version") or "") == WD_MODEL_VERSION
                and str(_rr_show.get("test_version") or "") == _RESIDUAL_ORDER_TEST_VERSION
                and _rr_show.get("status") == "tested"
            ):
                _rgg = _rr_show.get("groups") or {}
                _rg1, _rg2, _rg3 = _rgg.get("1") or {}, _rgg.get("2") or {}, _rgg.get("3") or {}
                _rm1, _rm2, _rm3, _rm4 = st.columns(4)
                _rm1.metric("Order 1 · C+XL jääk", "—" if not _rg1 else f"{float(_rg1.get('cxl_mean')):+.2f}")
                _rm2.metric("Order 2 · C+XL jääk", "—" if not _rg2 else f"{float(_rg2.get('cxl_mean')):+.2f}")
                _rm3.metric("Order 3 · C+XL jääk", "—" if not _rg3 else f"{float(_rg3.get('cxl_mean')):+.2f}")
                _rm4.metric("Order 3 · tegelik > prognoos", "—" if not _rg3 else f"{float(_rg3.get('cxl_positive_pct')):.0f}%")
                st.caption(
                    "Positiivne C+XL jääk tähendab, et praegune C/B + XL süsteem ennustas suuri vilju liiga vähe. "
                    + (
                        f"Order 3 ({int(_rg3.get('n', 0))} rida): C jääk {float(_rg3.get('c_mean')):+.2f}, "
                        f"XL jääk {float(_rg3.get('xl_mean')):+.2f}, C/B jääk {float(_rg3.get('cb_mean')):+.2f}; "
                        f"C+XL mediaan {float(_rg3.get('cxl_median')):+.2f}."
                        if _rg3 else ""
                    )
                )
                _ro3 = _rr_show.get("order3_only_correction") or {}
                _ral = _rr_show.get("all_orders_correction") or {}
                if _ro3:
                    _rc1, _rc2, _rc3, _rc4 = st.columns(4)
                    _rc1.metric("C+XL baas MAE", f"{float(_ro3.get('base_mae')):.2f}")
                    _rc2.metric("Order3 residual-korr MAE", f"{float(_ro3.get('candidate_mae')):.2f}")
                    _rc3.metric("Paranemine", f"{float(_ro3.get('improvement')):+.2f}")
                    _rc4.metric("Korrigeeritud ridu", str(int(_ro3.get('corrected_rows') or 0)))
                    st.caption(
                        f"Strict expanding test: korrektsioon õpitakse ainult varasematest sama-order OOS vigadest (min 4 rida). "
                        f"Korrigeeritud ridadel võidab "
                        f"{'—' if _ro3.get('corrected_win_pct') is None else f"{float(_ro3.get('corrected_win_pct')):.0f}%"}; "
                        f"halvim ajapool "
                        f"{'—' if _ro3.get('worst_half') is None else f"{float(_ro3.get('worst_half')):+.2f}"}. "
                        f"Kõigi orderite residual-korrektsioon: "
                        f"{'—' if not _ral else f"{float(_ral.get('base_mae')):.2f} → {float(_ral.get('candidate_mae')):.2f} ({float(_ral.get('improvement')):+.2f})"}. "
                        "Ametlikku prognoosi test ei muuda."
                    )
            elif isinstance(_rr_show, dict) and _rr_show.get("status") == "insufficient_rows":
                st.caption("Korjejäägi residual-test: piisavaid ühiseid walk-forward ridu veel ei ole.")
            ...
            if USE_WINDDRY_ENGINE:
                # AKTIIVSE MOOTORI LUKK. Research võib shadow-võtmesse kirjutada,
                # kuid abc_champion_state_json tähistab ainult operatiivset production-mootorit.
                champion_name = "WIND×DRY HIGH L3–7 · LEVEL+DELTA"
                champion_cols = list(WD_LOCKED_COLS)
                champion_mae = None
                champion_stats = None
            ...
            # -------------------------------------------------------------------------
            st.markdown("##### 9 päeva saagiprognoos · WIND×DRY")
            st.caption(
                f"A+B+C aktiivne mootor: {champion_name}. "
                f"Baas põhineb ilmal, intervallil, hooaja faasil ja põllu identiteedil; lukustatud lisatunnus on "
                f"WIND×DRY HIGH L3–7 LEVEL+DELTA. "
                f"A+B+C prognoosi juhib lukustatud WIND×DRY kiht; vanad champion-kandidaadid ei kirjuta seda üle. "
            ...
            # WIND×DRY live-map: forecast täidab tuleviku, mõõdetud/kontrollitud päev
            # kirjutab sama kuupäeva alati üle. HIGH lävi õpib ainult mõõdetud päevadest.
            _wd_live_by_day = {}
            _wd_measured_by_day = {}
            for _wr in all_weather_rows:
                _wkey = str(_wr.get("weather_date") or "")
                if not _wkey or _wd_daily_value(_wr) is None:
                    continue
                _kind = str(_wr.get("data_kind") or "").strip().lower()
                if _kind == "forecast":
                    _wd_live_by_day[_wkey] = _wr
            for _wr in all_weather_rows:
                _wkey = str(_wr.get("weather_date") or "")
                if not _wkey or _wd_daily_value(_wr) is None:
                    continue
                _kind = str(_wr.get("data_kind") or "").strip().lower()
                if _kind == "measured" and bool(_wr.get("checked")):
                    _wd_day = _wd_parse_day(_wkey)
                    if _wd_day is not None and _wd_day >= WD_WEATHER_START:
                        _wd_measured_by_day[_wkey] = _wr
                    # Väärtusaknas võidab kontrollitud measured alati sama päeva forecasti.
                    _wd_live_by_day[_wkey] = _wr
            ...
            def _weather_window_for_prediction(previous_day, target_day):
                ...
                if USE_WINDDRY_ENGINE:
                    _wdx = _wd_locked_features(
                        target_day, previous_day, _wd_live_by_day, _wd_measured_by_day, allow_delta=True
                    )
                    wx.update(_wdx)
            ...
            # Future forecast rotation is anchored to today's confirmed work plan.
            # Non-empty saved plans on FUTURE dates are deliberately ignored: they may be
            # stale or partial and must not jump the 14-field rotation. One exception is
            # intentional and authoritative: an explicitly saved EMPTY plan [] means
            # "korjet ei ole". On such a day the forecast shows no harvest and the rotation
            # does NOT advance; the same next field remains first on the following harvest day.
            try:
                _ce_saved_future_plans = db.get_harvest_plans() or {}
            except Exception:
                _ce_saved_future_plans = {}
            ...
            for offset in range(1, 10):
                ...
                # Only an explicit empty saved plan has future scheduling authority.
                # Missing key = no scheduling information. Non-empty value = ignored here.
                _saved_future_plan = _ce_saved_future_plans.get(target_day.isoformat())
                if _saved_future_plan is not None:
                    try:
                        _saved_future_plan_list = list(_saved_future_plan)
                    except TypeError:
                        _saved_future_plan_list = [_saved_future_plan]
                    if len(_saved_future_plan_list) == 0:
                        forecast_days.append((target_day, []))
                        continue
            ...
            # Legacy shadow/challenger and plant-index state are deliberately disabled.
            # They were not part of LAB44–47 and must not alter the official CYCLE output.
            try:
            ...
            try:
                if db.yield_forecasts_available():
                    _forecast_history_all = db.get_yield_forecasts(limit=5000)
                    forecast_history_rows = [
                        r for r in _forecast_history_all
                        if str(r.get("model_version") or "") == MODEL_VERSION
                    ]
            except db.DatabaseError:
                forecast_history_rows = []
            ...
            for target_day, rows_day in forecast_detail_days:
                ...
                if target_day == TODAY:
                    ...
                    st.caption("täna · CYCLE+WEATHER tööprognoos enne tänaste tegelike korjete mõju")
                elif lead >= 6:
                    header_html = (
                ...
                if not rows_day:
                    st.caption("Korjevaba päev · korjering ei liigu edasi.")
                    continue
                ...
                visible_cols = [
                    ...
                    if not str(c).startswith("_") and c not in {"Taimeindeks", "Alus"}
                ]
                day_df_visible = day_df[visible_cols]
                st.dataframe(day_df_visible.style.format({
                    "Taimeindeks": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                ...
                for r in rows_day:
                    ...
                    erow = {
                        ...
                        "Taimeindeks": effects.get("Taimeindeks", 0.0),
                ...
                if False and explain_rows:
                    st.caption("A+B+C CYCLE-selgitus · +/− = tsükli tunnuse panus kastides võrreldes PERSIST-põhise mudelibaasiga")
            ...
            _active_abc_mae = (
                float(_wd_acceptance_stats.get("Katse MAE"))
                if USE_WINDDRY_ENGINE and isinstance(_wd_acceptance_stats, dict) and _wd_acceptance_stats.get("Katse MAE") is not None
                else current_test_mae
            )
            abc_mae_text = f"{_active_abc_mae:.1f} kasti/põld" if _active_abc_mae is not None else "veel hindamata"
            total_mae_text = f"{current_total_test_mae:.1f} kasti/põld" if current_total_test_mae is not None else "veel hindamata"
            st.caption(
                f"Aktiivse A+B+C ajaline testviga: {abc_mae_text}. Kogusaagi baastestviga (ABC+XL): {total_mae_text}. "
            ...
            if forecast_store_ok:
                ...
                st.caption("Iga päev jääb alles aktiivse WIND×DRY production-mootori 1–9 päeva ette tehtud snapshot.")
                try:
                    ...
                    if "layered_trace_df" in locals() and not layered_trace_df.empty:
                        ...
                        with st.expander("Näita kõiki kihilisi BIO-mudeleid"):
                            _layered_fmt_all = {
                                ...
                                "Viimased 5 MAE": "{:.2f}",
                                "Viimased 5 Δ": "{:+.2f}",
                                "Viimased 5 võidab %": "{:.0f}%",
                            }
                            _layered_fmt = {k: v for k, v in _layered_fmt_all.items() if k in layered_trace_df.columns}

                            # Mobiilis / osas Streamliti versioonides ei ole st.dataframe rea-klikk
                            # usaldusväärselt valitav. Näitame tabelis fikseeritud reanumbrit ja
                            # valime kandidaadi eraldi selectboxist. Nii on valik üheselt töötav
                            # sõltumata brauserist ning kasutaja saab valida täpselt tabelirea järgi.
                            _layered_display_df = layered_trace_df.copy().reset_index(drop=True)
                            _layered_display_df.insert(0, "Rida", range(1, len(_layered_display_df) + 1))
                            st.dataframe(
                                _layered_display_df.style.format(_layered_fmt),
                                use_container_width=True,
                                hide_index=True,
                            )

                            st.caption(
                                "See on detailtabel. Kogu-hooaja OOS-i kandidaat vali nüüd lehe ülaosas "
                                "aktiivse WIND×DRY võrdluspunkti juures olevast tabelist."
                            )
                    else:
                        st.caption(
        ...
        # Forecast-dirty tohib maha võtta ainult siis, kui ametliku CYCLE mootori
        # snapshot salvestus päriselt õnnestus JA batchis oli vähemalt üks rida.
        # Nii ei saa "ring jooksis" enam tähendada "prognoosi pole, aga dirty=0".
        _official_forecast_saved = bool(
            locals().get("forecast_store_ok", False)
            and locals().get("forecast_payloads")
        )
        if _official_forecast_saved:
            db.set_app_setting("forecast_dirty", "0")
            db.set_app_setting("forecast_last_error", "")
        else:
            db.set_app_setting("forecast_dirty", "1")
            _payload_count = len(locals().get("forecast_payloads") or [])
            _store_flag = bool(locals().get("forecast_store_ok", False))
            db.set_app_setting(
                "forecast_last_error",
                f"CYCLE snapshot salvestamata: payload_rows={_payload_count}, store_ok={_store_flag}",
            )
    ...
    # Taustaringi ajutine sisu ei kuulu ühelegi kasutajavaatele.
    ...
    if page == "Täna" and _forecast_refresh_due and bool(locals().get("_official_forecast_saved", False)):
        st.rerun()
...
if page == "Prognoos":
    st.subheader("Prognoos")
    st.caption(
        "Viimane salvestatud korje-eelne prognoos. Uus snapshot tekib automaatselt, "
        "kui muutub ilm, korje või päev."
    )

    st.success("CYCLE + WEATHER · põllupõhine A+B+C prognoos")
    if not _history_gate_ok:
        st.warning(_history_gate_warning)

    try:
        _ro_saved_all = db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []
        _ro_saved = [
            r for r in _ro_saved_all
            if str(r.get("model_version") or "") == CYCLE_MODEL_VERSION
        ]
    except db.DatabaseError:
        _ro_saved = []
    ...
    # Hooaja lõpu tööplaan: kasutaja võib järgmise 9 päeva korjed ette määrata.
    # Tühi salvestatud valik tähendab teadlikult "korjet ei ole".
    try:
        _ro_future_plans = db.get_harvest_plans() or {}
    except Exception:
        _ro_future_plans = {}

    with st.expander("Muuda tulevast korjeplaani"):
        _plan_edit_date = st.date_input(
            "Päev",
            value=TODAY + timedelta(days=1),
            min_value=TODAY + timedelta(days=1),
            max_value=TODAY + timedelta(days=9),
            key="forecast_future_plan_date",
        )
        _plan_edit_key = _plan_edit_date.isoformat()
        _saved_edit_plan = _ro_future_plans.get(_plan_edit_key)
        if _saved_edit_plan is not None:
            try:
                _plan_edit_default = [int(f) for f in list(_saved_edit_plan)[:4]]
            except TypeError:
                _plan_edit_default = [int(_saved_edit_plan)]
            _plan_edit_status = "käsitsi salvestatud"
        else:
            # Reconstruct the same default 3-field cursor used by the forecast engine,
            # including any manually saved plans on earlier future days.
            try:
                _ui_today_plan = db.get_harvest_plan(TODAY)
            except Exception:
                _ui_today_plan = None
            if _ui_today_plan is None:
                try:
                    _ui_today_rows = db.get_harvest_for_day(TODAY)
                    _ui_hist = db.get_harvest_history()
                    _ui_today_plan = _planned_fields_for_day(TODAY, _ui_today_rows, _ui_hist)
                except Exception:
                    _ui_today_plan = []
            _ui_today_plan = [int(f) for f in list(_ui_today_plan or [])[:4]]
            if _ui_today_plan:
                _ui_cursor = _next_field(int(_ui_today_plan[-1]))
            else:
                try:
                    _ui_hist_rows = db.get_harvest_history()
                    _ui_hist_events = []
                    for _hr in _ui_hist_rows:
                        try:
                            _hd = date.fromisoformat(str(_hr.get("harvest_date"))[:10])
                            _hf = int(_hr.get("field_no"))
                            _ho = int(_hr.get("harvest_order") or 1)
                        except Exception:
                            continue
                        if _hd <= TODAY:
                            _ui_hist_events.append((_hd, _ho, _hf))
                    _ui_hist_events.sort()
                    _ui_cursor = _next_field(int(_ui_hist_events[-1][2])) if _ui_hist_events else 1
                except Exception:
                    _ui_cursor = 1

            _plan_edit_default = []
            _walk_day = TODAY + timedelta(days=1)
            while _walk_day <= _plan_edit_date:
                _walk_saved = _ro_future_plans.get(_walk_day.isoformat())
                if _walk_saved is not None:
                    _walk_fields = [int(f) for f in list(_walk_saved)[:4]]
                else:
                    _walk_fields = [
                        int(_ui_cursor),
                        int(_next_field(_ui_cursor)),
                        int(_next_field(_next_field(_ui_cursor))),
                    ]
                if _walk_day == _plan_edit_date:
                    _plan_edit_default = list(_walk_fields)
                    break
                if _walk_fields:
                    _ui_cursor = _next_field(int(_walk_fields[-1]))
                _walk_day += timedelta(days=1)
            _plan_edit_status = "automaatne ettepanek"

        _future_widget_key = f"future_plan_fields_{_plan_edit_key}"
        _future_date_state_key = f"future_plan_default_{_plan_edit_key}"
        if _future_date_state_key not in st.session_state:
            st.session_state[_future_widget_key] = list(_plan_edit_default)
            st.session_state[_future_date_state_key] = list(_plan_edit_default)

        _future_selected = st.multiselect(
            "Selle päeva korjepõllud",
            options=list(range(1, 15)),
            key=_future_widget_key,
            max_selections=4,
            help="Valiku järjekord on korjejärjekord. Jäta valik tühjaks, kui sellel päeval korjet ei ole.",
        )
        _future_selected = [int(f) for f in _future_selected]
        st.caption(
            f"Praegu: {_plan_edit_status}. Tühi valik = korjet ei ole. "
            "Tulevikuplaan mõjutab prognoosi simulatsiooni, mitte tegelikku korjeajalugu."
        )
        if st.button("Salvesta tulevikuplaan", key=f"save_future_plan_{_plan_edit_key}"):
            db.save_harvest_plan(_plan_edit_date, _future_selected)
            st.session_state[_future_date_state_key] = list(_future_selected)
            _mark_forecast_dirty(
                f"tuleviku korjeplaan muutus {_plan_edit_key}: "
                + (",".join(str(f) for f in _future_selected) if _future_selected else "korjet ei ole")
            )
            st.rerun()
    ...
    for _lead in range(0, 10):
        ...
        _explicit_no_harvest = False
        _visible_plan = None
        if _lead == 0:
            # Keep the original lead=0 batch in DB for no-hindsight audit, but the
            # visible today's forecast must follow the CURRENT confirmed work plan.
            try:
                _visible_plan = db.get_harvest_plan(TODAY)
            except Exception:
                _visible_plan = None
        else:
            _visible_plan = _ro_future_plans.get(_target_day.isoformat())

        if _visible_plan is not None:
            _visible_plan_list = [int(f) for f in list(_visible_plan)]
            _visible_plan_set = set(_visible_plan_list)
            if not _visible_plan_list:
                _rows = []
                _explicit_no_harvest = True
            else:
                _rows = [
                    _r for _r in _rows
                    if int(_r.get("field_no")) in _visible_plan_set
                ]
                if {int(_r.get("field_no")) for _r in _rows} != _visible_plan_set:
                    # Never show a stale/partial batch as if it matched the new work plan.
                    _rows = []
                else:
                    # Keep the user's planned order visible.
                    _order_map = {f: i for i, f in enumerate(_visible_plan_list)}
                    _rows = sorted(_rows, key=lambda _r: _order_map.get(int(_r.get("field_no")), 999))

        if not _history_gate_ok:
            _rows = _zero_forecast_rows(_rows)
        if not _rows and not _explicit_no_harvest:
            continue

        if _explicit_no_harvest:
            _total_day = 0.0
            _total_text = "0,0 kasti"
            if _lead >= 6:
                _header_html = (
                    '<div style="background:#fff3cd;border:1px solid #ffe69c;border-radius:10px;'
                    'padding:8px 12px;margin:12px 0 6px 0;">'
            ...
            st.caption(f"{_lead} p ette · korjet ei ole")
            _ro_shown += 1
            continue
        if _visible_plan is None:
        ...
        if _lead >= 6:
            ...
            st.markdown(f"### {_weekday_letter(_target_day)} {_short_date(_target_day)}  {_total_text}")
            if _lead == 0:
                st.caption("täna · praeguse tööplaani vaade · ametlik korje-eelne snapshot säilib auditis")
            else:
                _confidence = "kõrgem kindlus" if _lead <= 3 else "keskmine kindlus"
                _manual_plan = _ro_future_plans.get(_target_day.isoformat())
                if _manual_plan is not None:
                    _manual_fields = ", ".join(str(int(f)) for f in list(_manual_plan))
                    st.caption(f"{_lead} p ette · käsitsi plaan: põllud {_manual_fields} · {_confidence}")
                else:
        ...
        if False and _history_gate_ok and _explain_matches:
            _explain_rows = []
            _input_lines = []
            for _f in sorted(_snapshot_fields):
                ...
                _exrow = {
                    ...
                    "Eelmine tootlikkus": _effects.get("Eelmine tootlikkus", 0.0),
    ...
    if _ro_shown == 0:
        _ro_last_error = str(db.get_app_setting("forecast_last_error", "") or "").strip()
        if _ro_last_error:
            st.warning(
                "Ametlikku CYCLE prognoosi ei õnnestunud veel salvestada. "
                "Rakendus proovib snapshot'i uuesti; tehniline põhjus: " + _ro_last_error
            )
        else:
            st.info(
                "Salvestatud ametlikku CYCLE prognoosi veel pole. "
                "Rakendus käivitab esimese snapshot'i automaatselt."
            )
    else:
        try:
...
if page == "Mootor":
    st.subheader("Mootor")
    st.success("KurgiMootor V7.0 · aktiivne A+B+C mootor: CYCLE + WEATHER")
    st.caption(
        "Üks mootor, üks ametlik prognoos. Vana taimeindeks, WIND×DRY production-kiht, "
        "champion/shadow ja Jäljeotsija ei osale enam ametlikus A+B+C prognoosis."
    )

    _m1, _m2, _m3 = st.columns(3)
    _m1.metric("Ridge λ", "8")
    _m2.metric("Min train", "24 rida")
    try:
        _motor_train_n = int(float(db.get_app_setting("cycle_last_training_rows", "0") or 0))
    except Exception:
        _motor_train_n = 0
    _m3.metric("Viimane train N", str(_motor_train_n) if _motor_train_n else "—")

    st.markdown("#### A+B+C sisendid")
    st.markdown(
        "- sama põllu eelmise tsükli päevatootlikkus\n"
        "- korjeintervalli muutus ja hooaja vanus\n"
        "- tsükkel-vs-eelmine-tsükkel: radiatsioon, keskmine temperatuur, Tmin, "
        "WIND×DRY, sademed ja ET0"
    )

    st.markdown("#### Tööloogika")
    st.caption(
        "Ajalooline õppimine kasutab ainult kontrollitud mõõdetud ilma. Operatiivse state'i ja tulevikutsükli jaoks "
        "kasutatakse viimase värske päeva täielikku ajutist mõõdetud rida või lukustatud forecast-snapshot'i kuni checked ilma saabumiseni; "
        "tulevastel päevadel kasutatakse ilmaprognoosi. See fallback ei lähe treeningusse. "
        "Korjepäeva enda ilm ei lähe selle korje A+B+C sisendisse."
    )
    st.caption(
        "XL = eelmise tsükli XL päevatootlikkuse püsimine. C/B = viimase tegeliku sama põllu suhe. "
        "Need on väljundid ega toida A+B+C mootorit tagasi."
    )

    try:
        _motor_last_at = str(db.get_app_setting("forecast_last_checked_at", "") or "")
        if _motor_last_at:
            _motor_dt = datetime.fromisoformat(_motor_last_at).astimezone(ZoneInfo("Europe/Tallinn"))
            st.info(f"Viimane ametlik prognoosiring: {_motor_dt.strftime('%d.%m %H:%M')}")
    except Exception:
        pass

    with st.expander("⏱ Käivituse diagnostika", expanded=False):
        _first_boot = st.session_state.get("_km_startup_first_run")
        _last_boot = st.session_state.get("_km_startup_last_run")
        if _first_boot:
            _boot_c1, _boot_c2 = st.columns(2)
            _boot_c1.metric("Sessiooni esimene app.py ring", f"{float(_first_boot.get('total_s', 0)):.1f} s")
            if _last_boot:
                _boot_c2.metric("Viimane rerun", f"{float(_last_boot.get('total_s', 0)):.1f} s")
            try:
                _boot_dt = datetime.fromisoformat(str(_first_boot.get("captured_at"))).astimezone(ZoneInfo("Europe/Tallinn"))
                st.caption(f"Esimene ring mõõdetud {_boot_dt.strftime('%d.%m %H:%M:%S')} · {str(_first_boot.get('note') or 'tavaline ring')}")
            except Exception:
                pass
            _boot_df = pd.DataFrame(list(_first_boot.get("rows") or []))
            if not _boot_df.empty:
                st.dataframe(_boot_df, use_container_width=True, hide_index=True)
            st.caption(
                "See kell algab app.py esimesest reast. Kui telefonis mõõdetud avamisaeg on sellest palju pikem, "
                "on vahe Streamlit Cloudi ärkamises / konteineri käivitumises enne app.py käivitust."
            )
        else:
            st.caption("Esimese sessiooniringi mõõdik tekib pärast selle ringi lõpetamist või automaatset prognoosi-rerun'i.")
...
# Production-10 startup diagnostics: preserve first/last completed script run.
_km_startup_mark("script end")
_km_commit_startup_diag("normal script end")
