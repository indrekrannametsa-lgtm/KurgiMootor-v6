...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    def _ro_latest_snapshot_rows(target_day_value):
        picked = {}
        for _r in _ro_saved:
            if str(_r.get("target_date")) != target_day_value.isoformat():
                continue
            _fdate = str(_r.get("forecast_date") or "")
            if not _fdate:
                continue
            try:
                ...
                continue
            _key = (_fdate, _fno)
            _old = picked.get(_key)
            if _old is None or str(_r.get("generated_at") or "") > str(_old.get("generated_at") or ""):
                picked[_key] = _r
        if not picked:
            return []
        _by_date = {}
        for (_fdate, _fno), _row in picked.items():
            _by_date.setdefault(_fdate, {})[_fno] = _row
        _latest_date = sorted(_by_date.keys())[-1]
        return list(_by_date[_latest_date].values())
