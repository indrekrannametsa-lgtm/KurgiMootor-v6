...
if page == "Täna":
    # Väike märge ainult siis, kui kerge Jäljeotsija näeb lävendit ületavat tugevat jälge.
    _light_alert_raw = db.get_app_setting("light_discovery_alert_json", "")
    if _light_alert_raw:
        try:
            _la = json.loads(_light_alert_raw)
            _la_name = str(_la.get("name") or "uus kandidaat")
            _la_imp = float(_la.get("improvement"))
            st.warning(f"🟡 Mootoril on tugev uus jälg: {_la_name} · Δ {_la_imp:+.2f}")
        except Exception:
            pass
...
if page in ("Prognoos", "Mootori tähelepanekud") or _auto_light_check:
        ...
        if training_rows:
            ...
            try:
                ...
                if not _alert_candidates.empty:
                    ...
                    db.set_app_setting(
                        "light_discovery_alert_json",
                        json.dumps(_alert_payload, ensure_ascii=False),
                    )
                else:
                    # Kui uus kontrollring enam lävendit ületavat jälge ei näe,
                    # eemaldame vana märgi, et Avaleht ei jääks valet häiret näitama.
            ...
            MODEL_VERSION = "v6.5-layered-bio-v8-auto-trace-alert"
            forecast_payloads = []
            ...
            if valid_pred.any():
                ...
                if not trace_df.empty:
                    ...
                    _response_fade = _response_fade_diagnostic()
...
if page == "Mootori tähelepanekud":
    ...
    st.caption(
        "Igapäevane prognoos ei käivita laia uuringut. Lai kihiline uuring on käsitsi; "
