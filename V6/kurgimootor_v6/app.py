...
APP_PATCH_VERSION = "wd-leveldelta-production-4-season-oos-audit-2"  # diagnostic only: active-engine benchmark + filtered challenger season OOS
...
_season_oos_selected_parts = st.session_state.pop("_season_oos_selected_parts_run", None)
...
if page == "Mootori tähelepanekud":
    ...
    if isinstance(_season_oos_saved, dict) and (
        ...
        if _active_baseline:
            ...
            _fh = _active_baseline.get("daily_first_half_mae")
            _sh = _active_baseline.get("daily_second_half_mae")
            ...
            st.caption(
                ...
                + (f"Päev MAE: hooaja I pool {float(_fh):.2f}, II pool {float(_sh):.2f}. " if _fh is not None and _sh is not None else "")
        ...
        if not _season_rows.empty:
            ...
            _season_show = _season_rows.rename(columns={
                ...
                "daily_days": "Päevi",
                "daily_worst_half": "Päev halvim pool",
            })
            _season_cols = [
                "Kihte", "Koosseis", "OOS baas MAE", "OOS katse MAE", "OOS Δ", "OOS võidab %", "OOS ridu",
                "Päev baas MAE", "Päev katse MAE", "Päev Δ", "Päev võidab %", "Päevi", "Päev halvim pool",
                "Kinnitus stabiilne",
            ]
            ...
            st.dataframe(
                _season_show[_season_cols].style.format({
                    ...
                    "Päev Δ": "{:+.2f}", "Päev võidab %": "{:.0f}%", "Päev halvim pool": "{:+.2f}",
                }),
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            def _active_engine_full_oos_stats(base_pred, target):
                """Aktiivse production-mootori enda fikseeritud kogu-hooaja strict OOS benchmark.

                Erinevalt kandidaadivõrdlusest ei sõltu see ühegi challengeri puuduvatest
                ridadest: kasutatakse kõiki ridu, millel aktiivne mootor ja tegelik siht on olemas.
                Päev MAE-sse lähevad ainult täpselt 3 põlluga päevad.
                """
                ...
                daily_mae = first_half_mae = second_half_mae = worst_error = None
                worst_day = None
                if daily_rows:
                    _de = np.asarray([r[3] for r in daily_rows], dtype=float)
                    daily_mae = float(np.mean(_de))
                    _halves = np.array_split(np.arange(len(daily_rows)), 2)
                    if len(_halves) >= 1 and len(_halves[0]):
                        first_half_mae = float(np.mean(_de[_halves[0]]))
                    if len(_halves) >= 2 and len(_halves[1]):
                        second_half_mae = float(np.mean(_de[_halves[1]]))
                ...
                return {
                    ...
                    "daily_first_half_mae": first_half_mae,
                    "daily_second_half_mae": second_half_mae,
                    "daily_worst_error": worst_error,
                    "daily_worst_day": worst_day,
                }

            def _full_season_oos_stats(base_pred, candidate_pred, target):
                """Kogu strict walk-forward ajalugu + päris 3-põllu päevakoond.

                Kasutab ainult ridu, millel nii incumbent kui kandidaat on OOS-prediktsiooni
                andnud. Päevakoondi MAE-sse lähevad ainult päevad, kus samal ühisel maskil
                on täpselt 3 põldu; nii ei võrrelda osalist päeva täispäevaga.
                """
                base_pred = np.asarray(base_pred, dtype=float)
                ...
                day_base_mae = day_cand_mae = day_win = day_worst_half = None
                if daily_rows:
                    ...
                    for _h in _halves:
                        if len(_h):
                            _half_imps.append(float(np.mean(_db[_h]) - np.mean(_dc[_h])))
                    if _half_imps:
                        day_worst_half = float(min(_half_imps))
                ...
                return {
                    ...
                    "daily_days": int(len(daily_rows)),
                    "daily_worst_half": day_worst_half,
                }
            ...
            if _season_oos_run_now or _season_oos_selected_parts:
                ...
                _save_json_setting("layered_season_oos_json", {
                ...
                # Sama vajutuse järel kuva värskelt salvestatud vastused kohe üleval.
                st.rerun()
            ...
            if _layered_run_now:
                ...
                while (
                    ...
                    _row = {
                        ...
                        "daily_worst_half": (None if not _cand.get("full_oos") else _cand["full_oos"].get("daily_worst_half")),
                    }
                ...
                for _r in _top_rows:
                    layered_trace_rows.append({
                        ...
                        "Päev halvim pool": _r.get("daily_worst_half"),
                    })
