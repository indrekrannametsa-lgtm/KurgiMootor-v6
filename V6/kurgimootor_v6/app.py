...
APP_PATCH_VERSION = "wd-leveldelta-production-4-season-oos-audit-10-top-candidate-picker"  # diagnostic only: active-engine benchmark + filtered challenger season OOS
...
if _run_operational_cycle:
    ...
    if page != "Prognoos":
        ...
        # Streamliti pikk operatiivne arvutusplokk sisaldab ka Prognoosi renderdust.
        # Placeholderi tühjendamisest üksi ei piisanud mobiilivaates alati: vana sisu võis
        # Mootori tähelepanekute alla jääda. Pärast käsitsi OOS-/uurimisarvutust teeme ühe
        # odava puhta rerun'i. Arvutuslipud on selleks hetkeks juba maha võetud ja tulemus
        # salvestatud, seega rasket testi uuesti ei käivitata.
        _needs_clean_motor_rerun = bool(
            page == "Mootori tähelepanekud"
            and (
                _season_oos_run_now
                or bool(_season_oos_selected_parts)
                or _run_layered_manual_requested
                or bool(locals().get("_run_light_research", False))
                or _forecast_refresh_due
            )
        )
        if _needs_clean_motor_rerun:
            st.rerun()
