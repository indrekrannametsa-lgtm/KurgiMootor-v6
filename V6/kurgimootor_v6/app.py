        ...
        if training_rows:
            ...
            if _season_oos_run_now or _season_oos_selected_parts:
                ...
                _oos_payload = {
                    ...
                    "candidate_filter": "kasutaja valib ühe kandidaadi otse tabelireast",
                    "changes_official_forecast": False,
                }
                ...
                if not trace_df.empty:
                    ...
                    if "layered_trace_df" in locals() and not layered_trace_df.empty:
                        ...
                        with st.expander("Näita kõiki kihilisi BIO-mudeleid"):
                            ...
                            _layered_event = st.dataframe(
                                layered_trace_df.style.format(_layered_fmt),
                                use_container_width=True,
                                hide_index=True,
                                on_select="rerun",
                                selection_mode="single-row",
                                key="layered_candidate_row_select",
                            )
                            _selected_layered_rows = []
                            try:
                                _selected_layered_rows = list(_layered_event.selection.rows)
                            except Exception:
                                try:
                                    _selected_layered_rows = list((_layered_event or {}).get("selection", {}).get("rows", []))
                                except Exception:
                                    _selected_layered_rows = []
                            if _selected_layered_rows:
                                _sel_i = int(_selected_layered_rows[0])
                                if 0 <= _sel_i < len(layered_trace_df):
                                    ...
                                    st.caption(
                                        f"Valitud kandidaat: {_sel_parts} · avastus Δ "
                                ...
                                st.caption("Klõpsa tabelis ühel real, et valida kandidaat kogu-hooaja OOS-testiks.")
                    else:
                        st.caption(
