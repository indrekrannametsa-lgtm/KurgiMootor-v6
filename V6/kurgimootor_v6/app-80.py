...
APP_PATCH_VERSION = "wd-leveldelta-production-4-joint-size-shadow-1"  # diagnostic only: joint B/C/XL size-distribution test
...
# Ühekordne shadow-test: B/C/XL ei ole kolm sõltumatut maailma, vaid ühine
# suurusjaotus. Testime hierarhilist mudelit:
#   LARGE = (C+XL)/B
#   SPLIT = XL/C
# ning rekonstrueerime sellest B, C ja XL. Eelmise korje order 1/2/3
# lisatakse esmalt LARGE-tunnusele, mitte eraldi C- või XL-mudelile.
# Ametlikku prognoosi/championit see ei muuda.
_SIZE_DIST_TEST_VERSION = "joint-bcxl-largepool-prevorder-v1"
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            # -------------------------------------------------------------------------
            # LAB-159 / PRODUCTION-SHADOW: ühine B/C/XL suurusjaotus
            #
            # Probleem, mida test lahendab:
            #   korjejääk võib järgmise korje ajal olla kas C või XL. Kui C ja XL
            #   mudeldada sõltumatult, jaguneb üks bioloogiline signaal kahe sihtmärgi
            #   vahel ja võib mõlemas eraldi näida nõrgem.
            #
            # Hierarhiline parameeterdus:
            #   LARGE = (C + XL) / B          -> kui palju on "suurt vilja" B suhtes
            #   SPLIT = XL / C                -> kui suur osa suure vilja otsast on XL
            #   A_SHARE = A / (A+B+C)         -> ainult selleks, et WD ABC-st B+C maht saada
            #
            # Eelmise korje order 1/2/3 läheb esmases kandidaadis ainult LARGE-sse.
            # Nii ei anta orderile otsest "+C" või "+XL" reeglit; ta võib nihutada
            # kogu suurusjaotust suurema vilja poole. Ametlikku prognoosi see ei muuda.
            # -------------------------------------------------------------------------
            ...
            if USE_WINDDRY_ENGINE:
                ...
                _sd_large_log = np.where(
                    np.isfinite(_sd_b) & np.isfinite(_sd_c) & np.isfinite(_sd_xl) & (_sd_b >= 0) & ((_sd_c + _sd_xl) >= 0),
                    np.log((_sd_c + _sd_xl + _sd_eps) / (_sd_b + _sd_eps)),
                    np.nan,
                )
                _sd_xlc_log = np.where(
                    np.isfinite(_sd_c) & np.isfinite(_sd_xl) & (_sd_c >= 0) & (_sd_xl >= 0),
                    np.log((_sd_xl + _sd_eps) / (_sd_c + _sd_eps)),
                    np.nan,
                )
                ...
                # A osakaalu kasutatakse mõlema arhitektuuri absoluutsete B/C kastide
                # rekonstrueerimisel ühtemoodi; order A-sse ei lähe.
                ...
                _sd_large_base_log_pred = _sd_walk_transformed(_sd_large_log, [])
                _sd_split_base_log_pred = _sd_walk_transformed(_sd_xlc_log, [])
                _sd_order_cols = ["Eelmine korje order 2", "Eelmine korje order 3"]
                _sd_large_order_log_pred = _sd_walk_transformed(_sd_large_log, _sd_order_cols)
                _sd_split_order_log_pred = _sd_walk_transformed(_sd_xlc_log, _sd_order_cols)

                def _sd_ratio_from_log(values, lo, hi):
                    return np.exp(np.clip(values, np.log(lo), np.log(hi)))

                _sd_large_base = _sd_ratio_from_log(_sd_large_base_log_pred, 0.05, 25.0)
                _sd_split_base = _sd_ratio_from_log(_sd_split_base_log_pred, 0.01, 10.0)
                _sd_large_order = _sd_ratio_from_log(_sd_large_order_log_pred, 0.05, 25.0)
                _sd_split_order = _sd_ratio_from_log(_sd_split_order_log_pred, 0.01, 10.0)
                ...
                def _sd_reconstruct_joint(abc_pred, large_ratio, xl_over_c, a_share_pred):
                    bc_pool = np.maximum(0.0, abc_pred * (1.0 - a_share_pred))
                    large_ratio = np.clip(large_ratio, 0.05, 25.0)
                    xl_over_c = np.clip(xl_over_c, 0.01, 10.0)
                    # LARGE=(C+XL)/B ja SPLIT=XL/C.
                    cb_ratio = large_ratio / (1.0 + xl_over_c)
                    xl_over_b = large_ratio * xl_over_c / (1.0 + xl_over_c)
                    b_pred = bc_pool / (1.0 + cb_ratio)
                    c_pred = cb_ratio * b_pred
                    xl_pred = xl_over_b * b_pred
                    return b_pred, c_pred, xl_pred, cb_ratio

                _sd_cur = _sd_reconstruct_current(_sd_abc_pred, _sd_cb_inc, _sd_xl_inc, _sd_a_share_pred)
                _sd_joint_base = _sd_reconstruct_joint(_sd_abc_pred, _sd_large_base, _sd_split_base, _sd_a_share_pred)
                # Põhikandidaat: order mõjutab ainult C+XL ühist LARGE reservi.
                _sd_joint_order_large = _sd_reconstruct_joint(_sd_abc_pred, _sd_large_order, _sd_split_base, _sd_a_share_pred)
                # Diagnostika: kas orderist on lisakasu ka C-vs-XL splitis.
                _sd_joint_order_both = _sd_reconstruct_joint(_sd_abc_pred, _sd_large_order, _sd_split_order, _sd_a_share_pred)
                ...
                def _sd_metrics(pred_tuple):
                    ...
                    return {
                        ...
                        "mask": share_ok,
                        "row_dist_loss": row_dist_loss,
                    }

                _sd_m_cur = _sd_metrics(_sd_cur)
                _sd_m_base = _sd_metrics(_sd_joint_base)
                _sd_m_ord = _sd_metrics(_sd_joint_order_large)
                _sd_m_both = _sd_metrics(_sd_joint_order_both)

                def _sd_loss_compare(base_m, cand_m):
                    if not base_m or not cand_m:
                        return None
                    mask = base_m["mask"] & cand_m["mask"]
                    idx = np.where(mask)[0]
                    if len(idx) < 12:
                        return None
                    # Recompute composition loss on the exact shared rows.
                    def row_loss(pred_tuple):
                        pb, pc, px, _ = [np.asarray(x, dtype=float) for x in pred_tuple]
                    ...
                    base_tuple = _sd_cur
                    # caller will pass candidate tuple separately through closure below
                    return idx, row_loss(base_tuple)

                _sd_variants = {
                    "Praegune C/B + eraldi XL": (_sd_cur, _sd_m_cur),
                    "Ühine suurusjaotus": (_sd_joint_base, _sd_m_base),
                    "Ühine + eelmine order → C+XL/B": (_sd_joint_order_large, _sd_m_ord),
                    "Ühine + order → large + C/XL split": (_sd_joint_order_both, _sd_m_both),
                }
                _sd_valid_variants = {k:v for k,v in _sd_variants.items() if v[1] is not None}

                if _sd_m_cur and len(_sd_valid_variants) >= 2:
                    # Võrdle kõik kandidaadid praeguse arhitektuuriga täpselt samadel ridadel.
                    _sd_summary = {}
                    ...
                    # Parim kandidaat valitakse ainult jaotuse MAE järgi; see on diagnostika,
                    # mitte automaatne champion-promotion.
                    _sd_candidates_only = {
                        k:v for k,v in _sd_summary.items() if k != "Praegune C/B + eraldi XL"
                    }
                    _sd_best_name = min(_sd_candidates_only, key=lambda k: _sd_candidates_only[k]["dist_mae_pp"]) if _sd_candidates_only else None
                    _size_dist_stats = {
                        "status": "tested",
            ...
            st.markdown("##### Ühine B/C/XL suurusjaotus · shadow-test")
            if (
                ...
                _best_name = str(_sd_show.get("best_variant") or "")
                _best = _sv.get(_best_name) or {}
                _ord = _sv.get("Ühine + eelmine order → C+XL/B") or {}
                _js1, _js2, _js3, _js4 = st.columns(4)
                _js1.metric("Praegune jaotuse MAE", "—" if not _cur else f"{float(_cur.get('dist_mae_pp')):.2f} pp")
                _js2.metric("Parim ühine MAE", "—" if not _best else f"{float(_best.get('dist_mae_pp')):.2f} pp")
                _js3.metric("Paranemine", "—" if not _best else f"{float(_best.get('dist_improvement_pp')):+.2f} pp")
                _js4.metric("Parim variant", _best_name.replace("Ühine + ", "+ ") if _best_name else "—")
                if _best:
                    st.caption(
                        f"Parim ühine variant: C/B MAE {float(_cur.get('cb_mae')):.2f} → {float(_best.get('cb_mae')):.2f} · "
                        f"B MAE {float(_cur.get('b_mae')):.2f} → {float(_best.get('b_mae')):.2f} · "
                        f"C MAE {float(_cur.get('c_mae')):.2f} → {float(_best.get('c_mae')):.2f} · "
                        f"XL MAE {float(_cur.get('xl_mae')):.2f} → {float(_best.get('xl_mae')):.2f} · "
                        f"C+XL MAE {float(_cur.get('large_mae')):.2f} → {float(_best.get('large_mae')):.2f} · "
                        f"KOGU MAE {float(_cur.get('total_mae')):.2f} → {float(_best.get('total_mae')):.2f}. "
                        f"Jaotuse kandidaat võidab {float(_best.get('win_rows_pct')):.0f}% ridu; "
                        f"halvim ajapool {float(_best.get('worst_half_pp')):+.2f} pp. "
                        f"Ametlikku prognoosi test ei muuda."
                    )
                if _ord:
                    st.caption(
                        f"Korjejäägi hüpoteesi võtmetest — order ainult ühises LARGE=(C+XL)/B tunnuses: "
                        f"jaotuse MAE {float(_cur.get('dist_mae_pp')):.2f} → {float(_ord.get('dist_mae_pp')):.2f} pp "
                        f"({float(_ord.get('dist_improvement_pp')):+.2f} pp); "
                        f"C/B MAE {float(_cur.get('cb_mae')):.2f} → {float(_ord.get('cb_mae')):.2f}; "
                        f"XL MAE {float(_cur.get('xl_mae')):.2f} → {float(_ord.get('xl_mae')):.2f}."
                    )
            elif isinstance(_sd_show, dict) and _sd_show.get("status") == "insufficient_rows":
                st.caption("Ühise B/C/XL suurusjaotuse shadow-test: piisavaid ühiseid walk-forward ridu veel ei ole.")
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    if (
        ...
        _rbest_name = str(_ro_sd.get("best_variant") or "")
        _rbest = _rsv.get(_rbest_name) or {}
        _rord = _rsv.get("Ühine + eelmine order → C+XL/B") or {}
        _rs1, _rs2, _rs3, _rs4 = st.columns(4)
        _rs1.metric("Praegune jaotuse MAE", "—" if not _rcur else f"{float(_rcur.get('dist_mae_pp')):.2f} pp")
        _rs2.metric("Parim ühine MAE", "—" if not _rbest else f"{float(_rbest.get('dist_mae_pp')):.2f} pp")
        _rs3.metric("Paranemine", "—" if not _rbest else f"{float(_rbest.get('dist_improvement_pp')):+.2f} pp")
        _rs4.metric("Parim variant", _rbest_name.replace("Ühine + ", "+ ") if _rbest_name else "—")
        if _rbest:
            st.caption(
                f"Parim: C/B MAE {float(_rcur.get('cb_mae')):.2f} → {float(_rbest.get('cb_mae')):.2f} · "
                f"B {float(_rcur.get('b_mae')):.2f} → {float(_rbest.get('b_mae')):.2f} · "
                f"C {float(_rcur.get('c_mae')):.2f} → {float(_rbest.get('c_mae')):.2f} · "
                f"XL {float(_rcur.get('xl_mae')):.2f} → {float(_rbest.get('xl_mae')):.2f} · "
                f"C+XL {float(_rcur.get('large_mae')):.2f} → {float(_rbest.get('large_mae')):.2f} · "
                f"KOGU {float(_rcur.get('total_mae')):.2f} → {float(_rbest.get('total_mae')):.2f}. "
                f"Võidab {float(_rbest.get('win_rows_pct')):.0f}% ridu; halvim ajapool {float(_rbest.get('worst_half_pp')):+.2f} pp."
            )
        if _rord:
            st.caption(
                f"Order → ühine C+XL/B: jaotuse MAE {float(_rcur.get('dist_mae_pp')):.2f} → {float(_rord.get('dist_mae_pp')):.2f} pp "
                f"({float(_rord.get('dist_improvement_pp')):+.2f}); C/B {float(_rcur.get('cb_mae')):.2f} → {float(_rord.get('cb_mae')):.2f}; "
                f"XL {float(_rcur.get('xl_mae')):.2f} → {float(_rord.get('xl_mae')):.2f}."
            )
    elif (
        isinstance(_ro_sd, dict)
        and str(_ro_sd.get("test_version") or "") == _SIZE_DIST_TEST_VERSION
        ...
        st.warning("Ühise B/C/XL suurusjaotuse shadow-test käis, kuid ühiseid ridu ei olnud piisavalt.")
    else:
        st.info("Ühise B/C/XL suurusjaotuse shadow-test pole veel arvutatud; äpp käivitab selle ühe korra automaatselt.")
