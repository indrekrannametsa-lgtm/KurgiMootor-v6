...
# V6.5 CPU-kaitse: lai kihiline uuring on ainult käsitsi.
# Edukalt lõpetatud uuringut ei ole mõtet samal päeval korrata, kuid katkestus/viga
# ei tekita lukku. Korjepäevade arv EI ole enam käivituskriteerium.
...
if page == "Mootori tähelepanekud":
    ...
    if _layered_success_today:
        st.button(
            "🔬 Lai kihiline uuring täna juba edukalt tehtud",
            disabled=True,
            key="layered_manual_disabled",
        )
        st.caption("Sama andmeseisu pole mõtet samal päeval uuesti läbi arvutada.")
    else:
        _layered_run_now = st.button("🔬 Käivita lai kihiline uuring", key="layered_manual_run")
...
_auto_light_check = _model_is_dirty()

if page == "Prognoos" or _auto_light_check or _layered_run_now:
    _light_cycle_t0 = time.perf_counter()
    if _layered_run_now:
        _light_cycle_reason = "käsitsi lai uuring"
    elif _auto_light_check:
        _light_cycle_reason = db.get_app_setting("model_dirty_reason", "") or "uus sisend"
    else:
        _light_cycle_reason = "Prognoosi vaade"
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            layered_screen_layers = {
                ...
                "Mikrokliima": {
                    ...
                    "Sade + niiskus": ["Sademed Σ", "Niiskus kesk"],
                    "Sade + ET0": ["Sademed Σ", "ET0 Σ"],
                },
                "Hooaja kulumine": {
                    "Hooaja kaar": ["Hooajapäev²"],
                    "Hooaja faasivahetused": ["Hooaeg 35+", "Hooaeg 50+", "Hooaeg 65+"],
            ...
            # Sade jääb teadlikult mikrokliima/proxy kihiks. Üksik sade ei saa championiks.
            ...
            # Põllu seisundi valve (diagnostika, mitte prognoosisisend).
            # Kui sama põld jääb kahel viimasel ausal walk-forward testil järjest
            # baasmudelist märgatavalt allapoole, märgime võimaliku potentsiaali muutuse.
            field_state_watch = []
            try:
            ...
            # Kerge Jäljeotsija teavitus Avalehele.
            # See EI vaheta championit ega käivita laia kihilist uuringut.
            # Lävend kasutab sama konservatiivset 0.10 kasti MAE paranemise mõtet,
            # mida Jäljeotsija stabiilsusreeglid juba kasutavad.
            try:
            ...
            if _layered_run_now:
                ...
                # Katse algus on eraldi edukast lõpetamisest.
                try:
                    ...
                    pass
                # ---------------------------------------------------------------------
                # V6.5 kihiline champion-valik — PAARID ENNE, mitte top-1 üksiktunnus.
                # ---------------------------------------------------------------------
                # Üksiktunnuse enda MAE ei otsusta enam, kas ta pääseb kihilisse mudelisse.
                # Kõik bioloogiliselt lubatud variandid võivad osaleda esimese kahe kihi
                # PAARIDES. Alles päris kombinatsiooni walk-forward tulemus otsustab,
                # millised kooslused lähevad järgmisse kihti.
                #
                # See lubab näiteks üksikult nõrgal radiatsiooni- või hooajatunnusel jääda
                # mudelisse siis, kui ta koos termilise arengu või koormusega stabiliseerib
                # prognoosi. Samal ajal väldime kõigi võimalike 3000+ kombinatsioonide
                # pimesi läbiproovimist beam-searchiga.
                #
                # Kinnitusperiood on jätkuvalt täiesti puutumata kuni ühe lõpliku
                # kihilise kandidaadi valimiseni.
                ...
                def _merge_cols(left_cols, right_cols):
                ...
                def _eval_layered_candidate(name, layers, parts, cols, *, min_improvement=-0.10):
                    ...
                    return {
                        ...
                        "pred": cp,
                        "stats": stats,
                        "trend_accuracy": _trend_accuracy(cp, y, _abc_discovery_days),
                        "regime": _regime_stats(predictions, cp, y, _abc_discovery_days),
                        "overreaction": _overreaction_penalty(cp, y, _abc_discovery_days),
                    }

                def _day_level_series(pred_arr, target_arr, allowed_days):
                    """Day-level mean prediction/actual for trend and regime diagnostics."""
                    rows = []
                ...
                def _trend_accuracy(pred_arr, target_arr, allowed_days):
                    rows = _day_level_series(pred_arr, target_arr, allowed_days)
                    if len(rows) < 3:
                        return None
                    hits = 0
                    total = 0
                    for i in range(1, len(rows)):
                        pdiff = rows[i]["pred"] - rows[i - 1]["pred"]
                        adiff = rows[i]["actual"] - rows[i - 1]["actual"]
                        # Väga väike muutus = praktiliselt sama suund.
                        psgn = 0 if abs(pdiff) < 0.15 else (1 if pdiff > 0 else -1)
                        asgn = 0 if abs(adiff) < 0.15 else (1 if adiff > 0 else -1)
                        hits += int(psgn == asgn)
                        total += 1
                    return 100.0 * hits / total if total else None

                def _weather_regime_days(allowed_days):
                    """Leia avastusperioodi suuremad ilmamuutused ilma tulevikku vaatamata."""
                    usable = []
                    # Päevataseme proxy: hiline T + hiline radiatsioon + niiskus.
                    for _day in sorted(allowed_days):
                        ...
                        vals = {}
                        ok = True
                        for col in ["BIO hiline T", "BIO hiline radiatsioon", "Niiskus kesk"]:
                            if col not in model_df.columns:
                                ok = False
                                break
                            ...
                            if len(arr) == 0:
                                ok = False
                                break
                            vals[col] = float(np.mean(arr))
                        if ok:
                            usable.append((_day, vals))
                    if len(usable) < 4:
                        return set()

                    # Muutuse skoor standardiseeritakse ainult sama allowed-ploki enda muutustega.
                    diffs = []
                    for i in range(1, len(usable)):
                        prev = usable[i - 1][1]
                        cur = usable[i][1]
                        diffs.append([
                            cur["BIO hiline T"] - prev["BIO hiline T"],
                            cur["BIO hiline radiatsioon"] - prev["BIO hiline radiatsioon"],
                            cur["Niiskus kesk"] - prev["Niiskus kesk"],
                        ])
                    arr = np.asarray(diffs, dtype=float)
                    scale = np.nanstd(arr, axis=0)
                    scale = np.where(scale < 1e-6, 1.0, scale)
                    scores = np.sqrt(np.sum((arr / scale) ** 2, axis=1))
                    # Märgime umbes tugevama kolmandiku üleminekutest režiimimuutuseks.
                ...
                def _regime_stats(base_pred, cand_pred, target, allowed_days):
                    regime_days = _weather_regime_days(allowed_days)
                    if not regime_days:
                        return None
                    mask = (
                        np.isfinite(base_pred)
                    ...
                    if len(idx) < 3:
                        return None
                    base_mae = float(np.mean(np.abs(base_pred[idx] - target[idx])))
                    cand_mae = float(np.mean(np.abs(cand_pred[idx] - target[idx])))
                    return {
                        "days": len(set(dates[idx])),
                        "improvement": base_mae - cand_mae,
                        "cand_mae": cand_mae,
                        "base_mae": base_mae,
                    }

                def _overreaction_penalty(pred_arr, target_arr, allowed_days):
                    """Karista mudelit, mis muudab päevataset palju rohkem kui tegelik saak."""
                    rows = _day_level_series(pred_arr, target_arr, allowed_days)
                    if len(rows) < 3:
                        return 0.0
                    excess = []
                ...
                # Taime reaktsioonivõime langus on praegu DIAGNOSTIKA, mitte uus salajane
                # prognoositunnus. Kui viimase 3 testipäeva head BIO tingimused ei anna
                # saagile vastust, märgime hooaja/taime võimekuse jälgimiseks.
                def _response_fade_diagnostic():
                    if not {"BIO hiline T", "BIO hiline radiatsioon"}.issubset(model_df.columns):
                        return None
                    valid_days = sorted(set(
                        dates[np.where(np.isfinite(predictions) & np.isfinite(y))[0]]
                    ))
                    if len(valid_days) < 5:
                        return None
                    rows = []
                    for _day in valid_days:
                        idx = np.where((dates == _day) & np.isfinite(y))[0]
                        if len(idx) == 0:
                            continue
                        t = pd.to_numeric(model_df.loc[idx, "BIO hiline T"], errors="coerce").to_numpy(dtype=float)
                        r = pd.to_numeric(model_df.loc[idx, "BIO hiline radiatsioon"], errors="coerce").to_numpy(dtype=float)
                        if not (np.isfinite(t).any() and np.isfinite(r).any()):
                            continue
                        rows.append({
                            "day": _day,
                            "weather": float(np.nanmean(t)) + 0.08 * float(np.nanmean(r)),
                            "yield": float(np.mean(y[idx])),
                        })
                    if len(rows) < 5:
                        return None
                    recent = rows[-3:]
                    ...
                    if len(earlier) < 2:
                        return None
                    weather_good = np.mean([r["weather"] for r in recent]) >= np.median([r["weather"] for r in earlier])
                    yield_response = np.mean([r["yield"] for r in recent]) - np.mean([r["yield"] for r in earlier[-3:]])
                    return {
                        "flag": bool(weather_good and yield_response < -0.5),
                        "yield_change": float(yield_response),
                    }

                def _candidate_rank(c):
                    # Beam'i järjekord: MAE on põhinäitaja, aga soosime ka õiget trendi,
                    # ilmamuutuse stabiilsust ja väldime "seinast seina" ülereageerimist.
                    s = c["stats"]
                    penalty = 0.0
                    if s["Halvim pool"] < -0.10:
                        penalty += abs(s["Halvim pool"]) * 0.20
                    if s["Võidab ridu %"] < 50.0:
                        penalty += (50.0 - s["Võidab ridu %"]) * 0.002

                    trend = c.get("trend_accuracy")
                    if trend is not None and trend < 50.0:
                        penalty += (50.0 - trend) * 0.003

                    regime = c.get("regime")
                    if regime is not None and regime["improvement"] < 0:
                        penalty += abs(regime["improvement"]) * 0.25

                    penalty += 0.15 * float(c.get("overreaction", 0.0) or 0.0)
                    return s["Katse MAE"] + penalty

                def _build_recipe(recipe_name, layer_order, beam_width=10):
                    if len(layer_order) < 2:
                        return []

                    first_layer, second_layer = layer_order[0], layer_order[1]
                    first_specs = _variant_specs(first_layer)
                    second_specs = _variant_specs(second_layer)
                    if not first_specs or not second_specs:
                        return []

                    # 1. samm: TESTI KÕIK kahe esimese bioloogilise kihi paarid.
                    beam = []
                    for a in first_specs:
                        for b in second_specs:
                            cols = _merge_cols(a["cols"], b["cols"])
                            parts = [
                                f"{first_layer}: {a['variant']}",
                                f"{second_layer}: {b['variant']}",
                            ]
                            cand = _eval_layered_candidate(
                                recipe_name,
                    ...
                    # Alles PAARIDE päris tulemuse põhjal kärbime otsingut.
                    beam.sort(key=_candidate_rank)
                    beam = beam[:beam_width]

                    # 2.+ samm: lisa järgmisi kihte. Iga beam-kandidaat proovib KÕIKI
                    # järgmise kihi variante; nõrk üksiktunnus võib seega endiselt võita,
                    # kui ta annab olemasolevale kombinatsioonile uut infot.
                    for layer_name in layer_order[2:]:
                        specs = _variant_specs(layer_name)
                        if not specs:
                            return []
                        expanded = []
                        for base in beam:
                            for spec in specs:
                                cols = _merge_cols(base["cols"], spec["cols"])
                                parts = base["parts"] + [f"{layer_name}: {spec['variant']}"]
                                cand = _eval_layered_candidate(
                                    recipe_name,
                                    base["layers"] + [layer_name],
                                    parts,
                                    cols,
                                    min_improvement=-0.10,
                                )
                        ...
                        if not expanded:
                            return []
                        expanded.sort(key=_candidate_rank)
                        beam = expanded[:beam_width]

                    return beam

                # Bioloogiliselt mõistlikud kihijärjestused.
                # Järjestus määrab ainult arvutusliku beam-searchi tee, mitte tunnuse märki.
                _layer_recipes = [
                    ("Kasv + assimilatsioon",
                     ["Termiline areng", "Assimilaatide pakkumine"]),
                    ("Kasv + koormus",
                     ["Termiline areng", "Bioloogiline koormus"]),
                    ("Assimilatsioon + koormus",
                     ["Assimilaatide pakkumine", "Bioloogiline koormus"]),
                    ("Kasv + assimilatsioon + koormus",
                     ["Termiline areng", "Assimilaatide pakkumine", "Bioloogiline koormus"]),
                    ("Kasv + assimilatsioon + mikrokliima",
                     ["Termiline areng", "Assimilaatide pakkumine", "Mikrokliima"]),
                    ("Kasv + koormus + hooaja kulumine",
                     ["Termiline areng", "Bioloogiline koormus", "Hooaja kulumine"]),
                    ("BIO tervik",
                     ["Termiline areng", "Assimilaatide pakkumine",
                      "Bioloogiline koormus", "Mikrokliima", "Hooaja kulumine"]),
                ]

                layered_finalists = []
                for _recipe_name, _layer_order in _layer_recipes:
                    _beam = _build_recipe(_recipe_name, _layer_order, beam_width=10)
                    if not _beam:
                        continue

                    # Raportisse parim 3 iga retsepti kohta, et näeksime ka alternatiive.
                    for _cand in _beam[:3]:
                        _s = _cand["stats"]
                        layered_trace_rows.append({
                            "Mudel": _recipe_name,
                            "Kihte": len(_cand["layers"]),
                            "Koosseis": " | ".join(_cand["parts"]),
                            "Baas MAE": _s["Baas MAE"],
                            "Katse MAE": _s["Katse MAE"],
                            "Paranemine": _s["Paranemine"],
                            "Võidab ridu %": _s["Võidab ridu %"],
                            "Halvim pool": _s["Halvim pool"],
                            "Testiridu": _s["Testiridu"],
                            "Trend %": _cand.get("trend_accuracy"),
                            "Režiim Δ": (_cand.get("regime") or {}).get("improvement"),
                            "Ülereageerimine": _cand.get("overreaction"),
                            "Stabiilne": _s["Stabiilne"],
                        })

                    # Championivõistluse finalist peab avastuses juba mõistlikult püsima.
                    for _cand in _beam:
                        _s = _cand["stats"]
                        _reg = _cand.get("regime")
                        _trend = _cand.get("trend_accuracy")
                        if (
                            _s["Paranemine"] >= 0.05
                            and _s["Võidab ridu %"] >= 50.0
                            and _s["Halvim pool"] >= -0.10
                            and (_trend is None or _trend >= 40.0)
                            and (_reg is None or _reg["improvement"] >= -0.20)
                            and float(_cand.get("overreaction", 0.0) or 0.0) <= 1.5
                        ):
                            layered_finalists.append(_cand)
                ...
                # Avastus valib ÜHE kihilise finalisti. Kinnitus ei osalenud ühegi
                # tunnuse, paari, beam'i ega retsepti valikus.
                if layered_finalists and _abc_confirmation_days:
                    layered_finalists.sort(key=_candidate_rank)
                    selected = layered_finalists[0]
                    selected_name = selected["name"]
                    selected_cols = selected["cols"]
                    selected_pred = selected["pred"]
                    selected_discovery_stats = selected["stats"]
                    ...
                    if confirm_stats and confirm_stats["Stabiilne"]:
                        ...
                        champion_stats["Kihid"] = " | ".join(selected_parts)

                _layered_elapsed = time.perf_counter() - _layered_t0
                try:
                    ...
                    db.set_app_setting("layered_research_last_candidates", str(len(layered_trace_df)))
                except Exception as _e:
                    # Ebaõnnestunud salvestus/uuring ei märgi päeva edukaks.
            ...
            MODEL_VERSION = "v6.5-layered-bio-v13-observations-safe"
            forecast_payloads = []
        ...
        # Kerge Jäljeotsija/prognoosiring kontrollis olemasolevad kandidaadid
        # uute andmete peal üle. Lai kihiline uuring siia EI kuulu.
        db.set_app_setting("model_dirty", "0")
        db.set_app_setting("model_last_checked_complete_day_count", str(len(complete_harvest_days)))
        _light_cycle_finished_at = datetime.now(ZoneInfo("Europe/Tallinn"))
        db.set_app_setting("model_last_checked_at", _light_cycle_finished_at.isoformat())

        try:
            _light_duration_s = max(0.0, time.perf_counter() - _light_cycle_t0)
            db.set_app_setting("light_cycle_last_duration_s", f"{_light_duration_s:.1f}")
            db.set_app_setting("light_cycle_last_at", _light_cycle_finished_at.isoformat())
...
if page == "Mootori tähelepanekud":
    ...
    st.caption(
        "Mootori tähelepanekute avamine ei arvuta prognoosi uuesti. Lai kihiline uuring on käsitsi; "
        "seda tasub korrata siis, kui andmestikku on tulnud sisuliselt uut infot "
        "(nt ilmastiku- või saagirežiimi muutus), mitte lihtsalt uute päevade arvu pärast. "
        "Viimane edukas tulemus säilib vahepeal."
    )
    ...
    with st.expander("⚙️ Tehniline koormus"):
        ...
        if _lc_sec:
            ...
            st.caption("Kerge arvutusringi kestust pole veel mõõdetud.")

        if _lr_sec_tech:
            st.write(f"**Viimane lai kihiline uuring:** {_lr_sec_tech} s")
            if _lr_n_tech:
                st.caption(f"Raportis kandidaate: {_lr_n_tech}")
        else:
            st.caption("Laia kihilise uuringu kestust pole veel salvestatud.")

        st.caption(
            "Need näidud mõõdavad KurgiMootori enda arvutusringide kestust, "
            "mitte Streamlit Cloudi konto täpset CPU-protsenti."
        )
    ...
    if not _obs_ready:
        st.caption(
            "Leht näitab salvestatud tähelepanekuid. Prognoosi- ja Jäljeotsija arvutust "
            "ei käivitata ainult selle lehe avamise tõttu."
        )

        _saved_layered_raw = db.get_app_setting("layered_research_trace_json", "")
        try:
            _saved_layered_rows = json.loads(_saved_layered_raw) if _saved_layered_raw else []
            if not isinstance(_saved_layered_rows, list):
                _saved_layered_rows = []
        except Exception:
            _saved_layered_rows = []

        if _saved_layered_rows:
            _saved_layered_df = pd.DataFrame(_saved_layered_rows)
            if not _saved_layered_df.empty:
                _sort_col = "Paranemine" if "Paranemine" in _saved_layered_df.columns else None
                if _sort_col:
                    _saved_layered_df = _saved_layered_df.sort_values(_sort_col, ascending=False)
                _best_saved = _saved_layered_df.iloc[0]
                _name = str(_best_saved.get("Mudel") or _best_saved.get("Jälg") or "salvestatud kandidaat")
                try:
                    _imp = float(_best_saved.get("Paranemine"))
                    st.info(f"Viimase laia uuringu parim salvestatud kandidaat: **{_name}** · Δ {_imp:+.2f}")
                except Exception:
                    st.info(f"Viimase laia uuringu parim salvestatud kandidaat: **{_name}**")
                with st.expander("Näita viimase laia uuringu salvestatud tulemusi"):
                    st.dataframe(_saved_layered_df, use_container_width=True, hide_index=True)
    else:
        ...
        if _journal_day and _journal_expected and "forecast_history_rows" in locals():
            _picked = {}
            for _hist in forecast_history_rows:
                if str(_hist.get("target_date") or "") != _journal_day.isoformat():
                    continue
                _fdate = str(_hist.get("forecast_date") or "")
                if not _fdate or _fdate > _journal_day.isoformat():
                    continue
                try:
                    _fno = int(_hist.get("field_no"))
                except (TypeError, ValueError):
                    continue
                if _fno not in _journal_expected:
                    continue
                _key = (_fdate, _fno)
                _old = _picked.get(_key)
                if _old is None or str(_hist.get("generated_at") or "") > str(_old.get("generated_at") or ""):
                    _picked[_key] = _hist

            _by_fdate = {}
            for (_fdate, _fno), _hist in _picked.items():
                _by_fdate.setdefault(_fdate, {})[_fno] = _hist
        ...
        st.markdown("### 🔎 Üksiktunnuste ja vabade ideede radar")
