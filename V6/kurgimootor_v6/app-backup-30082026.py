...
APP_PATCH_VERSION = "wd-leveldelta-production-4-ui-speed-plan-3"  # UI speed + no-harvest/future-plan continuity; model unchanged
...
# Korjeplaani loogika versioon. Kui plaani tõlgendus muutub, peab vana salvestatud
# 9 päeva snapshot ühe korra uuesti arvutuma; muidu võib UI jääda näitama vana
# 1,2,3 plokki kuni järgmise ilma- või korjemuutuseni.
FORECAST_PLAN_LOGIC_VERSION = "plan-v3-no-harvest-continuity"

def _ensure_forecast_plan_logic_version() -> None:
    _key = "forecast_plan_logic_version"
    _current = db.get_app_setting(_key, "")
    if _current != FORECAST_PLAN_LOGIC_VERSION:
        db.set_app_setting(_key, FORECAST_PLAN_LOGIC_VERSION)
        _mark_forecast_dirty("korjeplaani loogika uuendus")

try:
    _ensure_forecast_plan_logic_version()
except db.DatabaseError:
    pass
...
if _run_operational_cycle:
    ...
    with _forecast_cycle_ui:
        ...
        if training_rows:
            ...
            # FUTURE PLAN CONTINUITY LOCK v3
            # ------------------------------
            # Tänane 0-põllu päev EI liiguta korjeringi. Homne automaatne plaan
            # tuletatakse samast helperist, mida Avaleht kasutab: viimase PÄRISELT
            # korjatud päeva viimase põllu järel. Seega viimase 3,4,5 korje järel
            # annab tänane 0-põllu päev homseks 6,7,8 ning intervallid arvutatakse
            # päris eelkorjest, mitte fiktiivsest vahepäevast.
            if today_plan:
                current_first = _next_field(int(today_plan[-1]))
            else:
                _auto_tomorrow = _planned_fields_for_day(TODAY + timedelta(days=1), [], harvest_rows)
                current_first = int(_auto_tomorrow[0]) if _auto_tomorrow else 1
            any_weather_imputation = set()
            for offset in range(1, 10):
                target_day = TODAY + timedelta(days=offset)
                if offset == 1:
                    # Homne tuleb alati tänase reaalsest/plaanitud jätkust.
                    day_fields = [current_first, _next_field(current_first), _next_field(_next_field(current_first))]
                else:
                    _saved_future_plan = saved_harvest_plans.get(target_day.isoformat())
                    if _saved_future_plan is not None:
                        day_fields = [int(f) for f in _saved_future_plan if 1 <= int(f) <= 14]
                    else:
                        day_fields = [current_first, _next_field(current_first), _next_field(_next_field(current_first))]
                day_rows = []
                for f in day_fields:
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    # Vastuvõtu-, shadow- ja residual-testid kuuluvad Mootori tähelepanekutesse.
    # Prognoos-leht näitab ainult kasutatavat prognoosi.
