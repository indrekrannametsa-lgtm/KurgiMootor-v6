...
APP_PATCH_VERSION = "app-128-3H-clean-test"  # TEST: LAB-147 fikseeritud 3H A+B+C mootor
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor V6.4 · 3H TEST", page_icon=APP_ICON, layout="wide")


# =============================================================================
# TESTMOOTOR: LAB-147 fikseeritud 3H A+B+C
# -----------------------------------------------------------------------------
# SHORT +1..+4 = OWN2 + MEM10 + BLOCK1 + D3CORE
# MID   +5..+7 = OWN2 + MEM10 + BLOCK1 + target-day weather
# LONG  +8..+9 = OWN2
#
# See kiht EI kasuta prognoositud saaki state'ina. Tulevased korjesündmused
# annavad ainult ajamärgi / kasvukestuse. XL ja C/B jäävad app-128 olemasolevasse
# mudelisse. TESTIS ei rakendata A+B+C-le ega XL-le hilishooaja taimeindeksit.
# =============================================================================

V3H_RIDGE_ALPHA = 24.0
V3H_RECENCY_HALFLIFE_DAYS = 28.0
V3H_SMALL_SAMPLE_PRIOR_ROWS = 12.0
V3H_MIN_TRAIN_ROWS = 6
V3H_DEFAULT_GROWTH_DAYS = 14.0 / 3.0
V3H_EXPECTED_FIELDS_PER_DAY = 3
V3H_BLOCK_FRESH_DAYS = 2
V3H_BLOCK_LOOKBACK_DAYS = 12

V3H_BASE_FEATURES = ["w10_gdd10", "w10_rad", "w10_et0", "block1_signal"]
V3H_D3_CORE = ["d3_gdd10", "d3_rad", "d3_et0"]
V3H_DAY0 = ["d0_gdd10", "d0_rad", "d0_et0", "d0_rh", "d0_rain", "d0_wind"]
V3H_SHORT = "SHORT-D3CORE"
V3H_MID = "MID-DAY0"
V3H_OWN2 = "OWN2"
V3H_VARIANTS = {
    "OWN2": [],
    "MEM10": ["w10_gdd10", "w10_rad", "w10_et0"],
    "FLAT": V3H_BASE_FEATURES,
    V3H_SHORT: V3H_BASE_FEATURES + V3H_D3_CORE,
    V3H_MID: V3H_BASE_FEATURES + V3H_DAY0,
}


def _v3h_route(lead: int) -> str:
    lead = int(lead)
    if 1 <= lead <= 4:
        return V3H_SHORT
    if 5 <= lead <= 7:
...
def _v3h_d(value) -> Optional[date]:
...
def _v3h_f(value) -> Optional[float]:
    try:
        x = float(value)
...
def _v3h_abc(row: dict) -> Optional[float]:
    vals = [_v3h_f(row.get(k)) for k in ("a", "b", "c")]
...
def _v3h_reliable(row: dict) -> bool:
...
def _v3h_sort_key(e: _V3HEvent):
    return (e.day, e.order, e.field)


def _v3h_prepare_events(harvest_rows_raw: List[dict]):
    events: List[_V3HEvent] = []
    actual_lookup: Dict[Tuple[date, int], dict] = {}
    for raw in harvest_rows_raw:
        dd = _v3h_d(raw.get("harvest_date"))
        try:
        ...
        try:
            ...
            order = 1
        reliable = _v3h_reliable(raw)
        abc = _v3h_abc(raw) if reliable else None
        source = "actual" if abc is not None else "estimated_event"
        events.append(_V3HEvent(dd, field, order, abc, abc is not None, source))
        if abc is not None:
            rr = dict(raw)
            rr["_abc"] = float(abc)
            rr["_day"] = dd
    ...
    events.sort(key=_v3h_sort_key)
    return events, actual_lookup


def _v3h_field_events_before(events, field, cutoff_exclusive):
    return sorted(
        [e for e in events if e.field == int(field) and e.day < cutoff_exclusive],
        key=_v3h_sort_key,
    )


def _v3h_growth_between(prev, cur_day, cur_order):
    g = float((cur_day - prev.day).days) + (int(cur_order) - int(prev.order)) * (3.0 / 24.0)
    return max(0.5, g)


def _v3h_growth_for_target(schedule_events, field, target_day, target_order):
    hist = _v3h_field_events_before(schedule_events, field, target_day)
    if hist:
        prev = hist[-1]
        return _v3h_growth_between(prev, target_day, target_order), prev
    return V3H_DEFAULT_GROWTH_DAYS, None


def _v3h_rate_history(events, field, cutoff_exclusive):
    hist = _v3h_field_events_before(events, field, cutoff_exclusive)
    out = []
    for i in range(1, len(hist)):
        prev, cur = hist[i - 1], hist[i]
        if not (cur.reliable and cur.abc is not None and cur.abc > 0):
            continue
        growth = _v3h_growth_between(prev, cur.day, cur.order)
...
def _v3h_complete_day_rate_medians(known_events, cutoff_exclusive):
    rates_by_day, fields_by_day = {}, {}
    for field in range(1, 15):
        for ev, rate in _v3h_rate_history(known_events, field, cutoff_exclusive):
    ...
    for dd in sorted(rates_by_day):
        if (
            len(fields_by_day.get(dd, set())) == V3H_EXPECTED_FIELDS_PER_DAY
            and len(rates_by_day[dd]) == V3H_EXPECTED_FIELDS_PER_DAY
        ):
            rows.append((dd, float(np.median(rates_by_day[dd]))))
    return rows


def _v3h_block3_fallback(known_events, cutoff_exclusive):
    meds = _v3h_complete_day_rate_medians(known_events, cutoff_exclusive)
    if meds:
        last3 = meds[-3:]
    ...
    for field in range(1, 15):
        rh = _v3h_rate_history(known_events, field, cutoff_exclusive)
        if not rh:
            continue
        ev, rate = rh[-1]
        if 0 <= (reference_day - ev.day).days <= V3H_BLOCK_LOOKBACK_DAYS:
            vals.append(float(rate))
    return float(np.median(vals)) if vals else None


def _v3h_own2_snapshot(known_events, field, cutoff_exclusive):
    hist_events = _v3h_field_events_before(known_events, field, cutoff_exclusive)
    rh = _v3h_rate_history(known_events, field, cutoff_exclusive)
    own = []
    ...
    fb = _v3h_block3_fallback(known_events, cutoff_exclusive)
    if fb is None or fb <= 0:
        return None
    return float(fb), True


def _v3h_block1_signal(known_events, cutoff_exclusive, own2_rate):
    meds = _v3h_complete_day_rate_medians(known_events, cutoff_exclusive)
    ...
    if age < 0 or age > V3H_BLOCK_FRESH_DAYS:
        return 0.0, dd
    sig = math.log(max(1e-6, block_rate) / max(1e-6, own2_rate))
    return float(sig), dd


def _v3h_weather_row(raw: dict):
    dd = _v3h_d(raw.get("weather_date"))
    required = (
        "temp_night_avg_c", "temp_day_avg_c", "temp_min_c", "temp_max_c",
        "wind_avg_ms", "radiation_mj_m2", "humidity_avg_pct",
        "precipitation_mm", "et0_mm",
    )
    vals = {k: _v3h_f(raw.get(k)) for k in required}
    if dd is None or any(v is None for v in vals.values()):
        return None
    return {
        "day": dd,
        "temp": 0.5 * (vals["temp_night_avg_c"] + vals["temp_day_avg_c"]),
        "temp_min": vals["temp_min_c"],
        "temp_max": vals["temp_max_c"],
        "wind": vals["wind_avg_ms"],
        "rad": vals["radiation_mj_m2"],
        "rh": vals["humidity_avg_pct"],
        "rain": vals["precipitation_mm"],
        "et0": vals["et0_mm"],
        "source": str(raw.get("data_kind") or ""),
    }


def _v3h_weather_maps(weather_rows_raw: List[dict], issue_day: date):
    measured: Dict[date, dict] = {}
    forecast: Dict[date, dict] = {}
    for raw in weather_rows_raw:
        row = _v3h_weather_row(raw)
        if row is None:
            continue
        dd = row["day"]
        kind = str(raw.get("data_kind") or "").strip().lower()
        if kind == "measured" and bool(raw.get("checked")):
            measured[dd] = row
        elif kind == "forecast":
            forecast[dd] = row

    # Live issue map: minevikus mõõdetud, tänase/future puudumisel forecast.
    live = dict(measured)
    for dd, row in forecast.items():
        if dd not in live:
            live[dd] = row
        elif dd >= issue_day and dd not in measured:
            live[dd] = row
    return measured, live


def _v3h_range_weather(wmap, start_day, end_day):
...
def _v3h_agg_weather(rows):
    ...
    return {
        ...
        "rad": float(np.mean([r["rad"] for r in rows])),
        "et0": float(np.mean([r["et0"] for r in rows])),
        "rh": float(np.mean([r["rh"] for r in rows])),
        "rain": float(np.mean([r["rain"] for r in rows])),
        "wind": float(np.mean([r["wind"] for r in rows])),
    }


def _v3h_delta_window(wmap, target_day, days):
    recent = _v3h_range_weather(
        wmap, target_day - timedelta(days=days), target_day - timedelta(days=1)
    )
    previous = _v3h_range_weather(
        wmap, target_day - timedelta(days=2 * days), target_day - timedelta(days=days + 1)
    )
    recent = _v3h_agg_weather(recent) if recent is not None else None
    previous = _v3h_agg_weather(previous) if previous is not None else None
    if recent is None or previous is None:
        return None
    return {k: float(recent[k] - previous[k]) for k in ("gdd10", "rad", "et0", "rh", "rain", "wind")}


def _v3h_memory_features(known_events, schedule_events, field, target_day, target_order, wmap):
    growth, _prev = _v3h_growth_for_target(schedule_events, field, target_day, target_order)
    out = {"growth_days": float(growth)}

    rows10 = _v3h_range_weather(wmap, target_day - timedelta(days=10), target_day - timedelta(days=1))
    mem10 = _v3h_agg_weather(rows10) if rows10 is not None else None
    for suffix in ("gdd10", "rad", "et0"):
        out[f"w10_{suffix}"] = None if mem10 is None else float(mem10[suffix])

    delta3 = _v3h_delta_window(wmap, target_day, 3)
    for suffix in ("gdd10", "rad", "et0"):
        out[f"d3_{suffix}"] = None if delta3 is None else float(delta3[suffix])

    day0 = wmap.get(target_day)
    if day0 is None:
        for suffix in ("gdd10", "rad", "et0", "rh", "rain", "wind"):
            out[f"d0_{suffix}"] = None
    else:
        out["d0_gdd10"] = max(0.0, float(day0["temp"]) - 10.0)
        out["d0_rad"] = float(day0["rad"])
        out["d0_et0"] = float(day0["et0"])
        out["d0_rh"] = float(day0["rh"])
        out["d0_rain"] = float(day0["rain"])
        out["d0_wind"] = float(day0["wind"])
    return out


def _v3h_build_record(
    known_events, schedule_events, field, target_day, target_order, wmap,
    actual_abc=None, frozen_own2=None, block_cutoff=None,
):
    if frozen_own2 is None:
        own2 = _v3h_own2_snapshot(known_events, field, target_day)
    else:
        own2 = frozen_own2
    if own2 is None:
        return None
    ...
    wf = _v3h_memory_features(
        known_events, schedule_events, field, target_day, target_order, wmap
    )
    if block_cutoff is None:
        block_cutoff = target_day
    block_sig, block_day = _v3h_block1_signal(known_events, block_cutoff, own2_rate)
...
def _v3h_make_historical_records(events, actual_lookup, measured):
    records = []
    for (dd, field), row in sorted(actual_lookup.items()):
        prior = [e for e in events if e.day < dd]
        if not _v3h_field_events_before(prior, field, dd):
            continue
        rec = _v3h_build_record(
            prior, prior, field, dd, int(row.get("_order") or 1),
            measured, actual_abc=float(row["_abc"]), block_cutoff=dd,
        )
        if rec is not None:
            records.append(rec)
...
def _v3h_fit_model(train_records, cutoff_day, feature_names):
    if not feature_names:
        return {"persistence": True, "n": len(train_records)}
    clean = []
    for r in train_records:
        y = _v3h_f(r.get("y"))
        vals = [_v3h_f(r.get(c)) for c in feature_names]
        if y is None or any(v is None for v in vals):
            continue
        age = max(0.0, float((cutoff_day - r["target_day"]).days))
        w = 0.5 ** (age / V3H_RECENCY_HALFLIFE_DAYS)
        clean.append((vals, y, w))
    if len(clean) < V3H_MIN_TRAIN_ROWS:
        return None
    ...
    yw = y * ws[:, 0]
    try:
        beta = np.linalg.solve(Zw.T @ Zw + V3H_RIDGE_ALPHA * np.eye(Z.shape[1]), Zw.T @ yw)
    except np.linalg.LinAlgError:
        beta = np.linalg.pinv(Zw.T @ Zw + V3H_RIDGE_ALPHA * np.eye(Z.shape[1])) @ (Zw.T @ yw)
...
def _v3h_predict_variant(model, rec, feature_names):
    ...
    vals = [_v3h_f(rec.get(c)) for c in feature_names]
    if any(v is None for v in vals):
        return None
    ...
    yhat *= n_train / (n_train + V3H_SMALL_SAMPLE_PRIOR_ROWS)
    pred_rate = max(0.0, own2_rate * math.exp(yhat))
    return float(pred_rate * growth)


def _v3h_live_predictions(
    harvest_rows_raw: List[dict],
    weather_rows_raw: List[dict],
    issue_day: date,
    today_planned_fields: List[int],
    future_plan: List[Tuple[date, List[int]]],
):
    """Tagasta {(target_day, field): 3H detail} ainult +1..+9 jaoks."""
    all_events, actual_lookup = _v3h_prepare_events(harvest_rows_raw)
    measured, live_weather = _v3h_weather_maps(weather_rows_raw, issue_day)

    # Ajalooline õppimine kasutab ainult mõõdetud ilma.
    historical = _v3h_make_historical_records(all_events, actual_lookup, measured)
    train = [r for r in historical if r["target_day"] <= issue_day]
    models = {
        name: _v3h_fit_model(train, issue_day, feats)
        for name, feats in V3H_VARIANTS.items()
    }

    # State: ainult päriselt teada olevad sündmused. Schedule: lisaks tänased veel
    # korjamata plaanid + kõik tulevased korjesündmused, ilma prognoositud saagita.
    known_events = [e for e in all_events if e.day <= issue_day]
    schedule_events = [
        _V3HEvent(e.day, e.field, e.order, e.abc, e.reliable, e.source)
        for e in known_events
    ]
    existing = {(e.day, e.field) for e in schedule_events}
    for order, field in enumerate(today_planned_fields, start=1):
        key = (issue_day, int(field))
        if key not in existing:
            schedule_events.append(_V3HEvent(issue_day, int(field), order, None, False, "simulated_schedule"))
            existing.add(key)
    for target_day, fields in future_plan:
        for order, field in enumerate(fields, start=1):
            key = (target_day, int(field))
            if key not in existing:
                schedule_events.append(_V3HEvent(target_day, int(field), order, None, False, "simulated_schedule"))
                existing.add(key)
    schedule_events.sort(key=_v3h_sort_key)
    ...
    for target_day, fields in future_plan:
        ...
        for order, field in enumerate(fields, start=1):
            ...
            if field not in frozen_own2:
                own = _v3h_own2_snapshot(known_events, field, issue_day + timedelta(days=1))
                if own is None:
                    continue
                frozen_own2[field] = own

            rec = _v3h_build_record(
                known_events, schedule_events, field, target_day, order, live_weather,
                actual_abc=None, frozen_own2=frozen_own2[field],
                block_cutoff=issue_day + timedelta(days=1),
            )
            if rec is None:
                continue

            flat = _v3h_predict_variant(models["FLAT"], rec, V3H_VARIANTS["FLAT"])
            if flat is None:
                flat = _v3h_predict_variant(models["MEM10"], rec, V3H_VARIANTS["MEM10"])
            if flat is None:
                flat = _v3h_predict_variant(models["OWN2"], rec, V3H_VARIANTS["OWN2"])

            preds = {}
            for name, feats in V3H_VARIANTS.items():
                if name in {"OWN2", "MEM10", "FLAT"}:
                    pred = _v3h_predict_variant(models[name], rec, feats)
                else:
                    vals_ok = all(_v3h_f(rec.get(c)) is not None for c in feats)
                    pred = _v3h_predict_variant(models[name], rec, feats) if (models[name] is not None and vals_ok) else flat
                preds[name] = float(flat if pred is None else pred)

            route = _v3h_route(lead)
            out[(target_day, field)] = {
                "abc": float(preds[route]),
                "route": route,
                "lead": lead,
                "growth_days": float(rec["growth_days"]),
                "own2": float(preds["OWN2"]),
                "flat": float(preds["FLAT"]),
                "fallback": int(rec["used_fallback"]),
                "train_n": int(models[route]["n"]) if models.get(route) is not None else 0,
            }
    ...
    diag = {
        ...
        "predictions": len(out),
        "model_n": {
            name: int(model["n"]) if model is not None else 0
...
def _v3h_selfcheck():
    """Odav puhas kontroll, mida failil saab staatiliselt testida."""
    assert _v3h_route(1) == V3H_SHORT
    assert _v3h_route(4) == V3H_SHORT
    assert _v3h_route(5) == V3H_MID
    assert _v3h_route(7) == V3H_MID
    assert _v3h_route(8) == V3H_OWN2
    assert _v3h_route(9) == V3H_OWN2
    prev = _V3HEvent(date(2026, 8, 1), 1, 1, 10.0, True, "actual")
    assert abs(_v3h_growth_between(prev, date(2026, 8, 6), 3) - 5.25) < 1e-12
    return True

V3H_STATIC_SELFCHECK_OK = _v3h_selfcheck()

st.warning("🧪 3H CLEAN TEST · A+B+C +1…+9 kasutab LAB-147 3-horisondi mootorit. Korje- ja ilmaandmeid test ei muuda.")
...
_v3h_test_done_today = db.get_app_setting("v3h_clean_test_last_run_date", "") == TODAY.isoformat()
_forecast_refresh_due = _forecast_is_dirty() or (not _v3h_test_done_today)
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            st.caption(
                "🧪 3H TEST: tuleviku A+B+C kasutab fikseeritud LAB-147 arhitektuuri: "
                "+1…+4 SHORT = OWN2+MEM10+BLOCK1+D3CORE; +5…+7 MID = OWN2+MEM10+BLOCK1+sihtpäeva ilm; "
                "+8…+9 LONG = OWN2. Prognoositud saaki ei söödetata 3H state'i tagasi. "
                f"XL jääb app-128 olemasolevale XL-mudelile ({xl_champion_name}) ja C/B mudelile ({cb_champion_name}); "
                "selles testis ei rakendata A+B+C-le ega XL-le taimeindeksit."
            )
            st.caption(
                f"3H live: ajaloolisi transition-ridu {_v3h_diag.get('historical_rows', 0)}, "
                f"treeningus {_v3h_diag.get('train_rows', 0)}, "
                f"9 päeva põlluprognoose {_v3h_diag.get('predictions', 0)}."
            )
            st.info(
            ...
            # TEST 3H: ehita täpselt sama 9 päeva põlluplaan, mida allpool kuvame.
            _v3h_future_plan = []
            _v3h_first = _next_field(today_plan[-1]) if today_plan else 1
            _v3h_current_first = _v3h_first
            for _v3h_offset in range(1, 10):
                _v3h_day = TODAY + timedelta(days=_v3h_offset)
                _v3h_fields = [
                    _v3h_current_first,
                    _next_field(_v3h_current_first),
                    _next_field(_next_field(_v3h_current_first)),
                ]
                _v3h_future_plan.append((_v3h_day, _v3h_fields))
                _v3h_current_first = _next_field(_v3h_fields[-1])

            try:
                _v3h_predictions, _v3h_diag = _v3h_live_predictions(
                    harvest_rows, all_weather_rows, TODAY, today_plan, _v3h_future_plan
                )
            except Exception as _v3h_exc:
                _v3h_predictions, _v3h_diag = {}, {"error": str(_v3h_exc)}
                st.error(f"3H TEST mootor ei käivitunud: {_v3h_exc}")
            ...
            for offset in range(1, 10):
                ...
                for f in day_fields:
                    prev = field_state.get(int(f))
                    if not prev:
                        day_rows.append({"Põld": f, "Taimeindeks": 1.0, "A+B+C": None, "C/B": None, "XL": None, "Kokku": None, "Intervall": None, "Alus": "puudub", "Hinnanguline ilm": "—"})
                        continue
                    result = _predict_one(int(f), prev, target_day)
                    if not result:
                        day_rows.append({"Põld": f, "Taimeindeks": 1.0, "A+B+C": None, "C/B": None, "XL": None, "Kokku": None, "Intervall": (target_day-prev["date"]).days, "Alus": prev["source"], "Hinnanguline ilm": "puudub"})
                        continue

                    # PUHAS 3H TEST: tuleviku A+B+C peab tulema ainult fikseeritud
                    # 3H-routerist. Kui 3H puudub, ei varja me seda app-128 ABC fallbackiga.
                    _v3 = _v3h_predictions.get((target_day, int(f)))
                    if not _v3:
                        day_rows.append({
                            "Põld": f, "Taimeindeks": 1.0, "A+B+C": None,
                            "C/B": result.get("cb"), "XL": None, "Kokku": None,
                            "Intervall": (target_day-prev["date"]).days,
                            "Alus": "3H puudub", "Hinnanguline ilm": "—",
                        })
                        continue

                    _v3h_abc_pred = float(_v3["abc"])
                    # XL jääb app-128 olemasolevast XL mudelist, kuid testis eemaldame
                    # hilishooaja taimeindeksi, et 3H katse oleks puhas.
                    _v3h_xl_pred = float(result.get("raw_xl"))
                    result["abc"] = _v3h_abc_pred
                    result["xl"] = _v3h_xl_pred
                    result["total"] = _v3h_abc_pred + _v3h_xl_pred
                    result["raw_abc"] = _v3h_abc_pred
                    result["raw_xl"] = _v3h_xl_pred
                    result["raw_total"] = result["total"]
                    result["plant_index"] = 1.0
                    result["abc_mode"] = f"3H {_v3['route']}"
                    result["abc_explain"] = {
                        "baseline": float(_v3["own2"]),
                        "effects": {"Muu": float(_v3h_abc_pred - float(_v3["own2"]))},
                        "raw_prediction": _v3h_abc_pred,
                        "prediction": _v3h_abc_pred,
                        "reconstructed": _v3h_abc_pred,
                        "v3h_route": _v3["route"],
                        "v3h_flat": float(_v3["flat"]),
                        "v3h_growth_days": float(_v3["growth_days"]),
                    }
            ...
            MODEL_VERSION = "v6.4-test-3h-clean-2026-08"

            # Enne esimese 3H test-snapshoti salvestamist säilita tänase issue
            # olemasolev ametlik snapshot app_settings JSON-is. Korje- ja ilmaandmeid
            # test ei muuda; see backup kaitseb lisaks ka tänast prognoosiseisu.
            _v3h_backup_key = f"v3h_pretest_snapshot_{TODAY.isoformat()}"
            try:
                if not _load_json_setting(_v3h_backup_key, {}):
                    _v3h_existing = db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []
                    _v3h_today_existing = [
                        r for r in _v3h_existing
                        if str(r.get("forecast_date") or "") == TODAY.isoformat()
                    ]
                    if _v3h_today_existing:
                        _save_json_setting(_v3h_backup_key, {
                            "saved_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                            "rows": _v3h_today_existing,
                        })
            except Exception:
                pass
            ...
            for target_day, rows_day in forecast_days:
                for row in rows_day:
                    ...
                    forecast_payloads.append({
                        ...
                        "interval_days": row.get("Intervall"),
                        "basis": (
                            f"{row.get('Alus') or ''}; abc_engine=3H-LAB147; "
                            f"xl_champion={xl_champion_name}; cb_champion={cb_champion_name}; "
                            f"taimeindeks=1.0000; "
                            f"raw_total={float(row.get('_Raw Kokku') or row.get('Kokku')):.6f}"
                        ),
                        "estimated_weather_days": row.get("Hinnanguline ilm") or "",
                        "model_version": MODEL_VERSION,
            ...
            try:
                if db.yield_forecasts_available():
                    ...
                    db.set_app_setting("v3h_clean_test_last_run_date", TODAY.isoformat())
                else:
