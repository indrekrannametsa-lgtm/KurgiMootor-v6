...
APP_PATCH_VERSION = "v7.0-production-7"  # CYCLE + WEATHER · monthly totals + neutral existing-harvest UI
...
if page == "Täna":
    ...
    with home_future_forecast_slot.container():
        ...
        for lead in range(1, 10):
            ...
            adj_pct = None if not _history_gate_ok else _home_forecast_adjustment(target_day, rows_day)
            adj_text = "—" if adj_pct is None else (
                "0%" if abs(adj_pct) < 0.5 else f"{'↑' if adj_pct > 0 else '↓'} {adj_pct:+.0f}%"
            )
            far_forecast = lead >= 6
...
def _run_cycle_weather_snapshot_fast():
    ...
    try:
        if db.yield_forecasts_available():
            ...
            forecast_store_ok = bool(forecast_payloads)
        else:
    ...
    complete_ok = bool(forecast_store_ok and forecast_payloads and not missing_forecasts)
...
if page == "Prognoos":
    ...
    for _lead in range(0, 10):
        ...
        if _lead == 0 and _rows:
            ...
            try:
                _live_today_plan = db.get_harvest_plan(TODAY)
            except Exception:
                _live_today_plan = None
            if _live_today_plan is not None:
                _live_today_set = {int(f) for f in _live_today_plan}
                _rows = [
                    _r for _r in _rows
                    if int(_r.get("field_no")) in _live_today_set
                ] if _live_today_set else []
        if not _history_gate_ok:
