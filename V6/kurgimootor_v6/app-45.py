...
if page == "Täna":
    ...
    # Avaleht EI käivita enam rasket mudeli walk-forward arvutust.
    # Kuvame viimase salvestatud yield_forecasts snapshoti. Prognoos-lehe avamine
    # arvutab mudeli uuesti ja kirjutab uue snapshoti.
    try:
        _home_saved = db.get_yield_forecasts(limit=1500) if db.yield_forecasts_available() else []
    except db.DatabaseError:
        _home_saved = []

    def _latest_snapshot_rows(target_day_value):
        _rows = [r for r in _home_saved if str(r.get("target_date")) == target_day_value.isoformat()]
        if not _rows:
            return []
        # Vali kõige uuem forecast_date; sama päeva duplikaadi korral uusim generated_at.
        _latest_date = max(str(r.get("forecast_date") or "") for r in _rows)
        _same = [r for r in _rows if str(r.get("forecast_date") or "") == _latest_date]
        _picked = {}
        for _r in _same:
            try:
                ...
                continue
            _old = _picked.get(_f)
            if _old is None or str(_r.get("generated_at") or "") > str(_old.get("generated_at") or ""):
                _picked[_f] = _r
        return list(_picked.values())

    def _render_saved_home_forecast(target_day_value, rows_day, compact=False):
        if not rows_day:
            return False
        try:
            _total = sum(float(r.get("total_forecast")) for r in rows_day)
            _abc = sum(float(r.get("abc_forecast")) for r in rows_day)
            _xl = sum(float(r.get("xl_forecast")) for r in rows_day)
        except (TypeError, ValueError):
            return False
        _cb_vals = []
        for _r in rows_day:
            try:
                if _r.get("cb_forecast") is not None:
                    _cb_vals.append(float(_r.get("cb_forecast")))
            except (TypeError, ValueError):
                pass
        _cb = float(np.mean(_cb_vals)) if _cb_vals else None
        _cb_text = "—" if _cb is None else _fmt(_cb, 2)
        _fdate = max(str(r.get("forecast_date") or "") for r in rows_day)
        if compact:
            st.markdown(
                f"**{_weekday_letter(target_day_value)} {_short_date(target_day_value)}**"
                f"  **{_fmt(_total)} kasti**  \n"
                f"A+B+C {_fmt(_abc)} · XL {_fmt(_xl)} · C/B ~{_cb_text}"
            )
        else:
            st.markdown(
                f"""
                <div style="border:2px solid rgba(76,160,92,.38);border-radius:12px;padding:12px 14px;">
                  ...
                  <div style="font-size:2.15rem;font-weight:950;line-height:1.05;margin-top:3px;">{_fmt(_total)} kasti</div>
                  <div style="font-size:.92rem;opacity:.76;margin-top:6px;">
                    A+B+C {_fmt(_abc)} · XL {_fmt(_xl)} · C/B ~{_cb_text}
                  </div>
                  <div style="font-size:.74rem;opacity:.55;margin-top:4px;">snapshot {_fdate}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    # Tänane tegelik on odav ja tuleb harvests tabelist.
    _actual_sum_home = _daily_summary(today_rows) if today_rows else {
        "total": 0.0, "a": 0.0, "b": 0.0, "c": 0.0, "xl": 0.0, "cb": None
    }
    _actual_cb_home = "—" if _actual_sum_home["cb"] is None else _fmt(_actual_sum_home["cb"], 2)
    with home_today_actual_slot.container():
        st.markdown(
            ...
            <div style="border:1px solid rgba(128,128,128,.25);border-radius:12px;padding:12px 14px;">
              ...
              <div style="font-size:2.15rem;font-weight:900;line-height:1.05;margin-top:3px;">{_fmt(_actual_sum_home['total'])} kasti</div>
              <div style="font-size:.92rem;opacity:.74;margin-top:6px;">
                A {_fmt(_actual_sum_home['a'])} · B {_fmt(_actual_sum_home['b'])} · C {_fmt(_actual_sum_home['c'])} · XL {_fmt(_actual_sum_home['xl'])} · C/B {_actual_cb_home}
              </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with home_today_forecast_slot.container():
        if not _render_saved_home_forecast(TODAY, _latest_snapshot_rows(TODAY)):
            st.caption("Tänase päeva salvestatud prognoosi pole veel.")

    # Mootori info: näita viimase snapshoti alust; ära treeni mudelit avalehel.
    _basis_rows = _latest_snapshot_rows(TODAY)
    if not _basis_rows:
        for _d in range(1, 10):
            _basis_rows = _latest_snapshot_rows(TODAY + timedelta(days=_d))
            if _basis_rows:
                break
    _basis = str(_basis_rows[0].get("basis") or "") if _basis_rows else ""
    _abc_name = "weather-first"
    _xl_name = "weather-first"
    _cb_name = "weather-first baas"
    _m = re.search(r"champion=([^;]+)", _basis)
    if _m:
        _abc_name = _m.group(1).strip()
    _m = re.search(r"xl_champion=([^;]+)", _basis)
    if _m:
        _xl_name = _m.group(1).strip()
    _m = re.search(r"cb_champion=([^;]+)", _basis)
    if _m:
        _cb_name = _m.group(1).strip()

    with home_engine_info_slot.container():
        st.caption(
            f"A+B+C mootor: {_abc_name} · XL mootor: {_xl_name} · "
            f"C/B hinnang: {_cb_name}"
        )

    with home_future_forecast_slot.container():
        _shown = 0
        for _lead in range(1, 10):
            _target = TODAY + timedelta(days=_lead)
            _rows = _latest_snapshot_rows(_target)
            if _rows and _render_saved_home_forecast(_target, _rows, compact=True):
                _shown += 1
        if _shown == 0:
...
if page == "Mootori tähelepanekud":
    ...
    if "AUTONOMOUS_DISCOVERY_ENABLED" in locals() and not AUTONOMOUS_DISCOVERY_ENABLED:
        st.info(
            "Autonoomne ideegeneraator on CPU-turvarežiimis ajutiselt pausil. "
            "Operatiivne prognoos ja tavapärane Jäljeotsija töötavad edasi."
        )
    st.caption(
