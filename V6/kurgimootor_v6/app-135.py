...
CYCLE_MODEL_VERSION = "v6.6-cycle-weather-production-3"
...
def _ensure_active_winddry_engine() -> None:
    ...
    if _current != CYCLE_MODEL_VERSION:
        ...
        _mark_forecast_dirty("CYCLE + WEATHER production-mootori kasutuselevõtt")
...
_run_operational_cycle = bool(
    _forecast_refresh_due
    or _run_light_research_requested
    or _run_layered_manual_requested
    # Käsitsi käivitatud kogu-hooaja OOS vajab sama production walk-forward
    # mudeliplokki. V6-s puudusid need kaks lippu siit tingimusest, mistõttu
    # UI näitas "Arvutan…", kuid arvutusplokki ei sisenetudki.
    or _season_oos_run_now
    or bool(_season_oos_selected_parts)
    or _wd_acceptance_due
    or _order_acceptance_due
    or _order_total_due
    or _size_dist_due
    or _residual_order_due
)
...
if _run_operational_cycle:
    ...
    with _forecast_cycle_ui:
        ...
        if training_rows:
            ...
            try:
                _ce_saved_plans = db.get_harvest_plans() if hasattr(db, "get_harvest_plans") else {}
                if not isinstance(_ce_saved_plans, dict):
                    _ce_saved_plans = {}
            except Exception:
                _ce_saved_plans = {}
            ...
            for offset in range(1, 10):
                ...
                _saved_plan = _ce_saved_plans.get(target_day.isoformat())
                if _saved_plan is not None:
                    try:
                        day_fields = [int(f) for f in _saved_plan if 1 <= int(f) <= 14]
                    except Exception:
                        day_fields = []
                else:
