...
APP_PATCH_VERSION = "wd-engine-parallel-6"  # paralleelne WIND×DRY mootor; production app-128 jääb puutumata
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor · WIND×DRY PARALLEEL", page_icon=APP_ICON, layout="wide")


# =============================================================================
# APP-128 WIND×DRY TEST · READ ONLY
# =============================================================================
# See test ei kirjuta Supabase'i ega värskenda ilma. Kogu app-128 kasutajaliides ja
# taimeindeks jäävad alles. Ainus prognoosimuudatus: tulevase A+B+C peale lisatakse
# LAB-151..154 lukustatud WIND×DRY HIGH L3–7 LEVEL+DELTA kiht.
READ_ONLY_WINDDRY_TEST = True
TEST_MODEL_VERSION = "wd-engine-parallel-6"

if READ_ONLY_WINDDRY_TEST:
    _real_save_yield_forecasts = getattr(db, "save_yield_forecasts", None)
    ...
    def _test_save_yield_forecasts(rows):
        if _real_save_yield_forecasts is None:
            return 0
        safe_rows = []
        for row in list(rows or []):
            rr = dict(row)
            rr["model_version"] = TEST_MODEL_VERSION
            safe_rows.append(rr)
        return _real_save_yield_forecasts(safe_rows)

    db.save_yield_forecasts = _test_save_yield_forecasts

    # Välisvõrku/ilma ei värskendata testäpis.
...
st.warning(
    "🧪 WIND×DRY PARALLEELMOOTOR — app-128 UI + sama taimeindeks + lukustatud "
    "WIND×DRY HIGH L3–7 LEVEL+DELTA. Productionit ei muudeta; salvestatakse ainult "
    "oma prognoosisnapshotid versiooniga wd-engine-parallel-6."
)

# --- Lukustatud LAB-154 parameetrid. Neid selles testis ei häälestata. ---
_WD_RIDGE_ALPHA = 10.0
_WD_MIN_TRAIN_ROWS = 24
_WD_TARGET_EPS = 0.20
_WD_HIGH_Q = 0.75
_WD_MIN_DAYS_FOR_HIGH_THRESHOLD = 10
_WD_BASE_COLS = ["prev_log_rate", "season_day", "growth", "growth_delta"]
_WD_EXTRA_COLS = ["high_days", "high_run", "d_high_days", "d_high_run"]
_WD_HOURS_PER_FIELD = 3.0
...
def _wd_abc(row):
    vals = [_wd_f(row.get(k)) for k in ("a", "b", "c")]
    if any(v is None for v in vals):
        return None
    return float(sum(vals))


def _wd_reliable(row):
    q = str(row.get("data_quality") or "").strip().lower()
    return q not in {"hinnanguline", "ligikaudne", "estimated", "approximate"}


def _wd_growth(prev, cur):
    g = float((cur["day"] - prev["day"]).days) + (int(cur["order"]) - int(prev["order"])) * (_WD_HOURS_PER_FIELD / 24.0)
    return max(0.5, g)


def _wd_l3_7_days(target_day):
    return [target_day - timedelta(days=lag) for lag in range(7, 2, -1)]
...
def _wd_weather_maps(rows):
    """Treeningul ainult kontrollitud mõõdetud ilm; live-aknas mõõdetud > prognoos."""
    measured = {}
    forecast = {}
    for r in rows:
        dd = _wd_day(r.get("weather_date"))
        if dd is None:
            continue
        wind = _wd_f(r.get("wind_avg_ms"))
        rh = _wd_f(r.get("humidity_avg_pct"))
        if wind is None or rh is None:
            continue
        rec = {"wd": wind * (100.0 - rh), "wind": wind, "rh": rh}
        ...
        if kind == "measured" and bool(r.get("checked")):
            ...
            forecast[dd] = rec
    live = dict(forecast)
    live.update(measured)
    return measured, live


def _wd_high_threshold(measured, before_day):
    vals = [float(w["wd"]) for dd, w in measured.items() if dd < before_day and math.isfinite(float(w["wd"]))]
    if len(vals) < _WD_MIN_DAYS_FOR_HIGH_THRESHOLD:
        return None
    return float(np.quantile(np.asarray(vals, dtype=float), _WD_HIGH_Q))


def _wd_window(target_day, values_weather, measured_for_threshold):
    days = _wd_l3_7_days(target_day)
    if any(d not in values_weather for d in days):
        return None
    threshold = _wd_high_threshold(measured_for_threshold, min(days))
    if threshold is None:
        return None
    vals = [float(values_weather[d]["wd"]) for d in days]
    ...
    return {
        ...
        "kinds": "".join(str(values_weather[d].get("kind") or "?") for d in days),
    }


def _wd_event_features(cur, prev, prevprev, values_weather, measured_for_threshold):
    current = _wd_window(cur["day"], values_weather, measured_for_threshold)
    previous = _wd_window(prev["day"], values_weather, measured_for_threshold)
    if current is None or previous is None:
        return None
    growth = _wd_growth(prev, cur)
    if prevprev is not None:
        prev_growth = _wd_growth(prevprev, prev)
    else:
        prev_growth = _wd_f(prev.get("interval"))
        if prev_growth is None or prev_growth <= 0:
            return None
    prev_rate = float(prev["abc"]) / max(0.5, float(prev_growth))
    if prev_rate <= 0:
        return None
    return {
        "prev_log_rate": math.log(prev_rate + _WD_TARGET_EPS),
        "season_day": float((cur["day"] - SEASON_START).days),
        "growth": float(growth),
        "growth_delta": float(growth - prev_growth),
        "high_days": float(current["high_days"]),
        "high_run": float(current["high_run"]),
        "d_high_days": float(current["high_days"] - previous["high_days"]),
        "d_high_run": float(current["high_run"] - previous["high_run"]),
        "wd_avg": float(current["avg"]),
        "threshold": float(current["threshold"]),
        "kinds": current["kinds"],
        "window": f"{current['days'][0].strftime('%d.%m')}–{current['days'][-1].strftime('%d.%m')}",
        "prev_rate": float(prev_rate),
        "growth_for_target": float(growth),
    }


def _wd_ridge_fit(X, y):
    ...
    reg = np.eye(Xd.shape[1]) * _WD_RIDGE_ALPHA
    reg[0, 0] = 0.0
    try:
...
def _wd_ridge_pred(model, x):
    x = np.asarray(x, dtype=float)
    z = (x - model["mu"]) / model["sd"]
    return float(np.r_[1.0, z] @ model["beta"])


def _wd_prepare_actual_events(harvest_rows):
    by_field = {f: [] for f in range(1, 15)}
    for r in harvest_rows:
        dd = _wd_day(r.get("harvest_date"))
        if dd is None or dd > TODAY or not _wd_reliable(r):
            continue
        try:
        ...
        by_field[field].append({
            "day": dd, "field": field, "order": order, "abc": float(abc),
            "interval": _wd_f(r.get("interval_days")), "source": "actual",
        })
    for f in by_field:
        by_field[f].sort(key=lambda e: (e["day"], e["order"], e["field"]))
    return by_field


def _wd_fit_locked_layer(harvest_rows, all_weather_rows):
    measured, live = _wd_weather_maps(all_weather_rows)
    hist = _wd_prepare_actual_events(harvest_rows)
    records = []
    for f in range(1, 15):
        hh = hist.get(f, [])
        for i in range(1, len(hh)):
            cur = hh[i]
            prev = hh[i - 1]
            prevprev = hh[i - 2] if i >= 2 else None
            feat = _wd_event_features(cur, prev, prevprev, measured, measured)
            if feat is None:
                continue
            growth = feat["growth_for_target"]
            cur_rate = float(cur["abc"]) / max(0.5, growth)
            prev_rate = feat["prev_rate"]
            y = math.log((cur_rate + _WD_TARGET_EPS) / (prev_rate + _WD_TARGET_EPS))
            records.append({"y": y, **feat})
    if len(records) < _WD_MIN_TRAIN_ROWS:
        return None, measured, live, len(records)
    df = pd.DataFrame(records)
    base_cols = list(_WD_BASE_COLS)
    cand_cols = base_cols + list(_WD_EXTRA_COLS)
    ...
    if int(ok.sum()) < _WD_MIN_TRAIN_ROWS:
        return None, measured, live, int(ok.sum())
    models = {
        "base": _wd_ridge_fit(xb[ok], y[ok]),
        "cand": _wd_ridge_fit(xc[ok], y[ok]),
        "n": int(ok.sum()),
    }
    return models, measured, live, int(ok.sum())


def _wd_layer_multiplier(models, feat):
    if not models or feat is None:
        return 1.0
    xb = [feat[c] for c in _WD_BASE_COLS]
    xc = [feat[c] for c in _WD_BASE_COLS + _WD_EXTRA_COLS]
    try:
        base = _wd_ridge_pred(models["base"], xb)
        cand = _wd_ridge_pred(models["cand"], xc)
        value = float(math.exp(cand - base))
        # Mitte käsitsi häälestatud mõju-lävi; ainult numbriline turvapiir juhuks,
        # kui väga kauge prognoos läheb treeningjaotusest täiesti välja.
        return max(0.35, min(1.65, value))
    except Exception:
        return 1.0


def _wd_apply_locked_layer(harvest_rows, all_weather_rows, today_plan, today_actual,
                           today_forecast_rows, forecast_days):
    """Muuda ainult kuvatava/salvestatava TEST A+B+C. Rekursiivne app-128 state on selleks ajaks juba valmis."""
    models, measured, live, train_n = _wd_fit_locked_layer(harvest_rows, all_weather_rows)
    if not models:
        return {"train_n": train_n, "changed": 0, "max_change_pct": 0.0}

    hist = _wd_prepare_actual_events(harvest_rows)

    # Kui tänane põld pole veel tegelikuna sisestatud, liiguta WIND×DRY variseisundit
    # app-128 tänase ALGSE prognoosiga. Testkihi enda väljundit ei söödeta tagasi.
    today_rows_by_field = {int(r.get("Põld")): r for r in today_forecast_rows if r.get("Põld") is not None}
    for order, f in enumerate(today_plan, start=1):
        f = int(f)
        if f in today_actual:
            continue
        rr = today_rows_by_field.get(f)
        if not rr or rr.get("A+B+C") is None:
            continue
        hist.setdefault(f, []).append({
            "day": TODAY, "field": f, "order": order, "abc": float(rr["A+B+C"]),
            "interval": _wd_f(rr.get("Intervall")), "source": "official_forecast",
        })
        hist[f] = sorted(hist[f], key=lambda e: (e["day"], e["order"], e["field"]))[-3:]
    ...
    for target_day, rows_day in forecast_days:
        for order, row in enumerate(rows_day, start=1):
            if row.get("Põld") is None or row.get("A+B+C") is None:
                continue
            f = int(row["Põld"])
            official_abc = float(row["A+B+C"])
            hh = hist.get(f, [])
            prev = hh[-1] if hh else None
            prevprev = hh[-2] if len(hh) >= 2 else None
            cur = {
                "day": target_day, "field": f, "order": order,
                "abc": official_abc, "interval": _wd_f(row.get("Intervall")), "source": "official_forecast",
            }
            feat = _wd_event_features(cur, prev, prevprev, live, measured) if prev is not None else None
            factor = _wd_layer_multiplier(models, feat)
            test_abc = official_abc * factor
            delta = test_abc - official_abc

            row["_App128 ABC"] = official_abc
            row["_WD kordaja"] = factor
            row["_WD detail"] = feat
            row["A+B+C"] = float(test_abc)
            if row.get("XL") is not None:
                row["Kokku"] = float(test_abc) + float(row["XL"])
            row["Alus"] = f"{row.get('Alus') or ''} · WD×DRY {factor:.3f}"

            ex = row.get("_ABC_selgitus")
            if isinstance(ex, dict):
                ex2 = dict(ex)
                effects = dict(ex2.get("effects") or {})
                effects["WIND×DRY"] = float(delta)
                ex2["effects"] = effects
                ex2["prediction"] = float(test_abc)
                ex2["reconstructed"] = float(test_abc)
                row["_ABC_selgitus"] = ex2

            pct = abs((factor - 1.0) * 100.0)
            if pct > 0.05:
                changed += 1
            max_change = max(max_change, pct)
            factors.append(factor)

            # Järgmise sama põllu WIND×DRY tunnuse variseisundisse läheb APP-128
            # ALGNE prognoos, mitte TEST ABC. Nii ei teki testkihi enesetagasisidet.
            hist.setdefault(f, []).append(cur)
            hist[f] = sorted(hist[f], key=lambda e: (e["day"], e["order"], e["field"]))[-3:]
...
if page == "Täna":
    ...
    try:
        ...
        _home_saved = [r for r in _home_saved_all if str(r.get("model_version") or "") == TEST_MODEL_VERSION]
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            # -------------------------------------------------------------
            # TEST: üks lukustatud WIND×DRY HIGH L3–7 LEVEL+DELTA kiht.
            # Rakendame ALLES pärast app-128 rekursiivse 9p state'i valmimist,
            # et TEST ABC ei saaks järgmise korje sisendiks tagasi minna.
            # -------------------------------------------------------------
            _wd_test_summary = _wd_apply_locked_layer(
                harvest_rows, all_weather_rows, today_plan, today_actual,
                today_forecast_rows, forecast_days,
            )
            st.info(
                f"🧪 WIND×DRY testkiht: treeningridu {_wd_test_summary.get('train_n', 0)} · "
                f"muudetud põlluridu {_wd_test_summary.get('changed', 0)} · "
                f"suurim ABC muutus {_wd_test_summary.get('max_change_pct', 0.0):.1f}% · "
                f"keskm kordaja {_wd_test_summary.get('mean_factor', 1.0):.3f}. "
                "Taimeindeks on app-128 oma; XL on muutmata; TEST väljundit ei söödeta rekursiooni tagasi."
            )
            ...
            try:
                if db.yield_forecasts_available():
                    saved_n = db.save_yield_forecasts(forecast_payloads)
                    forecast_store_ok = saved_n == len(forecast_payloads)
                else:
                    st.warning("Prognoosid arvutatakse, kuid neid ei salvestata: Supabase'is puudub yield_forecasts tabel.")
            except db.DatabaseError as exc:
                st.warning(f"Paralleelmootori prognoosiajaloo salvestamine ebaõnnestus: {exc}")
            ...
            for target_day, rows_day in forecast_detail_days:
                ...
                for r in rows_day:
                    ...
                    erow = {
                        ...
                        "WIND×DRY": effects.get("WIND×DRY", 0.0),
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    try:
        ...
        _ro_saved = [r for r in _ro_saved_all if str(r.get("model_version") or "") == TEST_MODEL_VERSION]
    except db.DatabaseError:
        _ro_saved = []
