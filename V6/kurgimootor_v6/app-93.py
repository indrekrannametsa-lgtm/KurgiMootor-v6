...
LAYERED_DAILY_BUDGET_S = 105.0  # jätab ~15 s varu võrreldes 2 minutiga
...
# Automaatse laia otsingu käivitame ainult siis, kui tänaseks on lisandunud uus
# täielik korjepäev. Ilmauuendus üksi EI tohi seda käivitada.
try:
...
if page == "Mootori tähelepanekud":
    ...
    if _layered_success_today:
        st.button(
            "🔬 Tänane lai uurimisvoor tehtud",
            disabled=True,
            key="layered_manual_disabled",
        )
        st.caption("Lai otsing saab uue täieliku korjepäeva järel ühe ajaliselt piiratud uurimisvooru.")
    else:
        _layered_run_now = st.button(
            "🔬 Käivita lai uurimisvoor käsitsi",
            key="layered_manual_run",
            help="Tavaliselt käivitub see ise uue täieliku korjepäeva järel."
        )
...
if page == "Prognoos" or _auto_light_check or _layered_run_now:
    ...
    with _forecast_page_placeholder.container():
        ...
        # Üks automaatne lai uurimisvoor iga UUE täieliku korjepäeva kohta.
        # Sama päeva ilmamuutused seda ei käivita.
        if last_complete_harvest is not None:
            _is_new_complete_harvest_trigger = str(_light_cycle_reason or "").startswith("täielik korjepäev")
            _layered_auto_due = (
                _is_new_complete_harvest_trigger
                and last_complete_harvest.isoformat() != str(_last_research_harvest_day or "")
                and not _layered_success_today
            )
            if _layered_auto_due:
        ...
        if training_rows:
            ...
            if _layered_run_now:
                ...
                def _serialize_research_state(state):
                    # Hoia ajalugu kompaktne; kuni 600 katset on selle hooaja jaoks enam kui piisav.
                    state = dict(state)
                    state["results"] = list(state.get("results", []))[-600:]
                ...
                def _result_score(row):
                    # Suund: MAE-paranemine on põhiline, kuid ainult see EI otsusta
                    # uurimisõigust. Trend ja stabiilsus annavad väikese lisakaalu.
                    try:
                ...
                def _mutate_from_key(parent_key):
                    ...
                    action = int(_rng.integers(0, 3))
                    if action == 0 and len(layers_present) < min(5, len(all_layers)):
                        # Lisa uus kiht.
                ...
                def _fresh_explore_combo():
                    # 2–5 eri kihti; ei eelda, et üksiktunnus oleks varem hea olnud.
                    layers = list(_specs_by_layer.keys())
                    if len(layers) < 2:
                        return None
                    max_k = min(5, len(layers))
                    # Rohkem kaalu 2–3 kihile, kuid 4–5 kihti saavad samuti võimaluse.
                    choices = np.arange(2, max_k + 1)
                    weights = np.array([0.38, 0.32, 0.20, 0.10][:len(choices)], dtype=float)
                    weights = weights / weights.sum()
                    k = int(_rng.choice(choices, p=weights))
                    picked_layers = list(_rng.choice(layers, size=k, replace=False))
                    return [
                        _rng.choice(_specs_by_layer[str(layer)])
                        for layer in picked_layers
                    ]

                # Esmalt kinnita osa seni tugevaid kombinatsioone uue andmeseisu peal,
                # seejärel ~75/25 exploit/explore. Nii on otsing suunatud, kuid avatud.
                _best_parent_keys = _historical_best_keys(limit=40)
                _revalidate_queue = []
                for _key in _best_parent_keys[:8]:
                ...
                while (
                    ...
                    if _revalidate_queue:
                        ...
                        exploit = bool(_best_parent_keys) and (_rng.random() < 0.75)
                        if exploit:
                    ...
                    if not _key or _key in _tested:
                        continue

                    # Kui aega on liiga vähe, uut kallist walk-forward'i ei alusta.
                    if time.perf_counter() >= (_layered_deadline - _layered_stop_margin_s):
                        break
                ...
                # Championiks pääseb endiselt ainult avastuse järel eraldi hilisemal
                # confirmation-perioodil kinnitust saanud kandidaat.
                if _current_results and _abc_confirmation_days:
                    _selected_row = _current_results[0]
                    _selected_specs = _combo_from_key(_selected_row["combo_key"])
                    if _selected_specs:
                        _selected_cols = _merge_cols_many(_selected_specs)
                        _selected_pred = _walk_forward_with_extra(_selected_cols)
                        _confirm_stats = _period_stability_stats(
                            predictions, _selected_pred, y, _abc_confirmation_days,
                            min_rows=6,
                            improvement_threshold=0.05,
                            min_half_threshold=-0.05,
                        )
                        if _confirm_stats and _confirm_stats["Stabiilne"]:
                            champion_name = "Avatud kihiline champion"
                            champion_cols = _selected_cols
                            champion_pred = _selected_pred
                            champion_mae = _confirm_stats["Katse MAE"]
                            champion_stats = dict(_confirm_stats)
                            champion_stats["Kinnitusperiood"] = True
                            champion_stats["Kihid"] = " | ".join(
                                _selected_row.get("parts") or []
                            )
            ...
            # Kõik kerge Jäljeotsija baas- ja kandidaadiennustused on nüüd
            # selle andmeseisu jaoks olemas. Salvesta need järgmise rerun'i tarbeks.
            ...
            MODEL_VERSION = "v6.5-layered-bio-v16-light-wf-cache"
            forecast_payloads = []
...
if page == "Mootori tähelepanekud":
    ...
    st.caption(
        "Mootori tähelepanekute avamine ei arvuta prognoosi uuesti. Lai kihiline otsing saab ühe piiratud uurimisvooru uue täieliku korjepäeva järel; "
