...
# -----------------------------------------------------------------------------
# APP-129 READ-ONLY TEST MODE
# Loeb tootmise Supabase andmeid, kuid ei kirjuta sinna midagi.
# Test käivitab Prognoos-lehel A+B+C arvutuse alati värskelt.
# -----------------------------------------------------------------------------
READ_ONLY_TEST = True
...
if READ_ONLY_TEST:
    ...
    # Kõik app.py otsesed DB-mutatsioonid blokeeritakse.
    for _name in (
    ...
    # Testapp ei tohi ilma automaatselt ega käsitsi Supabase'i uuendada.
...
APP_PATCH_VERSION = "app-129-test"  # READ-ONLY test; täpne kasvuaeg A+B+C baasmudelis
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor V6.4 TEST 129", page_icon=APP_ICON, layout="wide")
st.warning("🧪 APP-129 READ-ONLY TEST — tootmise andmeid ei muudeta. Võrdluseks kasuta Prognoos-lehte; Avalehe salvestatud snapshot võib olla app-128 oma.")
...
_forecast_dirty_reason = "app-129 read-only test" if READ_ONLY_TEST else (db.get_app_setting("forecast_dirty_reason", "") if _forecast_refresh_due else "")
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            # XL ja C/B jätkavad app-128 baasi peal (kalendripäevane Intervall p).
            # A+B+C kasutab app-129 kandidaadis täpset kasvuaega, et eristada nt ~4.2 vs ~4.9 p.
            # Nii ei muuda me korraga XL/C/B loogikat ega kirjuta intervallile käsitsi protsenti ette.
            base_cont_cols = [
            ...
            abc_base_cont_cols = [
                "Kasvuaeg p" if c == "Intervall p" else c
                for c in base_cont_cols
            ]
            ...
            X_base_abc = model_df[abc_base_cont_cols].astype(float).to_numpy()
            ...
            # -------------------------------------------------------------
            _light_cache_version = "v17-abc-exact-growthtime-base"
            ...
            def _abc_growth_walk_predict(extra_arrays, train_idx, test_idx, alpha=10.0, field_alpha=80.0, z_clip=2.5):
                return _engine_abc_growth_walk_predict(
                    X_base_abc, fields, log_y_abc, extra_arrays, train_idx, test_idx,
            ...
            champion_cols = [
                ...
                if str(c) in model_df.columns and str(c) != "Kasvuaeg p"
            ]
            ...
            if _layered_run_now:
                ...
                if _selected_confirmed_row is not None:
                    _selected_cols = [c for c in list(_selected_confirmed_row.get("cols") or []) if c != "Kasvuaeg p"]
                    if _selected_cols:
            ...
            def _fit_full_abc_growth(extra_arrays, alpha=10.0, field_alpha=80.0, z_clip=2.5):
                return _engine_fit_full_abc_growth(
                    X_base_abc, fields, log_y_abc, extra_arrays, alpha=alpha,
            ...
            def _abc_growth_explain(model, field_no, base_values, extra_values, extra_names):
                ...
                def _group_for_feature(name):
                    if name in {"Intervall p", "Kasvuaeg p"}:
                        return "Intervall"
                ...
                numeric_names = list(abc_base_cont_cols) + list(extra_names)
            ...
            def _weather_window_for_prediction(previous_day, target_day, previous_order=1, target_order=1):
                ...
                _calendar_interval = (target_day - previous_day).days
                _growth_time_days = float(_calendar_interval) + (float(target_order) - float(previous_order)) * (3.0 / 24.0)
                wx = {
                    "Intervall p": _calendar_interval,
                    "Kasvuaeg p": _growth_time_days,
            ...
            def _predict_one(field_no, state, target_day, target_order=1):
                wx, estimated_days = _weather_window_for_prediction(state["date"], target_day, state.get("order", 1), target_order)
                if wx is None:
                    return None
                generic_base_values = [wx[c] for c in base_cont_cols]
                abc_base_values = [wx[c] for c in abc_base_cont_cols]
                ...
                raw_abc_pred = _predict_full_abc_growth(
                    abc_model_used, field_no, abc_base_values, abc_extra_values,
                )
                abc_explain = _abc_growth_explain(
                    abc_model_used, field_no, abc_base_values, abc_extra_values, abc_extra_names,
                )

                if xl_champion_uses_memory and state.get("source") != "tegelik":
                    raw_xl_pred = _predict_full_generic(full_xl_base_model, field_no, generic_base_values, [])
                else:
                    raw_xl_pred = _predict_full_generic(
                        full_xl_model, field_no, generic_base_values,
                ...
                if cb_champion_uses_memory and state.get("source") != "tegelik":
                    cb_log_pred = _predict_full_generic(
                        full_cb_base_model, field_no, generic_base_values, [], floor_zero=False,
                    )
                else:
                    cb_log_pred = _predict_full_generic(
                        full_cb_model, field_no, generic_base_values,
                ...
                return {
                    ...
                    "interval": wx["Intervall p"], "growth_time": wx.get("Kasvuaeg p"), "estimated_days": estimated_days or set(), "abc_mode": abc_mode,
            ...
            # Hoia algse päevaplaani korjejärjekord alles ka siis, kui Avalehel mõni põld peidetakse.
            # Täpne kasvuaeg sõltub korjekellaajast/järjekorrast; valiku peitmine ei tohi seda muuta.
            _today_order_by_field = {int(f): i for i, f in enumerate(today_plan_default, start=1)}
            ...
            for _today_pos, f in enumerate(today_plan, start=1):
                prev = field_state.get(int(f))
                if not prev:
                    continue
                pred = _predict_one(int(f), prev, TODAY, target_order=_today_order_by_field.get(int(f), _today_pos))
                if not pred:
                    continue
            ...
            for f in today_plan:
                ...
                if actual:
                    try:
                        ...
                        field_state[int(f)] = {
                            ...
                            "load_index": load_index, "overload": overload, "load2_index": load2_index,
                            "peak": peak, "peak_prev": peak_hist[-1] if peak_hist else None,
                            "order": int(actual.get("harvest_order") or _today_order_by_field.get(int(f), today_plan.index(int(f)) + 1)),
                            "source": "tegelik",
                        }
                ...
                field_state[int(f)] = {
                    ...
                    "order": int(_today_order_by_field.get(int(f), today_plan.index(int(f)) + 1)),
                    "source": "prognoos",
                }
            ...
            for offset in range(1, 10):
                ...
                day_rows = []
                for _day_pos, f in enumerate(day_fields, start=1):
                    prev = field_state.get(int(f))
                    if not prev:
                        ...
                        continue
                    result = _predict_one(int(f), prev, target_day, target_order=_day_pos)
                    if not result:
            ...
            try:
                ...
                if forecast_days and _challenger_is_distinct:
                    ...
                    _tomorrow_order_by_field = {int(r.get("Põld")): i for i, r in enumerate(_tomorrow_official_rows, start=1) if r.get("Põld") is not None}
                    for _f, _orow in _official_rows_by_field.items():
                        ...
                        if not _state or _orow.get("XL") is None:
                            continue
                        _wx, _est = _weather_window_for_prediction(
                            _state["date"], _tomorrow_day, _state.get("order", 1), _tomorrow_order_by_field.get(int(_f), 1)
                        )
                        if _wx is None:
                            continue
                        _base_values = [_wx[c] for c in abc_base_cont_cols]
