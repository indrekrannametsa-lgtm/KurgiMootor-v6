...
APP_PATCH_VERSION = "v7.0-production-1"  # CYCLE + WEATHER · operational state continuity fix
...
def _run_cycle_weather_snapshot_fast():
    ...
    return bool(forecast_store_ok and forecast_payloads), len(forecast_payloads)
...
if _forecast_refresh_due:
    ...
    if _fast_cycle_saved:
        ...
        db.set_app_setting(
            "forecast_last_error",
            _fast_cycle_error or f"CYCLE snapshot salvestamata: payload_rows={_fast_cycle_payload_count}",
        )
...
if page == "Mootor":
    ...
    try:
        _motor_train_n = int(float(db.get_app_setting("light_cycle_last_training_rows", "0") or 0))
    except Exception:
        _motor_train_n = 0
