...
APP_PATCH_VERSION = "wd-leveldelta-production-4-season-oos-audit-5-recent-window-visible-result"  # diagnostic only: active-engine benchmark + filtered challenger season OOS
    ...
    if isinstance(_season_oos_saved, dict) and (
        ...
        if _active_baseline:
            ...
            st.caption(
                ...
                "Vali järgmine kandidaat ise allpool tabelireast; korraga arvutatakse ainult üks."
            )
                ...
                if not trace_df.empty:
                    ...
                    if "layered_trace_df" in locals() and not layered_trace_df.empty:
                        ...
                        with st.expander("Näita kõiki kihilisi BIO-mudeleid"):
                            ...
                            _row_choices = [0] + list(range(1, len(layered_trace_df) + 1))

                            def _layered_row_label(_row_no):
                                if int(_row_no) == 0:
                                    return "— vali tabelist rida —"
                                _rr = layered_trace_df.iloc[int(_row_no) - 1]
                                _imp = _rr.get("Paranemine")
                                _conf = _rr.get("Kinnitus Δ")
                                _parts = str(_rr.get("Koosseis") or "")
                                _short = _parts if len(_parts) <= 78 else (_parts[:75] + "…")
                            ...
                            _selected_row_no = st.selectbox(
                                "Vali kandidaat kogu-hooaja OOS-testiks",
                                options=_row_choices,
                                format_func=_layered_row_label,
                                key="layered_candidate_row_pick",
                                help="Vali sama reanumber, mida näed ülal tabeli esimeses veerus. Arvutatakse ainult see üks kandidaat.",
                            )

                            if int(_selected_row_no) > 0:
                                _sel_i = int(_selected_row_no) - 1
                                _sel_row = layered_trace_df.iloc[_sel_i]
                                _sel_parts = str(_sel_row.get("Koosseis") or "")
                                _sel_conf = _sel_row.get("Kinnitus Δ")
                                _sel_imp = _sel_row.get("Paranemine")
                                st.caption(
                                    f"Valitud rida {int(_selected_row_no)}: {_sel_parts} · avastus Δ "
                                    + (f"{float(_sel_imp):+.2f}" if pd.notna(_sel_imp) else "—")
                                    + " · kinnitus Δ "
                                    + (f"{float(_sel_conf):+.2f}" if pd.notna(_sel_conf) else "—")
                                )
                                if st.button(
                                    "📊 Testi valitud kandidaati kogu hooajal",
                                    key="layered_selected_candidate_oos_button",
                                    help="Arvutab strict walk-forward OOS-i ainult sellele ühele valitud kandidaadile ja võrdleb aktiivse WIND×DRY-ga.",
                                ):
                                    st.session_state["_season_oos_selected_parts_run"] = _sel_parts
                                    st.rerun()
                            else:
                                st.caption("Vali ülal rippmenüüst tabelirea number. Tabel ise jääb ainult vaatamiseks.")
                    else:
                        st.caption(
