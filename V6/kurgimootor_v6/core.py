...
class WeatherService:
    ...
    def _haademeeste_hour_temperature_from_page(self, day: date, hour: int) -> Dict[str, Any]:
        """Loeb Ilmateenistuse tunniandmete HTML-ist Häädemeeste ühe tunni temperatuuri."""
        params = {
            "lang": "et",
        ...
        temp = None
        if len(found_row) >= 2:
            raw = str(found_row[1]).strip().replace(",", ".")
            try:
                temp = float(raw)
            except ValueError:
                temp = None
    ...
    def test_haademeeste_day_profile(self, day: date) -> Dict[str, Any]:
        """Diagnostika: 24 Häädemeeste tunnitemperatuuri + päeva/öö koondid.

        Öö = 20:00–07:59 (20,21,22,23,0..7)
        Päev = 08:00–19:59 (8..19)
        Ei kirjuta Supabase'i.
        """
        hourly: List[Dict[str, Any]] = []
        for hour in range(24):
            hourly.append(self._haademeeste_hour_temperature_from_page(day, hour))
