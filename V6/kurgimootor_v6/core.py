...
class WeatherService:
    ...
    def refresh_measured(self, start_day: date, end_day: date) -> Dict[str, int]:
        """Mõõdetud päev: Häädemeeste T/tuul + Pärnu rad/RH/sademed.

        Tänast päeva siia ei anta. Häädemeeste 24/24 tunni olemasolu on
        mõõdetud temperatuuri/tuule kvaliteedilukk.
        """
        ...
        existing_rows = {
            ...
            for r in db.get_weather_rows(start_day, end_day)
        ...
        while d <= end_day:
            ...
            # Kui Häädemeeste täielik päev on juba salvestatud, ei lae 24 HTML-lehte uuesti.
            has_hdm = (
                "Häädemeeste" in str(existing.get("source_station") or "")
                and existing.get("temp_day_avg_c") is not None
                and existing.get("temp_night_avg_c") is not None
                and existing.get("temp_min_c") is not None
                and existing.get("temp_max_c") is not None
                and existing.get("wind_avg_ms") is not None
            )
            if has_hdm:
                t_min = self._as_float(existing.get("temp_min_c"))
                t_max = self._as_float(existing.get("temp_max_c"))
                temp_day_avg = self._as_float(existing.get("temp_day_avg_c"))
                temp_night_avg = self._as_float(existing.get("temp_night_avg_c"))
                wind_avg = self._as_float(existing.get("wind_avg_ms"))
                hdm_complete = True
            else:
                profile = self._haademeeste_day_profile(d)
                t_min = self._as_float(profile.get("tmin_c"))
                t_max = self._as_float(profile.get("tmax_c"))
                temp_day_avg = self._as_float(profile.get("day_avg_c"))
                temp_night_avg = self._as_float(profile.get("night_avg_c"))
                wind_avg = self._as_float(profile.get("wind_avg_ms"))
                hdm_complete = (
                    int(profile.get("hours_found") or 0) == 24
                    and int(profile.get("wind_hours_found") or 0) == 24
                    and int(profile.get("day_hours_found") or 0) == 12
                    and int(profile.get("night_hours_found") or 0) == 12
                    and all(v is not None for v in (
                        t_min, t_max, temp_day_avg, temp_night_avg, wind_avg
                    ))
                )
            ...
            # Kui Pärnu valideeritud päevaelemendid pole veel avaldatud, võib kasutada
            # ainult selle päeva varem salvestatud forecast-snapshoti. Häädemeeste
            # mõõdetud T/tuult prognoosiga ei asendata.
            ...
            et0_mm = self.calculate_et0_mm(
                d, t_min, t_max, wind_avg, rad, humidity_avg
            )

            parnu_complete = (
                key in radiation_daily
            ...
            if fully_official:
                check_message = (
                    "Kontrollitud · Häädemeeste 24h T/tuul + "
                    "Pärnu valideeritud radiatsioon/RH/sademed"
                )
            elif hdm_complete and all(v is not None for v in (
                rad, humidity_avg, precipitation_mm, et0_mm
            )):
                temporary += 1
                check_message = (
                    "Ajutine · Häädemeeste 24h T/tuul + "
                    + ("varem salvestatud prognoos: " + ", ".join(forecast_parts)
                       if forecast_parts else "Pärnu päevaelemendid osaliselt puudu")
                )
            else:
                missing = []
                if not hdm_complete:
                    missing.append("Häädemeeste 24h T/tuul")
                if rad is None:
                    missing.append("radiatsioon")
                if humidity_avg is None:
                    missing.append("niiskus")
                if precipitation_mm is None:
                    missing.append("sademed")
                if et0_mm is None:
                    missing.append("ET0")
                check_message = "Puudulik · " + ", ".join(missing)
            ...
            db.upsert_weather({
                ...
                "temp_min_c": t_min,
                "temp_max_c": t_max,
                "temp_day_avg_c": temp_day_avg,
                "temp_night_avg_c": temp_night_avg,
                "wind_avg_ms": wind_avg,
                "radiation_mj_m2": rad,
                "humidity_avg_pct": humidity_avg,
                "precipitation_mm": precipitation_mm,
                "et0_mm": et0_mm,
                "source_station": "Häädemeeste (T/tuul) + Pärnu (rad/RH/sade)",
                "radiation_station": (
                    "Pärnu" if key in radiation_daily
                    else ("Open-Meteo (varasem salvestatud prognoos)"
                          if "radiatsioon" in forecast_parts else "Pärnu")
                ),
    ...
    def backfill_haademeeste(self, start_day: date, end_day: date) -> Dict[str, Any]:
        """Ühekordne/batch backfill. Kasutab sama production refresh'i."""
    ...
    def refresh_forecast(self, start_day: date, days: int = 10) -> Dict[str, int]:
        ...
        hourly_temp_day: Dict[str, List[float]] = defaultdict(list)
        hourly_temp_night: Dict[str, List[float]] = defaultdict(list)
        for ts, value in zip(hourly.get("time", []), hourly.get("temperature_2m", [])):
            if value is None:
                continue
            key_day = str(ts)[:10]
            try:
                ...
                continue
            if 8 <= hour < 20:
                hourly_temp_day[key_day].append(float(value))
            else:
                hourly_temp_night[key_day].append(float(value))
        ...
        for i, key in enumerate(dates):
            ...
            temp_day_avg = mean(hourly_temp_day[key]) if hourly_temp_day.get(key) else None
            temp_night_avg = mean(hourly_temp_night[key]) if hourly_temp_night.get(key) else None
            et0_mm = self.calculate_et0_mm(
            ...
            if temp_day_avg is None or temp_night_avg is None: problems.append("päeva/öö temperatuuri prognoos puudub")
    ...
    def _haademeeste_hour_from_page(self, day: date, hour: int) -> Dict[str, Any]:
        """Loeb Ilmateenistuse tunniandmete HTML-ist Häädemeeste T + tuule.

        Tunniandmete tabeli Häädemeeste real:
        - veerg 1 = õhutemperatuur
        - veerg 5 = tuule suund
        - veerg 6 = tuule kiirus
        - veerg 7 = puhang
        Kontrollime väärtuste mõistlikkust enne kasutamist.
        """
        params = {
            "lang": "et",
            "filter[date]": day.strftime("%d.%m.%Y"),
            "filter[hour]": f"{int(hour)}:00",
        }
        response = self.http.get(HOURLY_PAGE, params=params, timeout=45)
        if not response.ok:
            raise WeatherError(
                f"Tunniandmete leht {response.status_code}: {response.text[:300]}"
            )
        ...
        for row in parser.rows:
            if not row:
                continue
            station = str(row[0]).strip().lower()
            if station in {"häädemeeste", "haademeeste"}:
                found_row = row
                break

        def _cell_num(index: int) -> float | None:
            if not found_row or len(found_row) <= index:
                return None
            raw = str(found_row[index]).strip().replace(",", ".")
            try:
                return float(raw)
            except (TypeError, ValueError):
                return None
        ...
        temp = _cell_num(1)
        wind = _cell_num(6)
        wind_dir = _cell_num(5)
        gust = _cell_num(7)

        if temp is not None and not -50.0 <= temp <= 50.0:
            temp = None
        if wind is not None and not 0.0 <= wind <= 50.0:
            wind = None
        if gust is not None and not 0.0 <= gust <= 80.0:
            gust = None
        if wind_dir is not None and not 0.0 <= wind_dir <= 360.0:
            wind_dir = None
    ...
    def _haademeeste_day_profile(self, day: date) -> Dict[str, Any]:
        """24 tunni Häädemeeste mõõdetud T/tuul ja päevakoond.

        Öö = 20:00–07:59 (12 tundi)
        Päev = 08:00–19:59 (12 tundi)
        """
        hourly: List[Dict[str, Any]] = [
            self._haademeeste_hour_from_page(day, hour)
        ...
        wind_values = [
            float(item["windspeed_ms"])
            for item in hourly
            if item.get("windspeed_ms") is not None
        ]
        day_values = [
            float(item["temperature_c"])
            for item in hourly
            if item.get("temperature_c") is not None and 8 <= int(item["hour"]) < 20
        ]
        night_values = [
            float(item["temperature_c"])
            for item in hourly
            if item.get("temperature_c") is not None
            and (int(item["hour"]) >= 20 or int(item["hour"]) < 8)
        ]

        return {
            "day": day.isoformat(),
            "hours_found": len(temp_values),
            "wind_hours_found": len(wind_values),
            "tmin_c": min(temp_values) if temp_values else None,
            "tmax_c": max(temp_values) if temp_values else None,
            "day_avg_c": mean(day_values) if day_values else None,
            "night_avg_c": mean(night_values) if night_values else None,
            "wind_avg_ms": mean(wind_values) if wind_values else None,
            "day_hours_found": len(day_values),
            "night_hours_found": len(night_values),
            "hourly": hourly,
        }

    def test_haademeeste_day_profile(self, day: date) -> Dict[str, Any]:
        """Diagnostika: sama Häädemeeste päevaprofiil, mida kasutab production."""
        return self._haademeeste_day_profile(day)
