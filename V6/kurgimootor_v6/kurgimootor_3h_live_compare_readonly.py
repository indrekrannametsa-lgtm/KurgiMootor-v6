...
"""
KurgiMootor LAB-147 · THREE-HORIZON ROUTER
===========================================
...
LAB-147 EI otsi uusi tunnuseid ega vali lead'i kaupa championit. See testib ühte enne
...
LAB-147 kogu replay number on arhitektuuri konsolideeriv test, MITTE sõltumatu holdout-tõend.
...
LAB_VERSION = "LAB-147-THREE-HORIZON-ROUTER-V1"
...
SCORE_NAMES = VARIANT_NAMES + [ROUTED_NAME]
MEMORY_WINDOWS = (10,)
...
# -----------------------------------------------------------------------------
# LIVE READ-ONLY VÕRDLUS
# -----------------------------------------------------------------------------

def _db_forecast_weather_map(rows: List[dict]) -> Dict[date, dict]:
    out: Dict[date, dict] = {}
    for r in rows:
        dd = _d(r.get("weather_date"))
        if dd is None:
            continue
        if str(r.get("data_kind") or "").strip().lower() != "forecast":
            continue
        if any(_f(r.get(c)) is None for c in MEASURED_REQUIRED):
            continue
        night = float(r["temp_night_avg_c"])
        ...
        out[dd] = {
            ...
            "rain": float(r["precipitation_mm"]),
            "et0": float(r["et0_mm"]),
            "source": "db_current_forecast",
        }
    return out


def _official_snapshot_batches(rows: List[dict]) -> Dict[date, List[dict]]:
    """Leia iga target päeva viimane NON-TEST ametlik snapshot-batch."""
    ...
    for r in rows:
        ...
        low = mv.lower()
        # Meie tänased LAB/3H test-snapshotid ei tohi olla võrdluse "production".
        if "test" in low or "lab" in low or "3h" in low:
            continue
    ...
    for td, batches in by_target.items():
        # Eelista tänase forecast_date'i; selle sees viimast generated_at'i.
        ranked = sorted(
            batches.items(),
...
def _live_3h_from_official_schedule(
    harvest_rows: List[dict],
    weather_rows: List[dict],
    official_by_target: Dict[date, List[dict]],
):
    events, actual_lookup = _prepare_events(harvest_rows)
    measured = _measured_weather_map(weather_rows)
    forecast = _db_forecast_weather_map(weather_rows)

    # Tänase hommiku puhul ei pruugi täispäeva measured veel olemas olla.
    # Kasuta tänase päeva forecasti live-feature'ites, aga ÄRA kasuta seda
    # historical treeningrekordite koostamisel.
    live_measured = dict(measured)
    if TODAY not in live_measured and TODAY in forecast:
        live_measured[TODAY] = forecast[TODAY]

    historical = _make_historical_records(events, actual_lookup, measured)

    synthetic_lookup = dict(actual_lookup)
    schedule_rows = []
    for lead in range(1, 10):
        td = TODAY + timedelta(days=lead)
        rows = official_by_target.get(td) or []
        rows = sorted(rows, key=lambda r: int(r.get("field_no") or 999))
        if not rows:
            continue
        for order, r in enumerate(rows, start=1):
            try:
                ...
                continue
            # Ainult schedule stub. _abc=nan on auditiväljaks; prognoos ei kasuta seda.
            synthetic_lookup[(td, field)] = {
                "_abc": np.nan,
                "_order": order,
                "_day": td,
                "_field": field,
            }
            schedule_rows.append((td, field, order))

    pred_rows = _predict_issue(
        issue_day=TODAY,
        all_events=events,
        actual_lookup=synthetic_lookup,
        measured=live_measured,
        historical_records=historical,
        forecast_map=forecast,
        run_label="LIVE-READONLY",
    )
    # _predict_issue sisaldab ka ainult issue+1..+9 stube; siiski filtreerime kaitseks.
    pred_rows = [
        r for r in pred_rows
        if 1 <= int(r.get("lead") or 0) <= 9
        and (r["target_day"], int(r["field"])) in {(d, f) for d, f, _o in schedule_rows}
    ]
    return pred_rows, {
        "historical_rows": len(historical),
        "measured_days": len(measured),
        "forecast_days": len(forecast),
        "schedule_fields": len(schedule_rows),
        "today_weather_source": (
            "measured" if TODAY in measured
            else "forecast" if TODAY in forecast
            else "missing"
        ),
    }
...
def main() -> None:
    ...
    st.set_page_config(page_title="KurgiMootor · 3H LIVE compare", layout="wide")
    st.error("🧪 3H LIVE VÕRDLUS · READ-ONLY · EI SALVESTA MIDAGI")
    st.title("app-128 vs 3H · sama live sisend, sama korjegraafik")
    st.caption(
        "Vasakul on viimane NON-TEST app-128 snapshot. Paremal arvutatakse LAB-147 3H "
        "otse praegustest korje- ja ilmaandmetest. 3H tulemust DB-sse ei kirjutata."
    )
    ...
    official_by_target = _official_snapshot_batches(saved)
    missing_schedule = [
        TODAY + timedelta(days=lead)
        for lead in range(1, 10)
        if not official_by_target.get(TODAY + timedelta(days=lead))
    ]
    if missing_schedule:
        st.error(
            "Ametlikust snapshotist puudub korjegraafik päevadele: "
            + ", ".join(d.strftime("%d.%m") for d in missing_schedule)
        )
        st.stop()

    try:
        pred_rows, diag = _live_3h_from_official_schedule(
            harvest_rows, weather_rows, official_by_target
        )
    except Exception as exc:
        st.exception(exc)
        st.stop()
    ...
    for lead in range(1, 10):
        td = TODAY + timedelta(days=lead)
        prod_rows = sorted(
            official_by_target.get(td) or [],
            key=lambda r: int(r.get("field_no") or 999),
        )
        if not prod_rows:
            continue
        ...
        for pr in prod_rows:
            ...
            pabc = float(pr["abc_forecast"])
            pxl = float(pr["xl_forecast"])
            ptotal = float(pr["total_forecast"])
            habc = float(h3[ROUTED_NAME])
            htotal = habc + pxl  # XL teadlikult sama kui app-128, et võrrelda ainult ABC mootorit.

            prod_abc += pabc
            prod_xl += pxl
            prod_total += ptotal
            h3_abc += habc
            routes.add(str(h3["route"]))

            field_rows.append({
                "Päev": td.strftime("%d.%m"),
                "Lead": lead,
                "Põld": field,
                "Route": str(h3["route"]),
                "app-128 ABC": pabc,
                "3H ABC": habc,
                "ABC vahe": habc - pabc,
                "ABC vahe %": 100.0 * (habc / pabc - 1.0) if pabc > 0 else np.nan,
                "app-128 XL": pxl,
                "app-128 kokku": ptotal,
                "3H kokku (sama XL)": htotal,
            })

        if not complete:
            continue

        h3_total = h3_abc + prod_xl
        day_rows.append({
            "Päev": td.strftime("%d.%m"),
            "Lead": lead,
            "Põllud": ", ".join(str(int(r["field_no"])) for r in prod_rows),
            "Route": " / ".join(sorted(routes)),
            "app-128 ABC": prod_abc,
            "3H ABC": h3_abc,
            "ABC vahe %": 100.0 * (h3_abc / prod_abc - 1.0) if prod_abc > 0 else np.nan,
            "app-128 kokku": prod_total,
            "3H kokku (sama XL)": h3_total,
            "Kokku vahe": h3_total - prod_total,
            "Kokku vahe %": 100.0 * (h3_total / prod_total - 1.0) if prod_total > 0 else np.nan,
        })
    ...
    if daily.empty:
        st.error("3H live võrdlusridu ei tekkinud.")
        st.json(diag)
        st.stop()

    max_abs_diff = float(np.nanmax(np.abs(daily["Kokku vahe %"].to_numpy(dtype=float))))
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Päevi võrreldud", int(len(daily)))
    c2.metric("3H train ridu", int(diag["historical_rows"]))
    c3.metric("Suurim päeva erinevus", f"{max_abs_diff:.1f}%")
    c4.metric("Tänase ilma allikas", str(diag["today_weather_source"]))

    if max_abs_diff < 0.2:
        st.warning(
            "3H ja app-128 annavad tõesti praktiliselt sama numbri. Siis pole tegu ainult kuvaveaga."
        )
    else:
        st.success(
            "3H erineb app-128-st. Eelmise testäpi 'täpselt sama' kuva tuli snapshoti/kuvatee segunemisest."
        )

    st.markdown("### Päevade võrdlus")
    st.dataframe(
        daily.style.format({
            "app-128 ABC": "{:.1f}",
            "3H ABC": "{:.1f}",
    ...
    with st.expander("Põldude kaupa", expanded=False):
        st.dataframe(
            fields.style.format({
                "app-128 ABC": "{:.2f}",
                "3H ABC": "{:.2f}",
    ...
    st.divider()
    st.caption(
        "READ-ONLY kontroll: ainult db.get_harvest_history, db.get_weather_rows ja "
        "db.get_yield_forecasts. Ei save_yield_forecasts, set_app_setting ega muid write-kutseid."
    )
