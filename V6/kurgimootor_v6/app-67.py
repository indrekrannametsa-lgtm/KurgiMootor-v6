...
APP_PATCH_VERSION = "wd-engine-parallel-9"  # eraldi paralleelmootor; production app-128 ei muutu
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor · WIND×DRY PARALLEEL v9", page_icon=APP_ICON, layout="wide")


# =============================================================================
# WIND×DRY TESTMOOTOR · READ ONLY
# =============================================================================
# Erinevalt varasemast võrdlus-/lõppkorrektsiooni testist EI kasutata siin A+B+C
# prognoosi otsustajana production app-128 salvestatud championit. A+B+C mootor on:
#   app-128 puhas weather-first baas + lukustatud LAB-154 tunnused
#   WIND×DRY HIGH L3–7 LEVEL+DELTA.
# Taimeindeks jääb productioniga samaks ja rakendub pärast uue mootori raw-ABC prognoosi.
NEW_WD_ENGINE_TEST = True
WD_MODEL_VERSION = "wd-engine-parallel-9"
PRODUCTION_MODEL_VERSION = "v6.5-v18-complete-daily-research-observation-snapshot"
WD_LOCKED_COLS = [
    "WD HIGH L3–7 päevad",
    "WD HIGH L3–7 jada",
...
if NEW_WD_ENGINE_TEST:
    def _test_noop(*args, **kwargs):
        return None

    # Testäpp ei muuda tootmise seadeid/korjeid/ilma ega käivita välisvõrgu ilmauuendust.
    # Ainus lubatud kirjutus on yield_forecasts, alati WD_MODEL_VERSION võtmega, et
    # paralleelmootori snapshotid säiliksid production app-128 ridadest eraldi.
    for _name in (
        "set_app_setting", "save_harvest",
        "save_harvest_plan", "delete_harvest", "upsert_weather",
        "save_weather_forecast_snapshot",
    ):
        if hasattr(db, _name):
            setattr(db, _name, _test_noop)
    WeatherService.auto_refresh_if_needed = lambda self, *args, **kwargs: None
    WeatherService.safe_refresh_all = lambda self, *args, **kwargs: {"test_mode": True}
    if hasattr(WeatherService, "_refresh_measured_incremental"):
        WeatherService._refresh_measured_incremental = lambda self, *args, **kwargs: {"test_mode": True}
...
def _wd_threshold(measured_map, before_day):
    ...
    for key, row in measured_map.items():
        ...
        if dd is None or dd >= before_day:
            continue
        # Lävi õpib ainult päriselt kontrollitud mõõdetud päevadest.
        if str(row.get("data_kind") or "").lower() == "measured" and not bool(row.get("checked")):
            continue
...
# =============================================================================
# V9: sõltumatu WIND×DRY prognoositee
# -----------------------------------------------------------------------------
# See tee EI sõltu productioni suure uurimisploki training_rows väravast.
# A+B+C mudel õpib ainult mõõdetud korjetest ja ilmast: intervall, hooajapäev,
# T, radiatsioon, sademed, RH, ET0, tuul, põllu identiteet + LAB-154 lukustatud
# WIND×DRY HIGH L3–7 LEVEL+DELTA. Varasemat saaki mudeli sisendina ei kasutata.
# XL ja C/B loetakse production app-128 snapshotist, sest need jäävad teadlikult
# productioni eraldi mudeliteks. Taimeindeks loetakse productioni hetkeseisust.
# =============================================================================
...
def _v9_weather_maps(rows):
    measured = {}
    live = {}
    for r in rows:
        key = str(r.get("weather_date") or "")[:10]
        if not key:
            continue
        kind = str(r.get("data_kind") or "").lower()
        if kind == "forecast":
            live[key] = r
    for r in rows:
        key = str(r.get("weather_date") or "")[:10]
...
def _v9_window_features(previous_day, target_day, weather_map):
    if not previous_day or target_day <= previous_day:
        return None
    rows = []
    ...
    while d <= target_day:
        r = weather_map.get(d.isoformat())
        if not r:
            return None
        vals = {k: _v9_num(r.get(k)) for k in required}
    ...
    if not rows:
        return None
    tmean = [(r["temp_min_c"] + r["temp_max_c"]) / 2.0 for r in rows]
    return [
        float((target_day - previous_day).days),
        float((target_day - SEASON_START).days),
        float(np.mean(tmean)),
        float(np.mean([r["temp_min_c"] for r in rows])),
        float(np.mean([r["temp_max_c"] for r in rows])),
        float(np.sum([r["radiation_mj_m2"] for r in rows])),
        float(np.sum([r["precipitation_mm"] for r in rows])),
        float(np.mean([r["humidity_avg_pct"] for r in rows])),
        float(np.sum([r["et0_mm"] for r in rows])),
        float(np.mean([r["wind_avg_ms"] for r in rows])),
    ]


def _v9_fit_ridge(rows, use_wd):
    if len(rows) < 24:
        return None
    Xn, fields, y = [], [], []
    for r in rows:
    ...
    Z = (Xn - mu) / sd
    F = np.zeros((len(fields), 14), dtype=float)
    for i, f in enumerate(fields):
        if 1 <= f <= 14:
            F[i, f - 1] = 1.0
    X = np.column_stack([np.ones(len(Z)), Z, F])
    # Sama mõte mis productionis: numeric ridge mõõdukas, põlluefekt tugevamalt kokku tõmmatud.
    reg = np.eye(X.shape[1], dtype=float) * 10.0
    reg[0, 0] = 0.0
    field_start = 1 + Z.shape[1]
    reg[field_start:, field_start:] = np.eye(14) * 80.0
    try:
        beta = np.linalg.solve(X.T @ X + reg, X.T @ y)
    except np.linalg.LinAlgError:
        beta = np.linalg.pinv(X.T @ X + reg) @ (X.T @ y)
    return {"mu": mu, "sd": sd, "beta": beta, "n": int(len(y)), "use_wd": bool(use_wd)}


def _v9_predict(model, field_no, base_vals, wd_vals):
    vals = list(base_vals) + (list(wd_vals) if model.get("use_wd") else [])
    ...
    if 1 <= int(field_no) <= 14:
        f[int(field_no) - 1] = 1.0
    row = np.r_[1.0, z, f]
    log_pred = float(row @ model["beta"])
    return float(np.exp(np.clip(log_pred, np.log(0.05), 6.0)))


def _v9_plant_indices():
    raw = db.get_app_setting(f"plant_index_state_{TODAY.year}", "")
    if not raw:
        return {f: 1.0 for f in range(1, 15)}
    try:
        obj = json.loads(raw)
        fields = obj.get("fields") or {}
        out = {}
        for f in range(1, 15):
            item = fields.get(str(f)) or {}
            out[f] = max(0.35, min(1.0, float(item.get("index", 1.0))))
        return out
    except Exception:
        return {f: 1.0 for f in range(1, 15)}


def _v9_build_training(harvest_rows, measured_map):
    by_field = {}
    for r in harvest_rows:
        try:
            d = date.fromisoformat(str(r.get("harvest_date"))[:10])
            f = int(r.get("field_no"))
            abc = float(r.get("a")) + float(r.get("b")) + float(r.get("c"))
        except Exception:
            continue
        # Tänast tegelikku saaki ei kasutata sama päeva pre-harvest prognoosi treenimiseks.
        if d >= TODAY:
            continue
        by_field.setdefault(f, []).append((d, abc))
    train = []
    last_actual_window = {}
    last_actual_day = {}
    for f, events in by_field.items():
        events = sorted(events, key=lambda x: x[0])
        prev_w = None
        for i in range(1, len(events)):
            prev_day, _ = events[i - 1]
            cur_day, abc = events[i]
            base = _v9_window_features(prev_day, cur_day, measured_map)
            cur_w = _wd_l3_7_window(cur_day, measured_map, measured_map)
            if base is None or cur_w is None:
                continue
            if prev_w is None:
                wd = [float(cur_w["high_days"]), float(cur_w["high_run"]), 0.0, 0.0]
            else:
                wd = [
                    float(cur_w["high_days"]), float(cur_w["high_run"]),
                    float(cur_w["high_days"] - prev_w["high_days"]),
                    float(cur_w["high_run"] - prev_w["high_run"]),
                ]
            train.append({"field": f, "base": base, "wd": wd, "y": math.log(max(abc, 0.05)), "day": cur_day})
            prev_w = cur_w
            last_actual_window[f] = cur_w
            last_actual_day[f] = cur_day
    return train, last_actual_window, last_actual_day


def _v9_prod_snapshots():
    try:
        rows = db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []
    except Exception:
        rows = []
    rows = [r for r in rows if str(r.get("model_version") or "") == PRODUCTION_MODEL_VERSION]
    by_target = {}
    for lead in range(0, 10):
        d = TODAY + timedelta(days=lead)
        batch = _latest_snapshot_batch_rows(rows, d)
        if batch:
            by_target[d] = sorted(batch, key=lambda r: int(r.get("field_no") or 999))
    return by_target


def _v9_render_forecast():
    st.subheader("Prognoos")
    st.caption("WIND×DRY PARALLEEL v9 · sõltumatu live-arvutus. A+B+C ei loe app-128 ABC prognoosi sisendina.")

    harvest_rows = db.get_harvest_history(limit=5000)
    weather_start = date(TODAY.year, 6, 1)
    weather_rows = db.get_weather_rows(weather_start, TODAY + timedelta(days=9))
    measured_map, live_map = _v9_weather_maps(weather_rows)
    train, last_window, last_day = _v9_build_training(harvest_rows, measured_map)
    base_model = _v9_fit_ridge(train, use_wd=False)
    wd_model = _v9_fit_ridge(train, use_wd=True)

    if base_model is None or wd_model is None:
        st.error(
            f"WIND×DRY mudel ei saanud piisavalt täielikke õppimisridu: {len(train)}. "
            "Vaja on vähemalt 24 rida. See on nüüd nähtav viga, mitte tühi Prognoosi leht."
        )
        return

    prod = _v9_prod_snapshots()
    if not prod:
        st.error("Production app-128 salvestatud 0–9 päeva plaani ei leitud; väljade ajakava ja XL/C/B puuduvad.")
        return

    plant = _v9_plant_indices()
    # Viimase tegeliku korje kuupäev igale põllule.
    latest_actual = {}
    for r in harvest_rows:
        try:
            d = date.fromisoformat(str(r.get("harvest_date"))[:10])
            ...
            continue
        if d < TODAY and (f not in latest_actual or d > latest_actual[f]):
            latest_actual[f] = d

    prev_target_day = dict(latest_actual)
    prev_w = dict(last_window)
    output_days = []
    payloads = []
    for target_day in sorted(prod.keys()):
        day_rows = []
        for pr in prod[target_day]:
            try:
                f = int(pr.get("field_no"))
            except Exception:
                continue
            pday = prev_target_day.get(f)
            if not pday:
                continue
            base = _v9_window_features(pday, target_day, live_map)
            cur_w = _wd_l3_7_window(target_day, live_map, measured_map)
            if base is None or cur_w is None:
                day_rows.append({
                    "Põld": f, "Taimeindeks": plant.get(f, 1.0), "Weather-first ABC": None,
                    "WD mõju": None, "A+B+C": None, "C/B": pr.get("cb_forecast"),
                    "XL": pr.get("xl_forecast"), "Kokku": None,
                    "Intervall": (target_day - pday).days,
                })
                continue
            pw = prev_w.get(f)
            wd = [
                float(cur_w["high_days"]), float(cur_w["high_run"]),
                float(cur_w["high_days"] - pw["high_days"]) if pw is not None else 0.0,
                float(cur_w["high_run"] - pw["high_run"]) if pw is not None else 0.0,
            ]
            raw_base = _v9_predict(base_model, f, base, wd)
            raw_wd = _v9_predict(wd_model, f, base, wd)
            if raw_base is None or raw_wd is None:
                continue
            idx = float(plant.get(f, 1.0))
            abc_base = raw_base * idx
            abc_wd = raw_wd * idx
            try:
                xl = float(pr.get("xl_forecast")) if pr.get("xl_forecast") is not None else None
            except Exception:
                xl = None
            try:
                cb = float(pr.get("cb_forecast")) if pr.get("cb_forecast") is not None else None
            except Exception:
                cb = None
            total = (abc_wd + xl) if xl is not None else None
            interval = (target_day - pday).days
            row = {
                "Põld": f,
                "Taimeindeks": idx,
                "Weather-first ABC": abc_base,
                "WD mõju": abc_wd - abc_base,
                "A+B+C": abc_wd,
                "C/B": cb,
                "XL": xl,
                "Kokku": total,
            ...
            payloads.append({
                ...
                "field_no": f,
                "lead_days": int((target_day - TODAY).days),
                "abc_forecast": float(abc_wd),
                "cb_forecast": cb,
                "xl_forecast": float(xl) if xl is not None else 0.0,
                "total_forecast": float(total) if total is not None else float(abc_wd),
                "interval_days": interval,
                "basis": (
                    f"WIND×DRY v9; weather_base_abc={abc_base:.6f}; wd_effect={abc_wd-abc_base:+.6f}; "
                    f"plant_index={idx:.4f}; wd_high_days={wd[0]:.0f}; wd_high_run={wd[1]:.0f}; "
                    f"d_high_days={wd[2]:+.0f}; d_high_run={wd[3]:+.0f}"
                ),
                "estimated_weather_days": "",
                "model_version": WD_MODEL_VERSION,
            })
            # Tulevase järgmise sama põllu DELTA kasutab ainult ilmaakent ja kuupäeva,
            # mitte eelmise prognoosi saaki.
            prev_target_day[f] = target_day
            prev_w[f] = cur_w
        output_days.append((target_day, day_rows))

    if payloads:
        try:
            db.save_yield_forecasts(payloads)
        except Exception as exc:
            st.warning(f"Prognoos arvutati, kuid paralleelsnapshoti salvestamine ebaõnnestus: {exc}")

    all_effects = [abs(float(r["WD mõju"])) for _, rows in output_days for r in rows if r.get("WD mõju") is not None]
    if all_effects:
        st.info(
            f"Õppimisridu {wd_model['n']} · WIND×DRY keskmine |mõju| {np.mean(all_effects):.2f} kasti · "
            f"suurim |mõju| {max(all_effects):.2f} kasti."
        )

    for target_day, rows in output_days:
        if not rows:
            continue
        vals = [r.get("Kokku") for r in rows]
        total = sum(float(v) for v in vals if v is not None) if vals and all(v is not None for v in vals) else None
        lead = (target_day - TODAY).days
        total_text = f"{_fmt(total)} kasti" if total is not None else "prognoos puudulik"
        st.markdown(f"### {_weekday_letter(target_day)} {_short_date(target_day)}  {total_text}")
        st.caption("täna" if lead == 0 else f"{lead} p ette")
        df = pd.DataFrame(rows)
        st.dataframe(df.style.format({
            "Taimeindeks": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
            "Weather-first ABC": lambda v: "—" if pd.isna(v) else f"{float(v):.1f}",
            "WD mõju": lambda v: "—" if pd.isna(v) else f"{float(v):+.2f}",
...
with _hdr_title:
    st.markdown("## KurgiMootor · WIND×DRY PARALLEEL v9")
    st.caption("PARALLEELTEST · uus A+B+C testmootor; production app-128 ja selle champion ei juhi seda prognoosi.")
...
page = st.radio(
    ...
    key="wd_parallel_v9_page",
)
if NEW_WD_ENGINE_TEST:
    st.info(
        "🧪 Testmootor: A+B+C = weather-first baas + WIND×DRY HIGH L3–7 LEVEL+DELTA; "
        "seejärel rakendub sama taimeindeks. XL ja C/B jäävad productioni eraldi mudeliteks. "
        "DB-sse salvestatakse ainult selle testmootori prognoosisnapshotid eraldi model_version võtmega."
    )

if page == "Prognoos" and NEW_WD_ENGINE_TEST:
    _v9_render_forecast()
    st.stop()
...
if page == "Täna":
    ...
    try:
        ...
        ] if NEW_WD_ENGINE_TEST else _home_saved_all
...
if NEW_WD_ENGINE_TEST:
    _run_light_research_requested = False
    _run_layered_manual_requested = False
    # Paralleelmootor arvutatakse live just Prognoos-vaates; mujal loeme tema salvestatud snapshotte.
    _forecast_refresh_due = bool(page == "Prognoos")
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            # -------------------------------------------------------------
            # TESTMOOTOR: lukustatud LAB-154 tunnused ajaloolisele õppimisandmestikule.
            ...
            if NEW_WD_ENGINE_TEST:
                ...
                champion_mae = None
                champion_stats = {"test_engine": True, "source": "LAB-154 locked"}
            layered_trace_rows = []
            ...
            if NEW_WD_ENGINE_TEST:
                # Lõplik turvalukk: vana salvestatud champion/Jäljeotsija ei saa A+B+C
                # testmootorit hilisema diagnostikakoodi kaudu tagasi üle kirjutada.
                champion_name = "WIND×DRY HIGH L3–7 · LEVEL+DELTA"
                champion_cols = list(WD_LOCKED_COLS)
                champion_stats = {"test_engine": True, "source": "LAB-154 locked"}
            ...
            # -------------------------------------------------------------------------
            st.markdown("##### 9 päeva saagiprognoos · WIND×DRY TESTMOOTOR")
            st.caption(
                f"A+B+C testmootor: {champion_name}. See EI kasuta production app-128 salvestatud ABC championit. "
                ...
                f"Jäljeotsija vana champion-valik on testis välja lülitatud. Diagnostilised vanad plokid ei juhi A+B+C prognoosi. "
            ...
            _plant_raw_key = f"plant_index_raw_forecasts_{TODAY.year}"
            _plant_state_key = f"plant_index_state_{TODAY.year}"
            _plant_raw = _load_json_setting(_plant_raw_key, {})
            if not isinstance(_plant_raw, dict):
                _plant_raw = {}

            # Esimesel kasutuselevõtul võta olemasolev app-126 snapshot algseks
            # puhtaks prognoosiks. Pärast taimeindeksi käivitumist tunneme uued
            # snapshotid basis-välja "taimeindeks=" järgi ära ega kasuta neid raw-ankruna.
            try:
                _plant_existing_forecasts_all = (
                    db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []
                )
                _plant_existing_forecasts = [
                    r for r in _plant_existing_forecasts_all
                    if str(r.get("model_version") or "") == PRODUCTION_MODEL_VERSION
                ]
            except db.DatabaseError:
                _plant_existing_forecasts = []
            _seed_day = PLANT_INDEX_START
            while _seed_day <= TODAY + timedelta(days=9):
                _seed_batch = _latest_snapshot_batch_rows(_plant_existing_forecasts, _seed_day)
                for _sr in _seed_batch:
                    _basis_seed = str(_sr.get("basis") or "")
                    if "taimeindeks=" in _basis_seed:
                        continue
                    try:
                        _sf = int(_sr.get("field_no"))
                        _sraw_total = float(_sr.get("total_forecast"))
                        _sraw_abc = float(_sr.get("abc_forecast"))
                        _sraw_xl = float(_sr.get("xl_forecast"))
                    except (TypeError, ValueError):
                        continue
                    _skey = f"{_seed_day.isoformat()}|{_sf}"
                    if _skey not in _plant_raw:
                        _plant_raw[_skey] = {
                            "target_date": _seed_day.isoformat(),
                            "field_no": _sf,
                            "raw_total": _sraw_total,
                            "raw_abc": _sraw_abc,
                            "raw_xl": _sraw_xl,
                            "captured_at": str(_sr.get("generated_at") or ""),
                            "source": "pre-index yield_forecasts",
                        }
                _seed_day += timedelta(days=1)
            ...
            def _predict_one(field_no, state, target_day):
                ...
                # Diagnostika: sama rea puhas weather-first baas ilma WIND×DRY lisatunnusteta.
                raw_weather_base_abc = _predict_full_abc_growth(
                    full_abc_base_model, field_no, base_values, [],
                )
                ...
                weather_base_abc = float(raw_weather_base_abc) * plant_index
                wd_effect_abc = float(abc_pred - weather_base_abc)
                ...
                return {
                    ...
                    "weather_base_abc": float(weather_base_abc), "wd_effect_abc": float(wd_effect_abc),
            ...
            for f in today_plan:
                ...
                today_forecast_rows.append({
                    ...
                    "A+B+C": pred["abc"],
                    "Weather-first ABC": pred.get("weather_base_abc"),
                    "WD mõju": pred.get("wd_effect_abc"),
                    "C/B": pred["cb"],
                    "XL": pred["xl"],
                    "Kokku": pred["total"],
            ...
            for offset in range(1, 10):
                ...
                for f in day_fields:
                    ...
                    day_rows.append({
                        ...
                        "A+B+C": result["abc"], "Weather-first ABC": result.get("weather_base_abc"),
                        "WD mõju": result.get("wd_effect_abc"), "C/B": result["cb"], "XL": result["xl"], "Kokku": result["total"],
            ...
            # Mudeliversioon on testmootoril eraldi võti, et paralleelne ajalugu ei seguneks app-128-ga.
            ...
            _wd_effects_visible = [
                float(r.get("WD mõju"))
                for _, _rows in forecast_detail_days for r in _rows
                if r.get("WD mõju") is not None
            ]
            if _wd_effects_visible:
                _wd_max_abs = max(abs(v) for v in _wd_effects_visible)
                _wd_mean_abs = float(np.mean([abs(v) for v in _wd_effects_visible]))
                st.info(
                    f"WIND×DRY on live-prognoosis sees · suurim |mõju| {_wd_max_abs:.2f} ABC kasti · "
                    f"keskmine |mõju| {_wd_mean_abs:.2f} kasti. "
                    "Veerg 'Weather-first ABC' on sama mudel ilma WIND×DRY tunnusteta."
                )
            ...
            if forecast_store_ok:
                ...
                st.caption("Iga päev jääb alles selle PARALLEELMOOTORI uus 1–9 päeva ette tehtud snapshot. Production app-128 ajalugu jääb eraldi model_version alla puutumata.")
                try:
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    try:
        ...
        _ro_saved = [
            ...
            if str(r.get("model_version") or "") == WD_MODEL_VERSION
        ] if NEW_WD_ENGINE_TEST else _ro_saved_all
    except db.DatabaseError:
        _ro_saved = []
