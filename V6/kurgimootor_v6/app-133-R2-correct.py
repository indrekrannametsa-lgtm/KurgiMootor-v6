...
# -----------------------------------------------------------------------------
# APP-133-R2 READ-ONLY DIAGNOSTIKA
# Kasutab tootmise lugemisandmeid ja app-128 olemasolevat cache-versiooni,
# kuid blokeerib kõik app.py otsesed kirjutused ning automaatse ilma uuenduse.
# -----------------------------------------------------------------------------
READ_ONLY_TEST = True
TEST_BUILD = "APP-133-R2"
...
APP_PATCH_VERSION = "app-120"  # TEST kasutab app-128 tootmise cache-signatuuri; uusi suuri mudeleid ei sunnita arvutama
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor TEST 133-R2", page_icon=APP_ICON, layout="wide")
st.error("🧪 APP-133-R2 AKTIIVNE · READ-ONLY · kiire režiimikihi diagnostika · tootmise andmeid ei muudeta")
...
with _hdr_title:
    st.markdown("## KurgiMootor V6.4 · TEST 133-R2")
    st.caption("🧪 READ-ONLY diagnostika · kiire režiimikiht · põhichampionit ega tootmisandmeid ei muudeta.")
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            if page == "Mootori tähelepanekud" and valid_pred.any():
                # -----------------------------------------------------------------
                # APP-133 diagnostika: kiire režiimikiht (väga odav CPU).
                # See EI muuda ametlikku prognoosi ega championit.
                # Idee: olemasoleva championi ühe sammu prognoosile lisatakse
                # latentne tasemekorrektor, mis õpib ainult juba saabunud
                # täielike korjepäevade ABC jääkidest. Silumise aste K valitakse
                # ajaliselt ausalt varasemate päevade põhjal.
                # -----------------------------------------------------------------
                try:
                    _rg = pd.DataFrame({
                        "Kuupäev": list(dates),
                        "Tegelik ABC": np.asarray(y_abc, dtype=float),
                        "Champion ABC": np.asarray(champion_pred, dtype=float),
                    })
                    _rg = _rg[
                        np.isfinite(_rg["Tegelik ABC"])
                        & np.isfinite(_rg["Champion ABC"])
                        & (_rg["Champion ABC"] > 0.05)
                    ].copy()
                    _rg_day = (
                        _rg.groupby("Kuupäev", as_index=False)
                        .agg({
                            "Tegelik ABC": "sum",
                            "Champion ABC": "sum",
                        })
                    )
                    _rg_counts = _rg.groupby("Kuupäev").size().rename("Ridu").reset_index()
                    _rg_day = _rg_day.merge(_rg_counts, on="Kuupäev", how="left")
                    _rg_day = _rg_day[_rg_day["Ridu"] >= 3].sort_values("Kuupäev").reset_index(drop=True)

                    if len(_rg_day) >= 6:
                        _gains = [0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.80]
                        _actual_d = _rg_day["Tegelik ABC"].to_numpy(dtype=float)
                        _base_d = _rg_day["Champion ABC"].to_numpy(dtype=float)
                        _sim = {}
                        for _k in _gains:
                            _state = 0.0
                            _preds_k = []
                            _factor_in = []
                            for _a, _b in zip(_actual_d, _base_d):
                                _fac = float(np.exp(_state))
                                _factor_in.append(_fac)
                                _preds_k.append(float(_b * _fac))
                                # Log-suhte klipp kaitseb ühe vigase päeva eest,
                                # kuid lubab siiski kiiret režiimimuutust.
                                _obs = float(np.clip(np.log((_a + 0.25) / (_b + 0.25)), -0.70, 0.55))
                                _state = (1.0 - _k) * _state + _k * _obs
                            _sim[_k] = {
                                "pred": np.asarray(_preds_k, dtype=float),
                                "factor": np.asarray(_factor_in, dtype=float),
                                "next_factor": float(np.exp(_state)),
                            }

                        # Meta-valik on ajaliselt aus: iga päeva K valitakse ainult
                        # sellele päevale eelnenud päevade vigade põhjal. Esimese
                        # nelja päeva jooksul kasutame kandidaatide mediaani.
                        _meta_pred = np.full(len(_rg_day), np.nan, dtype=float)
                        _meta_k = [None] * len(_rg_day)
                        for _i in range(len(_rg_day)):
                            if _i < 4:
                                _meta_pred[_i] = float(np.median([_sim[_k]["pred"][_i] for _k in _gains]))
                                continue
                            _past = slice(1, _i)  # esimene päev on filtri soojendus
                            _scores = {
                                _k: float(np.mean(np.abs(_sim[_k]["pred"][_past] - _actual_d[_past])))
                                for _k in _gains
                            }
                            _best_k_i = min(_scores, key=_scores.get)
                            _meta_k[_i] = _best_k_i
                            _meta_pred[_i] = float(_sim[_best_k_i]["pred"][_i])

                        _eval_mask = np.arange(len(_rg_day)) >= 4
                        _base_mae_rg = float(np.mean(np.abs(_base_d[_eval_mask] - _actual_d[_eval_mask])))
                        _meta_mae_rg = float(np.mean(np.abs(_meta_pred[_eval_mask] - _actual_d[_eval_mask])))

                        # Homseks valime K ainult kõigi tänaseks teada olevate
                        # ühe-sammu vigade põhjal. See annab ka otsese vastuse
                        # küsimusele "kui suur on silumise aste?"
                        _full_scores = {
                            _k: float(np.mean(np.abs(_sim[_k]["pred"][1:] - _actual_d[1:])))
                            for _k in _gains
                        }
                        _best_k_next = min(_full_scores, key=_full_scores.get)
                        _next_factor = float(_sim[_best_k_next]["next_factor"])

                        st.markdown("##### ⚡ Kiire režiimikiht — diagnostika")
                        st.caption(
                            "Odav state-space / EWMA tüüpi kalibreerija olemasoleva championi peal. "
                            "Ei treeni uut suurt mudelit, ei käivita Jäljeotsijat ega muuda ametlikku prognoosi. "
                            "ABC taseme muutus õpitakse ainult juba saabunud täielikest korjepäevadest."
                        )
                        _c1, _c2, _c3, _c4 = st.columns(4)
                        _c1.metric("Champion päev-MAE", f"{_base_mae_rg:.1f}")
                        _c2.metric("Režiimikiht MAE", f"{_meta_mae_rg:.1f}", delta=f"{_base_mae_rg - _meta_mae_rg:+.1f}")
                        _c3.metric("Homse K", f"{_best_k_next:.2f}")
                        _c4.metric("Homse ABC tegur", f"{_next_factor:.2f}")
                        st.caption(
                            f"K={_best_k_next:.2f} tähendab, et uus päev saab umbes {_best_k_next*100:.0f}% kaalu "
                            f"ja varasem seisund {(1.0-_best_k_next)*100:.0f}% kaalu. "
                            "See on mõõdetud kohanemiskiirus, mitte käsitsi valitud taimeindeks."
                        )

                        # Näita viimast kuni 8 päeva replay'd.
                        _rep = _rg_day.copy()
                        _rep["Režiim ABC"] = _meta_pred
                        _rep["Tegur"] = [
                            (float(_meta_pred[i] / _base_d[i]) if np.isfinite(_meta_pred[i]) and _base_d[i] > 0 else np.nan)
                            for i in range(len(_rep))
                        ]
                        _rep["K"] = [np.nan if v is None else float(v) for v in _meta_k]
                        _rep["Champ viga"] = _rep["Champion ABC"] - _rep["Tegelik ABC"]
                        _rep["Režiim viga"] = _rep["Režiim ABC"] - _rep["Tegelik ABC"]
                        _rep = _rep.tail(8).copy()
                        _rep["Kuupäev"] = _rep["Kuupäev"].map(lambda d: d.strftime("%d.%m") if hasattr(d, "strftime") else str(d))
                        st.dataframe(
                            _rep[["Kuupäev", "Tegelik ABC", "Champion ABC", "Režiim ABC", "Tegur", "K", "Champ viga", "Režiim viga"]]
                            .style.format({
                                "Tegelik ABC": "{:.1f}",
                                "Champion ABC": "{:.1f}",
                                "Režiim ABC": "{:.1f}",
                        ...
                        # Kui homne ametlik prognoos on selles arvutusringis olemas,
                        # näita ainult varinumbrit: ABC korrigeeritakse, XL jääb puutumata.
                        if 'forecast_days' in locals() and forecast_days:
                            _fd, _frows = forecast_days[0]
                            _off_abc = float(sum(float(r.get("A+B+C") or 0.0) for r in _frows))
                            _off_xl = float(sum(float(r.get("XL") or 0.0) for r in _frows))
                            _reg_abc = _off_abc * _next_factor
                            _reg_total = _reg_abc + _off_xl
                            st.info(
                                f"**Kiire variprognoos {_weekday_letter(_fd)} {_short_date(_fd)}:** "
                                f"ametlik ABC {_off_abc:.1f} + XL {_off_xl:.1f}; "
                                f"režiimikiht ABC {_reg_abc:.1f} + sama XL {_off_xl:.1f} = **{_reg_total:.1f} kasti**. "
                                "See number on ainult diagnostika."
                            )
                    else:
                        st.caption("Kiire režiimikihi jaoks on vaja vähemalt 6 täielikku walk-forward korjepäeva.")
                except Exception as _reg_exc:
                    st.caption(f"Kiire režiimikihi diagnostika jäi vahele: {_reg_exc}")
