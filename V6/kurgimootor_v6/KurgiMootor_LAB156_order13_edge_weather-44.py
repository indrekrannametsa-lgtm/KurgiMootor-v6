...
"""
KurgiMootor · edge_weather-45
=============================

CYCLE PRODUCTIVITY ENGINE · STRICT TIME-ORDERED REPLAY · READ ONLY
...
The earlier 5-harvest-day blocked leave-out is retained only as secondary context.
It is no longer the primary verdict.

Important interpretation
------------------------
- PERSIST is the practical benchmark: previous daily productivity × current interval.
- MODEL is the exact fixed ridge architecture from LAB44; no feature/window/lag search in this app.
- Time-ordered replay is the primary anti-hindsight check. Full-fit reconstruction is
  shown only to see whether the architecture can represent the season after training
  on all available data. It is NOT validation evidence.
...
def _time_ordered_replay(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """Strict expanding replay: each target date is predicted from earlier target dates only."""
    if df.empty:
        return df.copy(), pd.DataFrame()
    ...
    for dd in days:
        ...
        fit = _fit_ridge(train)
        ready = fit is not None and not test.empty
...
def main() -> None:
    st.set_page_config(page_title="KurgiMootor · strict time replay", layout="wide")
    st.title("KurgiMootor · frozen cycle engine · strict time-ordered replay")
    st.caption("edge_weather-45 · exact LAB44 architecture · expanding past-only fit · READ ONLY")

    st.info(
        "See LAB ei otsi enam uut mudelit. LAB44 tsüklimootor on külmutatud. Iga ajaloolise korjepäeva T ennustus "
        "fititakse ainult nende tsüklite pealt, mille target_date < T. Sama päeva ega tuleviku saaki treeningus ei ole. "
        "PERSIST jääb samaks: eelmise sama põllu päevatootlikkus × uus intervall."
    )
    ...
    if replay.empty or "model" not in replay.columns:
        st.error("Strict time replay ei jõudnud minimaalse treeningmahuni.")
        st.dataframe(replay_audit, use_container_width=True, hide_index=True)
        return
    replay_daily = _daily(replay)

    # Secondary context only: the preceding blocked discovery test, unchanged.
    blocked, blocks = _blocked_cv(cycles)
    blocked_daily = _daily(blocked) if not blocked.empty else pd.DataFrame()
    ...
    replay_pre_aug = replay_daily[replay_daily["target_date"] < JULY_CUTOFF].copy()
    replay_late = replay_daily[replay_daily["target_date"] >= date(2026, 8, 25)].copy()

    level, title, text_verdict = _replay_verdict(replay_daily, replay_aug, replay_pre_aug)
    st.header("1. Otsus · kas külmutatud tsüklimootor töötab ainult minevikust õppides?")
    if level == "success":
        st.success(f"✅ {title}\n\n{text_verdict}")
    elif level == "warning":
        st.warning(f"🟡 {title}\n\n{text_verdict}")
    ...
    first_day = min(replay_daily["target_date"]) if not replay_daily.empty else None
    last_day = max(replay_daily["target_date"]) if not replay_daily.empty else None
    st.caption(
        f"Replay muutub esimest korda valmis siis, kui varasemaid tsükliridu on vähemalt {MIN_TRAIN_ROWS}. "
        f"Skooritud periood: {first_day} … {last_day}. Varasemad päevad on warm-up, mitte nullprognoosid."
    )

    st.header("2. Strict expanding replay · kogu saadaval olev minevikupõhine periood")
    p_all = _day_metrics(replay_daily, "persist")
    m_all = _day_metrics(replay_daily, "model")
    summary = pd.DataFrame([
        _summary_row("A · PERSIST = eelmine päevatootlikkus × intervall", replay_daily, "persist"),
        _summary_row("B · FROZEN CYCLE MODEL · past-only", replay_daily, "model"),
    ])
    ...
    chart = replay_daily.set_index("target_date")[["actual", "persist", "model"]].rename(columns={
        "actual": "Tegelik ABC",
        "persist": "PERSIST",
        "model": "CYCLE MODEL · TIME REPLAY",
    })
    st.line_chart(chart)

    show = replay_daily[["target_date", "actual", "persist", "model"]].copy()
    show.columns = ["Päev", "Tegelik ABC", "PERSIST", "CYCLE MODEL"]
    st.dataframe(_fmt_table(show), use_container_width=True, hide_index=True)

    st.header("3. Puhas terve taime laine · 17.–24.08 · strict replay")
    if replay_aug.empty:
        st.warning("17.–24.08 ei ole strict replay jaoks piisavalt päevi.")
    else:
        pa = _day_metrics(replay_aug, "persist")
        ma = _day_metrics(replay_aug, "model")
        aug_t = pd.DataFrame([
            _summary_row("PERSIST", replay_aug, "persist"),
            {**_summary_row("CYCLE MODEL · TIME REPLAY", replay_aug, "model"), "Parandus %": _improve(pa["mae"], ma["mae"])},
        ])
        st.dataframe(_fmt_table(aug_t), use_container_width=True, hide_index=True)
        aug_show = replay_aug[["target_date", "actual", "persist", "model"]].copy()
        aug_show.columns = ["Päev", "Tegelik ABC", "PERSIST", "CYCLE MODEL"]
        st.dataframe(_fmt_table(aug_show), use_container_width=True, hide_index=True)

    st.header("4. Enne augustit · mida strict replay üldse jõuab kontrollida?")
    if replay_pre_aug.empty:
        st.warning(
            "Külmutatud ridge vajab enne esimest prognoosi vähemalt 24 varasemat tsüklirida; "
            "seetõttu ei ole ükski enne 01.08 päev strict expanding replay's skooritav. "
            "Seda ei tohi täita hilisema infoga."
        )
    else:
        pp = _day_metrics(replay_pre_aug, "persist")
        mp = _day_metrics(replay_pre_aug, "model")
        pre_t = pd.DataFrame([
            _summary_row("PERSIST", replay_pre_aug, "persist"),
            {**_summary_row("CYCLE MODEL · TIME REPLAY", replay_pre_aug, "model"), "Parandus %": _improve(pp["mae"], mp["mae"])},
        ])
        st.dataframe(_fmt_table(pre_t), use_container_width=True, hide_index=True)
        pre_show = replay_pre_aug[["target_date", "actual", "persist", "model"]].copy()
        pre_show.columns = ["Päev", "Tegelik ABC", "PERSIST", "CYCLE MODEL"]
        st.dataframe(_fmt_table(pre_show), use_container_width=True, hide_index=True)

    with st.expander("5. Warm-up / leakage audit · kas fitis on ainult varasemad targetid?", expanded=False):
        ...
        if not audit.empty:
            ...
            if bad.empty:
                st.success("Leakage lock OK: igal skooritud päeval on max train target rangelt varasem kui ennustatav päev; future rows fitis = 0.")
            else:
                st.error("LEAKAGE LOCK FAIL: vähemalt ühel päeval ei ole treening rangelt minevikus.")

    with st.expander("6. Eelmine blocked discovery · ainult kontekst, mitte uus verdict", expanded=False):
        if blocked_daily.empty:
            st.write("Blocked konteksti ei saanud arvutada.")
        else:
            pb = _day_metrics(blocked_daily, "persist")
            mb = _day_metrics(blocked_daily, "model")
            bt = pd.DataFrame([
                _summary_row("PERSIST · blocked", blocked_daily, "persist"),
                {**_summary_row("CYCLE MODEL · blocked", blocked_daily, "model"), "Parandus %": _improve(pb["mae"], mb["mae"])},
            ])
            st.dataframe(_fmt_table(bt), use_container_width=True, hide_index=True)
            st.dataframe(_fmt_table(blocks), use_container_width=True, hide_index=True)

    st.header("7. Mida täishooaja fit õppis? · diagnostika, mitte replay skoor")
    st.caption(
        "Koefitsiendid all on fititud kogu hooajale ainult arhitektuuri tõlgendamiseks. Strict replay ülal refitib sama "
        "mudeli iga päeva jaoks ainult varasematele ridadele."
    )
    coefs = _coef_table(full_fit)
    st.dataframe(_fmt_table(coefs, 3), use_container_width=True, hide_index=True)

    with st.expander("Täishooaja reconstruction · ainult kuju", expanded=False):
    ...
    with st.expander("25.08+ vananeva taime saba · strict replay info", expanded=False):
        if replay_late.empty:
            st.write("Pole strict replay ridu.")
        else:
            st.dataframe(_fmt_table(replay_late), use_container_width=True, hide_index=True)

    st.header("8. Lukud")
    st.code(
        "ARCHITECTURE = exact LAB44 cycle engine; no feature/lambda/window changes\n"
        ...
        f"ridge lambda = {RIDGE_LAMBDA} fixed; minimum train rows = {MIN_TRAIN_ROWS}\n"
        "STRICT REPLAY FIT = target_date < predicted target_date only\n"
        "same-day target rows = excluded; all future target rows = excluded\n"
        "blocked validation retained only as secondary context\n"
        "no PI / FAST / TOMO / RAD derivative / capacity gate / cap / lag search / window search\n"
