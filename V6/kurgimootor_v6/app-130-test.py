...
"""
KurgiMootor LAB-142 · VERIFY + DYNAMIC STATE
===============================================
...
LAB-142 ei otsi enam uut mälupikkust ega uut tunnust. Testis on üks fikseeritud
...
LAB_VERSION = "LAB-142-VERIFY-DYNAMIC-V1"
...
def _predict_models(models: dict, rec: dict, lead: int) -> Dict[str, float]:
    ...
    # Exact LAB-140 reference and the refactored implementation must be identical.
    ...
    out["MEM10-CAL"] = _predict_or_own2(models.get("MEM10-CAL"), rec, MEM10_FEATURES)
    ...
    out["DYNAMIC"] = _predict_or_own2(models.get("DYNAMIC"), dyn_rec, MEM10_BLOCK_FEATURES)
    out["LAB140-REF"] = float(ref140)  # audit-only; scoring table does not include it
    return out
...
def _self_test() -> None:
    ...
    # Regression test LAB-141 bugile: kui treeningridu on liiga vähe, learned mudel
    # peab kukkuma tagasi OWN2 peale, mitte muutuma NaN-iks, mida groupby.sum loeks 0-na.
    tiny_models = _fit_issue_models(train[:2], cutoff)
    tiny_preds = _predict_models(tiny_models, rec0, 1)
    own = _own2_pred(rec0)
    assert abs(tiny_preds["MEM10-ZERO"] - own) < 1e-12
    assert abs(tiny_preds["MEM10-CAL"] - own) < 1e-12
    assert abs(tiny_preds["FLAT-MEM10+BLOCK1"] - own) < 1e-12
    assert abs(tiny_preds["DYNAMIC"] - own) < 1e-12
...
def main() -> None:
    ...
    st.set_page_config(page_title="KurgiMootor LAB-142", layout="wide")
    st.error("🧪 LAB-142 · VERIFY + DYNAMIC STATE · READ-ONLY · 22.07 → +1…+9")
    st.title("KurgiMootor · OWN2 + MEM10 + hääbuv BLOCK1")
    st.caption(
        "Esmalt tõestab test, et LAB-140 MEM10 referents on bititäpselt taastatud. Seejärel üks fikseeritud kandidaat: OWN2 + MEM10 + kiire BLOCK1, "
...
- Täpseid õitsemise alguskuupäevi DB-s pole; LAB ei leiuta neid tagantjärele.
    ...
    if st.button("Värskenda DB andmed", type="secondary"):
        _load_db_data.clear()
        st.session_state.pop("lab142_results", None)
        st.rerun()
    ...
    c5.metric("Input fingerprint", _input_fingerprint(events, measured))

    run_requested = st.button("▶ VERIFY LAB-140 → jooksuta DYNAMIC replay", type="primary")
    if not run_requested and "lab142_results" not in st.session_state:
    ...
    if run_requested:
        ...
        runs: Dict[date, str] = {}
        progress = st.progress(0.0, text="LAB-140 parity + ECMWF + DYNAMIC replay…")
        ...
        st.session_state["lab142_results"] = {
            "field": field_df, "daily": daily, "errors": errors, "runs": runs,
            "max_parity": max_parity, "fingerprint": _input_fingerprint(events, measured),
        }

    result = st.session_state.get("lab142_results")
    if not result:
        st.stop()
