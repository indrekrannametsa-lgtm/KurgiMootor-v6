...
"""
KurgiMootor LAB-143 · LEARNED BLOCK STATE
============================================
...
LAB_VERSION = "LAB-143-LEARNED-BLOCK-V1"
...
MODEL_NAMES = ["OWN2", "MEM10-ZERO", "FLAT-MEM10+BLOCK1", "LEARNED-BLOCK"]
PARITY_TOL = 1e-9
MEMORY_WINDOWS = (10,)
...
def main() -> None:
    ...
    st.set_page_config(page_title="KurgiMootor LAB-143", layout="wide")
    st.error("🧪 LAB-143 · LEARNED BLOCK STATE · READ-ONLY · 22.07 → +1…+9")
    st.title("KurgiMootor · OWN2 + MEM10 + minevikust õpitud BLOCK1")
    st.caption(
        "Üks uus kandidaat: MEM10 jääb puutumatuks ja BLOCK1 saab ühe scalar beta, mis õpitakse ainult varasemate "
        "range walk-forward MEM10 jääkide pealt. Ei lead-decay'd, intercepti ega lead'i järgi championivalikut."
    )
...
- Täpseid õitsemise alguskuupäevi DB-s pole; LAB ei leiuta neid tagantjärele.
    ...
    if st.button("Värskenda DB andmed", type="secondary"):
        _load_db_data.clear()
        st.session_state.pop("lab143_results", None)
        st.rerun()
    ...
    run_requested = st.button("▶ VERIFY LAB-140 → jooksuta LEARNED-BLOCK replay", type="primary")
    if not run_requested and "lab143_results" not in st.session_state:
    ...
    if run_requested:
        ...
        runs: Dict[date, str] = {}
        progress = st.progress(0.0, text="LAB-140 parity + ECMWF + LEARNED-BLOCK replay…")
    ...
    t1.metric("MEM10 baas · MAPE", f"{float(mem10['MAPE %']):.1f}%")
    t2.metric("LEARNED-BLOCK · MAPE", f"{float(learned['MAPE %']):.1f}%", f"{float(learned['MAPE %']-mem10['MAPE %']):+.1f} pp vs MEM10")
    t3.metric("LEARNED-BLOCK · +1p", "—" if learned1 is None else f"{float(learned1['MAPE %']):.1f}%")
    t4.metric("FLAT referents", f"{float(flat['MAPE %']):.1f}%")
    t5.metric("Viimane õpitud β", "—" if not math.isfinite(latest_beta) else f"{latest_beta:+.3f}")

    st.markdown("### 1. Üks kandidaat · kogu replay")
    st.dataframe(
    ...
    st.markdown("### 3. Lead 1–9 · sama õpitud β kõigil lead'idel")
    ...
    st.markdown("### 4. β audit · kuidas BLOCK1 kaal minevikust õpib")
    beta_audit = (
    ...
    st.markdown("### 5. Viimased skooritavad sihtpäevad")
    ...
    st.divider()
    st.caption(
        f"{LAB_VERSION} · MEM10 LAB-140 parity · BLOCK beta: strict residual WF · ridge α={RIDGE_ALPHA:g} · "
