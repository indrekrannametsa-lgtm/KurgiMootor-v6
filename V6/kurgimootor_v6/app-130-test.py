...
"""
KurgiMootor LAB-139 · OWN2 + KOMPONENTIDE BENCHMARK
====================================================

Eesmärk
-------
Võtame LAB-138 võitja OWN2 baasiks ja lisame komponendid ÜHEKAUPA samas
22.07.2026 -> +1...+9 ajamasinas. Katse ei otsi replay tulemustest parimat feature-set'i;
kõik variandid on enne jooksu fikseeritud ja kuvatakse kõrvuti.

Fikseeritud variandid (kumulatiivsed)
------------------------------------
OWN2
    Sama põllu kuni 2 viimase usaldusväärse ABC/kasvupäev rate'i mediaan.
+GROWTH
    OWN2 + learned correction täpse kasvupäeva järgi.
+GDD
    eelmine + kasvuperioodi GDD10.
+RAD
    eelmine + kasvuperioodi globaalradiatsioon.
+ET0
    eelmine + kasvuperioodi ET0.
+MEM14
    eelmine + 14 päeva GDD/radiatsiooni/ET0 mälu.
+BLOCK1
    eelmine + ainult väga värske (<=2 p) viimase täieliku korjepäeva BLOCK1 signaal.

Aususe reeglid
--------------
- Issue-päev D näeb ainult D lõpuks teada olnud korjeid ja mõõdetud ilma.
- D+1...D+9 ilm tuleb D arhiveeritud ECMWF IFS HRES jooksust.
- Tuleviku tegelikust korjereast kasutatakse ainult kuupäeva/põldu/järjekorda.
- Sama issue +1...+9 sees ei söödetata prognoositud saaki tagasi state'iks.
- OWN2 state külmutatakse issue-päeval; tulevase sama põllu järgmise korje kasvuaeg
  kasutab vaid simuleeritud korjesündmuse ajamärki, mitte prognoositud saaki.
- Historical transition mudelid treenitakse ainult target_day <= issue_day ridadel.
...
Mõte
----
Kõigi learned variantide target on log(actual_rate / OWN2_rate). Ridge on sama,
fikseeritud regulariseerimisega. Seega näeme, kas iga järgmine komponent parandab
OWN2 baasi ausas replay's või mitte.
"""
...
LAB_VERSION = "LAB-139-OWN2-COMPONENTS-V1"
...
# Kumulatiivne, enne testi lukustatud feature ladder.
VARIANTS: Dict[str, List[str]] = {
    "OWN2": [],
    "+GROWTH": ["growth_days"],
    "+GDD": ["growth_days", "gw_gdd10"],
    "+RAD": ["growth_days", "gw_gdd10", "gw_rad"],
    "+ET0": ["growth_days", "gw_gdd10", "gw_rad", "gw_et0"],
    "+MEM14": [
        "growth_days", "gw_gdd10", "gw_rad", "gw_et0",
        "w14_gdd10", "w14_rad", "w14_et0",
    ],
    "+BLOCK1": [
        "growth_days", "gw_gdd10", "gw_rad", "gw_et0",
        "w14_gdd10", "w14_rad", "w14_et0", "block1_signal",
    ],
}
...
def _weather_features(
    ...
    growth, prev = _growth_for_target(schedule_events, field, target_day, target_order)
    if prev is not None:
    ...
    mem_rows = _range_weather(wmap, target_day - timedelta(days=14), target_day - timedelta(days=1))
    mem = _agg_weather(mem_rows) if mem_rows is not None else None
    if mem is None:
        return None
    ...
    return {
        ...
        "gw_rad": float(gw["rad"]),
        "gw_et0": float(gw["et0"]),
        "w14_gdd10": float(mem["gdd10"]),
        "w14_rad": float(mem["rad"]),
        "w14_et0": float(mem["et0"]),
    }
...
def _build_record(
    *,
    ...
    wf = _weather_features(schedule_events, field, target_day, target_order, wmap)
    if wf is None:
        return None
...
def _summary_table(daily: pd.DataFrame) -> pd.DataFrame:
    ...
    # Lisa juurde muutus eelmise redeliastme suhtes, ilma sortimata: komponentide mõju peab jääma nähtavaks.
    df["Δ MAPE eelmisest"] = df["MAPE %"].diff()
    return df
...
def _self_test() -> None:
    ...
    for i in range(60):
        ...
        measured[dd] = {
            ...
            "et0": 3.0 + 0.005*i, "source": "measured",
        }

    rows = []
    # 6 korduvat 3-põllu päeva; rate muutub aeglaselt koos ilmaga.
    days = [date(2026,7,5), date(2026,7,10), date(2026,7,15), date(2026,7,20), date(2026,7,25), date(2026,7,30)]
    for j, dd in enumerate(days):
        for order, f in enumerate((1, 2, 3), start=1):
            abc = 7.0 + 0.4*j + 0.15*f
            rows.append({
    ...
    own = _own2_snapshot(events, 1, date(2026,7,26))
    assert own is not None and own[0] > 0
    sig, _ = _block1_signal(events, date(2026,7,26), own[0])
    ...
    rec0 = hist[-1]
    for name, feats in VARIANTS.items():
        model = _fit_model(hist[:-1], rec0["target_day"], feats)
        pred = _predict_variant(model, rec0, feats)
        assert pred is not None and math.isfinite(pred) and pred >= 0

    # E2E issue: tuleviku actualid lookupis on ainult skooriks; prediction recordisse neid ei anta.
    issue = date(2026,7,20)
...
def main() -> None:
    ...
    st.set_page_config(page_title="KurgiMootor LAB-139", layout="wide")
    st.error("🧪 LAB-139 · OWN2 + KOMPONENDID · READ-ONLY · 22.07 → +1…+9")
    st.title("KurgiMootor · milline komponent päriselt aitab?")
    st.caption(
        "OWN2 on fikseeritud baas. Lisame sama ridge-raamistiku sees kasvuaega, GDD-d, radiatsiooni, ET0-t, "
        "14 päeva ilmamälu ja lõpuks värske BLOCK1 signaali ühekaupa. Tuleviku ilm = arhiveeritud ECMWF issue-jooks."
    )
...
- **+GROWTH:** learned correction ainult täpse kasvupäeva järgi.
- **+GDD:** lisab kasvuperioodi GDD10.
- **+RAD:** lisab kasvuperioodi radiatsiooni.
- **+ET0:** lisab kasvuperioodi ET0.
- **+MEM14:** lisab 14 päeva GDD/radiatsiooni/ET0 mälu.
- **+BLOCK1:** lisab viimase täieliku korjepäeva suhte OWN2 state'i vastu ainult siis, kui see on <=2 päeva värske.
- Kõik variandid on **kumulatiivsed** ja fikseeritud enne replay'd; katse ise neid ümber ei vali.
...
- Skoor: ainult täielikud 3-põllu päevad; ainult **A+B+C**.
    ...
    if st.button("Värskenda DB andmed", type="secondary"):
        _load_db_data.clear()
        st.session_state.pop("lab139_results", None)
        st.rerun()
    ...
    run_requested = st.button("▶ Jooksuta komponentide benchmark", type="primary")
    if not run_requested and "lab139_results" not in st.session_state:
    ...
    if run_requested:
        ...
        runs: Dict[date, str] = {}
        progress = st.progress(0.0, text="ECMWF issue-jooksud + komponentide replay…")
        ...
        st.session_state["lab139_results"] = {"field": field_df, "daily": daily, "errors": errors, "runs": runs}

    result = st.session_state.get("lab139_results")
    if not result:
        st.stop()
    ...
    if p1_best is not None:
        top3.metric("+1p parim", f"{float(p1_best['MAPE %']):.1f}%", str(p1_best["Variant"]))
    else:
        top3.metric("+1p parim", "—")
    top4.metric("Keskmine OWN fallback", f"{daily['fallback_pct'].mean():.0f}%")

    st.markdown("### 1. Komponentide mõju · kogu replay")
    st.caption("Negatiivne Δ MAPE tähendab, et järgmise komponendi lisamine parandas eelmist astet.")
    st.dataframe(
        overall.style.format({
            "MAPE %": "{:.1f}%", "±20% sees %": "{:.0f}%", "Bias %": "{:+.1f}%",
            "MAE ABC": "{:.1f}", "Δ MAPE eelmisest": lambda x: "—" if pd.isna(x) else f"{x:+.1f} pp",
        }),
    ...
    st.markdown("### 3. Lead 1–9 · kõik komponendiastmed")
    ...
    st.caption(
        ...
        f"small-sample prior={SMALL_SAMPLE_PRIOR_ROWS:g} · BLOCK1 fresh <= {BLOCK_FRESH_DAYS} p · READ ONLY"
    )
