...
# -----------------------------------------------------------------------------
# APP-130 READ-ONLY TEST MODE
# Katse: põllupõhine tootmisvanus + kiire, kuid usalduskaaluga varjatud taime seisund.
# Tootmise Supabase'i LOETAKSE, kuid sinna EI KIRJUTATA.
# -----------------------------------------------------------------------------
READ_ONLY_TEST = True
...
if READ_ONLY_TEST:
    ...
    for _name in (
        ...
        "save_harvest_plan", "delete_harvest",
    ):
        if hasattr(db, _name):
...
APP_PATCH_VERSION = "app-130-test"  # sisemine failiversioon; DB forecast MODEL_VERSION jääb teadlikult samaks
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor V6.4 TEST 130", page_icon=APP_ICON, layout="wide")
st.warning("🧪 APP-130 READ-ONLY TEST — tootmise andmeid ei muudeta. Katse lisab põllupõhise tootmisvanuse ja varjatud taime seisundi kihi; A+B+C põhimudel ise jääb app-128 omaks.")
...
_auto_research_dirty = False if READ_ONLY_TEST else _model_is_dirty()
_research_dirty_reason = "" if READ_ONLY_TEST else (db.get_app_setting("model_dirty_reason", "") if _auto_research_dirty else "")
_new_complete_harvest_trigger = False if READ_ONLY_TEST else bool(
...
_forecast_refresh_due = True if READ_ONLY_TEST else _forecast_is_dirty()
_forecast_dirty_reason = "app-130 read-only test" if READ_ONLY_TEST else (db.get_app_setting("forecast_dirty_reason", "") if _forecast_refresh_due else "")
...
_run_layered_manual_requested = False if READ_ONLY_TEST else bool(_layered_run_now)
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            # -----------------------------------------------------------------
            # APP-130 TEST: tootmisvanus + varjatud taime seisund
            # -----------------------------------------------------------------
            # Põhimõte:
            #   1) app-128 weather-first A+B+C jääb puutumata;
            #   2) tootmisvanuse kiht õpitakse app-128 ausate walk-forward jääkide pealt;
            #   3) varjatud taime seisund õpitakse ainult A+B+C-st, mitte XL-ist;
            #   4) indeksil on enne 15.08 juba "prior" viimaste ausate jääkide põhjal,
            #      seega üks korje ei pea nullist kogu seisundit otsustama;
            #   5) üks uus korje mõjutab indeksit kohe, kuid mõju kaal sõltub mudeli
            #      ajaloolisest jääkmürast (suurem ebakindlus => väiksem ülereaktsioon).
            PLANT_INDEX_START = date(TODAY.year, 8, 15)
            PLANT_INDEX_MIN = 0.55
            AGE_ANCHOR_START = date(TODAY.year, 6, 24)

            # Kasutaja teada olev 2026 põldude vanusejärjestus:
            # 1–2 alustasid ~24.06; 8–12 umbes nädal hiljem; 13–14 veel hiljem.
            # Testis kasutame ainult suhtelist vanuse OFFSET-i. Tugevuse õpib ajalugu.
            # Hiljem saab need offsetid asendada päris istutus-/esimese korje kuupäevadega.
            def _production_age_offset_days(field_no):
                f = int(field_no)
                if f <= 2:
                    return 0.0
                if f <= 8:
            ...
            def _production_age_days(field_no, day_value):
                return max(
                    0.0,
                    float((day_value - AGE_ANCHOR_START).days) - _production_age_offset_days(field_no),
                )

            # --- Vanusekõvera õppimine ausate A+B+C walk-forward jääkide pealt ---
            _age_hist_pred = np.asarray(champion_pred, dtype=float)
            _age_hist_actual = np.asarray(y_abc, dtype=float)
            _age_hist_fields = np.asarray(fields, dtype=int)
            _age_hist_dates = np.asarray(dates, dtype=object)
            _age_hist_days = np.array([
                _production_age_days(int(f), d)
                for f, d in zip(_age_hist_fields, _age_hist_dates)
            ], dtype=float)

            def _fit_age_slope(cutoff_day, threshold):
                mask = (
                    np.isfinite(_age_hist_pred)
                    & np.isfinite(_age_hist_actual)
                    & (_age_hist_pred > 0.10)
                    & (_age_hist_actual > 0.05)
                    & np.array([d < cutoff_day for d in _age_hist_dates], dtype=bool)
                )
                idx = np.where(mask)[0]
                if len(idx) < 18:
                    return 0.0
                late = np.maximum(0.0, _age_hist_days[idx] - float(threshold))
                if float(np.max(late)) < 1.0:
                    return 0.0
                log_resid = np.log(_age_hist_actual[idx] / _age_hist_pred[idx])

                # Ühine hilise vanuse kalle + põllu interceptid. Põllu intercepte ei
                # rakendata prognoosis; need aitavad vanusekallet eraldada püsivast maavahe efektist.
                X = np.zeros((len(idx), 2 + 14), dtype=float)
                X[:, 0] = 1.0
                X[:, 1] = late
                for r, ii in enumerate(idx):
                    f = int(_age_hist_fields[ii])
                    if 1 <= f <= 14:
                        X[r, 1 + f] = 1.0
                penalty = np.zeros(X.shape[1], dtype=float)
                penalty[1] = 1.0          # vanusekalle: kerge stabiliseerimine
                penalty[2:] = 20.0        # põlluefekt: tugev partial-pooling
                try:
                    beta = np.linalg.solve(X.T @ X + np.diag(penalty), X.T @ log_resid)
                    slope = float(beta[1])
                except np.linalg.LinAlgError:
                    return 0.0
                # Füsioloogiline vananemiskiht ei tohi hilises faasis saaki tõsta.
                return min(0.0, slope)

            # Lävi valitakse ainult ENNE taimeindeksi käivitust tehtud ausa walk-forward
            # vea järgi. Seega 15.–16.08 tulemused ei vali endale tagantjärele sobivat läve.
            _age_threshold_candidates = [35.0, 38.0, 41.0, 44.0, 47.0, 50.0]
            _age_candidate_scores = {}
            _selection_days = sorted({
                d for d in _age_hist_dates
                if d < PLANT_INDEX_START
            })
            for _thr in _age_threshold_candidates:
                _p = np.full(len(_age_hist_pred), np.nan, dtype=float)
                for _d in _selection_days:
                    _test_idx = np.where(_age_hist_dates == _d)[0]
                    if len(_test_idx) == 0:
                        continue
                    _s = _fit_age_slope(_d, _thr)
                    _late = np.maximum(0.0, _age_hist_days[_test_idx] - _thr)
                    _p[_test_idx] = _age_hist_pred[_test_idx] * np.exp(_s * _late)
                _m = (
                    np.isfinite(_p)
                    & np.isfinite(_age_hist_actual)
                    & np.array([d < PLANT_INDEX_START for d in _age_hist_dates], dtype=bool)
                )
                if int(_m.sum()) >= 9:
                    _age_candidate_scores[_thr] = float(np.mean(np.abs(_p[_m] - _age_hist_actual[_m])))

            if _age_candidate_scores:
                AGE_LATE_THRESHOLD = min(_age_candidate_scores, key=_age_candidate_scores.get)
            else:
                AGE_LATE_THRESHOLD = 42.0

            # Võrdle valitud vanusekihti baasi vastu samal eel-15.08 perioodil.
            _age_base_mask = (
                np.isfinite(_age_hist_pred)
                & np.isfinite(_age_hist_actual)
                & np.array([d < PLANT_INDEX_START for d in _age_hist_dates], dtype=bool)
            )
            _age_base_mae = float(np.mean(np.abs(_age_hist_pred[_age_base_mask] - _age_hist_actual[_age_base_mask]))) if _age_base_mask.any() else None
            _age_selected_mae = _age_candidate_scores.get(AGE_LATE_THRESHOLD)
            AGE_LAYER_ENABLED = bool(
                _age_selected_mae is not None
                and _age_base_mae is not None
                and _age_selected_mae <= _age_base_mae + 0.02
            )

            def _age_factor_for(field_no, day_value, *, fit_cutoff=None):
                if not AGE_LAYER_ENABLED:
                    return 1.0
                cutoff = fit_cutoff or day_value
                slope = _fit_age_slope(cutoff, AGE_LATE_THRESHOLD)
                late = max(0.0, _production_age_days(field_no, day_value) - AGE_LATE_THRESHOLD)
                # Turvapiir ainult testis: vanusekiht üksi ei tohi ühe hooaja väikese valimiga
                # võtta üle 30% A+B+C potentsiaalist.
                return max(0.70, min(1.0, float(np.exp(slope * late))))

            # --- Taimeindeksi raw snapshotid (nagu app-128), kuid signaaliks kasutatakse ABC-d ---
            _plant_raw_key = f"plant_index_raw_forecasts_{TODAY.year}"
            _plant_state_key = f"plant_index_state_test130_{TODAY.year}"
            ...
            # Ajalooline jääkmüra vanusekihi järel. See määrab, kui palju ühe uue
            # korje signaali uskuda: mürasema mudeli korral ei hüppa indeks nii järsult.
            _pre_mask = (
                np.isfinite(_age_hist_pred)
                & np.isfinite(_age_hist_actual)
                & (_age_hist_pred > 0.10)
            ...
            _pre_idx = np.where(_pre_mask)[0]
            _pre_log_resid = []
            _pre_ratio_by_field = {f: [] for f in range(1, 15)}
            _pre_slope = _fit_age_slope(PLANT_INDEX_START, AGE_LATE_THRESHOLD)
            for _ii in _pre_idx:
                _f = int(_age_hist_fields[_ii]); _d = _age_hist_dates[_ii]
                _late = max(0.0, _age_hist_days[_ii] - AGE_LATE_THRESHOLD)
                _af = max(0.70, min(1.0, float(np.exp(_pre_slope * _late)))) if AGE_LAYER_ENABLED else 1.0
                _expected = float(_age_hist_pred[_ii]) * _af
                if _expected <= 0:
                    continue
                _ratio = float(_age_hist_actual[_ii]) / _expected
                _ratio = max(0.45, min(1.35, _ratio))
                _pre_log_resid.append(float(np.log(_ratio)))
                _pre_ratio_by_field[_f].append((_d, _ratio))

            if _pre_log_resid:
                _arr = np.asarray(_pre_log_resid, dtype=float)
                _med = float(np.median(_arr))
                _mad = float(np.median(np.abs(_arr - _med)))
                PLANT_OBS_SIGMA = max(0.14, 1.4826 * _mad)
            else:
                PLANT_OBS_SIGMA = 0.22

            # Prior: viimased kuni 3 ausat jääki iga põllu kohta, tugeva shrinkage'iga 1.00 suunas.
            _plant_index_by_field = {f: 1.0 for f in range(1, 15)}
            _plant_seed_info = {}
            for _f in range(1, 15):
                _events = sorted(_pre_ratio_by_field.get(_f) or [], key=lambda x: x[0])[-3:]
                if not _events:
                    continue
                _weights = np.array([0.25, 0.35, 0.50], dtype=float)[-len(_events):]
                _weights = _weights / _weights.sum()
                _logs = np.array([np.log(max(0.50, min(1.20, r))) for _, r in _events], dtype=float)
                _raw_state = float(np.exp(np.sum(_weights * _logs)))
                _confidence = min(0.70, 0.22 * len(_events))
                _seed_idx = float(np.exp(_confidence * np.log(min(1.0, _raw_state))))
                _seed_idx = max(PLANT_INDEX_MIN, min(1.0, _seed_idx))
                _plant_index_by_field[_f] = _seed_idx
                _plant_seed_info[_f] = {
                    "n": len(_events), "raw_state": _raw_state, "seed_index": _seed_idx,
                }
            ...
            for _hr in sorted(
                ...
                try:
                    ...
                    _actual_abc = float(_hr.get("a")) + float(_hr.get("b")) + float(_hr.get("c"))
                except (TypeError, ValueError):
                    continue
                ...
                if not isinstance(_raw_rec, dict):
                    continue
                try:
                    _raw_abc = float(_raw_rec.get("raw_abc"))
                except (TypeError, ValueError):
                    continue
                if _raw_abc <= 0:
                    continue

                _age_factor = _age_factor_for(_hf, _hd, fit_cutoff=_hd)
                _expected_abc = _raw_abc * _age_factor
                if _expected_abc <= 0:
                    continue
                _obs_ratio = max(0.45, min(1.20, _actual_abc / _expected_abc))
                _old_idx = float(_plant_index_by_field.get(_hf, 1.0))

                # Kalmani-filtri laadne usalduskaal: võrreldakse uut vaatlust senise
                # latentse seisundiga ja ajaloolise jääkmüraga. Suur, kuid usutav
                # kõrvalekalle saab kiiresti mõju; üks prognoosiviga ei saa kogu indeksit omastada.
                _surprise = abs(float(np.log(max(0.45, _obs_ratio) / max(PLANT_INDEX_MIN, _old_idx))))
                _z = _surprise / max(0.08, PLANT_OBS_SIGMA)
                _gain = min(0.55, max(0.25, 0.25 + 0.10 * _z))
                if _obs_ratio > _old_idx:
                    _gain = min(0.45, _gain + 0.05)  # taastumist ei tehta kunstlikult aeglaseks
                _new_log = (1.0 - _gain) * np.log(max(PLANT_INDEX_MIN, _old_idx)) + _gain * np.log(max(0.45, _obs_ratio))
                _new_idx = max(PLANT_INDEX_MIN, min(1.0, float(np.exp(_new_log))))
                _plant_index_by_field[_hf] = _new_idx
                _plant_last_event[_hf] = {
                    "harvest_date": _hd.isoformat(),
                    "raw_abc_forecast": _raw_abc,
                    "age_factor": _age_factor,
                    "expected_abc_after_age": _expected_abc,
                    "actual_abc": _actual_abc,
                    "observation_ratio": _obs_ratio,
                    "sigma": PLANT_OBS_SIGMA,
                    "gain": _gain,
            ...
            _save_json_setting(_plant_state_key, {
                ...
                "age_layer_enabled": AGE_LAYER_ENABLED,
                "age_threshold": AGE_LATE_THRESHOLD,
                "age_base_mae": _age_base_mae,
                "age_selected_mae": _age_selected_mae,
                "obs_sigma": PLANT_OBS_SIGMA,
                "updated_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                "fields": {
                    str(f): {
                        "index": float(_plant_index_by_field[f]),
                        "seed": _plant_seed_info.get(f),
            ...
            # Testis näita mootori enda hinnangut, et saaksime seda kohe põllutundega võrrelda.
            if READ_ONLY_TEST:
                st.markdown("##### 🧪 Taimevanus + varjatud taime seisund")
                _diag_rows = []
                for _f in range(1, 15):
                    _last = _plant_last_event.get(_f) or {}
                    _diag_rows.append({
                        "Põld": _f,
                        "Tootmisvanus": _production_age_days(_f, TODAY),
                        "Vanusetegur täna": _age_factor_for(_f, TODAY, fit_cutoff=TODAY),
                        "Taimeindeks": _plant_index_by_field.get(_f, 1.0),
                        "Prior": (_plant_seed_info.get(_f) or {}).get("seed_index"),
                        "Viimase korje signaal": _last.get("observation_ratio"),
                        "Viimane korje": _last.get("harvest_date"),
                    })
                _diag_df = pd.DataFrame(_diag_rows)
                st.dataframe(
                    _diag_df.style.format({
                        "Tootmisvanus": lambda v: f"{float(v):.0f} p",
                        "Vanusetegur täna": lambda v: f"{float(v):.2f}",
                        "Taimeindeks": lambda v: f"{float(v):.2f}",
                        "Prior": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                        "Viimase korje signaal": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                    }),
                    use_container_width=True,
                    hide_index=True,
                )
                _age_status = "SEES" if AGE_LAYER_ENABLED else "EI PARANDANUD walk-forward'i — tegur 1.00"
                st.caption(
                    f"Vanusekiht: {_age_status} · hilise faasi lävi {AGE_LATE_THRESHOLD:.0f} p · "
                    f"baas-MAE {'—' if _age_base_mae is None else f'{_age_base_mae:.2f}'} · "
                    f"vanuse-MAE {'—' if _age_selected_mae is None else f'{_age_selected_mae:.2f}'} · "
                    f"taimeindeksi vaatlusmüra σ≈{PLANT_OBS_SIGMA:.2f}. "
                    "Prior tuleb viimastest ausatest walk-forward jääkidest; uus korje mõjutab indeksit kohe."
                )
            ...
            def _predict_one(field_no, state, target_day):
                ...
                age_factor = _age_factor_for(field_no, target_day, fit_cutoff=TODAY)
                age_adjusted_abc = float(raw_abc_pred) * age_factor
                plant_index = _plant_index_for(field_no, target_day)
                abc_pred = age_adjusted_abc * plant_index
                # XL jääb testis eraldi: suurem osa XL-ist on korjejääk / leidmata vili,
                # seega taime seisundi indeks ei tohi seda automaatselt samas suhtes kärpida.
                xl_pred = float(raw_xl_pred)

                if isinstance(abc_explain, dict):
                    _ae = dict(abc_explain)
                    _ae_effects = dict(_ae.get("effects") or {})
                    _ae_effects["Taime vanus"] = float(age_adjusted_abc - raw_abc_pred)
                    _ae_effects["Taimeindeks"] = float(abc_pred - age_adjusted_abc)
            ...
            for offset in range(1, 10):
                ...
                for f in day_fields:
                    prev = field_state.get(int(f))
                    if not prev:
                        day_rows.append({"Põld": f, "Vanusetegur": _age_factor_for(f, target_day, fit_cutoff=TODAY), "Taimeindeks": _plant_index_for(f, target_day), "A+B+C": None, "C/B": None, "XL": None, "Kokku": None, "Intervall": None, "Alus": "puudub", "Hinnanguline ilm": "—"})
                        continue
                    result = _predict_one(int(f), prev, target_day)
                    if not result:
                        day_rows.append({"Põld": f, "Vanusetegur": _age_factor_for(f, target_day, fit_cutoff=TODAY), "Taimeindeks": _plant_index_for(f, target_day), "A+B+C": None, "C/B": None, "XL": None, "Kokku": None, "Intervall": (target_day-prev["date"]).days, "Alus": prev["source"], "Hinnanguline ilm": "puudub"})
                        continue
                    ...
                    day_rows.append({
                        "Põld": f, "Vanusetegur": result.get("age_factor", 1.0), "Taimeindeks": result.get("plant_index", 1.0),
            ...
            for target_day, rows_day in forecast_detail_days:
                ...
                st.dataframe(day_df_visible.style.format({
                    "Vanusetegur": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                ...
                for r in rows_day:
                    ...
                    erow = {
                        ...
                        "Taime vanus": effects.get("Taime vanus", 0.0),
