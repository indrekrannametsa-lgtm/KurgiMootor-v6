...
"""
KurgiMootor LAB-146 · PRE-TURN SIGNAL SEARCH
=============================================

Eesmärk
-------
LAB-144/145 vea audit näitas, et senine OWN2 + MEM10 + BLOCK1 baas eksib eelkõige
ühise päevataseme amplituudi pööretel: kõrged päevad jäävad liiga madalaks ja madalad
päevad liiga kõrgeks. Saagist tuletatud REGIME-TREND ja COMMON-SHOCK ei lahendanud seda.

LAB-146 EI lisa uut production-kihti. See on ainult aus otsing, kas enne pööret oli
issue-päeval juba nähtav lisainfo:
1) 3 päeva ilma muutus vs eelmine 3 päeva;
2) 5 päeva ilma muutus vs eelmine 5 päeva;
3) target-päeva enda ECMWF ilmaprognoos;
4) põllu bioloogilise vanuse andmepõhine proxy = päevad esimesest usaldusväärsest korjest.

Kõik kandidaadid ehitatakse sama senise FLAT baasi (OWN2 + MEM10 + BLOCK1) peale
ja jooksevad samas +1...+9 ajamasinas. Ühtegi turn-kuupäeva ei kasutata mudeli sisendina.
Lisaks näitab audit, kas kandidaat parandab FLAT-i 5 halvimal sihtpäeval või ainult
rikub stabiilseid päevi.
...
- Target-päeva ilma kandidaat kasutab productionis forecasti; historical trainingus mõõdetud ilma.
- Tuleviku tegelikku saaki ei kasutata sisendina.
- Sama issue +1...+9 sees prognoositud saaki EI söödetata tagasi state'iks.
- Põllu vanus ankurdub esimesele usaldusväärsele korjele; täpset õitsemise algust DB-s pole
  ja LAB ei leiuta seda tagantjärele.
...
vaba intercept. See on exploratory benchmark, mitte championivalik.
"""
...
LAB_VERSION = "LAB-146-PRETURN-SIGNAL-SEARCH-V1"
...
# Kõik otsingukandidaadid on enne replay'd fikseeritud ja sõltumatud.
VARIANTS: Dict[str, List[str]] = {
    "OWN2": [],
    "MEM10": ["w10_gdd10", "w10_rad", "w10_et0"],
    "FLAT": BASE_FEATURES,
    "FLAT+D3CORE": BASE_FEATURES + D3_CORE,
    "FLAT+D3FULL": BASE_FEATURES + D3_FULL,
    "FLAT+D5CORE": BASE_FEATURES + D5_CORE,
    "FLAT+DAY0": BASE_FEATURES + DAY0,
    "FLAT+AGE": BASE_FEATURES + AGE,
    "FLAT+D3FULL+AGE": BASE_FEATURES + D3_FULL + AGE,
    "FLAT+D3FULL+DAY0": BASE_FEATURES + D3_FULL + DAY0,
    "FLAT+D3FULL+AGE+DAY0": BASE_FEATURES + D3_FULL + AGE + DAY0,
}
...
def main() -> None:
    ...
    st.set_page_config(page_title="KurgiMootor LAB-146", layout="wide")
    st.error("🧪 LAB-146 · PRE-TURN SIGNAL SEARCH · READ-ONLY · 22.07 → +1…+9")
    st.title("KurgiMootor · mis oli enne pööret juba nähtav?")
    st.caption(
        "Senine kontrollbaas on FLAT = OWN2 + MEM10 + BLOCK1. Otsime ainult enne targetit teada olnud "
        "ilmamuutuse, target-päeva ECMWF ilma ja põllu andmepõhise bioloogilise vanuse lisainfot. "
        "See on exploratory benchmark, mitte production champion."
    )

    with st.expander("Katse reeglid", expanded=False):
        st.markdown(
            """
- **FLAT baas:** OWN2 + MEM10 (10 p GDD10/radiatsioon/ET0) + BLOCK1.
- **D3CORE:** targetile eelneva 3 päeva GDD/rad/ET0 muutus võrreldes eelneva 3 päevaga.
- **D3FULL:** sama + RH/sademed/tuul.
- **D5CORE:** 5 päeva GDD/rad/ET0 muutus võrreldes eelneva 5 päevaga.
- **DAY0:** target-päeva enda ECMWF prognoos (historical trainingus mõõdetud target-päeva ilm).
- **AGE:** päevad sama põllu esimesest usaldusväärsest korjest + nõrk ruutliige.
- Täpset õitsemise algust DB-s ei ole; seda **ei leiutata**. AGE on praegu ainult kontrollitav bioloogilise kella proxy.
- Kõik kandidaadid jooksevad samas ajamasinas ja sama ridge α-ga. Intercept puudub.
- Top-5 halvimad päevad valitakse ainult **FLAT baasi** vea järgi ja neid kasutatakse üksnes auditis.
- Tuleviku saaki state'iks tagasi ei söödetata. Ainult A+B+C.
    ...
    if st.button("Värskenda DB andmed", type="secondary"):
        _load_db_data.clear()
        st.session_state.pop("lab146_results", None)
        st.rerun()
    ...
    run_requested = st.button("▶ Jooksuta PRE-TURN otsing", type="primary")
    if not run_requested and "lab146_results" not in st.session_state:
    ...
    if run_requested:
        ...
        runs: Dict[date, str] = {}
        progress = st.progress(0.0, text="ECMWF + PRE-TURN replay…")
        ...
        st.session_state["lab146_results"] = {
            "field": field_df, "daily": daily, "errors": errors, "runs": runs,
        }

    result = st.session_state.get("lab146_results")
    if not result:
        st.stop()
    ...
    base_row = overall.loc[overall["Variant"] == "FLAT"].iloc[0]
    search_only = overall[~overall["Variant"].isin(["OWN2", "MEM10", "FLAT"])].copy()
    best = search_only.loc[search_only["MAPE %"].idxmin()] if not search_only.empty else base_row
    plus1 = _summary_table(daily[daily["lead"] == 1])
    plus1_search = plus1[~plus1["Variant"].isin(["OWN2", "MEM10", "FLAT"])]
    best1 = plus1_search.loc[plus1_search["MAPE %"].idxmin()] if not plus1_search.empty else None
    bad_base = float(bad.loc[bad["Variant"] == "FLAT", "MAPE %"].iloc[0]) if not bad.empty else float("nan")
    bad_best = bad[~bad["Variant"].isin(["OWN2", "MEM10", "FLAT"])]
    bad_best_row = bad_best.loc[bad_best["MAPE %"].idxmin()] if not bad_best.empty else None

    t1, t2, t3, t4, t5 = st.columns(5)
    t1.metric("FLAT baas · MAPE", f"{float(base_row['MAPE %']):.1f}%")
    t2.metric("Otsingu parim · kogu", f"{float(best['MAPE %']):.1f}%", str(best["Variant"]))
    if best1 is not None:
        t3.metric("Otsingu parim · +1p", f"{float(best1['MAPE %']):.1f}%", str(best1["Variant"]))
    else:
        t3.metric("Otsingu parim · +1p", "—")
    t4.metric("FLAT · 5 halvimat päeva", "—" if not math.isfinite(bad_base) else f"{bad_base:.1f}%")
    if bad_best_row is not None:
        t5.metric("Parim · 5 halvimat", f"{float(bad_best_row['MAPE %']):.1f}%", str(bad_best_row["Variant"]))
    else:
        t5.metric("Parim · 5 halvimat", "—")

    st.markdown("### 1. PRE-TURN kandidaadid · kogu replay")
    st.caption("Kõik read on sama FLAT baasi peale lisatud fikseeritud tunnuserühmad. Parim siin on ainult otsinguleid, mitte champion.")
    st.dataframe(
    ...
    st.markdown("### 3. Kus on 30% viga? · FLAT 5 halvimat sihtpäeva")
    st.caption("Halvimad päevad valitakse ainult FLAT baasi järgi; kandidaadid ei mõjuta grupi valikut.")
    if bad_days:
        st.write("FLAT 5 halvimat sihtpäeva: **" + ", ".join(pd.Timestamp(d).strftime("%d.%m") for d in sorted(bad_days)) + "**")
    if not bad.empty:
        bad_show = bad.copy()
        bad_show["Δ vs FLAT"] = bad_show["MAPE %"] - float(bad.loc[bad["Variant"] == "FLAT", "MAPE %"].iloc[0])
        st.dataframe(
    ...
    st.markdown("### 4. Ülejäänud päevad · kas pöörde parandamine rikub stabiilset osa?")
    if not rest.empty:
        rest_show = rest.copy()
        rest_show["Δ vs FLAT"] = rest_show["MAPE %"] - float(rest.loc[rest["Variant"] == "FLAT", "MAPE %"].iloc[0])
        st.dataframe(
            rest_show.style.format({
                "MAPE %": "{:.1f}%", "±20% sees %": "{:.0f}%", "Bias %": "{:+.1f}%",
                "MAE ABC": "{:.1f}", "Δ MAPE vs OWN2": lambda x: f"{x:+.1f} pp",
                "Δ vs FLAT": lambda x: f"{x:+.1f} pp",
            }),
    ...
    st.markdown("### 6. Kõrge / keskmine / madal tegelik päevatase · ainult audit")
    levels = _level_bias_table(daily)
    if not levels.empty:
        fmts = {"N": "{:.0f}"}
        for name in VARIANT_NAMES:
            fmts[f"{name} bias %"] = "{:+.1f}%"
            fmts[f"{name} MAPE %"] = "{:.1f}%"
        st.dataframe(levels.style.format(fmts), use_container_width=True, hide_index=True)
    ...
    st.caption(
        ...
        f"small-sample prior={SMALL_SAMPLE_PRIOR_ROWS:g} · FLAT + weather-change / DAY0 / production-age search · READ ONLY"
    )
