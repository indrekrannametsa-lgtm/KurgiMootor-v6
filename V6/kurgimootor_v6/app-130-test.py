...
APP_PATCH_VERSION = "wd-engine-parallel-5"  # eraldi paralleelmootor; production app-128 ei muutu
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor · WIND×DRY TESTMOOTOR", page_icon=APP_ICON, layout="wide")
...
WD_MODEL_VERSION = "wd-engine-parallel-5"
...
if NEW_WD_ENGINE_TEST:
    ...
    # Lõplik kirjutuskaitse: ka prognoosisnapshot saab minna DB-sse ainult
    # selle paralleelmootori model_version võtmega.
    if hasattr(db, "save_yield_forecasts"):
        _wd_real_save_yield_forecasts = db.save_yield_forecasts
        def _wd_safe_save_yield_forecasts(rows):
            safe_rows = []
            for row in (rows or []):
                item = dict(row)
                item["model_version"] = WD_MODEL_VERSION
                safe_rows.append(item)
            return _wd_real_save_yield_forecasts(safe_rows)
        db.save_yield_forecasts = _wd_safe_save_yield_forecasts
...
with _hdr_title:
    st.markdown("## KurgiMootor · paralleelmootor")
    st.caption("Paralleeläpp · app-128 kõrval eraldi jooksev WIND×DRY A+B+C prognoosimootor.")
...
page = st.radio(
    ...
    key=("wd_test_page" if NEW_WD_ENGINE_TEST else "main_page"),
)
if NEW_WD_ENGINE_TEST:
    st.info(
...
if NEW_WD_ENGINE_TEST:
    ...
    # Paralleelmootor arvutab oma prognoosi ainult Prognoos-vaates.
    # Nii on Prognoosi lehel kohe värske 0–9 päeva arvutus, aga muu UI ei käivita rasket mudeliringi.
    _forecast_refresh_due = (page == "Prognoos")
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        st.subheader("Prognoos · paralleelmootor")
        ...
        if training_rows:
            ...
            def _predict_one(field_no, state, target_day):
                ...
                # Kontrollarv: sama field/weather sisendiga puhas app-128 weather-first baas.
                # Seda EI kasutata testmootori väljundi asemel; näitame ainult võrdluseks.
                raw_abc_base_compare = _predict_full_abc_growth(
                ...
                abc_base_compare = float(raw_abc_base_compare) * plant_index
                ...
                return {
                    ...
                    "base_abc_compare": float(abc_base_compare),
                    "wd_effect_abc": float(abc_pred - abc_base_compare),
            ...
            for f in today_plan:
                ...
                today_forecast_rows.append({
                    ...
                    "A+B+C": pred["abc"],
                    "128 baas ABC": pred.get("base_abc_compare"),
            ...
            for offset in range(1, 10):
                ...
                for f in day_fields:
                    ...
                    day_rows.append({
                        ...
                        "A+B+C": result["abc"], "128 baas ABC": result.get("base_abc_compare"),
                ...
                for row in today_forecast_rows:
                    ...
                    forecast_payloads.append({
                        ...
                        "basis": (
                            ...
                            f"wd_base_abc={float(row.get('128 baas ABC') or row.get('A+B+C')):.6f}; "
                            f"wd_effect_abc={float(row.get('WD mõju') or 0.0):.6f}; "
            ...
            for target_day, rows_day in forecast_detail_days:
                ...
                st.dataframe(day_df_visible.style.format({
                    ...
                    "128 baas ABC": lambda v: "—" if pd.isna(v) else f"{float(v):.1f}",
                    "WD mõju": lambda v: "—" if pd.isna(v) else f"{float(v):+.1f}",
    ...
    for _lead in range(0, 10):
        ...
        for _r in _rows:
            ...
            _table_rows.append({
                ...
                "128 baas ABC": _basis_number(_basis, "wd_base_abc", None),
                "WD mõju": _basis_number(_basis, "wd_effect_abc", None),
                "C/B": _cb,
