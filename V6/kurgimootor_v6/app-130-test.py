...
APP_PATCH_VERSION = "wd-engine-parallel-10"  # eraldi paralleelmootor; production app-128 ei muutu
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor · WIND×DRY PARALLEEL v10", page_icon=APP_ICON, layout="wide")
...
WD_MODEL_VERSION = "wd-engine-parallel-13-session-only"
...
if NEW_WD_ENGINE_TEST:
    ...
    # Testäpp ei kirjuta productioni seadeid, korjeid, ilma EGA yield_forecasts tabeleid.
    # WIND×DRY prognoos hoitakse ainult Streamliti sessioonis, et app-128 ei saaks
    # testmootori ridu kogemata enda ametliku prognoosina kuvada.
    # Enne kirjutuste blokeerimist puhastame ühe korra selles sessioonis varasemate
    # wd-engine-parallel-* versioonide read shared yield_forecasts tabelist.
    if not st.session_state.get("_wd_shared_forecasts_cleaned", False):
        try:
            if hasattr(db, "yield_forecasts_available") and db.yield_forecasts_available():
                _wd_existing = db.get_yield_forecasts(limit=10000)
                _wd_versions = sorted({
                    str(_r.get("model_version") or "")
                    for _r in _wd_existing
                    if str(_r.get("model_version") or "").startswith("wd-engine-parallel-")
                })
                _wd_delete_errors = []
                for _wd_ver in _wd_versions:
                    try:
                        db._execute(
                            db._client().table("yield_forecasts").delete().eq("model_version", _wd_ver)
                        )
                    except Exception as _wd_del_exc:
                        _wd_delete_errors.append(f"{_wd_ver}: {_wd_del_exc}")
                if _wd_delete_errors:
                    raise RuntimeError("; ".join(_wd_delete_errors))
            st.session_state["_wd_shared_forecasts_cleaned"] = True
            st.session_state["_wd_shared_cleanup_error"] = ""
        except Exception as _wd_cleanup_exc:
            st.session_state["_wd_shared_cleanup_error"] = str(_wd_cleanup_exc)

    for _name in (
        "set_app_setting", "save_yield_forecasts", "save_harvest",
...
with _hdr_title:
    st.markdown("## KurgiMootor · WIND×DRY PARALLEEL v13")
    st.caption("PARALLEELTEST v13 · WIND×DRY töötab productionist eraldi; testprognoose ei kirjutata yield_forecasts tabelisse.")
st.success("✅ BUILD v13 · sõltumatu sessiooniprognoos · production snapshotid puutumata")
if NEW_WD_ENGINE_TEST:
    _wd_cleanup_error = str(st.session_state.get("_wd_shared_cleanup_error", "") or "")
    if _wd_cleanup_error:
        st.warning("Vanade WIND×DRY test-snapshotide puhastamine ei õnnestunud: " + _wd_cleanup_error)
    elif st.session_state.get("_wd_shared_forecasts_cleaned"):
        st.caption("Vanad wd-engine-parallel-* read eemaldatud shared yield_forecasts tabelist; v13 enam sinna ei kirjuta.")
...
page = st.radio(
    ...
    key="wd_parallel_v13_page",
)
if NEW_WD_ENGINE_TEST:
    st.success("✅ BUILD v13 · päris WIND×DRY mudel · Prognoos arvutatakse selle lehe avamisel")
    st.info(
        ...
        "Testprognoos elab ainult selle äpi sessioonis; yield_forecasts production-tabelisse testäpp ei kirjuta."
    )
...
if page == "Täna":
    ...
    if NEW_WD_ENGINE_TEST:
        _home_saved = list(st.session_state.get("wd_session_forecasts", []) or [])
    else:
        try:
...
if NEW_WD_ENGINE_TEST:
    ...
    # V10: Prognoos leht arvutab alati värske WIND×DRY prognoosi; teised lehed ei käivita rasket ringi.
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            if NEW_WD_ENGINE_TEST:
                ...
                champion_mae = None
                champion_stats = None  # WIND×DRY champion is forced; production confirmation-stat keys do not apply
            layered_trace_rows = []
            ...
            # PARALLEELTEST v13: ära kirjuta shared yield_forecasts tabelisse.
            # Hoia viimane WIND×DRY prognoos ainult selle Streamliti sessioonis.
            # Kui tänane korje on juba alanud, säilita sessioonis varasem lead=0
            # korje-eelne prognoos täpselt samal põhimõttel nagu production seda lukustab.
            if today_rows_live:
                _wd_prev_session = list(st.session_state.get("wd_session_forecasts", []) or [])
                _wd_prev_today = [
                    _r for _r in _wd_prev_session
                    if str(_r.get("target_date") or "") == TODAY.isoformat()
                    and int(_r.get("lead_days") or 0) == 0
                ]
                if _wd_prev_today:
                    forecast_payloads = _wd_prev_today + [
                        _r for _r in forecast_payloads
                        if not (
                            str(_r.get("target_date") or "") == TODAY.isoformat()
                            and int(_r.get("lead_days") or 0) == 0
                        )
                    ]
            st.session_state["wd_session_forecasts"] = list(forecast_payloads)
            st.session_state["wd_session_forecast_generated_at"] = datetime.now(ZoneInfo("Europe/Tallinn")).isoformat()
            ...
            forecast_history_rows = list(st.session_state.get("wd_session_forecasts", []) or [])
            ...
            # DIAGNOSTIKA v11: loe app-128 enda salvestatud production-snapshotid, et
            # võrrelda sama target_date + field_no vastu. See EI mõjuta v11 prognoosi.
            try:
                _v11_prod_saved_all = db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []
                _v11_prod_saved = [
                    r for r in _v11_prod_saved_all
                    if str(r.get("model_version") or "") == PRODUCTION_MODEL_VERSION
                ]
            except db.DatabaseError:
                _v11_prod_saved = []
            ...
            for target_day, rows_day in forecast_detail_days:
                ...
                # Otsene võrdlus app-128 vastu. Kasutame ainult production MODEL_VERSION
                # viimast terviklikku batch'i samale sihtpäevale.
                _v11_prod_batch = _latest_snapshot_batch_rows(_v11_prod_saved, target_day)
                _v11_prod_map = {}
                for _pr in _v11_prod_batch:
                    try:
                        _v11_prod_map[int(_pr.get("field_no"))] = _pr
                    except (TypeError, ValueError):
                        continue
                if not day_df.empty and "Põld" in day_df.columns:
                    def _v11_prod_abc(_f):
                        try:
                            _r = _v11_prod_map.get(int(_f))
                            return float(_r.get("abc_forecast")) if _r and _r.get("abc_forecast") is not None else np.nan
                        except (TypeError, ValueError):
                            return np.nan
                    day_df["128 ABC"] = day_df["Põld"].map(_v11_prod_abc)
                    day_df["WD ABC"] = pd.to_numeric(day_df.get("A+B+C"), errors="coerce")
                    day_df["Δ ABC"] = day_df["WD ABC"] - day_df["128 ABC"]
                ...
                st.dataframe(day_df_visible.style.format({
                    ...
                    "128 ABC": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                    "WD ABC": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                    "Δ ABC": lambda v: "—" if pd.isna(v) else f"{float(v):+.2f}",
            ...
            if forecast_store_ok:
                ...
                st.caption("Testprognoosi DB-ajalugu on v13-s teadlikult välja lülitatud, et productioni snapshotid ei seguneks.")
                try:
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    if NEW_WD_ENGINE_TEST:
        _ro_saved = list(st.session_state.get("wd_session_forecasts", []) or [])
    else:
        try:
