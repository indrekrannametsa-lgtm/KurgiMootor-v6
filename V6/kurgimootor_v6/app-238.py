...
if page == "Mootori tähelepanekud":
    ...
    # Näita aktiivse mootori viimast OOS-vastust KOHE nupu all.
    # Session-state on esimene allikas, et sama vajutuse järel oleks tulemus nähtav
    # ka siis, kui püsiseadistuse kirjutamine/uuestilugemine viibib.
    ...
    if _season_oos_run_now and not _active_top:
        st.info("⏳ Aktiivse WIND×DRY kogu-hooaja OOS arvutus on järjekorras…")
    if _active_oos_error:
        st.error(str(_active_oos_error))
    if isinstance(_active_top, dict) and _active_top:
        ...
        _at1.metric("Põllu MAE", f"{float(_active_top.get('row_mae')):.2f}" if _active_top.get('row_mae') is not None else "—")
        _at2.metric("3 põllu päeva MAE", f"{float(_active_top.get('daily_mae')):.2f}" if _active_top.get('daily_mae') is not None else "—")
        _at3.metric("Viimase 5 päeva MAE", f"{float(_active_top.get('daily_recent_mae')):.2f}" if _active_top.get('daily_recent_mae') is not None else "—")
        _at4.metric("Täispäevi", str(int(_active_top.get('daily_days') or 0)))

        # Üherealine tabel teeb tulemuse nähtavaks ka siis, kui kasutaja otsib just tabelit.
        _active_table = pd.DataFrame([{
            "Mudel": "Aktiivne WIND×DRY",
            "Põllu OOS MAE": _active_top.get("row_mae"),
            "3 põllu päeva MAE": _active_top.get("daily_mae"),
            "Viimase 5 päeva MAE": _active_top.get("daily_recent_mae"),
            "OOS ridu": int(_active_top.get("rows") or 0),
            "Täispäevi": int(_active_top.get("daily_days") or 0),
        }])
        st.dataframe(
        ...
        _at_recent_start = _active_top.get("daily_recent_start")
        _at_recent_end = _active_top.get("daily_recent_end")
        _at_recent_days = int(_active_top.get("daily_recent_days") or 0)
        _at_we = _active_top.get("daily_worst_error")
        _at_wd = _active_top.get("daily_worst_day")
        _at_bits = []
        if _active_top.get("daily_recent_mae") is not None:
            _range_txt = ""
