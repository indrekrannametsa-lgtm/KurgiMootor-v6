...
APP_PATCH_VERSION = "wd-leveldelta-production-4-ui-speed-1"  # UI/DB speed patch only; forecast model unchanged
...
if _run_operational_cycle:
    ...
    with _forecast_cycle_ui:
        ...
        if training_rows:
            ...
            forecast_days = []
            # Kui täna ei korjata, ei alga ring homme suvaliselt põllust 1.
            # Tänase automaatse plaani esimene põld lükkub lihtsalt homsesse.
            # Kui täna korjatakse nt ainult 2 põldu, jätkub homne ring kohe
            # viimase päriselt plaanitud põllu järel.
            if today_plan:
                first_field = _next_field(today_plan[-1])
            elif today_plan_default:
                first_field = int(today_plan_default[0])
            else:
                first_field = 1
