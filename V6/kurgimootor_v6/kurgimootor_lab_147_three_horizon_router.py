...
def main() -> None:
    ...
    st.set_page_config(page_title="KurgiMootor LAB-147", layout="wide")
    st.error("🧪 LAB-147 · THREE-HORIZON ROUTER · READ-ONLY · 22.07 → +1…+9")
    st.title("KurgiMootor · 3 horisonti, 3 eri tööd")
    st.caption(
...
- `3H-ROUTED` on deterministlik router, mitte championivalik.
    ...
    if st.button("Värskenda DB andmed", type="secondary"):
        _load_db_data.clear()
        st.session_state.pop("lab147_results", None)
        st.rerun()
    ...
    run_requested = st.button("▶ Jooksuta 3-HORIZON replay", type="primary")
    if not run_requested and "lab147_results" not in st.session_state:
    ...
    if run_requested:
        ...
        runs: Dict[date, str] = {}
        progress = st.progress(0.0, text="ECMWF + 3-HORIZON replay…")
        ...
        st.session_state["lab147_results"] = {
            "field": field_df, "daily": daily, "errors": errors, "runs": runs,
        }

    result = st.session_state.get("lab147_results")
    if not result:
        st.stop()
    ...
    t1.metric("FLAT · kogu MAPE", f"{float(flat_row['MAPE %']):.1f}%")
    t2.metric(
        "3H · kogu MAPE", f"{float(routed_row['MAPE %']):.1f}%",
        f"{float(routed_row['MAPE %'] - flat_row['MAPE %']):+.1f} pp vs FLAT"
    )
    t3.metric("SHORT +1…+4", f"{float(bshort['3H MAPE %']):.1f}%", f"{float(bshort['Δ vs FLAT']):+.1f} pp")
    t4.metric("MID +5…+7", f"{float(bmid['3H MAPE %']):.1f}%", f"{float(bmid['Δ vs FLAT']):+.1f} pp")
    t5.metric("LONG +8…+9", f"{float(blong['3H MAPE %']):.1f}%", f"{float(blong['Δ vs FLAT']):+.1f} pp")

    st.markdown("### 1. Üks fikseeritud 3-horisondi arhitektuur · kogu replay")
    show_order = ["OWN2", "MEM10", "FLAT", SHORT_NAME, MID_NAME, ROUTED_NAME]
    ...
    if not bad.empty:
        bad_show = bad[bad["Variant"].isin(["OWN2", "FLAT", SHORT_NAME, MID_NAME, ROUTED_NAME])].copy()
    ...
    if not warm.empty:
        warm_show = warm[warm["Variant"].isin(["OWN2", "FLAT", SHORT_NAME, MID_NAME, ROUTED_NAME])].copy()
        st.dataframe(
    ...
    show_cols = ["Issue", "Siht", "lead", "Route", "actual_abc", "FLAT", SHORT_NAME, MID_NAME, "OWN2", ROUTED_NAME]
    st.dataframe(
    ...
    st.caption(
        ...
        f"small-sample prior={SMALL_SAMPLE_PRIOR_ROWS:g} · fixed route 1–4 / 5–7 / 8–9 · READ ONLY"
    )
