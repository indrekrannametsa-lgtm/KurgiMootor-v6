...
def score_features(did: pd.DataFrame, target: str = "delta_D") -> pd.DataFrame:
    candidates = [c for c in did.columns if c.startswith("delta_edge_")]
    ...
    for c in candidates:
        ...
        rp = _pearson(x, y)
        rs = _spearman(x, y)
        full_sign = np.sign(rp) if np.isfinite(rp) and abs(rp) > 1e-12 else 0
        loo: List[float] = []
        ...
        out.append({
            "feature": c.replace("delta_edge_", ""),
            "N": len(z),
            "Pearson r": rp,
            "Spearman ρ": rs,
            "LOO sama suund %": same,
            "LOO min |r|": min_abs,
            "score": (abs(rs) if np.isfinite(rs) else 0.0) * (same / 100.0 if np.isfinite(same) else 0.0),
        })
    return pd.DataFrame(out).sort_values(["score", "Spearman ρ"], ascending=[False, False]).reset_index(drop=True) if out else pd.DataFrame()


def main() -> None:
    st.set_page_config(page_title="KurgiMootor LAB-156", layout="wide")
    st.error("🧪 LAB-156 · ORDER 1 vs 3 SERVAILM · READ-ONLY")
    st.title("Kasvu servatundide audit")
    st.caption(
...
- tunnuste otsing on discovery. Productionit ei muudeta ja DB-sse ei kirjutata.
    ...
    scores = score_features(did, "delta_D")
    scores_rel = score_features(did, "delta_rel")
    if scores.empty:
        st.warning("Tunnuste skoorimiseks jäi liiga vähe täielikke ridu.")
        st.stop()

    nice = scores.copy()
    for c in ("Pearson r", "Spearman ρ", "LOO min |r|", "score"):
        nice[c] = nice[c].round(3)
    nice["LOO sama suund %"] = nice["LOO sama suund %"].round(0)
    st.dataframe(nice[["feature", "N", "Pearson r", "Spearman ρ", "LOO sama suund %", "LOO min |r|"]], use_container_width=True, hide_index=True)

    top = scores.iloc[0]
    st.markdown("#### Praeguse andmeseisu tugevaim vihje")
    st.metric(top["feature"], f"Spearman ρ {top['Spearman ρ']:+.2f}", f"LOO sama suund {top['LOO sama suund %']:.0f}%")
    ...
    st.warning(
        "Tõlgendus: väikese N tõttu on see mehhanismi-otsing, mitte tõestus. Kõige väärtuslikum on tunnus, "
        "mis on tugeva suunaga nii kastivahe kui suhtelise vahe puhul ja mille suund püsib leave-one-out kontrollis."
    )
