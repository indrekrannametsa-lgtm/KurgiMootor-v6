...
def _fit_model(
    ...
    # Sensible initialization on a per-growth-day scale.
    mean_daily = float(
        np.mean(
    ...
    # Data residual is log-scale to avoid high-yield intervals dominating.
    def residual(theta):
        beta = theta[:n_beta]
        gammas = theta[n_beta:]

        pred = _predict_intervals(
            train,
            beta,
            gammas,
            feature_names,
        ...
        for j, name in enumerate(feature_names):
            ...
            beta_pen.append(
                math.sqrt(lam) * beta[j]
            )

        field_pen = (
            math.sqrt(FIELD_RIDGE)
            * gammas
        )

        return np.concatenate([
            data_resid,
            np.asarray(beta_pen, dtype=float),
            field_pen,
        ])
