...
APP_PATCH_VERSION = "v7.0-production-3"  # CYCLE + WEATHER · harvest-history forecast gate
...
def _harvest_history_integrity_from(rows, plans, today):
    """Kontrolli, et kogu mineviku korjeajalugu oleks operatiivseks prognoosiks lõpetatud.

    Reegel alates päevaplaanide kasutuselevõtust:
    - igal möödunud kalendripäeval peab olema plaan (ka [] = korjet ei olnud),
      VÕI vana 3-põllu korje täielikult sisestatud;
    - kui plaan on olemas, peavad tegelikud korjeread vastama plaanile täpselt
      nii põldude kui järjekorra järgi;
    - tänast päeva ei blokeerita, sest tänane korje võib olla alles töös.

    Enne esimese salvestatud päevaplaani kuupäeva säilib legacy-kontroll:
    olemasoleval korjepäeval peab olema 3 unikaalset korjerida.
    """
    ...
    plan_dates = sorted(norm_plans)
    tracking_start = plan_dates[0] if plan_dates else None
    issues = []

    # Legacy harvest days before plan tracking: a visible harvest day must still be complete.
    for dd in sorted(by_day):
        if tracking_start is not None and dd >= tracking_start:
            continue
        ordered = sorted(by_day[dd], key=lambda x: (x[0], x[1]))
        fields = [f for _, f, _ in ordered]
    ...
    if tracking_start is not None:
        ...
        while dd <= yesterday:
            plan_exists = dd in norm_plans
            plan = norm_plans.get(dd)
            ordered = sorted(by_day.get(dd, []), key=lambda x: (x[0], x[1]))
            actual = [f for _, f, _ in ordered]

            if not plan_exists:
                # Kui sellel päeval on legacy-stiilis täielik 3-põllu korje, aktsepteeri see.
                # Täiesti tühi päev ilma plaanita on aga teadmata olek ja blokeerib prognoosi.
                if len(actual) == 3 and len(set(actual)) == 3:
                    dd += timedelta(days=1)
                    continue
                issues.append({
                    "day": dd,
            ...
            if len(plan) == 0:
                if actual:
                    issues.append({
                        "day": dd,
                        "reason": "korjeplaan ja saak ei klapi",
                        "expected": [],
                        "actual": actual,
                    })
                dd += timedelta(days=1)
    ...
    issues.sort(key=lambda x: x["day"])
    return {
        "ok": not issues,
        "issues": issues,
...
def _harvest_history_integrity():
    try:
        ...
        return _harvest_history_integrity_from(rows, plans, TODAY)
