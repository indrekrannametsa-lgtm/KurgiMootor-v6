...
# -----------------------------------------------------------------------------
# APP-133-R4 VERIFIED TEST: ainult lugemine + kiire režiimidiagnostika.
# Tootmisandmete kirjutused ja automaatne ilma uuendus on testis blokeeritud.
# -----------------------------------------------------------------------------
READ_ONLY_TEST = True
TEST_BUILD = "APP-133-R4"

if READ_ONLY_TEST:
    def _test_noop(*args, **kwargs):
        return None

    # Kõik db-kirjutused, mida app-128 ise kasutab.
    for _name in (
        "set_app_setting",
        "save_yield_forecasts",
        "save_harvest",
        "save_harvest_plan",
        "delete_harvest",
    ):
    ...
    # Testäpp ei uuenda ilma ega kirjuta weather_daily tabelisse.
...
st.set_page_config(page_title="KurgiMootor TEST 133-R4", page_icon=APP_ICON, layout="wide")
st.error("🧪 APP-133-R4 AKTIIVNE · READ-ONLY · ainult kiire režiimidiagnostika")
...
with _hdr_title:
    st.markdown("## KurgiMootor V6.4 · TEST 133-R4")
    st.caption("🧪 READ-ONLY · ametlikku mudelit ei treenita ega tootmisandmeid muudeta")
...
# APP-133-R4: testäpp ei tohi käivitada täismudelit, Jäljeotsijat ega laia uuringut.
# Diagnostika kasutab ainult juba salvestatud forecast-snapshotte + harvests ridu.
if READ_ONLY_TEST:
...
if page == "Mootori tähelepanekud":
    ...
    # -------------------------------------------------------------------------
    # APP-133-R4: kiire režiimikiht. Ainult DB lugemine + lihtne EWMA/state replay.
    # Ei treeni Ridge'i, ei käivita walk-forward Jäljeotsijat ega kirjuta DB-sse.
    # -------------------------------------------------------------------------
    st.markdown("#### ⚡ Kiire režiimikiht — diagnostika")
    st.caption(
        "Kasutab ainult varem salvestatud ametlikke A+B+C prognoose ja hiljem saabunud tegelikke korjeid. "
        "Ametlikku prognoosi see ei muuda."
    )
    try:
        _rg_h = db.get_harvest_history(limit=2000)
        _rg_f = db.get_yield_forecasts(limit=5000) if db.yield_forecasts_available() else []

        # Täielik päev = täpselt 3 eri põldu ning A/B/C on olemas.
        _rg_actual = {}
        for _r in _rg_h or []:
            try:
                _d = date.fromisoformat(str(_r.get("harvest_date")))
                _fno = int(_r.get("field_no"))
                _abc = float(_r.get("a")) + float(_r.get("b")) + float(_r.get("c"))
            except (TypeError, ValueError):
                continue
            _rg_actual.setdefault(_d, {})[_fno] = _abc

        _rg_rows = []
        for _d in sorted(_rg_actual):
            _act_fields = _rg_actual[_d]
            if len(_act_fields) != 3:
                continue
            _expected = set(_act_fields)

            # Võta selle päeva viimane aus snapshot, mis oli olemas hiljemalt samal päeval.
            _by_fd = _snapshot_batch_map(_rg_f, _d, max_forecast_date=_d)
            _cands = []
            for _fd, _field_rows in _by_fd.items():
                if set(_field_rows) != _expected:
                    continue
                try:
                    _abc_fc = sum(float(_field_rows[_f].get("abc_forecast")) for _f in _expected)
                except (TypeError, ValueError, KeyError):
                    continue
                _cands.append((_fd, _abc_fc))
            if not _cands:
                continue
            _cands.sort(key=lambda x: x[0])
            _fd, _abc_fc = _cands[-1]
            _abc_act = float(sum(_act_fields.values()))
            if _abc_fc <= 0.05:
                continue
            _rg_rows.append({
                "Kuupäev": _d,
                "Prognoositud": _fd,
                "Tegelik ABC": _abc_act,
                "Champion ABC": float(_abc_fc),
            })

        _rg_df = (
            pd.DataFrame(_rg_rows).sort_values("Kuupäev").reset_index(drop=True)
            if _rg_rows else pd.DataFrame()
        )

        # Ühe K replay. Päev i prognoos kasutab ainult enne i saabunud seisundit.
        def _rg_replay(values_actual, values_base, kval):
            state = 1.0
            preds = []
            for _a, _b in zip(values_actual, values_base):
                preds.append(float(_b) * state)
                obs = float(_a) / float(_b) if float(_b) > 0.05 else 1.0
                # Kaitse ühe vigase päeva eest, aga luba päris pöördel kiiresti liikuda.
                obs = max(0.45, min(1.25, obs))
                state = (1.0 - float(kval)) * state + float(kval) * obs
                state = max(0.55, min(1.12, state))
            return np.asarray(preds, dtype=float), float(state)

        if len(_rg_df) >= 6:
            _a = _rg_df["Tegelik ABC"].to_numpy(float)
            _b = _rg_df["Champion ABC"].to_numpy(float)
            _k_grid = [0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85]

            # Aus meta-replay: iga päeva K valitakse AINULT varasemate päevade pealt.
            _meta_pred = np.full(len(_a), np.nan, dtype=float)
            _meta_k = np.full(len(_a), np.nan, dtype=float)
            for _i in range(4, len(_a)):
                _best = None
                for _k in _k_grid:
                    _p0, _ = _rg_replay(_a[:_i], _b[:_i], _k)
                    _mae0 = float(np.mean(np.abs(_p0[2:] - _a[2:_i])))
                    if _best is None or _mae0 < _best[0]:
                        _best = (_mae0, _k)
                _k_i = float(_best[1])
                _, _state_i = _rg_replay(_a[:_i], _b[:_i], _k_i)
                _meta_pred[_i] = _b[_i] * _state_i
                _meta_k[_i] = _k_i

            _eval = np.isfinite(_meta_pred)
            _base_mae = float(np.mean(np.abs(_b[_eval] - _a[_eval])))
            _reg_mae = float(np.mean(np.abs(_meta_pred[_eval] - _a[_eval])))

            # Järgmise päeva K valitakse kogu tänaseks teada oleva ajaloo pealt.
            _best_next = None
            for _k in _k_grid:
                _pp, _st = _rg_replay(_a, _b, _k)
                _mae = float(np.mean(np.abs(_pp[2:] - _a[2:])))
                if _best_next is None or _mae < _best_next[0]:
                    _best_next = (_mae, _k, _st)
            _next_k = float(_best_next[1])
            _next_factor = float(_best_next[2])

            _m1, _m2, _m3, _m4 = st.columns(4)
            _m1.metric("Champion päev-MAE", f"{_base_mae:.1f}")
            _m2.metric("Režiimikiht MAE", f"{_reg_mae:.1f}", delta=f"{_base_mae-_reg_mae:+.1f}")
            _m3.metric("Järgmise päeva K", f"{_next_k:.2f}")
            _m4.metric("ABC tegur", f"{_next_factor:.2f}", delta=f"{(_next_factor-1.0)*100:+.0f}%")
            st.caption(
                f"K={_next_k:.2f}: uus päev saab umbes {_next_k*100:.0f}% ja varasem seisund "
                f"{(1.0-_next_k)*100:.0f}% kaalust. K on valitud ajaloolise replay järgi, mitte käsitsi."
            )

            _show = _rg_df.copy()
            _show["Režiim ABC"] = _meta_pred
            _show["K"] = _meta_k
            _show["Champ viga"] = _show["Champion ABC"] - _show["Tegelik ABC"]
            _show["Režiim viga"] = _show["Režiim ABC"] - _show["Tegelik ABC"]
            _show = _show.tail(8).copy()
            _show["Kuupäev"] = _show["Kuupäev"].map(lambda d: d.strftime("%d.%m"))
            st.dataframe(
                _show[["Kuupäev", "Tegelik ABC", "Champion ABC", "Režiim ABC", "K", "Champ viga", "Režiim viga"]]
                .style.format({
                    "Tegelik ABC": "{:.1f}",
                    "Champion ABC": "{:.1f}",
                    "Režiim ABC": lambda v: "—" if pd.isna(v) else f"{float(v):.1f}",
                    "K": lambda v: "—" if pd.isna(v) else f"{float(v):.2f}",
                    "Champ viga": "{:+.1f}",
                    "Režiim viga": lambda v: "—" if pd.isna(v) else f"{float(v):+.1f}",
                }),
                use_container_width=True,
                hide_index=True,
            )

            # Esimene tänane/tulevane salvestatud ametlik snapshot; ABC korrigeeritakse, XL ei muutu.
            _future = []
            for _lead in range(0, 10):
                _td = TODAY + timedelta(days=_lead)
                _rr = _latest_snapshot_batch_rows(_rg_f, _td)
                if not _rr:
                    continue
                try:
                    _off_abc = float(sum(float(x.get("abc_forecast")) for x in _rr))
                    _off_xl = float(sum(float(x.get("xl_forecast")) for x in _rr))
                except (TypeError, ValueError):
                    continue
                _future.append((_td, _off_abc, _off_xl))
                break
            if _future:
                _td, _off_abc, _off_xl = _future[0]
                _reg_abc = _off_abc * _next_factor
                _reg_total = _reg_abc + _off_xl
                st.info(
                    f"**Variprognoos {_weekday_letter(_td)} {_short_date(_td)}:** ametlik ABC {_off_abc:.1f} + XL {_off_xl:.1f}; "
                    f"režiimikiht ABC {_reg_abc:.1f} + sama XL {_off_xl:.1f} = **{_reg_total:.1f} kasti**. "
                    "Ametlikku prognoosi see ei muuda."
                )
        else:
            st.info(
                f"Režiimikihi ausaks replay'ks on vaja vähemalt 6 täielikku päeva koos salvestatud ametliku ABC prognoosiga. "
                f"Praegu leidsin {len(_rg_df)}."
            )
    except Exception as _rg_exc:
        st.warning(f"Kiire režiimidiagnostika ei saanud arvutada: {_rg_exc}")
