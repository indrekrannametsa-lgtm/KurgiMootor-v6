...
if page == "Mootori tähelepanekud":
    ...
    if _layered_success_today:
        ...
        st.button(
            "🔬 Uurimispott täna liiga väike",
            disabled=True,
            key="layered_manual_budget_disabled",
        )
        st.caption(
            f"Lai uuring vajab käivituseks vähemalt {CPU_LAYERED_MIN_LAUNCH_S:.0f} s vaba uurimisaega; "
            f"praegu on talle turvavaruga saadaval {_manual_layered_available_s:.1f} s."
        )
    else:
        _layered_run_now = st.button(
            "🔬 Käivita lai uurimisvoor käsitsi",
            key="layered_manual_run",
            help="Tavaliselt saab lai otsing ühe piiratud vooru uue täieliku korjepäeva järel."
        )
        if _layered_run_now:
...
if page == "Prognoos" or _auto_light_check or _layered_run_now:
    ...
    with _forecast_page_placeholder.container():
        ...
        # Sama päeva ilmamuutused seda ei käivita. Enne käivitust reserveerime
        # Jäljeotsijale tema sihtaja + väikese turvavaru. Kui potti ei jää piisavalt,
        # lai ring jäetakse sel korral vahele — operatiivne prognoos jätkub siiski.
        if last_complete_harvest is not None:
            ...
            if _layered_auto_due:
                ...
                _light_reserve_s = max(float(CPU_LIGHT_TARGET_S), _light_elapsed_before_wide_s)
                ...
                if _auto_layered_available_s >= CPU_LAYERED_MIN_LAUNCH_S:
                    ...
                    _layered_run_now = False
                    _layered_skip_reason = (
                        f"Lai uuring jäi vahele: pärast Jäljeotsija reservi ja turvavaru "
                        f"jäi uurimispotist {_auto_layered_available_s:.1f} s; käivituseks on vaja vähemalt "
        ...
        if training_rows:
            ...
            if _layered_run_now:
                ...
                def _result_score(row):
                    ...
                    # Positiivne halvim-pool saab samuti väikese boonuse; varem
                    # karistati ainult negatiivset ning stabiliseeriv lisakiht võis
                    # seetõttu otsingusuunas liiga vähe väärtust saada.
                ...
                def _fresh_explore_combo():
                    # Kõik olemasolevad rollid võivad kombinatsiooni jõuda.
                    # 2–3 kihti on odavamad ja sagedasemad, kuid 4+ kihid ei ole enam
                    # kunstliku lae taga — stabiliseerivad hilised kihid saavad reaalse võimaluse.
        ...
        # iga päris arvutusringi kogu diagnostilise seisu, mitte ainult laia uuringu.
        # ---------------------------------------------------------------------
        try:
            ...
            if last_complete_harvest is not None:
                ...
                try:
                    _picked = {}
                    for _hist in forecast_history_rows if "forecast_history_rows" in locals() else []:
                        if str(_hist.get("target_date") or "") != _jday.isoformat():
                            continue
                        _fdate = str(_hist.get("forecast_date") or "")
                        if not _fdate or _fdate > _jday.isoformat():
                            continue
                        _fno = int(_hist.get("field_no"))
                        if _fno not in _jexpected:
                            continue
                        _key = (_fdate, _fno)
                        ...
                        if _old is None or str(_hist.get("generated_at") or "") > str(_old.get("generated_at") or ""):
                            _picked[_key] = _hist
                    _by_fd = {}
                    for (_fd, _fn), _hist in _picked.items():
                        _by_fd.setdefault(_fd, {})[_fn] = _hist
                    _complete = []
        ...
        try:
            _light_cycle_total_duration_s = max(0.0, time.perf_counter() - _light_cycle_t0)
            # Varem sisaldas "kerge/Jäljeotsija ring" ka sama tsükli sees jooksnud
            # laia uuringu aega. See võis näidata nt ~70 s kerget ringi, kuigi sellest
            # ~50 s oli tegelikult lai uuring. Nüüd mõõdame need lahus.
            _light_duration_s = max(
                0.0,
                _light_cycle_total_duration_s - float(_layered_elapsed_s_this_cycle or 0.0),
            )
            db.set_app_setting("light_cycle_last_duration_s", f"{_light_duration_s:.1f}")
            db.set_app_setting("light_cycle_last_total_duration_s", f"{_light_cycle_total_duration_s:.1f}")
            ...
            # Uurimispotti arvestame kerge ringi uue täieliku korjepäeva järel ning
            # käsitsi laia uuringu kaasneva Jäljeotsija töö puhul. Tavaline ilma- või
            # Prognoosi-vaate cache-hit ei võta Jäljeotsija päevast reservi.
            _light_is_new_harvest = str(_light_cycle_reason or "").startswith("täielik korjepäev")
            _light_is_manual_research = str(_light_cycle_reason or "") == "käsitsi lai uuring"
            if _light_is_new_harvest or _light_is_manual_research:
                _cpu_record_research("light", _light_duration_s)
                if _light_is_new_harvest:
                    db.set_app_setting(
...
if page == "Mootori tähelepanekud":
    ...
    with st.expander("⚙️ Tehniline koormus"):
        ...
        st.caption(
            ...
            "Kui pott saab täis, uut laia uuringut sel päeval ei käivitata."
        )
    ...
    if not _obs_ready:
        ...
        if not _obs:
            st.info("Täielikku salvestatud tähelepanekute snapshot'i veel pole. See tekib järgmise päris prognoosi/Jäljeotsija ringiga.")
        else:
            ...
            if not _auto_df_saved.empty:
                ...
                st.caption(f"Viimases radaris oli {_n} ideed. Need ei lähe automaatselt championiks.")
