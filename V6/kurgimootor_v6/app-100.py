...
APP_PATCH_VERSION = "wd-engine-parallel-2"  # eraldi paralleelmootor; production app-128 ei muutu
...
WD_MODEL_VERSION = "wd-engine-parallel-2"
...
with _hdr_title:
    ...
    st.caption("Paralleeläpp · uus WIND×DRY A+B+C mootor töötab production app-128 kõrval eraldi.")
...
if page == "Täna":
    ...
    selected_today_fields = st.multiselect(
        ...
        max_selections=4,
        disabled=NEW_WD_ENGINE_TEST,
        help=(
            "Paralleeläpp loeb tööplaani production app-128-st; muutmine toimub productionis."
            if NEW_WD_ENGINE_TEST else
            "Muuda ainult tänase tööplaani. Saagi sisestamine käib Korjed-menüüs."
        ),
    )
    ...
    try:
        ...
        _home_saved = (
            [r for r in _home_saved_all if str(r.get("model_version") or "") == WD_MODEL_VERSION]
    ...
    if NEW_WD_ENGINE_TEST:
        st.info("Paralleeläpp on korjete osas ainult lugemiseks. Korje lisamine või parandamine toimub production app-128-s.")
        _ro_harvest = db.get_harvest_history(limit=5000)
        _ro_by_day = {}
        for _r in _ro_harvest:
            try:
                _d0 = date.fromisoformat(str(_r.get("harvest_date")))
            except Exception:
                continue
            _ro_by_day.setdefault(_d0, []).append(_r)
        if not _ro_by_day:
            st.caption("Korjeajalugu puudub.")
        else:
            for _d0 in sorted(_ro_by_day.keys(), reverse=True)[:30]:
                _rows0 = sorted(_ro_by_day[_d0], key=lambda _r: int(_r.get("harvest_order") or 99))
                _render_day_block(f"{_weekday_letter(_d0)} {_short_date(_d0)}", _rows0, show_quality=True)
        st.stop()
...
if NEW_WD_ENGINE_TEST:
    ...
    # Paralleeläpp käitub nagu äpp, mitte analüüsiskript: raske mootor käib ainult Prognoos-vaates.
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    try:
        ...
        _ro_saved = (
            [r for r in _ro_saved_all if str(r.get("model_version") or "") == WD_MODEL_VERSION]
