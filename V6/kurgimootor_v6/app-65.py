...
APP_PATCH_VERSION = "wd-engine-parallel-7"  # eraldi cache-signatuur paralleelmootorile
APP_ICON = Image.open(APP_ICON_PATH)
st.set_page_config(page_title="KurgiMootor · WIND×DRY PARALLEEL v7", page_icon=APP_ICON, layout="wide")
...
TEST_MODEL_VERSION = "wd-engine-parallel-7"
...
st.warning(
    "🧪 WIND×DRY PARALLEELMOOTOR v7 — app-128 UI + sama taimeindeks + lukustatud "
    "WIND×DRY HIGH L3–7 LEVEL+DELTA. Productionit ei muudeta; salvestatakse ainult "
    "oma prognoosisnapshotid versiooniga wd-engine-parallel-7."
)
...
with _hdr_title:
    st.markdown("## KurgiMootor V6.4 · WIND×DRY v7")
...
if page == "Prognoos":
    st.caption("🧪 WIND×DRY v7 · Prognoosi live-arvutus")
...
if _run_operational_cycle:
    ...
    with _forecast_page_placeholder.container():
        ...
        if training_rows:
            ...
            # -------------------------------------------------------------
            # WIND×DRY PARALLEEL v7: lukustatud LAB-154 HIGH L3–7 LEVEL+DELTA kiht.
            # Rakendub alles pärast app-128 rekursiivse 9p state'i valmimist,
            # seega testväljundit ei söödeta järgmise korje state'ina tagasi.
            # -------------------------------------------------------------
            ...
            if page == "Prognoos":
                st.success(
                    f"WIND×DRY aktiivne · treeningridu {_wd_test_summary.get('train_n', 0)} · "
                    f"muudetud ridu {_wd_test_summary.get('changed', 0)} · "
                    f"suurim ABC mõju {_wd_test_summary.get('max_change_pct', 0.0):.1f}%"
                )
            ...
            MODEL_VERSION = "wd-engine-parallel-7"
            forecast_payloads = []
