...
"""
KurgiMootor · edge_weather-46
=============================

CYCLE PRODUCTIVITY ENGINE · WEATHER ABLATION · STRICT TIME-ORDERED REPLAY · READ ONLY
...
The only verdict here is the strict expanding past-only ablation. Historical blocked CV is not used to decide weather value in this LAB.
...
- STATE-ONLY is a pre-declared ablation that removes all six weather-delta features and keeps only previous productivity, interval change and season age.
- Same ridge lambda, minimum train rows, target and strict past-only replay are used for both; no feature/window/lag search.
- Time-ordered replay is the only validation score in this LAB. Full-season coefficients are shown only for diagnostics and are NOT validation evidence.
...
def main() -> None:
    st.set_page_config(page_title="KurgiMootor · weather ablation", layout="wide")
    st.title("KurgiMootor · cycle engine · weather ablation · strict replay")
    st.caption("edge_weather-46 · PERSIST vs STATE-ONLY vs FULL CYCLE · exact past-only replay · READ ONLY")

    st.info(
        "See LAB ei muuda CYCLE ENGINE'i ega otsi uusi tunnuseid. Küsimus on ainult selles, kas ilm annab juba leitud "
        "tsüklimootorile päriselt lisaväärtust. STATE-ONLY hoiab alles eelmise päevatootlikkuse, intervallimuutuse ja "
        "hooaja vanuse. FULL lisab täpselt samad kuus cycle-to-cycle weather-delta tunnust nagu LAB44/45. Mõlemal on "
        "sama target, ridge λ=8, minimum train N=24 ja strict target_date < T replay."
    )
    ...
    if replay.empty or "state_model" not in replay.columns or "full_model" not in replay.columns:
        st.error("Strict weather ablation replay ei jõudnud minimaalse treeningmahuni.")
        st.dataframe(replay_audit, use_container_width=True, hide_index=True)
        return

    replay_daily = _daily(replay)
    replay_aug = _period(replay_daily, AUG_START, AUG_END)
    replay_late = replay_daily[replay_daily["target_date"] >= date(2026, 8, 25)].copy()

    level, title, text_verdict = _weather_value_verdict(replay_daily, replay_aug)
    st.header("1. Otsus · kas ilm lisab tsüklimootorile päriselt infot?")
    if level == "success":
        st.success(f"✅ {title}\n\n{text_verdict}")
    elif level == "warning":
        st.warning(f"🟡 {title}\n\n{text_verdict}")
    ...
    st.header("2. Strict expanding replay · PERSIST vs STATE-ONLY vs FULL")
    p = _day_metrics(replay_daily, "persist")
    s = _day_metrics(replay_daily, "state_model")
    f = _day_metrics(replay_daily, "full_model")
    overall = pd.DataFrame([
        {**_summary_row("A · PERSIST", replay_daily, "persist"), "Parandus vs PERSIST %": 0.0, "FULL vs STATE %": np.nan},
        {**_summary_row("B · STATE-ONLY CYCLE", replay_daily, "state_model"), "Parandus vs PERSIST %": _improve(p["mae"], s["mae"]), "FULL vs STATE %": np.nan},
        {**_summary_row("C · FULL CYCLE + WEATHER", replay_daily, "full_model"), "Parandus vs PERSIST %": _improve(p["mae"], f["mae"]), "FULL vs STATE %": _improve(s["mae"], f["mae"])},
    ])
    st.dataframe(_fmt_table(overall), use_container_width=True, hide_index=True)

    chart = replay_daily.set_index("target_date")[["actual", "persist", "state_model", "full_model"]].rename(columns={
        "actual": "Tegelik ABC",
        "persist": "PERSIST",
        "state_model": "STATE-ONLY",
        "full_model": "FULL + WEATHER",
    })
    st.line_chart(chart)

    show = replay_daily[["target_date", "actual", "persist", "state_model", "full_model"]].copy()
    show["Weather võit vs STATE"] = np.abs(show["state_model"] - show["actual"]) - np.abs(show["full_model"] - show["actual"])
    show.columns = ["Päev", "Tegelik ABC", "PERSIST", "STATE-ONLY", "FULL + WEATHER", "Weather võit kastides"]
    st.dataframe(_fmt_table(show), use_container_width=True, hide_index=True)

    st.header("3. Puhas terve taime laine · 17.–24.08 · sama strict ablation")
    if replay_aug.empty:
        st.warning("17.–24.08 ei ole strict replay jaoks piisavalt päevi.")
    else:
        pa = _day_metrics(replay_aug, "persist")
        sa = _day_metrics(replay_aug, "state_model")
        fa = _day_metrics(replay_aug, "full_model")
        aug_t = pd.DataFrame([
            {**_summary_row("PERSIST", replay_aug, "persist"), "Parandus vs PERSIST %": 0.0, "FULL vs STATE %": np.nan},
            {**_summary_row("STATE-ONLY", replay_aug, "state_model"), "Parandus vs PERSIST %": _improve(pa["mae"], sa["mae"]), "FULL vs STATE %": np.nan},
            {**_summary_row("FULL + WEATHER", replay_aug, "full_model"), "Parandus vs PERSIST %": _improve(pa["mae"], fa["mae"]), "FULL vs STATE %": _improve(sa["mae"], fa["mae"])},
        ])
        st.dataframe(_fmt_table(aug_t), use_container_width=True, hide_index=True)
        aug_show = replay_aug[["target_date", "actual", "persist", "state_model", "full_model"]].copy()
        aug_show["Weather võit vs STATE"] = np.abs(aug_show["state_model"] - aug_show["actual"]) - np.abs(aug_show["full_model"] - aug_show["actual"])
        aug_show.columns = ["Päev", "Tegelik ABC", "PERSIST", "STATE-ONLY", "FULL + WEATHER", "Weather võit kastides"]
        st.dataframe(_fmt_table(aug_show), use_container_width=True, hide_index=True)

    st.header("4. Mis osa FULL võidust tuleb ilmast?")
    rows = []
    for label, frame in [
        ("Kogu strict periood", replay_daily),
        ("Terve taime laine 17.–24.08", replay_aug),
        ("Vananeva taime saba 25.08+", replay_late),
    ]:
        if frame.empty:
            continue
        pm = _day_metrics(frame, "persist")
        sm = _day_metrics(frame, "state_model")
        fm = _day_metrics(frame, "full_model")
        rows.append({
            "Periood": label,
            "N": fm["n"],
            "PERSIST MAE": pm["mae"],
            "STATE MAE": sm["mae"],
            "FULL MAE": fm["mae"],
            "STATE vs PERSIST %": _improve(pm["mae"], sm["mae"]),
            "FULL vs PERSIST %": _improve(pm["mae"], fm["mae"]),
            "WEATHER lisaväärtus vs STATE %": _improve(sm["mae"], fm["mae"]),
        })
    st.dataframe(_fmt_table(pd.DataFrame(rows)), use_container_width=True, hide_index=True)
    st.caption(
        "Ablationi tõlgendus on lihtne: STATE vs PERSIST mõõdab tsüklilise state-loogika väärtust; FULL vs STATE mõõdab "
        "kuue ilmamuutuse lisaväärtust. Ühtegi koefitsienti, akent, lagi ega lambda väärtust ei valita selle tulemuse järgi."
    )

    with st.expander("5. Warm-up / leakage audit · mõlemal mudelil identne minevik?", expanded=False):
        audit = replay_audit.copy()
        st.dataframe(audit, use_container_width=True, hide_index=True)
        bad = audit[(audit["Replay valmis"] == True) & ((audit["Future rows fitis"] != 0) | (audit["Max train target"] >= audit["Päev"]))]
        if bad.empty:
            st.success("Leakage lock OK: STATE ja FULL kasutavad igal skooritud päeval identset train setti; max train target < ennustatav päev; future rows fitis = 0.")
        else:
            st.error("LEAKAGE LOCK FAIL: vähemalt ühel päeval ei ole treening rangelt minevikus.")

    st.header("6. Täishooaja koefitsiendid · ainult diagnostika")
    st.caption("Need ei ole replay skoor. Mõlemad fitid kasutavad kogu olemasolevat hooaja tsükliandmestikku ainult mõju suurusjärgu vaatamiseks.")
    state_fit = _fit_ridge(cycles, STATE_FEATURES)
    full_fit = _fit_ridge(cycles, FULL_FEATURES)
    c1, c2 = st.columns(2)
    with c1:
        st.subheader("STATE-ONLY")
        st.dataframe(_fmt_table(_coef_table(state_fit), 3), use_container_width=True, hide_index=True)
    with c2:
        st.subheader("FULL + WEATHER")
        st.dataframe(_fmt_table(_coef_table(full_fit), 3), use_container_width=True, hide_index=True)

    with st.expander("25.08+ vananeva taime saba · päev-päevalt", expanded=False):
        if replay_late.empty:
            st.write("Pole strict replay ridu.")
        else:
            late_show = replay_late[["target_date", "actual", "persist", "state_model", "full_model"]].copy()
            late_show.columns = ["Päev", "Tegelik ABC", "PERSIST", "STATE-ONLY", "FULL + WEATHER"]
            st.dataframe(_fmt_table(late_show), use_container_width=True, hide_index=True)
    ...
    st.code(
        ...
        f"ridge lambda = {RIDGE_LAMBDA} for BOTH; minimum train rows = {MIN_TRAIN_ROWS} for BOTH\n"
        "STRICT REPLAY FIT = target_date < predicted target_date only\n"
        "STATE and FULL use identical training/test dates\n"
        "no feature selection / lambda search / window search / lag search / cap / PI / FAST / TOMO / RAD derivative\n"
