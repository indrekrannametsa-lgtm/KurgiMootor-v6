...
# CAPACITY GATE · frozen after observing 28.08 discovery failure.
# Exact rule, no fitted thresholds:
# if the two previous COMPLETED harvest-day common residuals are both below zero
# and the latest one worsened further, a positive RAD delta is clipped to zero.
# Negative RAD deltas are never limited. 28.08 is DISCOVERY ONLY and excluded
# from every backward summary below.
CAPACITY_DISCOVERY_DAY = date(2026, 8, 28)
CAPACITY_CONTEXT_START = date(2026, 8, 26)
...
def _apply_capacity_gate(daily):
    """Apply the frozen two-day plant-capacity rule causally.

    For target day t only residuals from completed harvest days before t are
    inspected. The gate is ON when the two most recent completed common
    residuals are both negative and the latest is lower than the one before it.
    If ON, only a positive RAD-predicted delta is clipped to 0. Negative RAD
    deltas pass through unchanged. No threshold or coefficient is fitted.
    """
    out = daily.copy()
    out['capacity_gate'] = False
    out['capacity_prev2_resid'] = np.nan
    out['capacity_prev1_resid'] = np.nan
    out['capacity_rad_delta'] = np.nan
    out['capacity_resid_pred'] = np.nan
    out['capacity_forecast'] = np.nan
    out['capacity_clipped'] = False

    for i, row in out.iterrows():
        if pd.isna(row.get('rad_delta_pred')) or pd.isna(row.get('prev_common_resid')):
            continue
        prior = out.iloc[:i].dropna(subset=['common_resid']).copy()
        if len(prior) < 2:
            continue
        r2 = float(prior.iloc[-2]['common_resid'])
        r1 = float(prior.iloc[-1]['common_resid'])
        gate = bool(r2 < 0.0 and r1 < 0.0 and r1 < r2)
        raw_delta = float(row['rad_delta_pred'])
        cap_delta = 0.0 if (gate and raw_delta > 0.0) else raw_delta
        r_hat = float(row['prev_common_resid']) + cap_delta
        try:
            forecast = float(row['base']) * math.exp(r_hat)
        except OverflowError:
            forecast = np.nan

        out.at[i, 'capacity_gate'] = gate
        out.at[i, 'capacity_prev2_resid'] = r2
        out.at[i, 'capacity_prev1_resid'] = r1
        out.at[i, 'capacity_rad_delta'] = cap_delta
        out.at[i, 'capacity_resid_pred'] = r_hat
        out.at[i, 'capacity_forecast'] = forecast
        out.at[i, 'capacity_clipped'] = bool(gate and raw_delta > 0.0)
    return out


def _capacity_period_metrics(df, label):
    """Apples-to-apples BASE vs RAD vs CAPACITY on strict RAD-ready rows only."""
    x = df[['date','actual','base','n_fields','rad_forecast','capacity_forecast','capacity_gate','capacity_clipped']].dropna(
        subset=['actual','base','rad_forecast','capacity_forecast']
    ).copy()
    if x.empty:
        return {
            'Periood':label, 'N':0, 'BASE MAE':np.nan, 'RAD MAE':np.nan, 'CAP MAE':np.nan,
            'CAP vs RAD %':np.nan, 'Gate ON':0, 'Clipped':0,
        }
    bm = _forecast_metrics(x.assign(_base=x['base']), '_base')
    rm = _forecast_metrics(x, 'rad_forecast')
    cm = _forecast_metrics(x, 'capacity_forecast')
    improve = (
        100.0 * (rm['MAE'] - cm['MAE']) / rm['MAE']
        if pd.notna(rm['MAE']) and rm['MAE'] > 1e-12 and pd.notna(cm['MAE'])
        else np.nan
    )
    return {
        'Periood':label, 'N':int(len(x)), 'BASE MAE':bm['MAE'], 'RAD MAE':rm['MAE'], 'CAP MAE':cm['MAE'],
        'CAP vs RAD %':improve, 'Gate ON':int(x['capacity_gate'].sum()), 'Clipped':int(x['capacity_clipped'].sum()),
    }
...
def main():
    ...
    st.caption('LAB-43c · same BASE · exact LAB-37 4v4 weather · strict derivative unchanged · frozen 2-day capacity diagnostic added · READ ONLY')
    st.info(
    ...
    try:
        ...
        daily = _apply_capacity_gate(_walk_derivative_models(daily0))
    ...
    st.markdown('### 4. Taime võimekuse gate · fikseeritud 2-päeva reegel')
    st.info(
        'Reegel on lukustatud täpselt nii: kui kaks eelmist lõpetatud korjepäeva olid mõlemad BASE-st allpool '
        '(ühine log-jääk < 0) JA viimane neist halvenes veel, siis positiivne RAD Δ lõigatakse nulli. '
        'Negatiivset RAD mõju ei piirata. Ühtegi läve, akent ega koefitsienti ei otsita.'
    )
    st.warning(
        '28.08 on selle reegli AVASTUSPÄEV ja ei kuulu ühegi alloleva backward-mõõdiku sisse. '
        'See audit saab ainult öelda, kas sama muutmata reegel oleks varasematel päevadel aidanud või rikkunud; '
        'päris tõestus saab tulla alles järgmisest seni nägemata korjest.'
    )

    cap_rows = [
        _capacity_period_metrics(earlier, 'Varasem ≤18.08'),
        _capacity_period_metrics(focus, 'Puhas laine 19.–24.08'),
    ]
    cap_summary = pd.DataFrame(cap_rows)
    st.dataframe(cap_summary.style.format({
        'BASE MAE':lambda v:'—' if pd.isna(v) else f'{float(v):.2f}',
        'RAD MAE':lambda v:'—' if pd.isna(v) else f'{float(v):.2f}',
        'CAP MAE':lambda v:'—' if pd.isna(v) else f'{float(v):.2f}',
        'CAP vs RAD %':lambda v:'—' if pd.isna(v) else f'{float(v):+.1f}%',
    }), use_container_width=True, hide_index=True)

    cap_back = daily[(daily['date'] < CAPACITY_CONTEXT_START) & daily['rad_forecast'].notna()].copy()
    if not cap_back.empty:
        cap_back['RAD AE'] = np.abs(cap_back['rad_forecast'] - cap_back['actual'])
        cap_back['CAP AE'] = np.abs(cap_back['capacity_forecast'] - cap_back['actual'])
        cap_back['CAP võit'] = cap_back['RAD AE'] - cap_back['CAP AE']
        st.caption('Backward readout · kõik strict RAD-ready päevad enne 26.08. Gate vaatab ainult kahte juba lõpetatud eelmist korjepäeva.')
        st.dataframe(
            cap_back[[
                'date','fields','actual','base','capacity_prev2_resid','capacity_prev1_resid',
                'rad_delta_pred','capacity_gate','capacity_clipped','capacity_rad_delta',
                'rad_forecast','capacity_forecast','RAD AE','CAP AE','CAP võit'
            ]].rename(columns={
                'date':'Päev','fields':'Põllud','actual':'Tegelik ABC','base':'BASE',
                'capacity_prev2_resid':'Eelmine-2 jääk','capacity_prev1_resid':'Eelmine jääk',
                'rad_delta_pred':'RAD Δ','capacity_gate':'Gate ON','capacity_clipped':'Positiivne Δ lõigatud',
                'capacity_rad_delta':'CAP Δ','rad_forecast':'RAD forecast','capacity_forecast':'RAD+CAP forecast',
            }).style.format({
                'Päev':_fmt_day,'Tegelik ABC':'{:.1f}','BASE':'{:.1f}',
                'Eelmine-2 jääk':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
                'Eelmine jääk':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
                'RAD Δ':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
                'CAP Δ':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
                'RAD forecast':lambda v:'—' if pd.isna(v) else f'{float(v):.1f}',
                'RAD+CAP forecast':lambda v:'—' if pd.isna(v) else f'{float(v):.1f}',
                'RAD AE':'{:.1f}','CAP AE':'{:.1f}','CAP võit':'{:+.1f}',
            }), use_container_width=True, hide_index=True
        )

    context = daily[(daily['date'] >= CAPACITY_CONTEXT_START) & (daily['date'] < CAPACITY_DISCOVERY_DAY)].copy()
    if not context.empty:
        st.caption('26.–27.08 · juba nähtud kontekst, mitte puhas holdout.')
        context['RAD AE'] = np.abs(context['rad_forecast'] - context['actual'])
        context['CAP AE'] = np.abs(context['capacity_forecast'] - context['actual'])
        st.dataframe(
            context[['date','actual','base','rad_delta_pred','capacity_gate','capacity_rad_delta','rad_forecast','capacity_forecast','RAD AE','CAP AE']]
            .rename(columns={
                'date':'Päev','actual':'Tegelik ABC','base':'BASE','rad_delta_pred':'RAD Δ','capacity_gate':'Gate ON',
                'capacity_rad_delta':'CAP Δ','rad_forecast':'RAD forecast','capacity_forecast':'RAD+CAP forecast'
            }).style.format({
                'Päev':_fmt_day,'Tegelik ABC':'{:.1f}','BASE':'{:.1f}',
                'RAD Δ':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
    ...
    discovery = daily[daily['date'] == CAPACITY_DISCOVERY_DAY].copy()
    if not discovery.empty:
        st.caption('28.08 · AVASTUSPÄEV · kuvatakse ainult mehhanismi kontrolliks, mitte tõendina.')
        discovery['RAD AE'] = np.abs(discovery['rad_forecast'] - discovery['actual'])
        discovery['CAP AE'] = np.abs(discovery['capacity_forecast'] - discovery['actual'])
        st.dataframe(
            discovery[['date','actual','base','capacity_prev2_resid','capacity_prev1_resid','rad_delta_pred','capacity_gate','capacity_rad_delta','rad_forecast','capacity_forecast','RAD AE','CAP AE']]
            .rename(columns={
                'date':'Päev','actual':'Tegelik ABC','base':'BASE','capacity_prev2_resid':'Eelmine-2 jääk',
                'capacity_prev1_resid':'Eelmine jääk','rad_delta_pred':'RAD Δ','capacity_gate':'Gate ON',
                'capacity_rad_delta':'CAP Δ','rad_forecast':'RAD forecast','capacity_forecast':'RAD+CAP forecast'
            }).style.format({
                'Päev':_fmt_day,'Tegelik ABC':'{:.1f}','BASE':'{:.1f}',
                'Eelmine-2 jääk':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
    ...
    if not late.empty:
        with st.expander('26.08+ vananeva taime saba · ainult info, mitte verdict'):
            st.dataframe(late[['date','fields','actual','base','common_resid','delta_common_resid','wx4_d_rad','rad_delta_pred','capacity_gate','capacity_rad_delta','capacity_forecast']].rename(columns={
                ...
                'capacity_gate':'Gate ON','capacity_rad_delta':'CAP Δ','capacity_forecast':'RAD+CAP forecast'
            }).style.format({
                'Päev':_fmt_day,'Tegelik ABC':'{:.1f}','BASE':'{:.1f}',
                'Ühine log-viga':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
                'Δ ühine log-viga':lambda v:'—' if pd.isna(v) else f'{float(v):+.3f}',
    ...
    st.caption(
        ...
        'Capacity gate is also coefficient-free: exactly two previous completed negative residuals plus worsening; it can only clip positive RAD Δ to zero. 28.08 is excluded from backward summaries.'
    )
    st.success(
