...
APP_PATCH_VERSION = "wd-leveldelta-production-4-ui-speed-plan-2"  # UI speed + strict harvest-plan continuity; model unchanged
...
def _ensure_active_winddry_engine() -> None:
    """One-time bootstrap when WIND×DRY becomes the production A+B+C engine."""
    _engine_key = "active_forecast_engine"
    _current = db.get_app_setting(_engine_key, "")
    if _current != WD_MODEL_VERSION:
        db.set_app_setting(_engine_key, WD_MODEL_VERSION)
        _mark_forecast_dirty("WIND×DRY HIGH L3–7 LEVEL+DELTA mootori kasutuselevõtt")
...
with _hdr_title:
    ...
    st.caption("Saagi ennustamise tööriist. A+B+C aktiivne mootor: WIND×DRY HIGH L3–7 LEVEL+DELTA.")
...
if page == "Täna":
    ...
    if saved_today_plan != selected_today_fields:
        ...
        # Korjeplaan on operatiivse prognoosi päris sisend. Kui põlde on täna
        # 0/1/2/4 senise 3 asemel, peab nii tänane summa kui järgmiste päevade
        # põllujärjekord kohe muutuma. Uurimismootorit see EI käivita.
        _mark_forecast_dirty("tänase korjeplaan muutus")
    ...
    try:
        ...
        _home_saved = [
            ...
            if str(r.get("model_version") or "") == WD_MODEL_VERSION
        ]
    except db.DatabaseError:
        _home_saved = []
    ...
    # Tänane prognoos – kuva ainult TÄNASES tööplaanis olevad põllud.
    # Sama päeva varasem 3-põllu snapshot võib DB-s ajaloo jaoks alles olla,
    # kuid 0/1/2 põllu tööplaan ei tohi kasutajale vana summat näidata.
    _today_saved_all_rows = _latest_snapshot_rows(TODAY)
    _today_plan_set = {int(f) for f in selected_today_fields}
    today_saved_rows = [
        r for r in _today_saved_all_rows
        ...
        if not selected_today_fields:
            st.markdown(
                ...
                <div style="border:2px solid rgba(76,160,92,.38);border-radius:12px;padding:12px 14px;">
                  ...
                  <div style="font-size:2.15rem;font-weight:950;line-height:1.05;margin-top:3px;">0,0 kasti</div>
                  <div style="font-size:.92rem;opacity:.76;margin-top:6px;">Täna korjet ei ole</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            ...
            plant_idx = _combined_plant_index(today_saved_rows)
            plant_text = f"taimeindeks {_fmt(plant_idx, 2)}"
                ...
                <div style="border:2px solid rgba(76,160,92,.38);border-radius:12px;padding:12px 14px;">
                  ...
                  <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;margin-top:3px;">
                    ...
                    <div style="text-align:right;">
                      ...
                      <div style="font-size:.78rem;font-weight:700;opacity:.58;margin-top:2px;">{plant_text}</div>
                    </div>
                  </div>
    ...
    with home_future_forecast_slot.container():
        ...
        for lead in range(1, 10):
            ...
            plant_idx = _combined_plant_index(rows_day)
            plant_text = f"taimeindeks {_fmt(plant_idx, 2)}"
            far_forecast = lead >= 6
                ...
                <div style="background:{bg};border:1px solid {border};border-radius:10px;
                  ...
                  <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;">
                    ...
                    <div style="text-align:right;">
                      ...
                      <div style="font-size:.76rem;font-weight:700;opacity:.58;margin-top:1px;">{plant_text}</div>
                    </div>
                  </div>
...
if _run_operational_cycle:
    ...
    if page == "Prognoos":
        ...
        _forecast_cycle_ui = _forecast_page_placeholder.container()
    else:
    ...
    with _forecast_cycle_ui:
        ...
        if training_rows:
            ...
            # -----------------------------------------------------------------
            # Hilishooaja taimeindeks
            # -----------------------------------------------------------------
            # Kuni 14.08 on indeks alati 1.00. Alates 15.08 võib ta ainult
            # vähendada mudeli puhtast ilma/championi potentsiaalist saadavat saaki.
            # Indeks õpib PÕLLU KAUPA ainult varasema päris korje ja enne seda
            # lukustatud puhta (indeksita) prognoosi suhtest. See ei lähe tagasi
            # Jäljeotsija/championi treeningusse ega saa seega ilmamudelit "päästa".
            PLANT_INDEX_START = date(TODAY.year, 8, 15)
            PLANT_INDEX_ALPHA = 0.70   # kiire reaktsioon: 70% uusimast signaalist
            PLANT_INDEX_MIN = 0.50     # kaitse ühe väga erandliku korje eest
            # WIND×DRY productionil on OMA taimeindeksi ajalugu.
            # Vana app-128 raw/state võtmeid ega vana model_version snapshotte EI seemendata,
            # sest see tekitaks uue ilmamootori peale vana mootori vea topeltkorrektsiooni.
            _plant_namespace = "winddry_leveldelta_p3"
            _plant_raw_key = f"plant_index_raw_forecasts_{_plant_namespace}_{TODAY.year}"
            _plant_state_key = f"plant_index_state_{_plant_namespace}_{TODAY.year}"
            _plant_raw = _load_json_setting(_plant_raw_key, {})
            if not isinstance(_plant_raw, dict):
                _plant_raw = {}

            # Arvuta indeks alati deterministlikult algusest uuesti. Kui korjerida
            # parandatakse, muutub ka indeks õigesti; eraldi "juba töödeldud" lippu pole vaja.
            _plant_index_by_field = {f: 1.0 for f in range(1, 15)}
            _plant_last_event = {}
            for _hr in sorted(
                harvest_rows,
                key=lambda r: (str(r.get("harvest_date") or ""), int(r.get("harvest_order") or 99)),
            ):
                try:
                    _hd = date.fromisoformat(str(_hr.get("harvest_date")))
                    _hf = int(_hr.get("field_no"))
                    _actual_total = float(_hr.get("total"))
                except (TypeError, ValueError):
                    continue
                if _hd < PLANT_INDEX_START or _hd > TODAY or not (1 <= _hf <= 14):
                    continue
                if str(_hr.get("data_quality") or "").strip().lower() in {"hinnanguline", "ligikaudne"}:
                    continue
                _raw_rec = _plant_raw.get(f"{_hd.isoformat()}|{_hf}")
                if not isinstance(_raw_rec, dict):
                    continue
                try:
                    _raw_total = float(_raw_rec.get("raw_total"))
                except (TypeError, ValueError):
                    continue
                if _raw_total <= 0:
                    continue
                _ratio_raw = _actual_total / _raw_total
                # Plussi ei ole: üle ootuse korje annab signaaliks maksimaalselt 1.00.
                _signal = max(PLANT_INDEX_MIN, min(1.0, _ratio_raw))
                _old_idx = float(_plant_index_by_field.get(_hf, 1.0))
                _new_idx = (1.0 - PLANT_INDEX_ALPHA) * _old_idx + PLANT_INDEX_ALPHA * _signal
                _new_idx = max(PLANT_INDEX_MIN, min(1.0, _new_idx))
                _plant_index_by_field[_hf] = _new_idx
                _plant_last_event[_hf] = {
                    "harvest_date": _hd.isoformat(),
                    "raw_forecast": _raw_total,
                    "actual": _actual_total,
                    "actual_raw_ratio": _ratio_raw,
                    "signal": _signal,
                    "index_before": _old_idx,
                    "index_after": _new_idx,
                }

            _save_json_setting(_plant_state_key, {
                "start_date": PLANT_INDEX_START.isoformat(),
                "alpha": PLANT_INDEX_ALPHA,
                "min_index": PLANT_INDEX_MIN,
                "updated_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                "fields": {
                    str(f): {
                        "index": float(_plant_index_by_field[f]),
                        "last_event": _plant_last_event.get(f),
                    }
                    for f in range(1, 15)
                },
            })
            _save_json_setting(_plant_raw_key, _plant_raw)

            def _plant_index_for(field_no, target_day):
                if target_day < PLANT_INDEX_START:
                    return 1.0
                try:
                    return max(PLANT_INDEX_MIN, min(1.0, float(_plant_index_by_field.get(int(field_no), 1.0))))
                except (TypeError, ValueError):
                    return 1.0
            ...
            def _predict_one(field_no, state, target_day):
                ...
                raw_abc_pred = _predict_full_abc_growth(
                ...
                if xl_champion_uses_memory and state.get("source") != "tegelik":
                    raw_xl_pred = _predict_full_generic(full_xl_base_model, field_no, base_values, [])
                else:
                    raw_xl_pred = _predict_full_generic(
                ...
                plant_index = _plant_index_for(field_no, target_day)
                abc_pred = float(raw_abc_pred) * plant_index
                xl_pred = float(raw_xl_pred) * plant_index

                # Selgituses hoia championi tegurid puhtana ning näita taimeindeksi
                # mõju eraldi negatiivse kastipanusena. Nii jääb nähtavaks, mida
                # ilma/championi mudel ise arvas ja mida hilishooaja indeks muutis.
                if isinstance(abc_explain, dict):
                    _ae = dict(abc_explain)
                    _ae_effects = dict(_ae.get("effects") or {})
                    _ae_effects["Taimeindeks"] = float(abc_pred - raw_abc_pred)
                    _ae["effects"] = _ae_effects
                    _ae["raw_prediction"] = float(raw_abc_pred)
                    _ae["prediction"] = float(abc_pred)
                    _ae["reconstructed"] = float(abc_pred)
                    abc_explain = _ae

                return {
                    "abc": abc_pred, "xl": xl_pred, "cb": cb_pred, "total": abc_pred + xl_pred,
                    "raw_abc": float(raw_abc_pred), "raw_xl": float(raw_xl_pred),
                    "raw_total": float(raw_abc_pred + raw_xl_pred), "plant_index": float(plant_index),
            ...
            for f in today_plan:
                ...
                today_forecast_rows.append({
                    "Põld": int(f),
                    "Taimeindeks": pred.get("plant_index", 1.0),
                    "A+B+C": pred["abc"],
                    "C/B": pred["cb"],
                    "XL": pred["xl"],
                    "Kokku": pred["total"],
                    ...
                    "_Raw ABC": pred.get("raw_abc"), "_Raw XL": pred.get("raw_xl"),
                    "_Raw Kokku": pred.get("raw_total"),
            ...
            # FUTURE PLAN CONTINUITY LOCK
            # ---------------------------
            # Tuleviku korjering peab jätkuma viimasest PÄRISELT korjatud põllust.
            # UI automaatne tänase päeva default ei ole selleks piisavalt tugev alus:
            # 0-põllu päeva või ebatäieliku päeva järel võib see jätta vana/vaikimisi
            # 1,2,3 ploki ette. See omakorda teeb ka korjeintervalli valeks.
            #
            # Reegel:
            #   • kui täna on plaanis >=1 põld, liigub homne plaan viimase tänase
            #     plaanitud põllu järelt edasi (sest need põllud korjatakse täna);
            #   • kui täna korjet EI OLE, ei tehta ühtegi fiktiivset korjet ning
            #     homne plaan algab viimase tegeliku ajaloolise korje järgmise põlluga;
            #   • kui ajalugu puudub, alles siis kasutame 1,2,3 algust.
            def _latest_actual_field_before(day_value):
                latest = None
                for _seq, _r in enumerate(harvest_rows):
                    try:
                        _d = date.fromisoformat(str(_r.get("harvest_date") or ""))
                        _f = int(_r.get("field_no"))
                        _o_raw = _r.get("harvest_order")
                        _o = int(_o_raw) if _o_raw is not None else 99
                    except (TypeError, ValueError):
                        continue
                    if _d >= day_value or not (1 <= _f <= 14):
                        continue
                    # Kui vanal real harvest_order puudub, võidab sama päeva hiljem
                    # tagastatud rida; DB ajalugu on päeva sees korjejärjekorras.
                    _key = (_d, _o, _seq)
                    if latest is None or _key > latest[0]:
                        latest = (_key, _f)
                return int(latest[1]) if latest is not None else None

            if today_plan:
                first_field = _next_field(int(today_plan[-1]))
            else:
                _last_actual_field = _latest_actual_field_before(TODAY)
                first_field = _next_field(_last_actual_field) if _last_actual_field is not None else 1

            current_first = int(first_field)
            ...
            for offset in range(1, 10):
                ...
                for f in day_fields:
                    prev = field_state.get(int(f))
                    if not prev:
                        day_rows.append({"Põld": f, "Taimeindeks": _plant_index_for(f, target_day), "A+B+C": None, "C/B": None, "XL": None, "Kokku": None, "Intervall": None, "Alus": "puudub", "Hinnanguline ilm": "—"})
                        continue
                    result = _predict_one(int(f), prev, target_day)
                    if not result:
                        day_rows.append({"Põld": f, "Taimeindeks": _plant_index_for(f, target_day), "A+B+C": None, "C/B": None, "XL": None, "Kokku": None, "Intervall": (target_day-prev["date"]).days, "Alus": prev["source"], "Hinnanguline ilm": "puudub"})
                        continue
                    ...
                    day_rows.append({
                        "Põld": f, "Taimeindeks": result.get("plant_index", 1.0),
                        "A+B+C": result["abc"], "C/B": result["cb"], "XL": result["xl"], "Kokku": result["total"],
                        "Intervall": result["interval"], "Alus": source_label,
                        "Hinnanguline ilm": ", ".join(sorted(d.strftime("%d.%m") for d in result["estimated_days"])) or "—",
                        "_Raw ABC": result.get("raw_abc"), "_Raw XL": result.get("raw_xl"),
                        "_Raw Kokku": result.get("raw_total"),
            ...
            # Uuenda puhta (taimeindeksita) mudelipotentsiaali ankruid ainult
            # nende korjete jaoks, mis ei ole veel tegelikuna sisestatud. Nii jääb
            # pärast korjet alles täpselt viimane enne tegelikku korjet teada olnud raw-prognoos.
            if TODAY >= PLANT_INDEX_START:
                for _rr in today_forecast_rows:
                    try:
                        _rf = int(_rr.get("Põld"))
                        _raw_total = float(_rr.get("_Raw Kokku"))
                        _raw_abc = float(_rr.get("_Raw ABC"))
                        _raw_xl = float(_rr.get("_Raw XL"))
                    except (TypeError, ValueError):
                        continue
                    if _rf in today_actual:
                        continue
                    _plant_raw[f"{TODAY.isoformat()}|{_rf}"] = {
                        "target_date": TODAY.isoformat(), "field_no": _rf,
                        "raw_total": _raw_total, "raw_abc": _raw_abc, "raw_xl": _raw_xl,
                        "captured_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "source": "live pre-harvest raw",
                    }

            for _raw_day, _raw_rows in forecast_days:
                if _raw_day < PLANT_INDEX_START:
                    continue
                for _rr in _raw_rows:
                    try:
                        ...
                        continue
                    _plant_raw[f"{_raw_day.isoformat()}|{_rf}"] = {
                        "target_date": _raw_day.isoformat(), "field_no": _rf,
                        "raw_total": _raw_total, "raw_abc": _raw_abc, "raw_xl": _raw_xl,
                        "captured_at": datetime.now(ZoneInfo("Europe/Tallinn")).isoformat(),
                        "source": "live future raw",
                    }
            _save_json_setting(_plant_raw_key, _plant_raw)
                ...
                for row in today_forecast_rows:
                    ...
                    forecast_payloads.append({
                        ...
                        "basis": (
                            ...
                            f"taimeindeks={float(row.get('Taimeindeks', 1.0)):.4f}; "
                            f"raw_total={float(row.get('_Raw Kokku') or row.get('Kokku')):.6f}"
                        ),
    ...
    if page != "Prognoos":
        # The background cycle lived in a CSS-hidden container, so there is no
        # duplicate Prognoos page to clean up and no second rerun is needed.
...
if page == "Prognoos" and not _run_operational_cycle:
    ...
    # Variprognoos on prognoosiinfo, mitte uurimislehe sisu. Hoia see ainult
    # Prognoos-menüüs, et Mootori tähelepanekud jääksid puhtalt uuringu/diagnostika jaoks.
    ...
    # Vastuvõtutest on Prognoosi lehel ALATI nähtav ka lugemisvaates.
    # Tulemus on salvestatud sama WD_MODEL_VERSION-i production walk-forward ringist.
    _ro_wa = _load_json_setting("wd_production_acceptance_json", {})
    st.markdown("##### WD vastuvõtt")
    if (
        isinstance(_ro_wa, dict)
        and str(_ro_wa.get("model_version") or "") == WD_MODEL_VERSION
        and _ro_wa.get("Testiridu")
    ):
        _ro_wa_pass = bool(_ro_wa.get("Stabiilne"))
        _rw1, _rw2, _rw3, _rw4 = st.columns(4)
        _rw1.metric("Tulemus", "PASS" if _ro_wa_pass else "EI LÄBINUD")
        _rw2.metric("Weather-first MAE", f"{float(_ro_wa.get('Baas MAE')):.2f}")
        _rw3.metric("WIND×DRY MAE", f"{float(_ro_wa.get('Katse MAE')):.2f}")
        _rw4.metric("Paranemine", f"{float(_ro_wa.get('Paranemine')):+.2f}")
        st.caption(
            f"Production walk-forward · {_ro_wa.get('Testiridu')} rida / {_ro_wa.get('Testipäevi')} päeva · "
            f"WD võidab {float(_ro_wa.get('Võidab ridu %')):.0f}% ridu · "
            f"halvim ajapool {float(_ro_wa.get('Halvim pool')):+.2f} kasti · "
            f"taimeindeks testis ei osale."
        )
    elif isinstance(_ro_wa, dict) and str(_ro_wa.get("model_version") or "") == WD_MODEL_VERSION:
        st.warning("Production-vastuvõtutest käis, kuid ühiseid walk-forward ridu ei tekkinud piisavalt tulemuse kuvamiseks.")
    else:
        st.info("Production-vastuvõtutest pole selle mootoriversiooni kohta veel arvutatud; äpp käivitab selle ühe korra automaatselt.")

    _ro_oa = _load_json_setting("wd_prev_order_acceptance_json", {})
    st.markdown("##### Kõigi eelmiste korjejärjekordade shadow-test")
    if (
        isinstance(_ro_oa, dict)
        and str(_ro_oa.get("model_version") or "") == WD_MODEL_VERSION
        and str(_ro_oa.get("test_version") or "") == _ORDER_TEST_VERSION
        and _ro_oa.get("Testiridu")
    ):
        _ro_oa_pass = bool(_ro_oa.get("Stabiilne"))
        _ro1, _ro2, _ro3, _ro4 = st.columns(4)
        _ro1.metric("Tulemus", "KASU" if _ro_oa_pass else "EI PIISA")
        _ro2.metric("WIND×DRY MAE", f"{float(_ro_oa.get('Baas MAE')):.2f}")
        _ro3.metric("+ eelmine order 1/2/3 MAE", f"{float(_ro_oa.get('Katse MAE')):.2f}")
        _ro4.metric("Paranemine", f"{float(_ro_oa.get('Paranemine')):+.2f}")
        st.caption(
            f"Production A+B+C walk-forward · {_ro_oa.get('Testiridu')} rida / {_ro_oa.get('Testipäevi')} päeva · "
            f"kandidaat võidab {float(_ro_oa.get('Võidab ridu %')):.0f}% ridu · "
            f"halvim ajapool {float(_ro_oa.get('Halvim pool')):+.2f} kasti · "
            f"ametlikku prognoosi see test ei muuda."
        )
    elif isinstance(_ro_oa, dict) and str(_ro_oa.get("test_version") or "") == _ORDER_TEST_VERSION:
        st.warning("Kõigi eelmiste korjejärjekordade shadow-test käis, kuid ühiseid ridu ei olnud piisavalt.")
    else:
        st.info("Kõigi eelmiste korjejärjekordade shadow-test pole veel arvutatud; äpp käivitab selle ühe korra automaatselt.")

    _ro_ot = _load_json_setting("wd_prev_order_xl_total_acceptance_json", {})
    st.markdown("##### Korjejärjekord → XL + kogusaak shadow-test")
    if (
        isinstance(_ro_ot, dict)
        and str(_ro_ot.get("model_version") or "") == WD_MODEL_VERSION
        and str(_ro_ot.get("test_version") or "") == _ORDER_TOTAL_TEST_VERSION
        and _ro_ot.get("status") == "tested"
    ):
        _rox = _ro_ot.get("XL") or {}
        _rotl = _ro_ot.get("TOTAL_XL_ONLY") or {}
        _rotb = _ro_ot.get("TOTAL_BOTH") or {}
        _ro_best = _rotl if float(_rotl.get("Katse MAE", 1e9)) <= float(_rotb.get("Katse MAE", 1e9)) else _rotb
        _ro_best_label = "order ainult XL-s" if _ro_best is _rotl else "order ABC-s + XL-s"
        _rt1, _rt2, _rt3, _rt4 = st.columns(4)
        _rt1.metric("XL baas MAE", f"{float(_rox.get('Baas MAE')):.2f}")
        _rt2.metric("XL + order MAE", f"{float(_rox.get('Katse MAE')):.2f}")
        _rt3.metric("Kogu baas MAE", f"{float(_ro_best.get('Baas MAE')):.2f}")
        _rt4.metric("Parim kogu + order MAE", f"{float(_ro_best.get('Katse MAE')):.2f}")
        st.caption(
            f"XL paranemine {float(_rox.get('Paranemine')):+.2f} · "
            f"KOGU order ainult XL-s {float(_rotl.get('Baas MAE')):.2f} → {float(_rotl.get('Katse MAE')):.2f} "
            f"({float(_rotl.get('Paranemine')):+.2f}) · "
            f"KOGU order ABC+XL {float(_rotb.get('Baas MAE')):.2f} → {float(_rotb.get('Katse MAE')):.2f} "
            f"({float(_rotb.get('Paranemine')):+.2f}) · parim: {_ro_best_label}. "
            f"Ametlikku prognoosi see test ei muuda."
        )
    elif (
        isinstance(_ro_ot, dict)
        and str(_ro_ot.get("test_version") or "") == _ORDER_TOTAL_TEST_VERSION
        and _ro_ot.get("status") == "insufficient_rows"
    ):
        st.warning("XL + kogusaagi korjejärjekorra shadow-test käis, kuid ühiseid ridu ei olnud piisavalt.")
    else:
        st.info("XL + kogusaagi korjejärjekorra shadow-test pole veel arvutatud; äpp käivitab selle ühe korra automaatselt.")

    _ro_sd = _load_json_setting("joint_size_distribution_shadow_json", {})
    st.markdown("##### Otsene B/C/XL jaotus · shadow-test")
    if (
        isinstance(_ro_sd, dict)
        and str(_ro_sd.get("model_version") or "") == WD_MODEL_VERSION
        and str(_ro_sd.get("test_version") or "") == _SIZE_DIST_TEST_VERSION
        and _ro_sd.get("status") == "tested"
    ):
        _rsv = _ro_sd.get("variants") or {}
        _rcur = _rsv.get("Praegune C/B + eraldi XL") or {}
        _rprimary = _rsv.get(str(_ro_sd.get("primary_variant") or "")) or {}
        _rplain = _rsv.get("Otsene ALR baas") or {}
        _rs1, _rs2, _rs3, _rs4 = st.columns(4)
        _rs1.metric("Praegune jaotuse MAE", "—" if not _rcur else f"{float(_rcur.get('dist_mae_pp')):.2f} pp")
        _rs2.metric("Otsene ALR MAE", "—" if not _rprimary else f"{float(_rprimary.get('dist_mae_pp')):.2f} pp")
        _rs3.metric("Paranemine", "—" if not _rprimary else f"{float(_rprimary.get('dist_improvement_pp')):+.2f} pp")
        _rs4.metric("Võidab ridu", "—" if not _rprimary else f"{float(_rprimary.get('win_rows_pct')):.0f}%")
        if _rprimary:
            st.caption(
                f"Põhitest ilma korjejärjekorrata: C/B {float(_rcur.get('cb_mae')):.2f} → {float(_rprimary.get('cb_mae')):.2f} · "
                f"B {float(_rcur.get('b_mae')):.2f} → {float(_rprimary.get('b_mae')):.2f} · "
                f"C {float(_rcur.get('c_mae')):.2f} → {float(_rprimary.get('c_mae')):.2f} · "
                f"XL {float(_rcur.get('xl_mae')):.2f} → {float(_rprimary.get('xl_mae')):.2f} · "
                f"C+XL {float(_rcur.get('large_mae')):.2f} → {float(_rprimary.get('large_mae')):.2f} · "
                f"KOGU {float(_rcur.get('total_mae')):.2f} → {float(_rprimary.get('total_mae')):.2f}. "
                f"Halvim ajapool {float(_rprimary.get('worst_half_pp')):+.2f} pp."
            )
        if _rplain and _rprimary:
            st.caption(
                f"Diagnostika: ALR baas {float(_rplain.get('dist_mae_pp')):.2f} pp · "
                f"ALR + praegused lisatunnused {float(_rprimary.get('dist_mae_pp')):.2f} pp."
            )
    elif (
        isinstance(_ro_sd, dict)
        and str(_ro_sd.get("test_version") or "") == _SIZE_DIST_TEST_VERSION
        and _ro_sd.get("status") == "insufficient_rows"
    ):
        st.warning("Otsese B/C/XL jaotuse shadow-test käis, kuid ühiseid ridu ei olnud piisavalt.")
    else:
        st.info("Otsese B/C/XL jaotuse shadow-test pole veel arvutatud; äpp käivitab selle ühe korra automaatselt.")

    _ro_rr = _load_json_setting("wd_prev_order_residual_json", {})
    st.markdown("##### Korjejäägi residual-test")
    if (
        isinstance(_ro_rr, dict)
        and str(_ro_rr.get("model_version") or "") == WD_MODEL_VERSION
        and str(_ro_rr.get("test_version") or "") == _RESIDUAL_ORDER_TEST_VERSION
        and _ro_rr.get("status") == "tested"
    ):
        _rgg = _ro_rr.get("groups") or {}
        _rg1, _rg2, _rg3 = _rgg.get("1") or {}, _rgg.get("2") or {}, _rgg.get("3") or {}
        _rr1, _rr2, _rr3, _rr4 = st.columns(4)
        _rr1.metric("Order 1 · C+XL jääk", "—" if not _rg1 else f"{float(_rg1.get('cxl_mean')):+.2f}")
        _rr2.metric("Order 2 · C+XL jääk", "—" if not _rg2 else f"{float(_rg2.get('cxl_mean')):+.2f}")
        _rr3.metric("Order 3 · C+XL jääk", "—" if not _rg3 else f"{float(_rg3.get('cxl_mean')):+.2f}")
        _rr4.metric("Order 3 · tegelik > prognoos", "—" if not _rg3 else f"{float(_rg3.get('cxl_positive_pct')):.0f}%")
        if _rg3:
            st.caption(
                f"Order 3 ({int(_rg3.get('n', 0))} rida): C jääk {float(_rg3.get('c_mean')):+.2f} · "
                f"XL jääk {float(_rg3.get('xl_mean')):+.2f} · C/B jääk {float(_rg3.get('cb_mean')):+.2f} · "
                f"C+XL mediaan {float(_rg3.get('cxl_median')):+.2f}. Positiivne = mudel ennustas liiga vähe."
            )
        _r3c = _ro_rr.get("order3_only_correction") or {}
        _rac = _ro_rr.get("all_orders_correction") or {}
        if _r3c:
            _rq1, _rq2, _rq3, _rq4 = st.columns(4)
            _rq1.metric("C+XL baas MAE", f"{float(_r3c.get('base_mae')):.2f}")
            _rq2.metric("Order3 residual-korr MAE", f"{float(_r3c.get('candidate_mae')):.2f}")
            _rq3.metric("Paranemine", f"{float(_r3c.get('improvement')):+.2f}")
            _rq4.metric("Korrigeeritud ridu", str(int(_r3c.get('corrected_rows') or 0)))
            _win = "—" if _r3c.get("corrected_win_pct") is None else f"{float(_r3c.get('corrected_win_pct')):.0f}%"
            _worst = "—" if _r3c.get("worst_half") is None else f"{float(_r3c.get('worst_half')):+.2f}"
            _alltxt = "—" if not _rac else f"{float(_rac.get('base_mae')):.2f} → {float(_rac.get('candidate_mae')):.2f} ({float(_rac.get('improvement')):+.2f})"
            st.caption(
                f"Strict expanding order3-korrektsioon: korrigeeritud ridadel võidab {_win}; halvim ajapool {_worst}. "
                f"Kõik orderid eraldi: {_alltxt}. Ainult varasemad OOS vead; ametlikku prognoosi test ei muuda."
            )
    elif (
        isinstance(_ro_rr, dict)
        and str(_ro_rr.get("test_version") or "") == _RESIDUAL_ORDER_TEST_VERSION
        and _ro_rr.get("status") == "insufficient_rows"
    ):
        st.warning("Korjejäägi residual-test käis, kuid ühiseid ridu ei olnud piisavalt.")
    else:
        st.info("Korjejäägi residual-test pole veel arvutatud; äpp käivitab selle ühe korra automaatselt.")
    ...
    try:
        _ro_today_plan = db.get_harvest_plan(TODAY)
    except Exception:
        _ro_today_plan = None
    ...
    for _lead in range(0, 10):
        ...
        if _lead == 0 and _ro_today_plan is not None:
            _ro_today_set = {int(f) for f in _ro_today_plan}
            _rows = [
                r for r in _rows
                if int(r.get("field_no")) in _ro_today_set
            ] if _ro_today_set else []
            if not _ro_today_set:
                st.markdown(f"### {_weekday_letter(_target_day)} {_short_date(_target_day)}  0,0 kasti")
                st.caption("täna · korjet ei ole")
                _ro_shown += 1
                continue
        if not _rows:
            continue
        ...
        for _r in _rows:
            ...
            _table_rows.append({
                ...
                "Taimeindeks": _row_plant_index(_r),
        ...
        if _explain_matches:
            ...
            for _f in sorted(_snapshot_fields):
                ...
                _exrow = {
                    ...
                    "Taimeindeks": _effects.get("Taimeindeks", 0.0),
